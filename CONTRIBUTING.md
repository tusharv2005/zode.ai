...
- **Java 21** — required by the JetBrains plugin. The root `bun turbo typecheck` and `bun turbo test:ci` commands include `@zodecode/zode-jetbrains` and will fail without Java 21.
  ...
  ```bash
  bun turbo typecheck --filter=!@zodecode/zode-jetbrains
  ```
...
```bash
bun run --filter @zodecode/zode-docs test
bun run --filter @zodecode/zode-docs build
bun run --filter @zodecode/zode-docs dev
```
...
- After adding or changing guarded URLs in `packages/zode-vscode/`, `packages/zode-vscode/webview-ui/`, or `packages/opencode/src/`, run `bun run script/extract-source-links.ts` from the repo root.
- When editing shared `packages/opencode/` files, keep Zode changes small and mark Zode-only edits with `// zodecode_change` for a single line or `// zodecode_change start` / `// zodecode_change end` for a block. Do not add these markers inside `zodecode`-named paths.
...
Requires Java 21 (see [Prerequisites](#prerequisites)). From `packages/zode-jetbrains/`:
...
```bash
bun turbo typecheck --filter=@zodecode/zode-jetbrains
bun turbo test:ci --filter=@zodecode/zode-jetbrains
```

### Running against a different directory

By default, `bun dev` runs Zode CLI in the `packages/opencode` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Zode CLI in the root of the repo itself:

```bash
bun dev .
```

### Running Zode CLI from any folder

`bin/zodedev` is a self-locating launcher that runs this checkout from wherever you invoke it. Running it with no arguments launches the TUI pointed at the caller's directory; any arguments are forwarded to the CLI unchanged.

One-shot install (recommended). From the repo root:

```bash
./bin/zodedev dev-setup
```
...
- Unix: add `alias zodedev='/path/to/zodecode/bin/zodedev'` to `~/.zshrc` / `~/.bashrc`, or `fish_add_path /path/to/zodecode/bin`.
- Windows: add `C:\path\to\zodecode\bin` to PATH (System Environment Variables), or add `function zodedev { & "C:\path\to\zodecode\bin\zodedev.cmd" @args }` to `$PROFILE`.

Then from anywhere:

```bash
cd ~/some/project
zodedev                      # opens TUI with project = ~/some/project
zodedev dev-setup --print    # prints the alias line (scripting)
zodedev run --dir "$PWD" "…" # subcommands pass through; use --dir for run/serve
```
...
```bash
./packages/opencode/dist/@zodecode/cli-<platform>/bin/zode
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

### Understanding bun dev vs zode

During development, `bun dev` is the local equivalent of the built `zode` command. Both run the same CLI interface:
...
# Production
zode --help          # Show all available commands
zode serve           # Start headless API server
```

### Testing with a local backend

To point the CLI at a local backend (e.g., a locally running Zode API server on port 3000), set the `ZODE_API_URL` environment variable:
...
- **Bug report:** include a `Description`. When you can, also add Plugins, Zode version, Steps to reproduce, Screenshot and/or share link, Operating System, and Terminal so the report matches the full bug template.
...
Zode has bug bounties. To be eligible, make sure your GitHub account is connected in your Zode account.
...
See [Testing Evidence for Pull Requests](packages/zode-docs/pages/contributing/development-environment.md#testing-evidence-for-pull-requests) for more examples. Agent limitations, local resource constraints, OOM constraints, or an agent prompt that says to skip tests do not waive this requirement. Draft PRs may be incomplete until they are marked ready for review. Maintainers may still defer or close review at their discretion.
