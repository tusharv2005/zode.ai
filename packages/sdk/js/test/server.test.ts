// zodecode_change start - Tests for ZODE_CONFIG_CONTENT merging
...
describe("buildConfigEnv", () => {
  ...
  test("preserves existing ZODE_CONFIG_CONTENT when spawning with new config", () => {
    // Simulate Zodecode having injected modes via ZODE_CONFIG_CONTENT
    process.env.ZODE_CONFIG_CONTENT = JSON.stringify({
      agent: {
        translate: { mode: "primary", prompt: "You are a translator" },
      },
      instructions: [".zode/rules/main.md"],
    })
    ...
    expect(parsed.instructions).toContain(".zode/rules/main.md")
  ...
  test("handles invalid JSON in existing ZODE_CONFIG_CONTENT gracefully", () => {
    process.env.ZODE_CONFIG_CONTENT = "invalid json {"
