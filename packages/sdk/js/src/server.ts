...
// zodecode_change start - Merge existing ZODE_CONFIG_CONTENT with new config
// This preserves Zodecode-injected modes when spawning nested CLI instances
...
function parseExistingConfig(): Config | undefined {
  const content = process.env.ZODE_CONFIG_CONTENT
...
export async function createZodeServer(options?: ServerOptions) {
  ...
  const proc = launch(`zode`, args, {
    // zodecode_change
    env: {
      ...process.env,
      ZODE_CONFIG_CONTENT: buildConfigEnv(options.config), // zodecode_change
    },
  })
  let clear = () => {}
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    proc.stdout?.on("data", (chunk) => {
      ...
      for (const line of lines) {
        ...
        if (line.startsWith("zode server listening")) {
          // zodecode_change end
...
export function createZodeTui(options?: TuiOptions) {
  const args = []
