...
function rewrite(request: Request, directory?: string) {
  ...
  const value = pick(request.headers.get("x-zode-directory"), directory)
  ...
  const next = new Request(url.href, request) // zodecode_change
  next.headers.delete("x-zode-directory")
  return next
}

export function createZodeClient(config?: Config & { directory?: string }) {
  if (!config?.fetch) {
