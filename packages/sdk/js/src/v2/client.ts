...
import { ZodeClient } from "./gen/sdk.gen.js"
import { wrapClientError } from "../error-interceptor.js"
export { type Config as ZodeClientConfig, ZodeClient }
...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-zode-directory", "directory"],
    ["x-zode-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-zode-directory")
  next.headers.delete("x-zode-workspace")
  return next
}

export function createZodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-zode-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-zode-workspace": config.experimental_workspaceID,
    }
  }
