import { createZodeClient, type Config as EffectiveConfig } from "@zodecode/sdk/v2/client"
import type {
  ...
  Pty as PtyInfo,
  PermissionRequest,
  Project as ZodeProject,
  QuestionRequest,
  ProviderAuthAuthorization,
  ProviderAuthResponse,
  ProviderListResponse,
  Session as ZodeSession,
  SessionStatus,
  ToolIdsResponse,
  ToolListResponse,
...
export type ProjectItem = ZodeProject
...
function server(input: string) {
  ...
  const user = decode(url.username) || "zode"
  const pass = decode(url.password) || "zode"
  url.username = ""
  url.password = ""
  url.search = ""
  url.hash = ""
...
function owner(items: ProjectItem[], session: Pick<ZodeSession, "projectID" | "directory">) {
...
export async function loadProjectTerminals(input: ProjectQuery, dir: string): Promise<ProjectTerminalItem[]> {
  ...
  const sessions = new Map<string, ZodeSession>()
...
export async function createProjectPty(input: Query, dir: string, title = "Zode session"): Promise<ProjectPtyInfo> {
  const sdk = client({ url: input.url, dir })
  const result = await sdk.pty.create({ directory: dir, command: "zode", cwd: dir, title })
