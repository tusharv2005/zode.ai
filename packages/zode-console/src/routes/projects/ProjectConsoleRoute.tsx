...
import { Card } from "@zodecode/zode-web-ui/card"
import { Icon } from "@zodecode/zode-web-ui/icon"
...
export function ProjectConsoleRoute() {
  ...
  const [selected, setSelected] = createSignal(window.localStorage.getItem(`zode.console.${params.project}.dir`) ?? "")
  const [active, setActive] = createSignal(window.localStorage.getItem(`zode.console.${params.project}.pty`) ?? "")
  ...
  function labelKey(dir: string) {
    return `zode.console.${project()}.worktree.${encodeURIComponent(dir)}.label`
  }
  ...
  function remember(dir: string, pty?: string) {
    window.localStorage.setItem(`zode.console.${project()}.dir`, dir)
    if (pty) {
      window.localStorage.setItem(`zode.console.${project()}.pty`, pty)
      return
    }
    window.localStorage.removeItem(`zode.console.${project()}.pty`)
  }
  ...
  function addWorktree() {
    ...
    run("Creating worktree", async () => {
      ...
      window.localStorage.setItem(`zode.console.${project()}.dir`, next.directory)
    })
  }
  ...
  function addSession(item = current()) {
    ...
    const label = `Zode ${terminalsFor(item.dir).length + 1}`
      ...
      <main class="project-console-main">
        ...
        <Show when={!terminal() && !snap.loading && !snap.error && !failure()}>
          <div class="project-terminal-empty">
            ...
            <span>Use + next to a worktree to start Zode CLI.</span>
          </div>
        </Show>
      </main>
