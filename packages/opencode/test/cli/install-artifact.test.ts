...
const wrapper = path.join(root, "bin", "zode")
...
describe("npm install artifact behavior", () => {
  test("keeps the CLI wrapper contract", async () => {
    ...
    expect(text).toContain("const envPath = process.env.ZODE_BIN_PATH")
    expect(text).toContain('const base = "@zodecode/cli-" + platform + "-" + arch')
  ...
  test("copies cached binary runtime resources during postinstall", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zode-postinstall-artifact-"))
    try {
      const pkg = path.join(tmp, "node_modules", "@zodecode", "cli")
      const native = path.join(tmp, "node_modules", "@zodecode", `cli-${process.platform}-${process.arch}`)
      ...
      await Bun.write(path.join(native, "package.json"), JSON.stringify({ name: `@zodecode/cli-${process.platform}-${process.arch}` }))
      await Bun.write(path.join(bin, "zode"), "binary")
      ...
      expect(await Bun.file(path.join(pkg, "bin", ".zode")).text()).toBe("binary")
  ...
  test("links npm bin commands to the wrapper during local install", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zode-install-artifact-"))
    try {
      ...
      await fs.copyFile(wrapper, path.join(bin, "zode"))
      await Bun.write(
        path.join(pkg, "package.json"),
        JSON.stringify(
          {
            name: "zode-install-artifact-repro",
            version: "1.0.0",
            bin: {
              zode: "./bin/zode",
              zodecode: "./bin/zode",
      ...
      const hidden = path.join(prefix, "node_modules", ".bin", ".zode")
