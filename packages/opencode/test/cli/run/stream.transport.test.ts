...
import { ZodeClient, type GlobalEvent } from "@zodecode/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ZodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ZodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ZodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ZodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ZodeClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: ZodeClient["event"]["subscribe"]
    globalEvent?: ZodeClient["global"]["event"]
    promptAsync?: ZodeClient["session"]["promptAsync"]
    status?: ZodeClient["session"]["status"]
    messages?: ZodeClient["session"]["messages"]
    children?: ZodeClient["session"]["children"]
    permissions?: ZodeClient["permission"]["list"]
    questions?: ZodeClient["question"]["list"]
  } = {},
) {
  const client = new ZodeClient()

  const subscribe: ZodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ZodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ZodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ZodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ZodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ZodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ZodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ZodeClient["question"]["list"] = input.questions ?? (() => ok([]))
