...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/@zodecode%2Fcli/${InstallationChannel}`) // zodecode_change
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("Zode-Org/tap/zode")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("zode")) return "zode"
          // zodecode_change end
          return ""
        },
      )
    ...
    test("reads brew tap info JSON via CLI", async () => {
      ...
      const layer = testLayer(
        ...
        (cmd, args) => {
          // zodecode_change start
          if (cmd === "brew" && args.includes("Zode-Org/tap/zode") && args.includes("--formula")) return "zode"
          // zodecode_change end
