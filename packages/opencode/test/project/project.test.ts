...
describe("Project.fromDirectory", () => {
  ...
  test("should handle git repository with commits", async () => {
    ...
    const zodeFile = path.join(tmp.path, ".git", "zode")
    expect(await Bun.file(zodeFile).exists()).toBe(true)
  })
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "zode")
...
describe("Project.fromDirectory with bare repos", () => {
  test("worktree from bare repo should cache in bare repo, not parent", async () => {
    ...
    try {
      ...
      const correctCache = path.join(barePath, "zode") // zodecode_change
      const wrongCache = path.join(parentDir, ".git", "zode") // zodecode_change
  ...
  test("different bare repos under same parent should not share project ID", async () => {
    ...
    try {
      ...
      const cacheA = path.join(bareA, "zode")
      const cacheB = path.join(bareB, "zode")
      const wrongCache = path.join(parentDir, ".git", "zode")
      // zodecode_change end
