...
describe("InstanceStore", () => {
  ...
  // zodecode_change - InstanceStore.boot wraps bootstrap.run in the Instance ALS so
  // ZodecodeBootstrap (and anything it forkDetaches, e.g. ZodeIndexing.init) can read
  // Instance.directory. Upstream runs bootstrap without the ALS; this regression test
  // pins the Zode contract.
  it.live("installs Instance ALS around bootstrap for Zode bootstrap compatibility", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
