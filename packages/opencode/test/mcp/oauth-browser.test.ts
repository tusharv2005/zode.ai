...
void mock.module("open", () => ({
  default: async (url: string) => {
    ...
    if (openShouldFail) {
      // zodecode_change start - buffer the error until the consumer attaches
...
// zodecode_change start - reset mock state and wait for OAuth redirect signal in CI
...
test("BrowserOpenFailed event is published when open() throws", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      // zodecode_change start - attach rejection handler before stopping callback server
      ...
      await waitFor(() => events.length > 0) // zodecode_change
...
test("BrowserOpenFailed event is NOT published when open() succeeds", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      // zodecode_change
      openShouldFail = false // zodecode_change
      ...
      await waitFor(() => openCalledWith !== undefined) // zodecode_change
      await new Promise((resolve) => setTimeout(resolve, 600)) // zodecode_change - let authenticate await callbackPromise before stop rejects it
