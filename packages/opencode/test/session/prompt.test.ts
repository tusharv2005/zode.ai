...
import { afterEach, expect, mock, spyOn } from "bun:test" // zodecode_change - spy on review telemetry
import { Telemetry } from "@zodecode/zode-telemetry" // zodecode_change - assert review command telemetry
...
import { Suggestion } from "../../src/zodecode/suggestion" // zodecode_change - accept suggestion in telemetry test
...
// zodecode_change start - make timing-sensitive prompt tests wait on observable state
...
function makeHttp() {
  ...
  return Layer.mergeAll(
    ...
    SessionPrompt.layer.pipe(
      ...
      Layer.provideMerge(question), // zodecode_change - SessionPrompt now dismisses questions via its service dependency
...
// zodecode_change start - restore any spies between tests so review telemetry spy never leaks
afterEach(() => {
  mock.restore()
})
// zodecode_change end
...
// zodecode_change start - replacement prompts unblock pending Question service requests
...
// zodecode_change - skipped: tracked in #9958
...
it.live(
    ...
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const tool = yield* Effect.promise(async () => {
          ...
          while (Date.now() < end) {
            const msgs = await Effect.runPromise(MessageV2.filterCompactedEffect(chat.id))
            const assistant = msgs.findLast((item) => item.info.role === "assistant" && item.info.agent === "code") // zodecode_change
  ...
  10_000, // zodecode_change
)

// zodecode_change start - child task failures stay tool errors so the parent can recover
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      // zodecode_change start - hold the model response instead of cancelling an infinite stream
      Effect.fnUntraced(function* ({ llm }) {
...
unix(
  // zodecode_change - skip flaky cancel test on Windows CI
...
// zodecode_change start - handleSubtask propagates child session cost to wrapper (#6321)
it.live(
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      // zodecode_change start - gate the failing stream so both callers join the same run
      Effect.fnUntraced(function* ({ llm }) {
        const gate = defer<void>()
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        ...
        yield* llm.wait(1)
        const b = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
        yield* Effect.sleep(50) // zodecode_change - let b attach to a's done deferred before gate resolves
        gate.resolve()
...
// zodecode_change start - #9492: the upstream fork-based shape of this test
...
// can be exercised. The Zode queue semantics (in-flight stream drains, second
// LLM request ends with the queued user message) are covered end-to-end in
// packages/opencode/test/zodecode/session-prompt-queue.test.ts — keep this
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        const status = yield* SessionStatus.Service // zodecode_change
        const chat = yield* sessions.create({
          title: "Pinned",
          permission: [{ permission: "*", pattern: "*", action: "allow" }],
        })
        ...
        const sh = yield* prompt
          ...
          .pipe(Effect.forkChild)
        // zodecode_change start - wait for shell to actually be running before forking loop
  ...
  30_000, // zodecode_change - Windows CI process startup can exceed 3s
)
    ...
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const sh = yield* prompt
          .shell({ sessionID: chat.id, agent: "build", command: "sleep 0.2" })
          .pipe(Effect.forkChild)
        // zodecode_change start - wait for shell to actually be running before forking loop callers
        yield* waitFor("shell busy", status.get(chat.id).pipe(Effect.map((s) => (s.type === "busy" ? s : undefined))))
        // zodecode_change end
    ...
    withSh(() =>
      provideTmpdirInstance(
        (_dir) =>
          Effect.gen(function* () {
            const { prompt, run, chat } = yield* boot()
            // zodecode_change start - wait for shell to actually be running before cancelling
            const status = yield* SessionStatus.Service
            // zodecode_change end
            ...
            const sh = yield* prompt
              ...
              .pipe(Effect.forkChild)
            // zodecode_change start - avoid racing cancellation against async shell startup on Linux CI
            yield* waitFor(
              "shell busy",
...
// zodecode_change start - cancel persists aborted shell result when shell ignores TERM
unix(
...
// zodecode_change start - /review subtask path tags child completions for telemetry
it.live(
      ...
      (_dir) =>
        Effect.gen(function* () {
          ...
          if (Exit.isFailure(exit)) {
            ...
            expect(NamedError.Unknown.isInstance(err)).toBe(true)
            if (NamedError.Unknown.isInstance(err)) {
              expect(err.data.message).toContain("code") // zodecode_change - "build" renamed to "code"
            }
          }
        }),
      { git: true },
