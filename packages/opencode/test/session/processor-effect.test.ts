...
import { SessionNetwork } from "../../src/session/network" // zodecode_change
...
it.live("session.processor effect tests reset reasoning state across retries", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        // zodecode_change start — auto-reply to network reconnection prompts triggered by reset()
        ...
        offAsk() // zodecode_change — cleanup subscriber
      }),
    { git: true, config: (url) => providerCfg(url) },
  ),
)
