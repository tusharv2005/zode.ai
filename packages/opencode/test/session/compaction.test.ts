...
describe("SessionNs.getUsage", () => {
  ...
  // zodecode_change start - Test for OpenRouter provider cost
  ...
  test("uses upstreamInferenceCost for Zode provider", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    const provider = { id: "zode" } as Provider.Info
    const result = SessionNs.getUsage({
      model,
      provider,
      usage: {
    ...
    // Should use upstreamInferenceCost for Zode provider (BYOK)
  ...
  test("uses regular cost when upstreamInferenceCost is missing for Zode", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    // When upstream cost is missing for Zode, fall back to regular cost field
    expect(result.cost).toBe(0.01)
  })

  // Tests for Anthropic Messages / OpenAI Responses / Vercel AI Gateway cost extraction
  // live in test/zodecode/provider-cost.test.ts (zodecode_change).
  // zodecode_change end
