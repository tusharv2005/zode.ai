...
// zodecode_change - skip on windows: address windows ci failures #9496
...
`,
          ...
          process.env.ZODE_TEST_HOME = dir
  ...
  it.live("built-in zode-config includes named command lookup guidance", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          const home = process.env.ZODE_TEST_HOME
          ...
          const result = yield* tool.execute({ name: "zode-config" }, ctx)

          expect(result.metadata.dir).toBe("builtin")
          expect(result.output).toContain("Finding a named command")
          expect(result.output).toContain("~/.config/zode/")
          expect(result.output).toContain("~/.zodecode/")
