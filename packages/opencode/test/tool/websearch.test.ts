...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.ZODE_WEBSEARCH_PROVIDER

    try {
      process.env.ZODE_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.ZODE_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.ZODE_WEBSEARCH_PROVIDER
      else process.env.ZODE_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for zode or explicit websearch provider flags", () => {
    // zodecode_change
    expect(webSearchEnabled(ProviderID.zode, { exa: false, parallel: false })).toBe(true) // zodecode_change
    expect(webSearchEnabled(ProviderID.opencode, { exa: false, parallel: false })).toBe(false) // zodecode_change
