...
import { MessageID, PartID, SessionID } from "../../src/session/schema" // zodecode_change - SessionID used by cost propagation tests
import { ModelID, ProviderID } from "../../src/provider/schema"
import { Provider } from "../../src/provider/provider" // zodecode_change
...
const it = testEffect(
  Layer.mergeAll(
    ...
    Provider.defaultLayer, // zodecode_change
...
// zodecode_change start - stub signature + prompt body extended to persist assistant cost for propagation tests
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // zodecode_change start — use arrayContaining: Zode appends inherited caller restrictions
        ...
        expect(seen?.tools).toEqual({
          todowrite: false,
          task: false, // zodecode_change - Zode disallows nested subagents
          bash: false,
          read: false,
        })
      }),
    {
      config: {
        ...
        experimental: {
          ...
          openTelemetry: true, // zodecode_change
        },
      },
    },
  )

  // zodecode_change start - terminal child assistant errors fail the task tool boundary
...
// zodecode_change start - subagent cost propagation coverage (#6321)
