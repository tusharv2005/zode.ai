...
// zodecode_change start - upstream #24416 fixture adapted for @zodecode/plugin
async function markPluginDependenciesReady(dir: string) {
  await mkdir(path.join(dir, "node_modules"), { recursive: true })
  await Bun.write(
    path.join(dir, "package-lock.json"),
    JSON.stringify({ packages: { "": { dependencies: { "@zodecode/plugin": "0.0.0" } } } }),
  )
}
// zodecode_change end
...
function paid(providers: Awaited<ReturnType<typeof list>>) {
  ...
  if (!item) return 0 // zodecode_change - Zode drops opencode provider without apiKey/auth
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // zodecode_change start - upstream #24416 avoids real plugin dependency installs
