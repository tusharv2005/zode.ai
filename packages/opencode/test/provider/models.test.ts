...
import { ModelCache } from "../../src/provider/model-cache" // zodecode_change
import { Config } from "../../src/config/config" // zodecode_change
import { Auth } from "../../src/auth" // zodecode_change
import { it } from "../lib/effect"
import { rm, writeFile, utimes, mkdir } from "fs/promises"
import path from "path"

// test/preload.ts pins ZODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.ZODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.ZODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.ZODE_MODELS_PATH = undefined
  Flag.ZODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.ZODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.ZODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
// zodecode_change start - skip: upstream tests assert raw-fixture passthrough but Zode's
// ModelsDev.get() filters/injects providers based on effect-native config access (zode-allowed
// gating, apertis options, and Zode provider injection).
// zodecode_change end
