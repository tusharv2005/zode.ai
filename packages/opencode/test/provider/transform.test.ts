...
describe("ProviderTransform.variants", () => {
  ...
  // zodecode_change start
  describe("@zodecode/zode-gateway", () => {
    test("claude models return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "zode/anthropic/claude-sonnet-4",
        providerID: "zode",
        capabilities: { reasoning: false },
        api: {
          id: "anthropic/claude-sonnet-4",
    ...
    test("anthropic models in api.id return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "zode/anthropic/claude-opus-4",
        providerID: "zode",
        capabilities: { reasoning: false },
        api: {
    ...
    // zodecode_change start
    test("mercury-2 uses server-provided variants from zode gateway", () => {
  ...
  describe("ProviderTransform.smallOptions", () => {
    describe("@zodecode/zode-gateway", () => {
      ...
      test("google models disable reasoning", () => {
        const model = createMockModel({
          id: "zode/google/gemini-2.0-flash",
          providerID: "zode",
          api: {
...
// zodecode_change start - tests for reasoningSummary guard
describe("ProviderTransform.options - OpenAI Responses API params guard", () => {
  ...
  test("includes reasoningSummary for @zodecode/zode-gateway", () => {
    const result = ProviderTransform.options({
      model: gpt5Model("@zodecode/zode-gateway", "zode"),
      sessionID,
    })
    expect(result.reasoningSummary).toBe("auto")
  })
