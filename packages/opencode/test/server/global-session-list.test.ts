...
import { RemoteSender } from "../../src/zode-sessions/remote-sender" // zodecode_change
...
describe("session.listGlobal", () => {
  ...
  // zodecode_change start - project-family filter across worktrees (stale .git/zode project ID)
  test("filters by project family across worktrees when project IDs drift", async () => {
    ...
    try {
      ...
      // Now write a stale project ID to .git/zode — this overrides the root's cached ID
      await Bun.write(path.join(first.path, ".git", "zode"), "stale-project-id")

      const root = await WithInstance.provide({
        directory: first.path,
        fn: async () => svc.create({ title: "root-session" }),
      })
      await Bun.file(path.join(first.path, ".git", "zode")).delete()
