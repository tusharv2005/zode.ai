 ...
 *
 * The script intentionally isolates `ZODE_DB` before importing modules that touch
...
import { zodeScenarios } from "../../zodecode/server/httpapi-exercise-scenarios" // zodecode_change
...
const scenarios: Scenario[] = [
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-zode-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-zode-directory")
  }),
