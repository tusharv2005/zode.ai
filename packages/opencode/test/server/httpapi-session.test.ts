...
describe("session HttpApi", () => {
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        const headers = { "x-zode-directory": tmp.path }
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        ...
        const response = yield* request(pathFor(SessionPaths.get, { sessionID: session.id }), {
          headers: { "x-zode-directory": tmp.path },
        })
  ...
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false, share: "disabled" } }, (tmp) =>
      Effect.gen(function* () {
        const headers = { "x-zode-directory": tmp.path, "content-type": "application/json" }
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false, share: "disabled" } }, (tmp) =>
      Effect.gen(function* () {
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-zode-directory": tmp.path, "content-type": "application/json" },
