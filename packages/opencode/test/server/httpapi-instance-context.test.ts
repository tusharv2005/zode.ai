...
// zodecode_change start - avoid InstanceLayer.layer's lazy bootstrap import in this test graph.
...
const it = testEffect(
  Layer.mergeAll(
    ...
    NodeServices.layer,
    // zodecode_change start - local equivalent of InstanceLayer.layer.
  ...
  it.live("provides selected workspace id on control-plane routes", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.get(`/session?workspace=${workspace.id}`).pipe(
        HttpClientRequest.setHeader("x-zode-directory", dir),
        HttpClient.execute,
      )
