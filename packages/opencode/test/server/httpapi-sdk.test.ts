...
type Sdk = ReturnType<typeof createZodeClient>
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.ZODE_SERVER_PASSWORD = input?.password
  Flag.ZODE_SERVER_USERNAME = input?.username
  ...
  httpapi(
    ...
    withStandardProject("raw", ({ directory }) =>
      Effect.gen(function* () {
        ...
        // zodecode_change start - match Hono AuthMiddleware username default ("zode")
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("zode", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("zode", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
        // zodecode_change end
