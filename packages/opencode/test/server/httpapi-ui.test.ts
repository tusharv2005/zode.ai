...
const original = {
  ZODE_DISABLE_EMBEDDED_WEB_UI: Flag.ZODE_DISABLE_EMBEDDED_WEB_UI,
  ZODE_SERVER_PASSWORD: Flag.ZODE_SERVER_PASSWORD,
  ZODE_SERVER_USERNAME: Flag.ZODE_SERVER_USERNAME,
  envPassword: process.env.ZODE_SERVER_PASSWORD,
  envUsername: process.env.ZODE_SERVER_USERNAME,
}

afterEach(() => {
  Flag.ZODE_DISABLE_EMBEDDED_WEB_UI = original.ZODE_DISABLE_EMBEDDED_WEB_UI
  Flag.ZODE_SERVER_PASSWORD = original.ZODE_SERVER_PASSWORD
  Flag.ZODE_SERVER_USERNAME = original.ZODE_SERVER_USERNAME
  restoreEnv("ZODE_SERVER_PASSWORD", original.envPassword)
  restoreEnv("ZODE_SERVER_USERNAME", original.envUsername)
})
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            ZODE_SERVER_PASSWORD: input?.password,
            ZODE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  // zodecode_change start - embedded UI is the only supported fallback; never proxy to app.opencode.ai
  test("returns not found without proxying when embedded UI is disabled", async () => {
    Flag.ZODE_DISABLE_EMBEDDED_WEB_UI = true
    let proxied = false
  ...
  test("requires server password for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "zode" }).request("/")
  ...
  test("accepts auth token for the web UI", async () => {
    ...
    const response = await uiApp({
      password: "secret",
      username: "zode",
    }).request(`/?auth_token=${btoa("zode:secret")}`)
  ...
  test("accepts basic auth for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "zode" }).request("/", {
      headers: { authorization: `Basic ${btoa("zode:secret")}` },
    })
  ...
  test("allows web UI preflight without auth", async () => {
    const response = await app({ password: "secret", username: "zode" }).request("/", {
      method: "OPTIONS",
      headers: {
