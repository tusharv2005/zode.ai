...
const describeProvider = process.platform === "win32" ? describe.skip : describe // zodecode_change - scoped temp cleanup is flaky on Windows CI
...
describeProvider("provider HttpApi", () => {
  ...
  it.live(
    ...
    withProviderProject((dir) =>
      Effect.gen(function* () {
        const headers = { "x-zode-directory": dir, "content-type": "application/json" }
        const server = app()
  ...
  it.live("serves provider lists when auth loaders add runtime fetch options", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "opencode-test-" })
      const previous = process.env.ZODE_AUTH_CONTENT
      ...
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          if (previous === undefined) delete process.env.ZODE_AUTH_CONTENT
          if (previous !== undefined) process.env.ZODE_AUTH_CONTENT = previous
        }),
      )
      const headers = { "x-zode-directory": dir }
