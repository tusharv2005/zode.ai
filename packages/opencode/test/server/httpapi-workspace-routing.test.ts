    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          ...
          "x-zode-directory": "/secret/path",
          "x-zode-workspace": "internal",
        }),
        HttpClient.execute,
      )
      ...
      expect(forwarded?.headers["x-zode-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-zode-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-zode-directory", headerDir),
        HttpClient.execute,
      )
