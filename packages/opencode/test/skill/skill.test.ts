...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.ZODE_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.ZODE_TEST_HOME = prev
      }),
  )

const discovered = <T extends { location: string }>(list: readonly T[]) =>
  list.filter((s) => s.location !== Skill.BUILTIN_LOCATION) // zodecode_change

const disabled = Flag.ZODE_DISABLE_CLAUDE_CODE_SKILLS

beforeAll(() => {
  Flag.ZODE_DISABLE_CLAUDE_CODE_SKILLS = false
})

afterAll(() => {
  Flag.ZODE_DISABLE_CLAUDE_CODE_SKILLS = disabled
})

describe("skill", () => {
  // zodecode_change start
  it.live("discovers skills from .zode/skill/ directory", () =>
    // zodecode_change end
      ...
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".zode", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const list = discovered(yield* skill.all()) // zodecode_change
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".zode", "skill", "dir-skill", "SKILL.md"), // zodecode_change: .zode is primary
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".zode", "skill", "dir-skill")) // zodecode_change: .zode is primary
  ...
  // zodecode_change start
  it.live("discovers multiple skills from .zode/skill/ directory", () =>
    // zodecode_change end
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".zode", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".zode", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".zode", "skill", "no-frontmatter", "SKILL.md"), // zodecode_change: .zode is primary
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect(discovered(yield* skill.all())).toEqual([]) // zodecode_change
        }),
      { git: true },
    ),
  )
