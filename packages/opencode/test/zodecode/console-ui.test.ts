...
import { ConsoleAssets } from "../../src/zodecode/console/assets"

const original = process.env.ZODE_CONSOLE_ASSET_DIR

afterEach(() => {
  if (original === undefined) delete process.env.ZODE_CONSOLE_ASSET_DIR
  else process.env.ZODE_CONSOLE_ASSET_DIR = original
})
...
async function assets(dir: string) {
  ...
  await Bun.write(path.join(dir, "assets", "app.js"), "console.log('zode')")
}

describe("Zode Console UI routes", () => {
  test("serves the console index for /console and SPA routes", async () => {
    await using tmp = await tmpdir()
    process.env.ZODE_CONSOLE_ASSET_DIR = tmp.path
  ...
  test("serves console assets without falling back on missing files", async () => {
    ...
    expect(await Bun.file(asset.file).text()).toContain("zode")
