// Regression test: OAuth accountId must flow into model fetch as zodecodeOrganizationId
...
type Options = Parameters<ModelCache.ZodeModels["fetch"]>[0]

function layer(info: Auth.Info | undefined, captured: Ref.Ref<Options | undefined>) {
  const auth = Layer.mock(Auth.Service)({
    get: (id) => Effect.succeed(id === "zode" ? info : undefined),
  })
...
it.live("model fetch uses accountId from OAuth auth as zodecodeOrganizationId", () =>
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) => cache.fetch("zode")).pipe(Effect.provide(layer(info, captured)))
    expect(yield* Ref.get(captured)).toMatchObject({
      zodecodeToken: "test-oauth-token",
      zodecodeOrganizationId: "org-enterprise-123",
    })
  }),
)

it.live("model fetch without OAuth accountId does not set zodecodeOrganizationId", () =>
  Effect.gen(function* () {
    const captured = yield* Ref.make<Options | undefined>(undefined)
    const info = new Auth.Oauth({
      type: "oauth",
    ...
    yield* ModelCache.Service.use((cache) => cache.fetch("zode")).pipe(Effect.provide(layer(info, captured)))
    expect(yield* Ref.get(captured)).toMatchObject({ zodecodeToken: "test-personal-token" })
    expect((yield* Ref.get(captured))?.zodecodeOrganizationId).toBeUndefined()
  }),
)
  ...
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) =>
      Effect.gen(function* () {
        ...
        expect(yield* cache.get("zode")).toBeDefined()

        yield* cache.clear("zode")
        expect(yield* cache.get("zode")).toBeUndefined()
