...
it.instance("shares and unshares sessions through Zode public URLs", () => {
  const urls: string[] = []
  ...
  return Effect.gen(function* () {
    ...
    yield* auth.set("zode", { type: "api", key: "test-token" })
    ...
    expect(result.url).toBe("https://app.__PRESERVE_ZODE_AI__/s/public-1")
    expect((yield* session.get(info.id)).share?.url).toBe("https://app.__PRESERVE_ZODE_AI__/s/public-1")
    ...
    Effect.ensuring(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        yield* auth.remove("zode").pipe(Effect.ignore)
        request.mockRestore()
      }),
    ),
