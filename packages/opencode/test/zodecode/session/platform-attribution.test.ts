  ...
  test("child sessions inherit the root platform override", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const attr = ZodeSession.attribution(child.id)

        expect(ZodeSession.getPlatformOverride(root.id)).toBe("agent-manager")
        expect(ZodeSession.getPlatformOverride(child.id)).toBe("agent-manager")
        expect(ZodeSession.resolvePlatform(child.id)).toBe("agent-manager")
  ...
  test("child sessions expose parent and root lineage", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        expect(ZodeSession.resolveParent(root.id)).toBeUndefined()
        expect(ZodeSession.resolveParent(child.id)).toBe(root.id)
        expect(ZodeSession.resolveParent(leaf.id)).toBe(child.id)
        expect(ZodeSession.resolveRoot(leaf.id)).toBe(root.id)
