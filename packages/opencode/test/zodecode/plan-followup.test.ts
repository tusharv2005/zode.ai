...
import { formatTodos, generateHandover, PlanFollowup, PlanFollowupRuntime } from "../../src/zodecode/plan-followup"
...
describe("plan follow-up", () => {
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { ZodeSessionPromptQueue } = await import("../../src/zodecode/session/prompt-queue")
      ...
      ZodeSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = ZodeSessionPromptQueue.scope(seeded.sessionID, all)
