// Unit tests for ZodeSessionPrompt history-trim / media-strip helpers.
...
import { ZodeSessionMessageOrder } from "../../src/zodecode/session/message-order"
...
describe("ZodeSessionPrompt.hasCompletedSummary", () => {
  test("returns false for empty array", () => {
    expect(ZodeSessionPrompt.hasCompletedSummary([])).toBe(false)
  })
  ...
  test("returns false when only summary has an error", () => {
    ...
    expect(ZodeSessionPrompt.hasCompletedSummary(msgs)).toBe(false)
  })
  ...
  test("returns true when summary has finish and no error", () => {
    ...
    expect(ZodeSessionPrompt.hasCompletedSummary(msgs)).toBe(true)
  })
})
...
describe("MessageV2.latest", () => {
  test("selects chronological state after retained pre-compaction tail", () => {
    ...
    const state = ZodeSessionMessageOrder.latest(msgs)
  ...
  test("keeps queued subtasks moved after a chronologically later assistant", () => {
    ...
    ZodeSessionMessageOrder.annotate([active, queued, done])

    const state = ZodeSessionMessageOrder.latest([active, done, queued])
...
describe("ZodeSessionPrompt.trimBeforeLastSummary", () => {
  ...
  test("trims to summary's text-user parent (reference-session bug)", () => {
    ...
    const result = ZodeSessionPrompt.trimBeforeLastSummary(filtered)
  ...
  test("keeps the newest summary when retained history contains an older summary", () => {
    ...
    expect(ZodeSessionPrompt.trimBeforeLastSummary(filtered)).toBe(filtered)
  })
  ...
  test("is idempotent", () => {
    ...
    const first = ZodeSessionPrompt.trimBeforeLastSummary(msgs)
    const second = ZodeSessionPrompt.trimBeforeLastSummary(first)
...
describe("ZodeSessionPrompt.stripHistoricalMedia", () => {
  test("replaces image file part in historical user message with placeholder text", () => {
    ...
    const result = ZodeSessionPrompt.stripHistoricalMedia(msgs)
  ...
  test("does not mutate the input", () => {
    ...
    const result = ZodeSessionPrompt.stripHistoricalMedia(input)
...
describe("ZodeSessionPrompt.maybeStripHistoricalMedia", () => {
  test("returns input unchanged when no completed summary exists", () => {
    const msgs = [
      user("msg_hist", [filePart("msg_hist", "image/png", "hist.png")]),
      user("msg_last", [textPart("msg_last", "follow-up")]),
    ]
    const result = ZodeSessionPrompt.maybeStripHistoricalMedia(msgs)
    expect(result).toBe(msgs)
  })
