...
//
// Unit tests for ZodeSnapshotTrack.wrap — the slow-repo guard that sits
...
import { ZodeSnapshotTrack } from "../../src/zodecode/snapshot/track"
...
const makeHooks = (
  answer: ZodeSnapshotTrack.Answer | Promise<ZodeSnapshotTrack.Answer>,
): { hooks: ZodeSnapshotTrack.Hooks; calls: Calls } => {
  const calls: Calls = { ask: 0, persist: 0, progress: [] }
  let counter = 0
  const hooks: ZodeSnapshotTrack.Hooks = {
    async ask() {
      calls.ask += 1
      return answer
    },
...
describe("ZodeSnapshotTrack progress indicator", () => {
  ...
  test("slow-but-succeeding path (under timeout) starts then ends the indicator", async () => {
    ...
    expect(withoutFrame(firstText.text)).toBe(
      ZodeSnapshotTrack.formatProgress(ZodeSnapshotTrack.PROGRESS_INITIALIZING, "").trim(),
    )
  ...
  test("timed-out path keeps the initializing label (no text escalation)", async () => {
    ...
    const base = ZodeSnapshotTrack.formatProgress(ZodeSnapshotTrack.PROGRESS_INITIALIZING, "").trim()
    for (const evt of events) {
...
describe("ZodeSnapshotTrack constants", () => {
  test("TIMEOUT_MS defaults to 10s and respects env override", () => {
    ...
    // with ZODE_SNAPSHOT_TRACK_TIMEOUT_MS, which this test suite does not set.
    expect(ZodeSnapshotTrack.TIMEOUT_MS).toBe(10_000)
  })

  test("exposes stable answer labels", () => {
    expect(ZodeSnapshotTrack.ANSWER_CONTINUE).toBe("Continue with snapshots")
    expect(ZodeSnapshotTrack.ANSWER_DISABLE).toBe("Disable for this project")
  })
})
