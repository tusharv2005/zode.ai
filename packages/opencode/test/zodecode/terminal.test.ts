...
import { kitty, sequences } from "../../src/zodecode/cli/cmd/tui/util/terminal"

const keys = ["TERM_PROGRAM", "MSYSTEM", "ZODE_DISABLE_KITTY_KEYBOARD", "ZODE_ENABLE_KITTY_KEYBOARD"] as const
...
test("allows explicitly enabling Kitty keyboard", () => {
  env({ ZODE_ENABLE_KITTY_KEYBOARD: "1", MSYSTEM: "MINGW64" })
...
test("allows explicitly disabling Kitty keyboard", () => {
  env({ ZODE_DISABLE_KITTY_KEYBOARD: "1", ZODE_ENABLE_KITTY_KEYBOARD: "1" })
...
test("resets common terminal input modes", () => {
  env({ ZODE_DISABLE_KITTY_KEYBOARD: "1" })
