...
import { ZodeSessionTuiSync } from "../../src/zodecode/session/tui-sync"
...
function syncVariant(input: { current: string | undefined; message: Message; parts?: readonly { type: string }[] }) {
  if (!ZodeSessionTuiSync.model({ role: input.message.role, parts: input.parts })) return input.current
  return input.message.model?.variant ?? "default"
}

describe("ZodeSessionTuiSync.model", () => {
  test("syncs normal user messages", () => {
    expect(ZodeSessionTuiSync.model({ role: "user", parts: [{ type: "text" }] })).toBe(true)
  })

  test("skips compaction marker user messages", () => {
    expect(ZodeSessionTuiSync.model({ role: "user", parts: [{ type: "compaction" }] })).toBe(false)
  })

  test("skips messages before parts load", () => {
    expect(ZodeSessionTuiSync.model({ role: "user" })).toBe(false)
  })
  ...
  test("skips messages checked with stored parts", () => {
    ...
    expect(ZodeSessionTuiSync.model({ role: msg.role, parts })).toBe(false)
  })

  test("skips non-user messages", () => {
    expect(ZodeSessionTuiSync.model({ role: "assistant", parts: [{ type: "text" }] })).toBe(false)
  })
