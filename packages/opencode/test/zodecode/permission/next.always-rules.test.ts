...
afterAll(async () => {
  ...
  for (const file of ["zode.jsonc", "zode.json", "config.json", "opencode.json", "opencode.jsonc"]) {
    ...
    withDir({ git: true }, () =>
      Effect.gen(function* () {
        const fiber = yield* ask({
          ...
          permission: "bash",
          patterns: ["zode-permission-8353 test"],
          metadata: { rules: ["zode-permission-8353 *", "zode-permission-8353 test"] },
          always: ["zode-permission-8353 *", "zode-permission-8353 test"],
          ruleset: [],
        }).pipe(Effect.forkScoped)
        ...
        yield* saveAlwaysRules({
          ...
          approvedAlways: ["zode-permission-8353 test"],
        })
        ...
        expect(cfg.permission?.bash).toMatchObject({ "zode-permission-8353 test": "allow" })
        expect(cfg.permission?.bash).not.toMatchObject({ "zode-permission-8353 *": "allow" })

        const broad = yield* ask({
          id: PermissionID.make("permission_saved_always_broad"),
          sessionID: SessionID.make("session_saved_always"),
          permission: "bash",
          patterns: ["zode-permission-8353 install"],
          metadata: {},
          always: [],
          ruleset: [],
        }).pipe(Effect.forkScoped)
