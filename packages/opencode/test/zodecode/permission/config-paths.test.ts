...
import { ConfigProtection } from "../../../src/zodecode/permission/config-paths"
...
describe("ConfigProtection.isRequest", () => {
  ...
  const legacy = ZodecodePaths.globalDirs().map((d) => path.resolve(d))
  ...
  test("returns false for file-tool external_directory targeting global config", () => {
    const result = ConfigProtection.isRequest({
      permission: "external_directory",
      patterns: [config + "/*"],
      metadata: { filepath: path.join(config, "zode.json"), parentDir: config },
    })
  ...
  test("returns true for edit targeting root config files", () => {
    for (const file of ["zode.json", "zode.jsonc", "AGENTS.md"]) {
      const result = ConfigProtection.isRequest({
        permission: "edit",
        patterns: [file],
      })
