...
import { McpMigrator } from "../../src/zodecode/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.ZodecodeMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .zode/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".zode")
          await Bun.write(
    ...
    test("reads from legacy .zodecode/mcp.json when .zode/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".zodecode")
          await Bun.write(
            path.join(settingsDir, "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    // Regression: malformed .zodecode/mcp.json must not prevent .zode/mcp.json from loading
    test("loads .zode/mcp.json even when .zodecode/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".zodecode", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".zode", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".zode/mcp.json overrides .zodecode/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".zodecode", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test("does not read project-level .zode/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".zode")
          await Bun.write(
