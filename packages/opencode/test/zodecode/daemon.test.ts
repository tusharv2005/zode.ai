...
import { Daemon } from "../../src/zodecode/daemon/daemon"
import { DaemonClient } from "../../src/zodecode/daemon/client"
import { tmpdir } from "../fixture/fixture"

const original = {
  state: process.env.ZODE_TEST_DAEMON_STATE_DIR,
  log: process.env.ZODE_TEST_DAEMON_LOG_DIR,
  disabled: process.env.ZODE_NO_DAEMON,
}

afterEach(async () => {
  if (process.env.ZODE_TEST_DAEMON_STATE_DIR !== original.state) await Daemon.stop().catch(() => undefined)
  restore()
})

function restore() {
  if (original.state === undefined) delete process.env.ZODE_TEST_DAEMON_STATE_DIR
  else process.env.ZODE_TEST_DAEMON_STATE_DIR = original.state
  if (original.log === undefined) delete process.env.ZODE_TEST_DAEMON_LOG_DIR
  else process.env.ZODE_TEST_DAEMON_LOG_DIR = original.log
  if (original.disabled === undefined) delete process.env.ZODE_NO_DAEMON
  else process.env.ZODE_NO_DAEMON = original.disabled
}

function dirs(root: string) {
  process.env.ZODE_TEST_DAEMON_STATE_DIR = path.join(root, "state")
  process.env.ZODE_TEST_DAEMON_LOG_DIR = path.join(root, "log")
  return {
...
describe("daemon manager", () => {
  ...
  test("does not forward bundled bun entrypoints to the daemon child", () => {
    const proc = {
      argv: ["/tmp/zode", "/$bunfs/root/src/index.js", "daemon", "start"],
      execArgv: ["--user-agent=zode/test", "--use-system-ca", "--"],
      execPath: "/tmp/zode",
    }
    expect(Daemon.command(undefined, proc)).toStrictEqual(["/tmp/zode"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/zode.exe", "B:/~BUN/root/src/index.js", "daemon", "start"],
        execPath: "C:/tmp/zode.exe",
      }),
    ).toStrictEqual(["C:/tmp/zode.exe"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/zode.exe", "b:\\~BUN\\root\\src\\index.js", "daemon", "start"],
  ...
  test("forwards source entrypoints to the daemon child", () => {
    expect(
      Daemon.command(undefined, {
        argv: ["/tmp/bun", "/tmp/zode/src/index.ts", "daemon", "start"],
        execArgv: ["--conditions=browser"],
        execPath: "/tmp/bun",
      }),
    ).toStrictEqual(["/tmp/bun", "--conditions=browser", "/tmp/zode/src/index.ts"])
  })
