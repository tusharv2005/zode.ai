...
`,
      ...
      const client = process.env["ZODE_CLIENT"]
      ...
      try {
        ...
        yield* Effect.promise(() => BackgroundProcess.stopSession(sessionID))
        if (client === undefined) delete process.env["ZODE_CLIENT"]
        else process.env["ZODE_CLIENT"] = client
      }
    }),
  )
...
`,
      ...
      const client = process.env["ZODE_CLIENT"]
      const scans = process.env["ZODE_BACKGROUND_PROCESS_PORTS"]
      process.env["ZODE_CLIENT"] = "cli"
      process.env["ZODE_BACKGROUND_PROCESS_PORTS"] = "true"
      ...
      try {
        ...
        if (scans === undefined) delete process.env["ZODE_BACKGROUND_PROCESS_PORTS"]
        else process.env["ZODE_BACKGROUND_PROCESS_PORTS"] = scans
      }
    }),
  )
...
`,
      ...
      const scans = process.env["ZODE_BACKGROUND_PROCESS_PORTS"]
      process.env["ZODE_CLIENT"] = "cli"
      delete process.env["ZODE_BACKGROUND_PROCESS_PORTS"]
