...
const zode = { id: "zode" } as Provider.Info

// Calculated cost for the `model()` + `baseUsage` pair: 1M input * $3 + 100k output * $15 = 3 + 1.5
const fallback = 3 + 1.5

describe("ZodeSession.providerCost — Anthropic Messages / OpenAI Responses", () => {
...
describe("ZodeSession.providerCost — Vercel AI Gateway", () => {
    ...
    const result = SessionNs.getUsage({
      ...
      metadata: {
        gateway: {
          ...
          // which Zode doesn't pass on to end users — must be ignored.
          cost: "0",
...
describe("ZodeSession.providerCost — fallback", () => {
