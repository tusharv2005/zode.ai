...
import { ZodecodeConfigOverlay } from "../../../src/zodecode/config/overlay"
...
async function config(dir: string, value: unknown) {
  await Bun.write(path.join(dir, "zode.json"), JSON.stringify(value, null, 2))
}
...
describe("config overlay routes", () => {
  test("ignores unsafe patch paths", () => {
    const patched = ZodecodeConfigOverlay.patch({
      scope: "project",
      unset: [
        ["__proto__", "polluted"],
  ...
  test.serial("removes local scalar override and falls back to global", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir({ config: { model: "zode/project-model", username: "alice" } })
    ;(Global.Path as { config: string }).config = global.path
    await config(global.path, { model: "zode/global-model" })
    await invalidate()
  ...
  test.serial("writes project mcp overrides without copying inherited servers", async () => {
    ...
    const saved = (await Bun.file(path.join(project.path, ".zode", "zode.json")).json()) as {
