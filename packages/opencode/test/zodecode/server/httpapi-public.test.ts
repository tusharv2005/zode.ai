...
import { AgentBuilderPaths } from "../../../src/zodecode/server/httpapi/groups/agent-builder"
import { BackgroundProcessPaths } from "../../../src/zodecode/server/httpapi/groups/background-process"
import { ConfigConsolePaths } from "../../../src/zodecode/server/httpapi/groups/config-console"
import { IndexingPaths } from "../../../src/zodecode/server/httpapi/groups/indexing"
import { ZodeGatewayPaths } from "../../../src/zodecode/server/httpapi/groups/zode-gateway"
import { NetworkPaths } from "../../../src/zodecode/server/httpapi/groups/network"
import { TelemetryPaths } from "../../../src/zodecode/server/httpapi/groups/telemetry"
...
describe("Zode PublicApi OpenAPI contract", () => {
  test("uses Zode branding", () => {
    const spec = OpenApi.fromApi(PublicApi)
    expect(spec.info.title).toBe("zode")
    expect(spec.info.description).toBe("zode api")
  })
  ...
  test("keeps directory routing queries on Zode Console routes", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
  ...
  test("keeps workspace routing queries on all Zode-owned routed endpoints", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
  ...
  test("keeps personal organization resets nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[ZodeGatewayPaths.organization]?.post?.requestBody as Body | undefined
    const schema = body?.content?.["application/json"]?.schema
    const props = schema?.properties
    expect(props?.organizationId).toEqual({ anyOf: [{ type: "string" }, { type: "null" }] })
  })

  test("keeps Zode gateway responses nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    ...
    const profile = response(ZodeGatewayPaths.profile)?.properties
    ...
    const sessions = response(ZodeGatewayPaths.cloudSessions)?.properties
    ...
    const claw = response(ZodeGatewayPaths.clawStatus)?.properties
    ...
    expect(response(ZodeGatewayPaths.clawChatCredentials)).toEqual({
  ...
  test("keeps transcription prompts in the public contract", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[ZodeGatewayPaths.audioTranscriptions]?.post?.requestBody as Body | undefined
    const schema = body?.content?.["application/json"]?.schema
