...
export const zodeScenarios: Scenario[] = [
  ...
  http.protected.get("/zode/profile", "zode.profile").probe({ path: "/path" }).status(401),
  http.protected.get("/zode/modes", "zode.modes").json(200, (body) => {
    object(body)
    array(body.modes)
  }),
  http.protected
    .post("/zode/fim", "zode.fim")
    .at((ctx) => ({ path: "/zode/fim", headers: ctx.headers(), body: { prefix: "const value = ", suffix: "\n" } }))
    .status(401),
  http.protected
    .post("/zode/edit", "zode.edit")
    .at((ctx) => ({ path: "/zode/edit", headers: ctx.headers(), body: edit }))
    .status(401),
  http.protected
    .post("/zode/audio/transcriptions", "zode.audio.transcriptions")
    .at((ctx) => ({
      path: "/zode/audio/transcriptions",
      headers: ctx.headers(),
      body: { model: "whisper-1", input_audio: { data: "", format: "wav" } },
    }))
    .status(401),
  http.protected.get("/zode/notifications", "zode.notifications").json(200, array),
  http.protected
    .post("/zode/organization", "zode.organization.set")
    .at((ctx) => ({ path: "/zode/organization", headers: ctx.headers(), body: { organizationId: null } }))
    .status(401),
  http.protected.get("/zode/claw/status", "zode.claw.status").probe({ path: "/path" }).status(401),
  http.protected.get("/zode/claw/chat-credentials", "zode.claw.chatCredentials").probe({ path: "/path" }).status(401),
  http.protected.get("/zode/cloud-sessions", "zode.cloudSessions").probe({ path: "/path" }).status(401),
  http.protected
    .get("/zode/cloud/session/{id}", "zode.cloud.session.get")
    .probe({ path: "/path" })
    .at((ctx) => ({ path: route("/zode/cloud/session/{id}", { id: "httpapi-missing" }), headers: ctx.headers() }))
    .status(401),
  http.protected
    .post("/zode/cloud/session/import", "zode.cloud.session.import")
    .at((ctx) => ({ path: "/zode/cloud/session/import", headers: ctx.headers(), body: { sessionId: "missing" } }))
    .status(401),
  ...
  http.protected
    ...
    .status(400),
  http.protected
    .post("/zodecode/heap/snapshot", "zodecode.heap.snapshot")
    .mutating()
    .jsonEffect(200, (body) =>
      Effect.gen(function* () {
  ...
  http.protected
    .post("/zodecode/skill/remove", "zodecode.removeSkill")
    .mutating()
    .preserveDatabase()
    .seeded((ctx) => file(ctx, ".opencode/skill/httpapi-remove/SKILL.md", "# HTTP API remove\n"))
    .at((ctx) => ({ path: "/zodecode/skill/remove", headers: ctx.headers(), body: { location: ctx.state } }))
  ...
  http.protected
    .post("/zodecode/agent/remove", "zodecode.removeAgent")
    .mutating()
    .seeded((ctx) =>
      file(ctx, ".opencode/agent/httpapi-remove.md", "---\ndescription: HTTP API remove\n---\nRemove me.\n"),
    )
    .at((ctx) => ({ path: "/zodecode/agent/remove", headers: ctx.headers(), body: { name: "httpapi-remove" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
  ...
  http.protected
    .post("/zodecode/session-import/project", "zodecode.sessionImport.project")
    .mutating()
    .at((ctx) => ({
      path: "/zodecode/session-import/project",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/zodecode/session-import/session", "zodecode.sessionImport.session")
    .mutating()
    .seeded((ctx) => ctx.project())
    .at((ctx) => ({
      path: "/zodecode/session-import/session",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/zodecode/session-import/message", "zodecode.sessionImport.message")
    .mutating()
    .seeded((ctx) => ctx.session({ title: "Import message" }))
    .at((ctx) => ({
      path: "/zodecode/session-import/message",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/zodecode/session-import/part", "zodecode.sessionImport.part")
    .mutating()
    .seeded((ctx) =>
      Effect.gen(function* () {
    ...
    .at((ctx) => ({
      path: "/zodecode/session-import/part",
      headers: ctx.headers(),
      body: {
