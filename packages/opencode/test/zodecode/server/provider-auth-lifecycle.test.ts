...
import { invalidateAfterProviderAuthChange } from "../../../src/zodecode/server/provider-auth-lifecycle"
...
it.effect("clears provider models before disposing instances after auth changes", () =>
  Effect.gen(function* () {
    ...
    yield* invalidateAfterProviderAuthChange("zode").pipe(Effect.provide(layer(events)))

    expect(yield* Ref.get(events)).toEqual(["clear:zode", "dispose"])
  }),
)
