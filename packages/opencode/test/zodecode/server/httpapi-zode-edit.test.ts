...
async function send(body: Record<string, unknown> = edit, signal?: AbortSignal) {
  ...
  return app().request(ZodeGatewayPaths.edit, {
    method: "POST",
    headers: { "content-type": "application/json", "x-zode-directory": tmp.path },
...
function authenticate() {
  process.env.ZODE_AUTH_CONTENT = JSON.stringify({
    zode: {
      type: "oauth",
...
describe("HttpApi Zode next edit", () => {
  test("requires Zode Gateway authentication for the Zode-backed model", async () => {
    process.env.ZODE_AUTH_CONTENT = "{}"
  ...
  test("proxies Zode-backed edits with gateway auth and autocomplete headers", async () => {
    authenticate()
