...
import { PlanFollowup } from "../../src/zodecode/plan-followup"
...
describe("plan_exit detection", () => {
        ...
        tools: [
          {
            tool: "plan_exit",
            input: {},
            output: "Plan is ready at .zode/plans/plan.md. Ending planning turn.",
          },
        ],
      })
  ...
  test("ZodeSessionPrompt resolves plan follow-up through the supplied question service", () =>
    withInstance(async () => {
      const seeded = await seed({
        text: "Here is the plan",
        tools: [
      ...
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const pending = ZodeSessionPrompt.askPlanFollowup({
            sessionID: seeded.sessionID,
            messages: seeded.messages,
            abort: AbortSignal.any([]),
            question,
  ...
  test("ZodeSessionPrompt cleans listener-local plan follow-up when aborted outside instance context", () => {
  ...
  test("JetBrains client enables plan follow-up with custom answer", () =>
    withInstance(async () => {
      ...
      try {
        process.env.ZODE_CLIENT = "jetbrains"
        const seeded = await seed({
          text: "Here is the plan",
          tools: [
            {
        ...
        else process.env.ZODE_CLIENT = prev
      }
    }))
  ...
  test("plan reminder reuses custom plan_exit path when refining", () =>
    withInstance(async () => {
      ...
      await ZodeSessionPrompt.insertPlanReminders({
  ...
  test("plan reminder prefers project plan path instructions over fallback", () =>
    withInstance(async () => {
      ...
      expect(text).toContain("Do not choose .zode/plans/")
