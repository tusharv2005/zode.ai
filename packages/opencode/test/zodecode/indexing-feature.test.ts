...
} from "../../src/zodecode/indexing-feature"
...
describe("indexing plugin helpers", () => {
  test("detects plugin-enabled configs", () => {
    ...
    expect(indexingEnabled({ plugin: ["@zodecode/zode-indexing@1.0.0"] })).toBe(true)
  })
  ...
  test("does not add duplicate indexing plugin", () => {
    const list = ensureIndexingPlugin(["@zodecode/zode-indexing@1.0.0"], INDEXING_PLUGIN)
    expect(list).toEqual(["@zodecode/zode-indexing@1.0.0"])
  })
