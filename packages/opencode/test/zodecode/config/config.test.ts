...
import { ZodecodeConfig } from "../../../src/zodecode/config/config"
...
afterEach(async () => {
  delete process.env.ZODE_MD_TEST
  await clear()
...
describe("markdown substitutions", () => {
  test("applies file and env substitutions to parsed markdown body", async () => {
    process.env.ZODE_MD_TEST = "env content"
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, "body.md"), "file content")
        await Filesystem.write(
          path.join(dir, "SKILL.md"),
          ["---", "name: test", "description: Test", "---", "{file:body.md}", "{env:ZODE_MD_TEST}"].join("\n"),
        )
      },
    })
...
describe("zodecode indexing config", () => {
    ...
    try {
      ...
      await WithInstance.provide({
        ...
        fn: async () => {
          ...
          const config = await load()
          const input = ZodeIndexing.input(config.indexing, global.indexing)
  ...
  test("global indexing enabled applies when project indexing is disabled", async () => {
    const input = ZodeIndexing.input({ enabled: false }, { enabled: true })
    expect(input.enabled).toBe(true)
  })
  ...
  test("accepts delete sentinels for indexing model overrides", () => {
    ...
    const merged = ZodecodeConfig.mergeConfig(
      {
        indexing: {
          provider: "openai",
    ...
    const input = ZodeIndexing.input(patch.indexing)
...
describe("agent config", () => {
    ...
    const merged = ZodecodeConfig.mergeConfig(
      {
        agent: {
          explore: {
            model: "zode/anthropic/claude-sonnet-4-6",
            variant: "high",
          },
        },
      },
  ...
  test("removes an agent variant override without removing its model", () => {
    ...
    expect(patch.agent?.explore?.variant).toBeNull()
    expect(merged.agent?.explore).toEqual({ model: "zode/anthropic/claude-sonnet-4-6" })
  })

  test("removes agent model and variant overrides from global JSONC config", async () => {
    await using globalTmp = await tmpdir()
    const file = path.join(globalTmp.path, "zode.jsonc")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
