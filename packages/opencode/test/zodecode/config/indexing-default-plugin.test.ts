...
import { ZodecodeDefaultPlugins } from "../../../src/zodecode/config/default-plugins"
import { INDEXING_PLUGIN } from "../../../src/zodecode/indexing-feature"
...
describe("zodecode default indexing plugin", () => {
  afterEach(async () => {
    await clear()
  ...
  test("injects indexing without registering an external plugin origin", () => {
    ...
    ZodecodeDefaultPlugins.apply(config, { disabled: false })
  ...
  test("removes a persisted indexing marker from external plugin origins", () => {
    ...
    ZodecodeDefaultPlugins.apply(config, { disabled: true })
  ...
  test("does not hard-enable indexing plugin when default plugins are disabled", async () => {
    const prev = process.env["ZODE_DISABLE_DEFAULT_PLUGINS"]
    ...
    try {
      ...
      if (prev === undefined) delete process.env["ZODE_DISABLE_DEFAULT_PLUGINS"]
      else process.env["ZODE_DISABLE_DEFAULT_PLUGINS"] = prev
    }
  })
})
