...
import { ModesMigrator } from "../../src/zodecode/modes-migrator"
...
describe("ModesMigrator", () => {
  ...
  describe("convertPermissions", () => {
    ...
    test("converts fileRegex groups to restricted permissions", () => {
      const groups: ModesMigrator.ZodecodeMode["groups"] = [
        "read",
    ...
    test("handles tuple without fileRegex", () => {
      const groups: ModesMigrator.ZodecodeMode["groups"] = [["edit", {}]]
      const permissions = ModesMigrator.convertPermissions(groups)
  ...
  describe("convertMode", () => {
    test("converts full mode to agent config", () => {
      const mode: ModesMigrator.ZodecodeMode = {
        slug: "translate",
        name: "Translate",
  ...
  - slug: translate
    ...
    groups:
      - read`,
          ...
          // Create project .zodecodemodes (should win)
          await Bun.write(
            path.join(dir, ".zodecodemodes"),
            `customModes:
  - slug: translate
