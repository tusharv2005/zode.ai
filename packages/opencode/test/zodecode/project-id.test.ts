...
import { getZodeProjectId } from "../../src/zodecode/project-id"
...
describe("project-id", () => {
  describe("normalization", () => {
    test("extracts repo name from HTTPS git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin https://github.com/Zode-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from SSH git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin git@github.com:Zode-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from HTTPS URL without .git extension", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/Zode-Org/handbook`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from ssh:// URL", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin ssh://git@github.com/Zode-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("truncates long repo names to 100 characters", async () => {
      ...
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/Zode-Org/${longName}.git`.cwd(dir).quiet()
        },
      })
  ...
  describe("config file priority", () => {
    test("uses project.id from .zode/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          ...
          await Bun.write(
            path.join(dir, ".zode", "config.json"),
            JSON.stringify({
              project: {
    ...
    test("falls back to .zodecode/config.json when .zode/config.json is absent", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".zodecode"), { recursive: true })
          await Bun.write(
            path.join(dir, ".zodecode", "config.json"),
            JSON.stringify({
              project: {
                id: "legacy-project",
              },
    ...
    test("prefers .zode/config.json over .zodecode/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".zode"), { recursive: true })
          await Bun.write(path.join(dir, ".zode", "config.json"), JSON.stringify({ project: { id: "new-project" } }))
          await fs.mkdir(path.join(dir, ".zodecode"), { recursive: true })
          await Bun.write(
            path.join(dir, ".zodecode", "config.json"),
      ...
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.write(
            ...
            JSON.stringify({
              project: {
                id: "https://github.com/Zode-Org/another-repo.git",
              },
            }),
          )
        },
  ...
  describe("fallback behavior", () => {
    ...
    test("handles malformed JSON in config file gracefully", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".zode"), { recursive: true })
          await Bun.write(path.join(dir, ".zode", "config.json"), "{ invalid json")
  ...
  describe("edge cases", () => {
    test("handles git URLs with port numbers", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com:443/Zode-Org/handbook.git`.cwd(dir).quiet()
        },
      })
