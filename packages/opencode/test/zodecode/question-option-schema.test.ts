/**
 * Contract tests for Zode-specific fields on QuestionOption / QuestionInfo.
 *
 * packages/opencode/src/question/index.ts is a shared upstream file.
 * Two Zode additions to the Option schema have been silently dropped by
...
describe("QuestionOption schema — Zode-specific field contract", () => {
  ...
  // Static source checks — guard the zodecode_change markers so a conflict
  // resolution that drops the fields is caught immediately.
  test("source declares mode as an optional field inside a zodecode_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/zodecode_change start[^\n]*hint to UI clients/)
    expect(src).toMatch(/mode:\s*Schema\.optional\(Schema\.String\)/)
    expect(src).toMatch(/zodecode_change end/)
  })

  test("source declares labelKey and descriptionKey inside a zodecode_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/zodecode_change start[^\n]*i18n keys/)
