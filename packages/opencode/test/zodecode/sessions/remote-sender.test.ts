...
import { RemoteSender } from "../../../src/zode-sessions/remote-sender"
import type { RemoteWS } from "../../../src/zode-sessions/remote-ws"
import type { RemoteProtocol } from "../../../src/zode-sessions/remote-protocol"
...
import { Suggestion } from "../../../src/zodecode/suggestion" // zodecode_change
...
describe("RemoteSender", () => {
    ...
    expect(calls).toEqual([
      {
        ...
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderID.make("zode"), modelID: ModelID.make("anthropic/claude-sonnet-4-20250514") },
      },
    ])
  })

  test("send_message keeps zodecode-prefixed model unchanged before internal conversion", async () => {
    const { conn } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      type: "command",
      id: "req_model_zodecode",
      command: "send_message",
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "zodecode/gpt-5-mini",
      },
    })
    ...
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderID.make("zode"), modelID: ModelID.make("gpt-5-mini") },
      },
    ])
  })
  ...
  test("send_message rejects structured model on remote path", () => {
    ...
    sender.handle({
      ...
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: "zodecode", modelID: "gpt-5-mini" },
      },
    })
  ...
  test("send_message does not special-case zode-prefixed model", async () => {
    const { conn, sent } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      ...
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "zode/gpt-5-mini",
      },
    })

    await new Promise((r) => setTimeout(r, 0))

    expect(sent[0]).toEqual({ type: "response", id: "req_model_zode", result: {} })
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderID.make("zode"), modelID: ModelID.make("zode/gpt-5-mini") },
      },
    ])
  })
  // zodecode_change end
