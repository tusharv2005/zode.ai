...
import { ZodeSession } from "../../src/zodecode/session"
    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<ZodeSession.CloseReason>()
          const unsub = yield* bus.subscribeCallback(ZodeSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(ZodeSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("ZodeSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = ZodeSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: ZodeSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = ZodeSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: ZodeSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
