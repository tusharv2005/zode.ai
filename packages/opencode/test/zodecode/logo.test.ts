...
import { plain, session, supports, tui } from "../../src/zodecode/cli/logo"

describe("zodecode logo", () => {
  ...
  test("allows an override", () => {
    expect(supports({ ZODE_UNICODE_LOGO: "1", SSH_TTY: "/dev/pts/0" }, "linux")).toBe(true)
    expect(supports({ ZODE_UNICODE_LOGO: "0" }, "linux")).toBe(false)
  })

  test("uses modern and fallback logo variants", () => {
    expect(tui({ ZODE_UNICODE_LOGO: "1" }, "linux").join("\n")).toContain("🬺🬏")
