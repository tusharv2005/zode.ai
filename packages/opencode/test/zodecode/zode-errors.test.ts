...
import { ZODE_ERROR_CODES, isZodeError, parseZodeErrorCode } from "../../src/zodecode/zode-errors"
...
describe("parseZodeErrorCode", () => {
  ...
  it("extracts PROMOTION_MODEL_LIMIT_REACHED from { code } (top-level)", () => {
    ...
    expect(parseZodeErrorCode(error)).toBe("PROMOTION_MODEL_LIMIT_REACHED")
  })
  ...
  it("returns undefined for non-Zode error codes", () => {
    const error = makeAPIError({
      statusCode: 429,
      responseBody: JSON.stringify({ error: { code: "SOME_OTHER_ERROR" } }),
    })
    expect(parseZodeErrorCode(error)).toBeUndefined()
  })
...
describe("isZodeError", () => {
  it("returns true for PAID_MODEL_AUTH_REQUIRED", () => {
    const error = makeAPIError({
      statusCode: 401,
      responseBody: JSON.stringify({ error: { code: "PAID_MODEL_AUTH_REQUIRED" } }),
    })
    expect(isZodeError(error)).toBe(true)
  })
  ...
  it("returns false for regular 429 errors without Zode code", () => {
    const error = makeAPIError({
      statusCode: 429,
      isRetryable: true,
      message: "Too Many Requests",
    })
    expect(isZodeError(error)).toBe(false)
  })
...
describe("SessionRetry.retryable with Zode errors", () => {
