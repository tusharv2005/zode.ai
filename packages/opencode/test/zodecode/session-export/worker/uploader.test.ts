...
import { Storage } from "@/zodecode/session-export/worker/storage"
import { Uploader, backoffFor } from "@/zodecode/session-export/worker/uploader"
import { Config } from "@/zodecode/session-export/config"
...
describe("Uploader", () => {
  ...
  beforeEach(() => {
    token = process.env.ZODE_SESSION_EXPORT_AUTH_TOKEN
    delete process.env.ZODE_SESSION_EXPORT_AUTH_TOKEN
  ...
  afterEach(() => {
    ...
    if (token === undefined) delete process.env.ZODE_SESSION_EXPORT_AUTH_TOKEN
    else process.env.ZODE_SESSION_EXPORT_AUTH_TOKEN = token
  })

  test("2xx response marks rows uploaded and deletes them", async () => {
    const telemetry: unknown[] = []
    const calls: Array<{ input: string; init: RequestInit }> = []
    process.env.ZODE_SESSION_EXPORT_AUTH_TOKEN = "local-token"
    ...
    expect(headers.get("x-zode-export-api-version")).toBe("1")
    expect(headers.get("x-zode-export-schema-version")).toBe("1")
    expect(headers.get("x-zode-export-agent-version")).toBe("v0")
    expect(headers.get("x-zode-export-root-session-id")).toBe("s1")
    expect(headers.get("x-zode-export-session-id")).toBe("s1")
    expect(headers.get("x-zode-export-seq-start")).toBe("0")
    expect(headers.get("x-zode-export-seq-end")).toBe("0")
    expect(headers.get("x-zode-export-event-count")).toBe("1")
    expect(headers.get("x-zode-export-content-encoding")).toBe("identity")
    expect(headers.get("x-zode-export-client-sent-at")).toMatch(/^\d{4}-\d{2}-\d{2}T/)
    expect(headers.get("x-zode-export-payload-sha256")).toBe(await sha256(body))
  ...
  test("sends anonymous id with all export headers when auth token is absent", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    const names = [
      "x-zode-export-api-version",
      "x-zode-export-schema-version",
      "x-zode-export-agent-version",
      "x-zode-export-surface",
      "x-zode-export-root-session-id",
      "x-zode-export-session-id",
      "x-zode-export-batch-id",
      "x-zode-export-seq-start",
      "x-zode-export-seq-end",
      "x-zode-export-event-count",
      "x-zode-export-payload-sha256",
      "x-zode-export-client-sent-at",
      "x-zode-export-content-encoding",
    ]

    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-zode-anon-id")).toBe("install_abc")
  ...
  test("falls back to telemetry id when anon id is not provided", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-zode-anon-id")).toBe("install_from_disk")
  })
  ...
  test("includes surface in batch metadata and upload headers", async () => {
    ...
    expect(headers.get("x-zode-export-surface")).toBe("vscode-extension")
