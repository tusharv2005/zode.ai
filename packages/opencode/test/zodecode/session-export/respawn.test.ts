...
import { getKillSwitchReason, resetEligibility } from "@/zodecode/session-export/eligibility"

describe("SessionExport worker respawn", () => {
  let feature: string | undefined

  beforeEach(() => {
    feature = process.env.ZODECODE_FEATURE
    resetEligibility()
  })
  ...
  afterEach(async () => {
    ...
    resetEligibility()
    if (feature === undefined) delete process.env.ZODECODE_FEATURE
    else process.env.ZODECODE_FEATURE = feature
  })

  test("passes surface to worker init", () => {
    const workers: FakeWorker[] = []
    process.env.ZODECODE_FEATURE = "cli"
    SessionExport.init({
      agentVersion: "v0",
      dbPath: ":memory:",
      subscribeAll: () => () => {},
