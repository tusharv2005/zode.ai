...
test("compaction export derives root session from ZodeSession", async () => {
  const root = join(import.meta.dir, "../../..")
  const text = await Bun.file(join(root, "src/session/compaction.ts")).text()
  const block = text.slice(
    text.indexOf("// zodecode_change start - export self-contained compaction capture"),
    text.indexOf("yield* prune({ sessionID"),
  )
  expect(block).toContain("ZodeSession.resolveRoot(input.sessionID)")
})

test("llm export does not retain raw stream parts", async () => {
  const root = join(import.meta.dir, "../../..")
  const llm = await Bun.file(join(root, "src/session/llm.ts")).text()
  const events = await Bun.file(join(root, "src/zodecode/session-export/events.ts")).text()
...
test("workspace git subprocesses are hidden on Windows", async () => {
  const root = join(import.meta.dir, "../../..")
  const text = await Bun.file(join(root, "src/zodecode/session-export/workspace-provider.ts")).text()
