...
import { ZodeSessionPromptQueue } from "@/zodecode/session/prompt-queue"
import { Suggestion } from "../../src/zodecode/suggestion"
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      ZodeSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => ZodeSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const ids = await Effect.runPromise(
      ZodeSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          ZodeSessionPromptQueue.retarget(sessionID, injected)
          return ZodeSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)
        }),
        Effect.succeed([]),
      ),
    )
        ...
        const result = await Effect.runPromise(
          ZodeSessionPromptQueue.enqueue(
            ...
            Effect.promise(async () => {
              ...
              return { compact, ids: ZodeSessionPromptQueue.scope(session.id, messages).map((item) => item.info.id) }
            }),
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      ZodeSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: ZodeSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: ZodeSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      ZodeSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: ZodeSessionPromptQueue.hasFollowup(sessionID) })
    ...
    expect(ZodeSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
    ...
    const third = Effect.runPromise(
      ZodeSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: ZodeSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(ZodeSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
          ...
          scoped(tmp.path, async (prompt) => {
            ...
            const ids = await Effect.runPromise(
              ZodeSessionPromptQueue.enqueue(
                ...
                Effect.succeed(ZodeSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
                Effect.succeed([]),
              ),
            )
            expect(ids).toEqual([])
            expect(ZodeSessionPromptQueue.hasFollowup(session.id)).toBe(false)
          }),
      })
    } finally {
      server.stop(true)
