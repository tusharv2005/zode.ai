...
const script = join(import.meta.dir, "..", "..", "bin", "zode")
const platform = process.platform === "win32" ? "windows" : process.platform
const binary = platform === "windows" ? "zode.exe" : "zode"

describe("bin/zode tree-sitter resources", () => {
  async function setup(root: string, nested: boolean) {
    const dir = nested
      ? join(root, "node_modules", "@zodecode", `cli-${platform}-${process.arch}`, "bin")
      : join(root, "node_modules", "@zodecode", "cli", "bin")
    const wasm = join(dir, "tree-sitter")
    const bin = join(dir, nested ? binary : ".zode")
    ...
    return { bin, log, wasm, wrapper: join(dir, "zode") }
  }

  async function run(root: string, bin: string | undefined, log: string, wrapper?: string, failCached?: boolean) {
    const capture = `
const { EventEmitter } = require("events")
const zodeFs = require("fs")
const zodeChild = require("child_process")
const log = process.argv[1]
const wrapper = process.argv[2]
const failCached = process.argv[3] === "true"
const realpathSync = zodeFs.realpathSync
zodeFs.realpathSync = (file) => file === __filename ? wrapper || process.cwd() : realpathSync(file)
zodeChild.spawn = (target) => {
  if (failCached && target.endsWith(".zode")) throw new Error("cached binary failed")
  zodeFs.writeFileSync(log, process.env.ZODE_TREE_SITTER_WASM_DIR || "")
...
`
  ...
  test("exports co-located tree-sitter WASM dir for optional package binary", async () => {
    const root = await mkdtemp(join(tmpdir(), "zode-bin-tree-sitter-"))
    try {
