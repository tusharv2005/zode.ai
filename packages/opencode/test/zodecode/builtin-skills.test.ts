...
import { BUILTIN_SKILLS } from "../../src/zodecode/skills/builtin"
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const skill = yield* Skill.Service
      const item = yield* skill.get("zode-config")
      expect(item).toBeDefined()
      expect(item!.name).toBe("zode-config")
      expect(item!.location).toBe(Skill.BUILTIN_LOCATION)
      expect(item!.content).toContain("zode")
    }),
  { git: true },
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const dir = path.join(instance.directory, ".zode", "skill", "zode-config")
      yield* Effect.promise(() =>
        Bun.write(
          path.join(dir, "SKILL.md"),
          `---
name: zode-config
description: User override of zode-config.
---
...
`,
      ...
      const item = yield* skill.get("zode-config")
      expect(item).toBeDefined()
      expect(item!.description).toBe("User override of zode-config.")
      expect(item!.location).not.toBe(Skill.BUILTIN_LOCATION)
      expect(item!.location).toContain(path.join("skill", "zode-config", "SKILL.md"))
    }),
  { git: true },
)
