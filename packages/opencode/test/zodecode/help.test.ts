...
import { generateHelp, generateCommandTable } from "../../src/zodecode/help"
...
import { ProvidersCommand } from "../../src/cli/cmd/providers" // zodecode_change — upstream renamed auth → providers
...
import { HelpCommand } from "../../src/zodecode/help-command"
import { ProfileCommand } from "../../src/zodecode/cli/cmd/profile"
...
const AttachStub = {
  command: "attach <url>",
  describe: "attach to a running zode server",
  handler() {},
}
...
describe("zode help --all (markdown)", () => {
  test("contains ## heading for each known top-level command", async () => {
    ...
    for (const cmd of ["run", "auth", "debug", "mcp", "session", "agent", "profile"]) {
      expect(output).toContain(`## zode ${cmd}`)
    }
  })

  test("contains headings for nested subcommands", async () => {
    const output = await generateHelp({ all: true, format: "md", commands })
    expect(output).toContain("zode auth login")
    expect(output).toContain("zode auth logout")
    expect(output).toContain("zode debug config")
  })
})

describe("zode help --all (text)", () => {
...
describe("zode help <command>", () => {
  test("zode help auth contains auth subcommand headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).toContain("zode auth login")
    expect(output).toContain("zode auth logout")
    expect(output).toContain("zode auth list")
  })

  test("zode help auth does NOT contain run or debug headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).not.toContain("## zode run")
    expect(output).not.toContain("## zode debug")
  })
})
...
describe("edge cases", () => {
  ...
  test("zode help nonexistent throws unknown command error", async () => {
...
describe("generateCommandTable", () => {
  ...
  test("contains rows for known commands", async () => {
    ...
    for (const name of ["run", "auth", "debug", "mcp"]) {
      expect(output).toContain(`zode ${name}`)
    }
  })

  test("default command appears as zode [project], not $0", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`zode [project]`")
  ...
  test("skips commands with no describe", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).not.toContain("`zode generate`")
  })

  test("contains zode completion row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`zode completion`")
  })

  test("contains zode help row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`zode help")
  })
})
...
describe("commands.ts stays in sync with index.ts", () => {
  test("every .command() in index.ts has an entry in the commands array", async () => {
    ...
    const barrel = await Bun.file(path.resolve(import.meta.dir, "../../src/zodecode/commands.ts")).text()
