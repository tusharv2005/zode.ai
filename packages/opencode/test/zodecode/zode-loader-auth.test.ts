...
// Tests that unauthenticated Zode models are assembled with paid models and autoloaded anonymously.

import { expect } from "bun:test"
import { AppFileSystem } from "@opencode-ai/core/filesystem"
import { Effect, Layer } from "effect"
import { FetchHttpClient } from "effect/unstable/http"
import { zodeCustomLoaders } from "../../src/zodecode/provider/provider"
...
it.live("assembles paid Zode models without auth", () =>
  Effect.gen(function* () {
    const providers = yield* ModelsDev.Service.use((models) => models.get()).pipe(Effect.provide(layer()))
    const zode = Provider.fromModelsDevProvider(providers.zode)

    expect(zode.models["paid-model"]).toMatchObject({
      id: "paid-model",
      providerID: "zode",
      cost: { input: 1, output: 2 },
    })
...
it.live("marks zero-cost Zode models as free when the catalog omits isFree", () =>
  Effect.gen(function* () {
    const providers = yield* ModelsDev.Service.use((models) => models.get()).pipe(Effect.provide(layer()))
    const zode = Provider.fromModelsDevProvider(providers.zode)

    expect(zode.models["free-model"].isFree).toBe(true)
  }),
)
...
it.effect("enables a paid catalog when config apiKey is present", () =>
  Effect.gen(function* () {
    const result = yield* load({ config: { provider: { zode: { options: { apiKey: "test-key" } } } } })
    expect(result.autoload).toBe(true)
