/**
 * Contract test for prompt.ts Zode-specific invariants.
 *
 * prompt.ts is a shared upstream file. The Zode-specific "new prompt unblocks
 * pending suggestions/questions then enqueues without cancelling the in-flight
 * stream" behaviour lives inside a zodecode_change block. An upstream merge
...
describe("prompt.ts Zode-specific invariants", () => {
  test("imports Suggestion from zodecode/suggestion", () => {
    const content = fs.readFileSync(PROMPT_FILE, "utf-8")
    expect(content).toMatch(/import\s*\{[^}]*Suggestion[^}]*\}\s*from\s*["']@\/zodecode\/suggestion["']/)
  })
  ...
  test("dismissAll for suggestions and questions runs before enqueue, without cancelling the in-flight fiber", () => {
    ...
    // Critically, the block must NOT call state.cancel or ZodeSessionPromptQueue.reserve —
    ...
    const block = content.match(
      /zodecode_change start[^\n]*unblock tools[\s\S]*?Suggestion\.dismissAll[\s\S]*?question\.dismissAll[\s\S]*?ZodeSessionPromptQueue\.enqueue/,
    )
    expect(block).not.toBeNull()
    expect(content).not.toMatch(/state\.cancel\(input\.sessionID\)/)
    expect(content).not.toMatch(/ZodeSessionPromptQueue\.reserve/)
  })
  ...
  test("runLoop breaks out between LLM steps when a newer prompt was enqueued", () => {
    ...
    expect(content).toContain("ZodeSessionPromptQueue.hasFollowup(sessionID)")
  })
})
