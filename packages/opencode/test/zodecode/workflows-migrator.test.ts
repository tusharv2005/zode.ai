...
import { WorkflowsMigrator } from "../../src/zodecode/workflows-migrator"
...
Actual description here.`
  ...
  describe("discoverWorkflows", () => {
    test("discovers project workflows", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".zode", "workflows")
    ...
    test("discovers workflows from legacy .zodecode/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("discovers global workflows from ~/.zode/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".zode", "workflows", "global.md"), "# Global\n\nGlobal workflow")
      ...
      expect(
        workflows.some((w) => w.source === "global" && w.path.includes(path.join(".zode", "workflows", "global.md"))),
      ).toBe(true)
    })

    test("applies markdown substitutions to workflow content", async () => {
      process.env.ZODE_WORKFLOW_TEST = "env content"
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".zode", "workflows")
          ...
          await Bun.write(
            ...
            ["# Workflow", "", "{file:../../guide.md}", "{env:ZODE_WORKFLOW_TEST}"].join("\n"),
          )
        },
      })
      ...
      try {
        ...
        delete process.env.ZODE_WORKFLOW_TEST
      }
    })
  })
  ...
  describe("convertToCommand", () => {
    test("converts workflow to command format", () => {
      const workflow: WorkflowsMigrator.ZodecodeWorkflow = {
        name: "code-review",
    ...
    test("project workflows override global workflows with same name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const projectDir = path.join(dir, ".zode", "workflows")
