...
import { ZodeCompactionChunks } from "../../src/zodecode/session/compaction-chunks"
...
describe("ZodeCompactionChunks", () => {
  test("splits oversized history into chronological chunks", async () => {
    ...
    const chunks = await Effect.runPromise(ZodeCompactionChunks.split({ messages, model, size: 2_000 }))
