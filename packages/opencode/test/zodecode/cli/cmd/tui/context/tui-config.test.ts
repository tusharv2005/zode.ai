...
describe("ZodeTuiConfig.makeStore", () => {
  test("reactive reads update when set() reconciles a new config", () => {
    const store = ZodeTuiConfig.makeStore(TuiConfig.resolve(cfg({ keybinds: { app_exit: "ctrl+c" }, theme: "zode" })))
    ...
    expect(themes).toEqual(["zode", "nord"])
