...
const managedConfigDir = process.env.ZODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "zode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "zode.json") {
...
// zodecode_change start
test("preserves Zode provider free model metadata", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://app.kilo.ai/config.json",
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      const config = await load()
      const model = config.provider?.zode?.models?.["free-e2e"]
...
test("updates config and preserves empty shell sentinel", async () => {
  // zodecode_change - upstream hardcodes project config to config.json; Zode writes to zode.json
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "zode.json"))
...
test("updates global config and omits empty shell key in json", async () => {
  // zodecode_change - globalConfigFile() prefers zode.json over opencode.json
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
...
// zodecode_change start
test("prefers .zode directory config over legacy .zodecode", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "zode.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["ZODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ZODE_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["ZODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ZODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("reports warning for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "zode.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .zode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zode")
...
test("loads agents from .zode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .zode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .zode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
// zodecode_change start
test("prefers .zode commands over legacy .zodecode commands", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, ".zodecode", "command", "hello.md"),
        `---
...
Hello from legacy command`,
      ...
      await Filesystem.write(
        path.join(dir, ".zode", "command", "hello.md"),
        `---
...
test("does not try to install dependencies in read-only ZODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ZODE_CONFIG_DIR
  process.env.ZODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ZODE_CONFIG_DIR
    else process.env.ZODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ZODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .zode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".zode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .zode config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .zode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "zode.json"),
        JSON.stringify({
          $schema: "https://app.kilo.ai/config.json",
...
// Note: preload.ts sets ZODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("permission config preserves user key order", async () => {
  ...
  // zodecode_change start — isolate from global config to prevent cross-test contamination
...
// zodecode_change start - regression for `env` alias on local MCP entries
...
test("project config can override MCP server enabled status", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // zodecode_change start — base config in .json, override in .jsonc (jsonc loads second and wins)
...
test("local .zode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .zode directory config enables it
      const opencodeDir = path.join(dir, ".zode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "zode.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "zode.json") // zodecode_change
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.zode/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.zode/plugin/demo.ts", "file:///project/.zode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.zode/plugin/demo.ts"])
  })
...
describe("ZODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ZODE_DISABLE_PROJECT_CONFIG"]
    process.env["ZODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ZODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ZODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .zode/ directories when flag is set", async () => {
    // zodecode_change - test .zode/ directory (the test body writes to .zode/command/)
    const originalEnv = process.env["ZODE_DISABLE_PROJECT_CONFIG"]
    process.env["ZODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .zode directory with a command
          const opencodeDir = path.join(dir, ".zode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await WithInstance.provide({
        ...
        fn: async () => {
          ...
          // Project .zode should NOT be in directories list  // zodecode_change
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ZODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ZODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ZODE_CONFIG_DIR"]
      process.env["ZODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ZODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ZODE_CONFIG_DIR"]
      } else {
        process.env["ZODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ZODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ZODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ZODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ZODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ZODE_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from ZODE_CONFIG_DIR, not project
...
describe("ZODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ZODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ZODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ZODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ZODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ZODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ZODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ZODE_CONFIG_CONTENT"]
