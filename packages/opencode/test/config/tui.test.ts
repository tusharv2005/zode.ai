...
afterEach(async () => {
  delete process.env.ZODE_CONFIG
  delete process.env.ZODE_TUI_CONFIG
  // zodecode_change start
  await fs.rm(path.join(Global.Path.config, "zode.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "zode.jsonc"), { force: true }).catch(() => {})
  // zodecode_change end
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".zode") // zodecode_change
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "zode.json"), // zodecode_change
        JSON.stringify(
          {
      ...
      await Bun.write(
        path.join(local, "zode.json"), // zodecode_change
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".zode"), { recursive: true }) // zodecode_change
      await Bun.write(
        path.join(dir, ".zode", "tui.json"), // zodecode_change
...
test("migrates tui-specific keys from zode.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "zode.json"))) // zodecode_change
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "zode.json.tui-migration.bak"))).toBe(true) // zodecode_change
...
test("migrates project legacy tui keys even when global tui.json already exists", async () => {
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "zode.json")))
  expect(server.theme).toBeUndefined()
  expect(server.tui).toBeUndefined()
})
...
test("skips migration when zode.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "zode.jsonc.tui-migration.bak"))).toBe(false) // zodecode_change
  const source = await Filesystem.readText(path.join(tmp.path, "zode.jsonc")) // zodecode_change
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "zode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "zode.json.tui-migration.bak"))).toBe(false) // zodecode_change
})

test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "zode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "zode.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "zode.jsonc.tui-migration.bak")) // zodecode_change
...
// zodecode_change start
test("migrates legacy tui keys across multiple zode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "zode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "zode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
      // zodecode_change end
    },
  })
...
test("project config takes precedence over ZODE_TUI_CONFIG (matches ZODE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.ZODE_TUI_CONFIG = custom
    },
  })
...
test("ZODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from ZODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "zode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.ZODE_CONFIG = path.join(customDir, "zode.json") // zodecode_change
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
// zodecode_change start
test("loads .zode/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".zode"), { recursive: true })
      await Bun.write(path.join(dir, ".zode", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
      // zodecode_change end
    },
  })
  const config = await getTuiConfig(tmp.path)
