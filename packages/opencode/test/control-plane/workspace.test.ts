...
const originalWorkspacesFlag = Flag.ZODE_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  ZODE_AUTH_CONTENT: process.env.ZODE_AUTH_CONTENT,
...
beforeEach(() => {
  Database.close()
  Flag.ZODE_EXPERIMENTAL_WORKSPACES = true
  restoreEnv()
})
...
afterEach(async () => {
  ...
  Flag.ZODE_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config core.autocrlf false`.cwd(dir).quiet() // zodecode_change - align test repos with Git service patch behavior
...
const winSkip = process.platform === "win32" ? test.skip : test // zodecode_change - git patch application is covered on Linux CI
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.ZODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      ...
      expect(JSON.parse(recorded.calls.create[0].env.ZODE_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.ZODE_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.ZODE_EXPERIMENTAL_WORKSPACES).toBe("true")
  ...
  winSkip("sessionWarp applies source workspace patch to local target workspace", async () => {
    ...
    await withInstance(async (dir) => {
      ...
      await $`git add new.txt`.cwd(previousDir).quiet() // zodecode_change - avoid unrelated untracked patch path
...
describe("workspace sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.ZODE_EXPERIMENTAL_WORKSPACES = false
