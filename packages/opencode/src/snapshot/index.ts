...
import { DiffFull } from "../zodecode/snapshot/diff-full" // zodecode_change
import { ZodeSnapshotTrack } from "../zodecode/snapshot/track" // zodecode_change
import type { MessageID, SessionID } from "../session/schema" // zodecode_change
...
// zodecode_change start - lightweight FileDiff without `patch` for session.summary.diffs (keeps DB payload small)
...
export interface Interface {
  ...
  // zodecode_change start - accept prompt context so slow snapshots can target UI or honor managed caller policy
  readonly track: (opts?: {
    sessionID?: SessionID
    messageID?: MessageID
    snapshotInitialization?: ZodeSnapshotTrack.SnapshotInitialization
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Snapshot.state")(function* (ctx) {
        ...
        const enabled = Effect.fnUntraced(function* () {
          ...
          // zodecode_change start - ACP guard: disable snapshots for ACP clients
          if (Flag.ZODE_CLIENT === "acp") return false
          // zodecode_change end
        ...
        const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
          return yield* locked(
            Effect.gen(function* () {
              ...
              // zodecode_change start — route patches through git (DiffFull.batch) instead of the
    ...
    // zodecode_change start - Snapshot.Service-scoped state for the slow-repo track wrapper
    const trackState = ZodeSnapshotTrack.makeState()
    // zodecode_change end
    ...
    return Service.of({
      ...
      // zodecode_change start - timeout guard with interactive and managed wait policies
      track: Effect.fn("Snapshot.track")(function* (opts) {
        return yield* ZodeSnapshotTrack.wrap({
      ...
      diffFull: Effect.fn("Snapshot.diffFull")(function* (from: string, to: string) {
        // zodecode_change start - cache full diffs at the service boundary
        if (from === to) return []
        const key = `${from}:${to}`
