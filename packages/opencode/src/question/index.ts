...
import { ZodeQuestion } from "@/zodecode/question" // zodecode_change
...
export class Option extends Schema.Class<Option>("QuestionOption")({
  ...
  // zodecode_change start - optional i18n keys so clients can translate while still
  ...
  // zodecode_change end
  // zodecode_change start - hint to UI clients to switch the active agent/mode picker
...
const base = {
  ...
  // zodecode_change start - optional i18n keys for question text and header
...
export interface Interface {
  readonly ask: (input: {
    ...
    blocking?: boolean // zodecode_change
    tool?: Tool
  ...
  readonly dismissAll: (sessionID: SessionID) => Effect.Effect<void> // zodecode_change
}
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Question.ask")(function* (input: {
      ...
      const info = Schema.decodeUnknownSync(Request)({
        ...
        blocking: input.blocking, // zodecode_change
        tool: input.tool,
      })

      // zodecode_change start
      yield* ZodeQuestion.guardFollowup(input.sessionID, () => new RejectedError())
      // zodecode_change end
    ...
    // zodecode_change start - body lives in @/zodecode/question/ZodeQuestion.makeDismissAll
    const dismissAll = ZodeQuestion.makeDismissAll({
      state,
    ...
    return Service.of({ ask, reply, reject, list, dismissAll }) // zodecode_change
  }),
)
