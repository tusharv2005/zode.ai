...
import * as ZodeOAuthCallback from "../zodecode/mcp-oauth-callback" // zodecode_change
...
<head>
  ...
  <title>Zode - Authorization Successful</title>
  <!-- zodecode_change end -->
  <style>
...
<body>
  <div class="container">
    ...
    <!-- zodecode_change start -->
    <p>You can close this window and return to Zode.</p>
    <!-- zodecode_change end -->
  </div>
...
<head>
  <!-- zodecode_change start -->
  <title>Zode - Authorization Failed</title>
  <!-- zodecode_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // zodecode_change start - delegate Zode-specific callback binding from here because OAuth state lives in this module
  await ZodeOAuthCallback.ensureRunning({
    redirectUri,
  ...
  await new Promise<void>((resolve, reject) => {
    // zodecode_change start - EADDRINUSE can still fire when another process
    ...
    server!.listen(currentPort, () => {
      server!.off("error", onError) // zodecode_change
