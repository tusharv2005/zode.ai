...
import { TsCheck } from "../zodecode/ts-check" // zodecode_change
...
// zodecode_change start - tsgo native LSP or lightweight diagnostic client
// When ZODE_EXPERIMENTAL_LSP_TOOL is enabled, spawn tsgo --lsp --stdio as a
...
export const Typescript: Info = {
  ...
  async spawn(root) {
    if (!Flag.ZODE_EXPERIMENTAL_LSP_TOOL) return undefined
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.ZODE_DISABLE_LSP_DOWNLOAD) return
...
export const Ty: Info = {
  ...
  async spawn(root) {
    if (!Flag.ZODE_EXPERIMENTAL_LSP_TY) {
      return undefined
    }
  ...
  async spawn(root) {
    ...
    return {
      process: spawn(
        java,
        [
          "-Djava.import.generatesMetadataFilesAtProjectRoot=false", // zodecode_change
          "-jar",
          launcherJar,
          "-configuration",
          configFile,
