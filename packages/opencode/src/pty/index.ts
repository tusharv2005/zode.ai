...
import { ZodePtySelfCommand } from "@/zodecode/pty/self-command" // zodecode_change
...
export const Info = Schema.Struct({
  ...
  pid: PositiveInt,
  sessionID: Schema.optional(Schema.NullOr(SessionID)), // zodecode_change
})
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Pty.create")(function* (input: CreateInput) {
      ...
      const resolved = ZodePtySelfCommand.resolve(input) // zodecode_change
      const command = resolved.command || Shell.preferred(cfg.shell) // zodecode_change
      const args = resolved.args || [] // zodecode_change
      if (Shell.login(command)) {
        args.push("-l")
      }

      const cwd = resolved.cwd || s.dir // zodecode_change
      ...
      const env = {
        ...
        TERM: "xterm-256color",
        ZODE_TERMINAL: "1",
        ZODE_PTY_ID: id, // zodecode_change
      } as Record<string, string>
      // zodecode_change start
      // Don't leak the zode server's auth credential into user shells.
      ...
      // with zero effort. Users who genuinely need `zode run`/`zode tui
      // attach` to auto-connect from inside a zode-spawned terminal can
      // pass `--password` explicitly or re-export the env themselves.
      delete env.ZODE_SERVER_PASSWORD
      delete env.ZODE_SERVER_USERNAME
      // zodecode_change end
