...
import { matchLegacyZodeOpenApi } from "@/zodecode/server/httpapi/public" // zodecode_change
import * as ZodeServer from "@/zodecode/server/server" // zodecode_change
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /experimental/session worktrees": { type: "boolean" }, // zodecode_change
  "GET /zode/cloud-sessions cursor": { type: "string" }, // zodecode_change
  "GET /zode/cloud-sessions limit": { type: "number" }, // zodecode_change
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for the zode serve command", // zodecode_change
...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  matchLegacyZodeOpenApi(input) // zodecode_change
  return input
}
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  // zodecode_change start - preserve model reset sentinels in indexing config patches
...
function pathParameterSchema(route: string, name: string) {
  ...
  if (name === "processID" && route.includes(" /background-process/")) return { type: "string", pattern: "^bgp.*" } // zodecode_change
  ...
  // /network/* reuses QuestionID (prefix "que"), not a separate brand. // zodecode_change
  if (name === "requestID" && route.startsWith("POST /network/")) return { type: "string", pattern: "^que.*" } // zodecode_change
  return undefined
}

export const PublicApi = OpenCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: ZodeServer.DOC_TITLE, // zodecode_change
    version: "1.0.0",
    description: ZodeServer.DOC_DESCRIPTION, // zodecode_change
