...
import { ZodeSessionHttpApi } from "@/zodecode/server/httpapi/session-fork" // zodecode_change
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const forkRaw = ZodeSessionHttpApi.forkRaw(fork) // zodecode_change - carry upstream bodyless full-session fork support
    ...
    const prompt = Effect.fn("SessionHttpApi.prompt")(function* (ctx: {
      ...
      return HttpServerResponse.stream(
        Stream.fromEffect(
          promptSvc
            // zodecode_change - cast to bridge schema-readonly→PromptInput-mutable; matches legacy Hono session.ts
    ...
    const promptAsync = Effect.fn("SessionHttpApi.promptAsync")(function* (ctx: {
      ...
      // zodecode_change start - cast to bridge schema-readonly→PromptInput-mutable; matches legacy Hono session.ts
      yield* promptSvc
        .prompt({ ...ctx.payload, sessionID: ctx.params.sessionID } as unknown as SessionPrompt.PromptInput)
        .pipe(
    ...
    const viewed = Effect.fn("SessionHttpApi.viewed")(function* (ctx: { payload: typeof ViewedPayload.Type }) {
      const { ZodeSessions } = yield* Effect.promise(() => import("@/zode-sessions/zode-sessions"))
      ZodeSessions.setViewedSessions({ focused: ctx.payload.focused ?? [], open: ctx.payload.open ?? [] })
      return true
    })
    // zodecode_change end
    ...
    return handlers
      ...
      .handleRaw("fork", forkRaw) // zodecode_change - carry upstream bodyless full-session fork support
      .handle("abort", abort)
      .handle("init", init)
      .handle("share", share)
      ...
      .handle("viewed", viewed) // zodecode_change
  }),
)
