...
import { EffectBridge } from "@/effect/bridge" // zodecode_change
...
import { Review } from "@/zodecode/review/review" // zodecode_change
import { WorktreeDiff } from "@/zodecode/review/worktree-diff" // zodecode_change
import { WorktreeFamily } from "@/zodecode/worktree-family" // zodecode_change
import { Worktree } from "@/worktree"
import { Effect, Option } from "effect"
import * as HttpServerResponse from "effect/unstable/http/HttpServerResponse"
import { HttpApiBuilder, HttpApiError } from "effect/unstable/httpapi"
import * as Log from "@opencode-ai/core/util/log" // zodecode_change
...
export const experimentalHandlers = HttpApiBuilder.group(InstanceHttpApi, "experimental", (handlers) =>
  Effect.gen(function* () {
    ...
    // zodecode_change start - worktree diff endpoints for agent manager
    ...
    const session = Effect.fn("ExperimentalHttpApi.session")(function* (ctx: { query: typeof SessionListQuery.Type }) {
      ...
      const sessions = Array.from(
        Session.listGlobal({
          projectID, // zodecode_change
          directory: ctx.query.worktrees ? undefined : ctx.query.directory, // zodecode_change
          directories, // zodecode_change
      ...
      // zodecode_change start - resolve worktree folder name for each session
      const result = sorted
      ...
      return HttpServerResponse.jsonUnsafe(list, {
        headers:
          result.length > limit && list.length > 0 // zodecode_change
