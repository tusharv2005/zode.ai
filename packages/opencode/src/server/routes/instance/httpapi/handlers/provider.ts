...
import { mapValues, pickBy } from "remeda" // zodecode_change
import { ModelCache } from "@/provider/model-cache" // zodecode_change
import { disposeAllInstancesAfterProviderAuthCallback } from "@/zodecode/server/provider-auth-lifecycle" // zodecode_change
  ...
  Effect.gen(function* () {
    ...
    const callback = Effect.fn("ProviderHttpApi.callback")(function* (ctx: {
      ...
      yield* svc
        ...
        .pipe(Effect.catch(() => Effect.fail(new HttpApiError.BadRequest({}))))
      yield* disposeAllInstancesAfterProviderAuthCallback() // zodecode_change
      return true
    })
