...
import type { AssistantMessage, Event, ZodeClient, SessionMessageResponse, ToolPart } from "@zodecode/sdk/v2"
import { applyPatch } from "diff"
import { InstallationVersion } from "@opencode-ai/core/installation/version"

import { fetchDefaultModel } from "@zodecode/zode-gateway" // zodecode_change
...
export function init({ sdk: _sdk }: { sdk: ZodeClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `zode auth login` in the terminal",
      name: "Login with Zode",
      id: "zode-login",
    }
    // zodecode_change end
    ...
    if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
      authMethod._meta = {
        "terminal-auth": {
          command: "opencode",
          args: ["auth", "login"],
          label: "Zode Login", // zodecode_change
        },
      }
    }
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const zodeProvider = providers.find((p) => p.id === "zode")
  if (zodeProvider) {
    const [best] = Provider.sort(Object.values(zodeProvider.models))
    if (best) {
      return {
  ...
  // zodecode_change start - prefer zode free-tier default before erroring.
  // Only fall back to the Zode provider if it was present in the available
  ...
  if (providers.some((p) => p.id === "zode")) {
    const freeModel = await fetchDefaultModel()
    return { providerID: ProviderID.zode, modelID: ModelID.make(freeModel) }
  }
  // zodecode_change end
