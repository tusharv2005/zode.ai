...
} from "@zodecode/zode-indexing/engine"
import type { IndexingStatus } from "@zodecode/zode-indexing/status"
import { withTimeout } from "@/util/timeout"
import type { Message, Request, Result } from "./indexing-worker-protocol"

declare global {
  const ZODE_INDEXING_WORKER_PATH: string
}
...
export namespace IndexingWorker {
  ...
  const worker = (directory: string, root: string, hooks: Hooks): Driver => {
    const file =
      typeof ZODE_INDEXING_WORKER_PATH !== "undefined"
        ? ZODE_INDEXING_WORKER_PATH
      ...
      init(config) {
        const request: Request = {
          ...
          input: {
            ...
            root,
            config,
            lancedbPath: process.env.ZODE_LANCEDB_PATH,
          },
        }
