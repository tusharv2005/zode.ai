...
export namespace ZodecodePaths {
  const home = () => process.env.HOME || process.env.USERPROFILE || os.homedir()

  /**
   * Get the platform-specific VSCode global storage path for Zodecode extension.
   * - macOS: ~/Library/Application Support/Code/User/globalStorage/zodecode.zode-code
   * - Windows: %APPDATA%/Code/User/globalStorage/zodecode.zode-code
   * - Linux: ~/.config/Code/User/globalStorage/zodecode.zode-code
   */
  export function vscodeGlobalStorage(): string {
    const home = os.homedir()
    switch (process.platform) {
      case "darwin":
        return path.join(home, "Library", "Application Support", "Code", "User", "globalStorage", "zodecode.zode-code")
      case "win32":
        return path.join(
      ...
      default:
        return path.join(home, ".config", "Code", "User", "globalStorage", "zodecode.zode-code")
    }
  }

  /** Global Zode directories in user home: ~/.zodecode and ~/.zode (legacy first, .zode wins later) */
  export function globalDirs(): string[] {
    return [path.join(home(), ".zodecode"), path.join(home(), ".zode")]
  }

  /**
   * Discover Zode directories containing skills.
   * Returns parent directories (.zodecode/ and .zode/) for glob pattern "skills/[*]/SKILL.md".
   *
   * - Walks up from projectDir to worktreeRoot for .zodecode/ and .zode/
   * - Includes global ~/.zodecode/ and ~/.zode/
  ...
  export async function skillDirectories(opts: {
    ...
    if (!opts.skipGlobalPaths) {
      // 1. Global ~/.zodecode/ and ~/.zode/ (loaded first so project-level overrides)
    ...
    // 3. Walk up from project dir to worktree root for .zodecode/ and .zode/
    ...
    for (const target of [".zodecode", ".zode"] as const) {
