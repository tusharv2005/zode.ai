...
/**
 * Zode-specific home footer plugin.
 *
 ...
 */
import type { TuiPlugin, TuiPluginApi, TuiPluginModule } from "@zodecode/plugin/tui"
...
const id = "internal:zode-home-footer"
...
// ---------------------------------------------------------------------------
// RemoteIndicator – adapted from @/zodecode/remote-tui for plugin API usage
// ---------------------------------------------------------------------------

function RemoteIndicator(props: { api: TuiPluginApi; zode: boolean }) {
  ...
  onMount(() => {
    void props.api.client.remote
      ...
      .catch(() => undefined)
    const off = props.api.event.on("zode-sessions.remote-status-changed", (evt) => setStatus(evt.properties))
    onCleanup(off)
  })
...
// ---------------------------------------------------------------------------
// Sub-components (mirror upstream home/footer with zode additions)
// ---------------------------------------------------------------------------
...
function View(props: { api: TuiPluginApi }) {
  const zode = createMemo(() => props.api.state.provider.some((p) => p.id === "zode"))
  const theme = () => props.api.theme.current
  const sync = useSync()
  ...
  return (
    ...
    >
      ...
      <box gap={1} flexDirection="row" flexShrink={0}>
        <RemoteIndicator api={props.api} zode={zode()} />
        <Mcp api={props.api} />
