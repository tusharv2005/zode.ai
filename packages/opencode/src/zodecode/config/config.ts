...
import { fetchOrganizationModes } from "@zodecode/zode-gateway"
...
export namespace ZodecodeConfig {
  const log = Log.create({ service: "zodecode.config" })
  ...
  /** Zode-specific config file names (highest-to-lowest precedence within zode). */
  export const ZODE_CONFIG_FILES = ["zode.jsonc", "zode.json"] as const

  /** All config file names in precedence order (zode + opencode). */
  export const ALL_CONFIG_FILES = ["zode.jsonc", "zode.json", "opencode.jsonc", "opencode.json"] as const

  /** Directory suffixes that Zode recognizes in addition to .opencode. */
  export const ZODE_DIR_SUFFIXES = [".zode", ".zodecode"] as const

  /** All config directory suffixes Zode can update, including upstream .opencode. */
  export const ALL_CONFIG_DIR_SUFFIXES = [".zode", ".zodecode", ".opencode"] as const

  /** Path patterns for resolving zode agent names from file paths. */
  export const AGENT_PATTERNS = ["/.zode/agent/", "/.zode/agents/", "/.zodecode/agent/", "/.zodecode/agents/"] as const

  /** Path patterns for resolving zode command names from file paths. */
   ...
   *
   * This mirrors the Zode project-config load chain: prefer existing config files
   * in ancestor config directories, then existing root config files, and create
   * `.zode/zode.json` when no project config exists yet.
   */
  export const projectConfigUpdateTarget = Effect.fn("ZodecodeConfig.projectConfigUpdateTarget")(function* (input: {
    ...
    return files.find((file) => existsSync(file)) ?? path.join(input.directory, ".zode", "zode.json")
  })

  export const updateProjectConfig = Effect.fn("ZodecodeConfig.updateProjectConfig")(function* (input: {
    fs: AppFileSystem.Interface
    directory: string
    worktree?: string
    config: Config.Info
  ...
  /**
   * Load all Zodecode legacy configs (modes, workflows, rules, MCP, ignore).
  ...
  export async function loadLegacyConfigs(input: {
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        ...
        log.debug("loaded zodecode custom modes", {
          count: Object.keys(migration.agents).length,
          modes: Object.keys(migration.agents),
        })
      }
      for (const skipped of migration.skipped) {
        log.debug("skipped zodecode mode", { slug: skipped.slug, reason: skipped.reason })
      }
    } catch (err) {
      log.warn("failed to load zodecode modes", { error: err })
    }

    // Load Zodecode workflows as commands
    try {
      ...
      if (Object.keys(migration.commands).length > 0) {
        ...
        log.debug("loaded zodecode workflows as commands", {
          count: Object.keys(migration.commands).length,
          commands: Object.keys(migration.commands),
        })
      }
    } catch (err) {
      log.warn("failed to load zodecode workflows", { error: err })
    }
    ...
    try {
      ...
      if (migration.instructions.length > 0) {
        ...
        log.debug("loaded zodecode rules", {
          count: migration.instructions.length,
          files: migration.instructions,
        })
      }
      for (const warning of migration.warnings) {
        log.debug("zodecode rules warning", { warning })
      }
    } catch (err) {
      log.warn("failed to load zodecode rules", { error: err })
    }

    // Load Zodecode MCP servers (skip global VSCode extension paths unless running in an editor or Console daemon)
    const skipGlobal = process.env["ZODE_PLATFORM"] !== "vscode" && process.env["ZODECODE_FEATURE"] !== "daemon"
    const mcp = await McpMigrator.loadMcpConfig(input.projectDir, skipGlobal)
    if (Object.keys(mcp).length > 0) {
      result = input.merge(result, { mcp })
    }

    // Load .zodecodeignore patterns
    try {
      const permission = await IgnoreMigrator.loadIgnoreConfig(input.projectDir)
      if (Object.keys(permission).length > 0) {
        result = input.merge(result, { permission })
        log.debug("loaded zodecode ignore patterns", {
          hasRead: !!(permission as Record<string, unknown>).read,
          hasEdit: !!(permission as Record<string, unknown>).edit,
        })
      }
    } catch (err) {
      log.warn("failed to load zodecode ignore patterns", { error: err })
    }
  ...
  /**
   * Load organization custom modes from the Zode Cloud API.
  ...
  export async function loadOrganizationModes(
    ...
    const warnings: Config.Warning[] = []
    try {
      const zode = auth["zode"]
      if (zode?.type === "oauth" && zode.access && zode.accountId) {
        const modes = await fetchOrganizationModes(zode.access, zode.accountId)
        if (modes.length > 0) {
  ...
  const GLOBAL_CONFIG_FILES = ["config.json", "zode.json", "zode.jsonc", "opencode.json", "opencode.jsonc"]
  ...
  export function isConfigDir(dir: string, flagDir?: string): boolean {
    return dir.endsWith(".zode") || dir.endsWith(".zodecode") || dir.endsWith(".opencode") || dir === flagDir
  }
}
