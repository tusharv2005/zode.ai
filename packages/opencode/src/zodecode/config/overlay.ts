...
import { ZodecodeConfigSources } from "./sources"

export namespace ZodecodeConfigOverlay {
  ...
  export const Result = z.object({
    ...
    sources: z.array(ZodecodeConfigSources.Source),
    targets: z.object({
  ...
  export type Input = {
    ...
    global: Config.Info
    sources: ZodecodeConfigSources.Source[]
  }

  const files = ["zode.jsonc", "zode.json", "opencode.jsonc", "opencode.json"] as const
  const dirs = [".zode", ".zodecode", ".opencode"] as const
  ...
  export async function project(input: { directory: string; worktree?: string }): Promise<Config.Info> {
    ...
    return configs.reduce((result, cfg) => ZodecodeConfig.mergeConfig(result, cfg), {} as Config.Info)
  }
  ...
  export async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return candidates.find((file) => existsSync(file)) ?? path.join(input.directory, ".zode", "zode.json")
  }
  ...
  function globalDirs() {
    return [
      Global.Path.config,
      path.join(Global.Path.home, ".zodecode"),
      path.join(Global.Path.home, ".zode"),
  ...
  async function withAgents(input: Config.Info, dirs: string[]): Promise<Config.Info> {
    ...
    const next = ZodecodeConfig.mergeConfig(ZodecodeConfig.mergeConfig(input, { agent }), { agent: mode })
