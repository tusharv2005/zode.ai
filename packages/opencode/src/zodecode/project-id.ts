...
/**
 * Read project ID from .zode/config.json, falling back to .zodecode/config.json
...
async function getProjectIdFromConfig(directory: string): Promise<string | undefined> {
  ...
  for (const dir of [".zode", ".zodecode"]) {
...
/**
 * Resolve project ID with priority: .zode/config.json -> .zodecode/config.json -> git origin URL
 * @returns Normalized project ID or undefined
 */
async function resolveProjectId(): Promise<string | undefined> {
  const dir = Instance.directory

  // Priority 1: .zode/config.json (falls back to .zodecode/config.json)
...
export namespace ZodeProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@zodecode/ZodeProjectID") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const state = yield* InstanceState.make(
        Effect.fn("ZodeProjectID.state")(function* () {
...
const { runPromise } = makeRuntime(ZodeProjectID.Service, ZodeProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getZodeProjectId(): Promise<string | undefined> {
