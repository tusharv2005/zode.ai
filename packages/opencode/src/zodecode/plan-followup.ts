...
export const PlanFollowupRuntime = {
  ...
  handover(input: LLM.StreamInput, signal: AbortSignal) {
    return llm().runPromise((svc) => ZodeLLM.text(svc.stream(input)).pipe(Effect.orDie), { signal })
  },
      ...
      questions: [
        {
          ...
          custom: Flag.ZODE_CLIENT === "cli" || Flag.ZODE_CLIENT === "jetbrains",
          options: [
            {
