...
export namespace BackgroundProcess {
  ...
  class StateService extends Context.Service<StateService, { readonly get: () => Effect.Effect<State> }>()(
    "@zodecode/BackgroundProcess.State",
  ) {}
  ...
  function infer() {
    return Flag.ZODE_CLIENT === "cli" && process.env.ZODE_BACKGROUND_PROCESS_PORTS === "true"
  }
  ...
  function env() {
    ...
    delete result.ZODE_SERVER_PASSWORD
    delete result.ZODE_SERVER_USERNAME
    delete result.ZODE_BACKGROUND_PROCESS_PORTS
    return result
  }
