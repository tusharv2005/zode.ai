...
export namespace McpMigrator {
  const log = Log.create({ service: "zodecode.mcp-migrator" })

  // Remote transport types used by the Zodecode extension
  const REMOTE_TYPES = new Set(["streamable-http", "sse"])

  function isRemote(server: ZodecodeMcpServer): boolean {
    return !!server.type && REMOTE_TYPES.has(server.type)
  }

  // Zodecode MCP server structure
  export interface ZodecodeMcpServer {
    command?: string
    args?: string[]
  ...
  export interface ZodecodeMcpSettings {
    mcpServers: Record<string, ZodecodeMcpServer>
  }
  ...
  export async function readMcpSettings(filepath: string): Promise<ZodecodeMcpSettings | null> {
    if (!(await Filesystem.exists(filepath))) return null

    try {
      const content = await fs.readFile(filepath, "utf-8")
      return JSON.parse(content) as ZodecodeMcpSettings
    } catch (err) {
  ...
  export function convertServer(name: string, server: ZodecodeMcpServer): ConfigMCP.Info | null {
    if (isRemote(server)) {
      if (!server.url) {
  ...
  export async function migrate(options?: {
    ...
    const allServers: Array<{ name: string; server: ZodecodeMcpServer }> = []

    if (!options?.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global MCP settings)
      const vscodeSettingsPath = path.join(ZodecodePaths.vscodeGlobalStorage(), "settings", "mcp_settings.json")
    ...
    // Check .zode/mcp.json and .zodecode/mcp.json for project-level settings
    // (not "mcp_settings.json" which is only used for global settings)
    // .zodecode is loaded first (lower precedence), .zode second (higher precedence)
    if (options?.projectDir) {
      for (const dir of [".zodecode", ".zode"]) {
    ...
    const serversByName = new Map<string, ZodecodeMcpServer>()
  ...
  /**
   * Load Zodecode MCP servers and return them as an opencode config partial.
  ...
  export async function loadMcpConfig(
    ...
    try {
      ...
      if (Object.keys(result.mcp).length > 0) {
        log.debug("loaded zodecode MCP servers", {
      ...
      for (const skipped of result.skipped) {
        log.debug("skipped zodecode MCP server", { name: skipped.name, reason: skipped.reason })
      }

      for (const warning of result.warnings) {
        log.warn("zodecode MCP migration warning", { warning })
      }

      return result.mcp
    } catch (err) {
      log.warn("failed to load zodecode MCP servers", { error: err })
      return {}
    }
  }
}
