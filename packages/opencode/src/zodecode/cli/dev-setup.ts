...
const MARKER_START = "# >>> zodedev launcher >>>"
const MARKER_END = "# <<< zodedev launcher <<<"

export const DevSetupCommand = cmd({
  command: "dev-setup",
  describe: "install a `zodedev` shell alias for this checkout",
  builder: (y) =>
    y
      .option("shell", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}Zode CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}zodedev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `zode dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "zodedev")
  const bat = path.join(repo, "bin", "zodedev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias zodedev ${q(sh)}`
  if (shell === "powershell") return `function zodedev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias zodedev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.zodedev.bak.${stamp()}`
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.ZODE_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/opencode/dist/<target>/bin/zode, so
  ...
  throw new Error(
    "cannot locate zodecode source checkout; set ZODE_DEV_REPO=/path/to/zodecode or run ./bin/zodedev dev-setup from the repo",
  )
}
