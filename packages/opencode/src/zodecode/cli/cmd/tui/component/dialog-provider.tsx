...
/**
 * Zode-specific overrides for the provider dialog.
 *
...
import type { ProviderAuthAuthorization } from "@zodecode/sdk/v2"
import { ZodeAutoMethod } from "@/zodecode/components/dialog-zode-auto-method"
...
/**
 * If the provider is Zode Gateway, renders the custom `ZodeAutoMethod`
...
export function renderAutoMethod(opts: {
  ...
  if (opts.providerID !== "zode") return undefined
  return () => (
    <ZodeAutoMethod
 ...
 * provider is Zode Gateway. Returns `undefined` otherwise.
 */
export function renderApiDescription(
  ...
  if (providerID !== "zode") return undefined
  return () => (
    <box gap={1}>
      <text fg={theme.textMuted}>
