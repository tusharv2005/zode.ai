...
import { fetchBalance, fetchProfile, type ZodecodeBalance, type ZodecodeProfile } from "@zodecode/zode-gateway"
...
interface Args {
  ...
  getProfile?: (token: string) => Promise<ZodecodeProfile>
  getBalance?: (token: string, organizationId?: string) => Promise<ZodecodeBalance | null>
...
export const ProfileCommand = cmd({
  command: "profile",
  describe: "show Zode account profile",
  builder: (yargs: Argv) =>
    yargs.option("json", {
