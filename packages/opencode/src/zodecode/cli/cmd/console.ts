...
export const ZodeConsoleCommand = cmd({
  command: "console",
  describe: "open the local Zode Console",
  builder: (yargs) => withNetworkOptions(yargs),
  handler: async (args) => {
    const opts = await AppRuntime.runPromise(resolveNetworkOptions(args))
    const result = await Daemon.start(opts)
    const state = result.state
    if (!state) throw new Error("Zode daemon did not provide connection state")
