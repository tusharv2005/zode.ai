...
export function matchLegacyZodeOpenApi(input: Record<string, unknown>) {
  rebrand(input)
  ...
  const body = spec.paths?.["/zode/organization"]?.post?.requestBody?.content?.["application/json"]?.schema
  ...
  const profile = json("/zode/profile")?.schema?.properties
  ...
  const sessions = json("/zode/cloud-sessions")?.schema?.properties
  ...
  const claw = json("/zode/claw/status")?.schema?.properties
  ...
  const credentials = json("/zode/claw/chat-credentials")
  ...
  const fim = spec.paths?.["/zode/fim"]?.post?.responses
  if (!fim) return
  fim["200"] = {
