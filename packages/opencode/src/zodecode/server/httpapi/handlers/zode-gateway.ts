...
import {
  ...
  clearModesCache,
  fetchBalance,
  fetchZodecodeNotifications,
  fetchOrganizationModes,
  fetchProfile,
} from "@zodecode/zode-gateway"
import { DIRECT_FIM_ENV, requestMistralFim, resolveFimTarget } from "@zodecode/zode-gateway/fim"
import { DIRECT_EDIT_ENV, extractFencedBody, resolveEditTarget } from "@zodecode/zode-gateway/edit"
import { buildMercuryEditPrompt } from "@zodecode/zode-gateway/edit-prompt"
import { buildZodeHeaders } from "@zodecode/zode-gateway"
...
import { AudioTranscriptionsBody, ClawStatus, EditBody, FimBody } from "../groups/zode-gateway"

const FIM_TIMEOUT_MS = 30_000
const log = Log.create({ service: "zode-gateway" })
...
export const zodeGatewayHandlers = HttpApiBuilder.group(InstanceHttpApi, "zode", (handlers) =>
  Effect.gen(function* () {
    const auth = yield* Auth.Service
    const store = yield* InstanceStore.Service
    const cache = yield* ModelCache.Service

    const profile = Effect.fn("ZodeGatewayHttpApi.profile")(function* () {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
    ...
    const proxyAuth = Effect.fn("ZodeGatewayHttpApi.proxyAuth")(function* () {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return {
        auth: info,
        token: getToken(info),
    ...
    const modes = Effect.fn("ZodeGatewayHttpApi.modes")(function* () {
    ...
    const fim = Effect.fn("ZodeGatewayHttpApi.fim")(function* (ctx: { payload: typeof FimBody.Type }) {
      const target = resolveFimTarget(ctx.payload.provider, ctx.payload.model)
      const info = target.provider === "zode" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "zode") return info?.token
      ...
      if (target.provider === "zode" && !info?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
          ...
          const run = async (url: string): Promise<Response> => {
            ...
            return fetch(url, {
              ...
              headers: {
                ...
                ...(target.provider === "zode"
                  ? buildZodeHeaders(undefined, { zodecodeOrganizationId: info?.organizationId })
                  : {}),
                ...(target.provider === "zode" ? { [HEADER_FEATURE]: "autocomplete" } : {}),
              },
              signal,
              body: JSON.stringify({
                model: target.model,
    ...
    const edit = Effect.fn("ZodeGatewayHttpApi.edit")(function* (ctx: { payload: typeof EditBody.Type }) {
      const target = resolveEditTarget(ctx.payload.provider, ctx.payload.model)
      if (target.provider === "zode" && !target.url) {
        return yield* Effect.fail(new HttpApiError.BadRequest({}))
      }
      const proxy = target.provider === "zode" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "zode") return proxy?.token
        const item = yield* auth.get(target.provider).pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
        if (item?.type === "api") return item.key
        return DIRECT_EDIT_ENV[target.provider].map((key) => process.env[key]).find(Boolean)
      })
      if (target.provider === "zode" && !proxy?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const audioTranscriptions = Effect.fn("ZodeGatewayHttpApi.audioTranscriptions")(function* (ctx: {
      ...
      const response = yield* Effect.tryPromise({
        try: () =>
          fetch(`${ZODE_API_BASE}/api/gateway/v1/audio/transcriptions`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    const notifications = Effect.fn("ZodeGatewayHttpApi.notifications")(function* () {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
    ...
    const organization = Effect.fn("ZodeGatewayHttpApi.organization")(function* (ctx) {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      yield* cache.clear("zode")
      clearModesCache()
      yield* store.disposeAll().pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return true
    })

    const clawStatus = Effect.fn("ZodeGatewayHttpApi.clawStatus")(function* () {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.ServiceUnavailable({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
        ...
        Effect.match({
          onFailure: (err) => {
            if (err instanceof GatewayError)
              return jsonError(`ZodeClaw request failed: ${err.status} ${err.message}`, err.status)
            logError("claw/status", err)
            return jsonError("Failed to reach ZodeClaw", 502)
          },
    ...
    const clawChatCredentials = Effect.fn("ZodeGatewayHttpApi.clawChatCredentials")(function* () {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessions = Effect.fn("ZodeGatewayHttpApi.cloudSessions")(function* (ctx) {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSession = Effect.fn("ZodeGatewayHttpApi.cloudSession")(function* (ctx) {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessionImport = Effect.fn("ZodeGatewayHttpApi.cloudSessionImport")(function* (ctx) {
      const info = yield* auth.get("zode").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
