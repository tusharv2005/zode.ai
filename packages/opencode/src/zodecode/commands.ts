...
import { ProvidersCommand } from "../cli/cmd/providers" // zodecode_change — upstream renamed auth → providers
...
import { ZodeConsoleCommand } from "./cli/cmd/console"
...
// from `zode help --all` and the docs table. They're also guarded the same way
// at the yargs registration site in src/index.ts, so the commands-in-sync
// regex in test/zodecode/help.test.ts sees DevSetup/DevAlias on neither side.
...
export const commands = [
  ...
  GenerateCommand,
  DebugCommand,
  ProvidersCommand, // zodecode_change — upstream renamed AuthCommand → ProvidersCommand
  AgentCommand,
  UpgradeCommand,
  UninstallCommand,
  ServeCommand,
