...
export namespace Daemon {
  ...
  function root() {
    return process.env.ZODE_TEST_DAEMON_STATE_DIR ?? Global.Path.state
  }

  function logs() {
    return process.env.ZODE_TEST_DAEMON_LOG_DIR ?? Global.Path.log
  }
    ...
    try {
      const child = spawn(cmd[0], [...cmd.slice(1), ...args(input)], {
        ...
        env: {
          ...process.env,
          ...input.env,
          ZODE_SERVER_USERNAME: username,
          ZODE_SERVER_PASSWORD: password,
          ZODECODE_FEATURE: "daemon",
        },
  ...
  async function line(out: string) {
    ...
    const match = text.match(/zode server listening on http:\/\/([^:\s]+):(\d+)/)
