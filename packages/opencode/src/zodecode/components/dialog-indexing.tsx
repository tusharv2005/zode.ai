...
import { formatZodeEmbeddingModelLabel } from "@zodecode/zode-indexing/embedding-models"
import { fetchZodeEmbeddingModelCatalog } from "@zodecode/zode-gateway"
import { useSync } from "@tui/context/sync"
import { useToast } from "@tui/ui/toast"
import { createResource, Show } from "solid-js"
import { reconcile } from "solid-js/store"
import type { IndexingConfig, Config } from "@zodecode/sdk/v2"
import { hasZodeIndexingAuth, resolveZodeIndexingAuth, shouldDefaultIndexingToZode } from "../indexing-auth"
...
function hasZodeAuth(sync: ReturnType<typeof useSync>): boolean {
  const provider = sync.data.provider_next.all.find((item) => item.id === "zode")
  return hasZodeIndexingAuth({ config: sync.data.config, provider })
}
...
function defaultIndexing(sync: ReturnType<typeof useSync>, global?: IndexingConfig): IndexingConfig {
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "zode")
  const auth = resolveZodeIndexingAuth({ config: sync.data.config, provider })
  if (!shouldDefaultIndexingToZode({ ...global, ...indexing }, auth)) return indexing
  return { ...indexing, provider: "zode", model: null, dimension: null }
}
...
function providerSettingsDescription(
  ...
  if (provider === "zode") return hasZodeAuth(sync) ? "uses Zode account" : "sign in to Zode"
...
function ProviderSelect(props: SubDialogProps) {
  ...
  const options: DialogSelectOption<EmbeddingProvider>[] = (
    ...
    .filter(([value]) => value !== "zode" || hasZodeAuth(sync) || indexing.provider === "zode")
    .map(([value, title]) => ({
      value,
      title,
...
function ZodeModelSelect(props: SubDialogProps) {
  const dialog = useDialog()
  const sync = useSync()
  const sdk = props.useSDK()
  const toast = useToast()
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "zode")
  const auth = resolveZodeIndexingAuth({ config: sync.data.config, provider })
  const [catalog] = createResource(() => fetchZodeEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey }))
  const options = () =>
    (catalog()?.models ?? []).map((model) => ({
      value: model.id,
      title: formatZodeEmbeddingModelLabel(model),
    }))
  const current = () => {
    const cfg = catalog()
...
export function DialogIndexing(props: DialogIndexingProps) {
  ...
  const options: DialogSelectOption<string>[] = [
    ...
    {
      value: "model",
      title: indexing.provider === "zode" ? "Zode Model Preset" : "Embedding Model",
      category: "Embedding",
      description: indexing.provider === "zode" ? "hosted presets only" : (indexing.model ?? "default"),
    },
    {
      value: "dimension",
      ...
      onSelect={async (option) => {
        switch (option.value) {
          case "globalToggle": {
            ...
            const updated =
              enabled && !globalCfg().provider && !getIndexing(sync).provider && hasZodeAuth(sync)
          ...
          case "projectToggle": {
            ...
            const updated =
              enabled && !current.provider && hasZodeAuth(sync) ? { ...defaultIndexing(sync), enabled } : { enabled }
          ...
          case "providerSettings":
            ...
            break
          case "model": {
            if (indexing.provider === "zode") {
              dialog.replace(() => <ZodeModelSelect useSDK={props.useSDK} />)
              break
            }
