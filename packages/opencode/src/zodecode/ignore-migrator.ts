...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "zodecode.ignore-migrator" })

  const ZODECODEIGNORE_FILE = ".zodecodeignore"
  const GLOBAL_ZODECODEIGNORE = path.join(os.homedir(), ".zodecode", ZODECODEIGNORE_FILE)
  ...
  /**
   * Parse .zodecodeignore content into patterns.
  ...
  /**
   * Load patterns from a .zodecodeignore file
   */
  ...
  /**
   * Migrate .zodecodeignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .zodecodeignore (lower priority)
    if (!options.skipGlobalPaths) {
      const globalPatterns = await loadIgnoreFile(GLOBAL_ZODECODEIGNORE, "global")
      allPatterns.push(...globalPatterns)

      if (globalPatterns.length > 0) {
        log.debug("loaded global .zodecodeignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .zodecodeignore (higher priority - added last)
    const projectIgnorePath = path.join(options.projectDir, ZODECODEIGNORE_FILE)
    const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
    allPatterns.push(...projectPatterns)

    if (projectPatterns.length > 0) {
      log.debug("loaded project .zodecodeignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .zodecodeignore and return permission config.
  ...
  export async function loadIgnoreConfig(
    ...
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .zodecodeignore patterns", {
      ...
      log.warn("failed to load .zodecodeignore", { error: err })
      return {}
    }
  }
}
