...
async function headers(args: HeaderArgs): Promise<Headers> {
  ...
  const out = new Headers({
    ...
    "x-zode-export-api-version": "1",
    "x-zode-export-schema-version": "1",
    "x-zode-export-agent-version": args.agentVersion,
    "x-zode-export-surface": args.surface,
    "x-zode-export-root-session-id": first.rootSessionId,
    "x-zode-export-session-id": first.sessionId,
    "x-zode-export-batch-id": args.batchId,
    "x-zode-export-seq-start": String(Math.min(...seqs)),
    "x-zode-export-seq-end": String(Math.max(...seqs)),
    "x-zode-export-event-count": String(args.rows.length),
    "x-zode-export-payload-sha256": await sha256Hex(args.body),
    "x-zode-export-client-sent-at": new Date().toISOString(),
    "x-zode-export-content-encoding": "identity",
  })
  const token = process.env.ZODE_SESSION_EXPORT_AUTH_TOKEN
  if (token) out.set("authorization", `Bearer ${token}`)
  const anon = args.anonId ?? (await anonId(args.anonIdPath))
  if (!token && anon) out.set("x-zode-anon-id", anon)
  return out
}
