...
import { resolveZodeIndexingAuth } from "@/zodecode/indexing-auth"
...
export async function getAuthOrgId(): Promise<OrgState> {
  try {
    const [cfg, info] = await Promise.all([
      ...
      auth.runPromise((svc) => svc.get("zode")),
    ])
    const id = resolveZodeIndexingAuth({ config: cfg, auth: info }).organizationId
