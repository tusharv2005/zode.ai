...
declare global {
  const ZODE_SESSION_EXPORT_WORKER_PATH: string
}
...
function target(): WorkerTarget {
  if (typeof ZODE_SESSION_EXPORT_WORKER_PATH !== "undefined") return ZODE_SESSION_EXPORT_WORKER_PATH
...
function currentSurface(): string {
  return process.env.ZODECODE_FEATURE?.trim() || "unknown"
}
