...
function planEditRules(worktree: string) {
  return {
    "*": "deny" as const,
    [path.join(".zode", "plans", "*.md")]: "allow" as const,
...
export interface ZodeData {
  mcpRules: Record<string, "allow" | "ask" | "deny">
  defaultsPatch: Permission.Ruleset
}

// Prepare zode-specific data derived from config. Call once per state initialization.
export function prepare(cfg: Config.Info): ZodeData {
...
// Patch the base agents map in-place with all zode-specific changes:
// - Rename build → code
// - Patch plan with readOnlyBash, mcpRules, .zode paths
...
export function patchAgents(
  ...
  if (agents.plan) {
    agents.plan = {
      ...
      permission: Permission.merge(
        defaults,
        planGuard(worktree, zode.mcpRules),
        user,
  ...
  agents.ask = {
    ...
    prompt: PROMPT_ASK,
    options: {},
    permission: Permission.merge(defaults, askGuard(zode.mcpRules), user, askEditGuard(), denies(user)),
    mode: "primary",
    native: true,
  }
}
 ...
 * removing it from legacy .zodecodemodes YAML files.
 * Scans all config directories for agent/mode .md files matching the name,
 * then also checks the .zodecodemodes files the ModesMigrator reads.
 */
export async function remove(name: string) {
  ...
  // 2. Remove from legacy .zodecodemodes YAML files (read by ModesMigrator)
  const { ModesMigrator } = await import("@/zodecode/modes-migrator")
  const { ZodecodePaths } = await import("@/zodecode/paths")
  const os = await import("os")
  const matter = (await import("gray-matter")).default
  const home = os.default.homedir()
  const modesFiles = [
    path.join(ZodecodePaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml"),
    path.join(home, ".zodecode", "cli", "global", "settings", "custom_modes.yaml"),
    path.join(home, ".zodecodemodes"),
    path.join(Instance.directory, ".zodecodemodes"),
  ]
