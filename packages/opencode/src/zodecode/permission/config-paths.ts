...
import { ZodecodePaths } from "@/zodecode/paths"

export namespace ConfigProtection {
  /**
   * Config directory prefixes (relative paths, forward-slash normalized).
   * Matches .zode/, .zodecode/, .opencode/ at any depth within the project.
   */
  const CONFIG_DIRS = [".zode/", ".zodecode/", ".opencode/"]
   ...
   */
  const CONFIG_ROOT_FILES = new Set(["zode.json", "zode.jsonc", "opencode.json", "opencode.jsonc", "AGENTS.md"])
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".zode"
      // Match at root (e.g. ".zode/foo") or nested (e.g. "packages/sub/.zode/foo")
  ...
  function configs(): string[] {
    return Array.from(
      new Set([Global.Path.config, process.env.XDG_CONFIG_HOME ? path.join(process.env.XDG_CONFIG_HOME, "zode") : ""]),
    ).filter(Boolean)
  }
  ...
  function fallback(p: string): boolean {
    ...
    return keys(p).some(
      (key) =>
        key.endsWith("/config/zode") ||
        key.includes("/config/zode/") ||
        key.endsWith("/.config/zode") ||
        key.includes("/.config/zode/"),
    )
  }
  ...
  export function isAbsolute(filepath: string): boolean {
    ...
    // ~/.config/zode/ (XDG config)
    for (const dir of configs()) {
      if (within(filepath, dir)) return true
    }

    // ~/.zode/ and ~/.zodecode/ (legacy global dirs)
    for (const dir of ZodecodePaths.globalDirs()) {
      if (within(filepath, dir)) return true
    }
