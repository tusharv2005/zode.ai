# Zode CLI Configuration Reference

All config lives in `zode.json` (or `zode.jsonc`). Precedence low-to-high: remote well-known, global (`~/.config/zode/zode.json`), env `ZODE_CONFIG`, project `./zode.json`, `.zode/zode.json`, `ZODE_CONFIG_CONTENT`, managed (see Config File Locations). Deep-merged; later wins.

This also covers where Zode looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.zode/`, `.zodecode/`, `.opencode/`, and `~/.config/zode/`, plus Agent Manager setup/run scripts in the VS Code extension.

## Commands (`.zode/command/*.md`)

Markdown files with YAML frontmatter. The filename (minus `.md`) becomes the command name invoked via `/name`. Commands can live in `.zode/`, `.zodecode/`, `.opencode/`, and global config roots, with both `command/` and `commands/` directory names supported. See Config File Locations for the full search order.
...
3. `~/.zodecode/`
4. `~/.opencode/`
5. The `ZODE_CONFIG_DIR` directory (if the env var is set)
6. project `.zode/`, `.zodecode/`, and `.opencode/` directories from the current working directory up to the worktree root
...
## Agents (`.zode/agent/*.md`)

Also loaded from `.zodecode/` and `.opencode/` directories (legacy), and plural `agents/` variants.
...
Markdown files in `.zode/workflows/` or `.zodecode/workflows/` (project-level) and `~/.zode/workflows/` or `~/.zodecode/workflows/` (global). These are automatically converted to commands at startup. The filename (minus `.md`) becomes the command name. Project workflows override global ones with the same name.
...
Agent Manager worktrees usually live under `.zode/worktrees/`. Think of each worktree as a separate checkout on its own branch: it enables parallel edits, but dependencies, build output, caches, databases, and generated files can consume significant disk space across many worktrees.
...
|---|---|
| macOS / Linux | `.zode/setup-script`, `.zode/setup-script.sh` |
| Windows | `.zode/setup-script.ps1`, `.zode/setup-script.cmd`, `.zode/setup-script.bat` |
...
- For port conflicts (`EADDRINUSE`, browser/tests hitting the wrong worktree), inspect the app's dev-server config as well as `.zode/run-script`. If fixing it requires application changes, ask whether the user wants the app made configurable or only wants a run-script workaround.
...
For example, this configuration will hide all models from the built-in Zode Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.
...
The provider ID is the lowercase name used in the `provider/model` format (e.g., `zode`, `openai`, `anthropic`, `google`, `groq`).
...
Skills are markdown files at `skills/<name>/SKILL.md` (or `skill/<name>/SKILL.md`) with `name` and `description` in frontmatter. Discovered inside `.zode/`, `.zodecode/`, and `.opencode/` directories.
...
|---|---|---|---|
| Switch theme | `<leader>t` | `/themes` | Pick from 35+ built-in themes (zode, catppuccin, dracula, github, gruvbox, nord, tokyonight, etc.) |
| Toggle appearance (dark/light) | — | — | Ctrl+P → "Toggle appearance" |

Custom themes: place JSON files in `~/.config/zode/themes/` or `.zode/themes/`.
...
### Config files (zode.json)

| Scope | Path |
|---|---|
| Project | `./zode.json`, `./zode.jsonc`, `./opencode.json` (legacy), `./opencode.jsonc` (legacy) |
| Global | `~/.config/zode/zode.json`, `~/.config/zode/zode.jsonc`, `~/.config/zode/opencode.json` (legacy), `~/.config/zode/opencode.jsonc` (legacy), `~/.config/zode/config.json` (legacy) |
| Managed | Linux: `/etc/zode/`, macOS: `/Library/Application Support/zode/`, Windows: `%ProgramData%\zode\` — loads `zode.json`, `zode.jsonc`, `opencode.json`, `opencode.jsonc` (enterprise, highest priority) |

Each config directory (`.zode/`, `.zodecode/`, `.opencode/`) can also contain `zode.json`, `zode.jsonc`, `opencode.json`, or `opencode.jsonc`.

### Config directories

Three directory names are scanned: `.zode` (modern), `.zodecode` (legacy), `.opencode` (legacy). All three are checked at each level:

- **Project**: walks up from CWD to the git worktree root, checking for all three at each directory level
- **Home**: `~/.zode/`, `~/.zodecode/`, `~/.opencode/`
- **XDG global**: `~/.config/zode/` (always loaded, lowest file-based precedence)
...
Example: `~/.config/zode/command/*.md` (modern global), `~/.zodecode/command/*.md` (legacy global), `.opencode/commands/*.md` (legacy project) all load commands.
...
|---|---|
| `ZODE_CONFIG` | Path to an additional config file (loaded after global) |
| `ZODE_CONFIG_DIR` | Path to an additional config directory (appended to search list) |
| `ZODE_CONFIG_CONTENT` | Inline JSON config string (high precedence, after project dirs) |
| `ZODE_DISABLE_PROJECT_CONFIG` | Skip all project-level config (files and directories) |
