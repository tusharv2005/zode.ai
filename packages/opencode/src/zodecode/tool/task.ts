...
const log = Log.create({ service: "zodecode-task-model" })
...
export namespace ZodeTask {
  ...
  /** Zode keeps delegation one level deep to avoid recursive subagent chains. */
  ...
  const saved = Effect.fn("ZodeTask.savedModel")(function* (name: string) {
    if (Flag.ZODE_CLIENT !== "cli") return undefined
  ...
  export const resolveModel = Effect.fn("ZodeTask.resolveModel")(function* (input: {
    name: string
