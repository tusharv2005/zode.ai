...
const log = Log.create({ service: "zodecode-tool-registry" })
type Deps = { agent: Agent.Interface; truncate: Truncate.Interface }

export namespace ZodeToolRegistry {
  const hint =
  ...
  /** Resolve Zode-specific tool Infos outside any InstanceState, so their Truncate/Agent deps are
  ...
  /** Finalize Zode-specific tools into Tool.Defs. Call this inside the InstanceState state Effect —
  ...
  function semanticTool(deps: Deps) {
    return Effect.gen(function* () {
      const ready = yield* Effect.tryPromise(() =>
        import("@/zodecode/indexing").then((mod) => mod.ZodeIndexing.ready()),
      ).pipe(
        Effect.catch((err) =>
          Effect.sync(() => {
      ...
      const mod = yield* Effect.tryPromise(() => import("@/zodecode/tool/semantic-search")).pipe(
        Effect.catch((err) =>
          Effect.sync(() => {
  ...
  /** Zode-specific tools to append to the builtin list */
  export function extra(
    ...
    return [
      ...
      tools.recall,
      ...(Flag.ZODE_CLIENT === "cli" || Flag.ZODE_CLIENT === "vscode" ? [tools.process] : []),
      // The extension is the only client that can consume the Agent Manager start event.
      ...(Flag.ZODE_CLIENT === "vscode" ? [tools.manager] : []),
    ]
  }
