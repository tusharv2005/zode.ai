...
import { Telemetry, type ReviewCommand } from "@zodecode/zode-telemetry"
import { SessionNetwork } from "@/session/network"
import type { SessionID } from "@/session/schema"
import type { SessionStatus } from "@/session/status"
import { MessageV2 } from "@/session/message-v2"
import { isRecord } from "@/util/record"
import { isReviewCommand, parseReviewCommand } from "@/zodecode/review/command"
...
export namespace ZodeSessionProcessor {
  const log = Log.create({ service: "session.processor.zode" })
  ...
  export const PROVIDER_FINISH_ERROR_MESSAGE =
    "The provider ended the response with an error before returning details. Start a new message to retry; Zode will compact the oversized conversation first if needed."
  ...
  /**
   * Returns the Zode-specific retry policy options (limit + offline handler).
  ...
  export function retryOpts(input: {
    ...
    return {
      limit: Flag.ZODE_SESSION_RETRY_LIMIT,
