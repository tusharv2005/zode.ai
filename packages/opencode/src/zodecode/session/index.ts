...
import { ENV_FEATURE } from "@zodecode/zode-gateway"

export namespace ZodeSession {
  const log = Log.create({ service: "session.zode" })
  ...
  export function attribution(id: string): { rootID: string; feature?: string } {
    ...
    const platform = resolvePlatform(rootID) ?? process.env["ZODE_PLATFORM"]
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (Zode / OpenRouter / Vercel AI Gateway)
  // ---------------------------------------------------------------------------
   ...
   *                                      (`costDetails.upstreamInferenceCost` for Zode)
   ...
   *
   * Zode does not charge end users a per-request fee, so for the Zode provider the
  ...
  export function providerCost(input: {
    ...
    const isZode = (input.provider?.id ?? input.providerID) === "zode"
    ...
    if (orUsage) {
      ...
      // Zode doesn't charge a fee on top of the upstream inference cost, so for Zode
      ...
      const cost = isZode && upstream !== undefined ? upstream : regular
    ...
    //    Zode doesn't charge end users a per-request fee, so the top-level `cost` field
    ...
    //    gateway fee that Zode would pass through, but Zode doesn't charge end users a
  ...
  export function shareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { ZodeSessions } = await import("@/zode-sessions/zode-sessions")
      return ZodeSessions.share(id)
  ...
  export function unshareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { ZodeSessions } = await import("@/zode-sessions/zode-sessions")
      await ZodeSessions.unshare(id)
    }).pipe(Effect.catchCause((cause) => Effect.fail(Cause.squash(cause))))
  }

  export async function removeSession(id: string): Promise<void> {
    const { ZodeSessions } = await import("@/zode-sessions/zode-sessions")
    await ZodeSessions.remove(id).catch(() => {})
  }
...
export const zodeSessionFork = fn(
  ...
  async (input) => {
    ...
    return AppRuntime.runPromise(
      Effect.gen(function* () {
        ...
        yield* ZodeSession.remapChildren(session.id)
        return session
      }),
    )
  },
