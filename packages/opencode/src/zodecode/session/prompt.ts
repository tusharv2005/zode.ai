...
import { PlanFollowup } from "@/zodecode/plan-followup"
import { PlanFile } from "@/zodecode/plan-file"
import { ZodeSession } from "@/zodecode/session"
import { ZodeSessionMessageOrder } from "@/zodecode/session/message-order"
import { Permission } from "@/permission"
import { Question } from "@/question"
import { environmentDetails, type EditorContext } from "@/zodecode/editor-context"
...
export namespace ZodeSessionPrompt {
  ...
  export function shouldAskPlanFollowup(input: { messages: MessageV2.WithParts[]; abort: AbortSignal }) {
    ...
    if (!["cli", "vscode", "jetbrains"].includes(Flag.ZODE_CLIENT)) return false
  ...
  export const recoverDanglingAssistant = Effect.fn("ZodeSessionPrompt.recoverDanglingAssistant")(function* (input: {
    sessionID: SessionID
  ...
  export const recoverProviderFinishError = Effect.fn("ZodeSessionPrompt.recoverProviderFinishError")(
    function* (input: {
      sessionID: SessionID
      status: Pick<SessionStatus.Interface, "get">
      sessions: Pick<Session.Interface, "messages" | "removeMessage">
  ...
  export const askPermission = Effect.fn("ZodeSessionPrompt.askPermission")(function* (input: {
   ...
   * OneDrive ReparsePoint directories in some Node versions (zodecode#9755).
   */
    ...
    if (mode(input.agent.name) === "plan") {
      add(
        [
          ...
          "If none is specified, create a plan in .zode/plans/ using a concise kebab-case filename based on the plan details.",
          "Do not choose .zode/plans/ when instructions specify an allowed plan path such as .plans/.",
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, ZodeSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): ZodeSession.CloseReason {
  ...
  export function trimBeforeLastSummary(msgs: MessageV2.WithParts[]): MessageV2.WithParts[] {
    const summary = msgs.reduce<{ msg: MessageV2.WithParts; index: number } | undefined>((latest, msg, index) => {
      ...
      if (!latest || ZodeSessionMessageOrder.compare(msg, latest.msg, index, latest.index) > 0) return { msg, index }
      return latest
    }, undefined)
