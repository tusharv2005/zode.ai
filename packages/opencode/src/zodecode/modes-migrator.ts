...
import type { OrganizationMode } from "@zodecode/zode-gateway"

export namespace ModesMigrator {
  // Zodecode mode structure
  export interface ZodecodeMode {
    slug: string
    name: string
  ...
  export interface ZodecodeModesFile {
    customModes: ZodecodeMode[]
  }
  ...
  export function convertPermissions(groups: ZodecodeMode["groups"]): ConfigPermission.Info {
  ...
  export function convertMode(mode: ZodecodeMode): ConfigAgent.Info {
  ...
  export async function readModesFile(filepath: string): Promise<ZodecodeMode[]> {
    try {
      const content = await fs.readFile(filepath, "utf-8")
      // Wrap YAML content in frontmatter delimiters so gray-matter can parse it
      const wrapped = `---\n${content}\n---`
      const parsed = matter(wrapped).data as ZodecodeModesFile
  ...
  export async function migrate(options: {
    ...
    const allModes: ZodecodeMode[] = []

    if (!options.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global modes)
      const vscodeGlobalPath = path.join(ZodecodePaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(vscodeGlobalPath)))

      // 2. CLI global settings (fallback/alternative location)
      const cliGlobalPath = path.join(os.homedir(), ".zodecode", "cli", "global", "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(cliGlobalPath)))

      // 3. Home directory .zodecodemodes
      const homeModesPath = path.join(os.homedir(), ".zodecodemodes")
    ...
    // 5. Project .zodecodemodes
    const projectModesPath = path.join(options.projectDir, ".zodecodemodes")
    allModes.push(...(await readModesFile(projectModesPath)))

    // Deduplicate by slug (later entries win)
    const modesBySlug = new Map<string, ZodecodeMode>()
