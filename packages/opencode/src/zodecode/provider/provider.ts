...
//
// Zode-specific provider logic extracted from packages/opencode/src/provider/provider.ts
...
// calls at well-defined injection points (each marked with zodecode_change).

import { createZode, type ZodeProvider, AI_SDK_PROVIDERS, PROMPTS } from "@zodecode/zode-gateway"
...
export const ZODE_BUNDLED_PROVIDERS: Record<string, () => Promise<(options: any) => BundledSDK>> = {
  "@zodecode/zode-gateway": async () => createZode as unknown as (options: any) => BundledSDK,
}
...
export const ZODE_MODEL_SCHEMA_EXTENSIONS = {
...
// ---------------------------------------------------------------------------
// fromModelsDevModel patch — returns zode-specific fields
// ---------------------------------------------------------------------------

export function patchModelsDevModel(providerID: string, source: any) {
  const free = providerID === "zode" && source.cost?.input === 0 && source.cost?.output === 0
  return {
    variants: providerID === "zode" ? (source.variants ?? {}) : {},
...
// ---------------------------------------------------------------------------
// Config model patch — merges zode-specific fields from config + existing
// ---------------------------------------------------------------------------
...
export function zodeCustomLoaders(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    zode: Effect.fnUntraced(function* (input: any) {
      ...
      const hasKey = yield* Effect.gen(function* () {
        ...
        if ((yield* dep.config()).provider?.["zode"]?.options?.apiKey) return true
        return false
      })

      const options: Record<string, string> = {}
      if (env.ZODE_ORG_ID) {
        options.zodecodeOrganizationId = env.ZODE_ORG_ID
      }
      if (!hasKey) {
      ...
      return {
        ...
        options,
        async getModel(sdk: ZodeProvider, modelID: string) {
  ...
  switch (providerID) {
    ...
    case "cerebras":
      result.options.headers = {
        ...result.options.headers,
        "X-Cerebras-3rd-Party-Integration": "zode",
      }
      break
    case "azure": {
    ...
    // in provider.ts with single-line zodecode_change markers
  }
}
...
export function zodeSmallModelPriority(providerID: string): string[] | undefined {
  if (providerID.startsWith("zode")) return ["zode-auto/small"]
  return undefined
}
