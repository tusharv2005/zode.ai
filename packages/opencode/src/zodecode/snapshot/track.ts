...
//          persist `"snapshot": false` to `.zode/zode.json`, and skip. All
...
//     Instead we write the file directly via `ZodecodeConfig.updateProjectConfig`
...
//
// All of this is Zode-specific — the upstream snapshot module remains a thin
...
export namespace ZodeSnapshotTrack {
  const log = Log.create({ service: "snapshot.track" })

  export const TIMEOUT_MS = iife(() => {
    const raw = process.env["ZODE_SNAPSHOT_TRACK_TIMEOUT_MS"]
    if (raw) {
