...
export namespace RulesMigrator {
  // Only support .zodecoderules (no migration for .roorules or .clinerules)
  const LEGACY_RULE_FILE = ".zodecoderules"
  const home = () => process.env.ZODE_TEST_HOME || process.env.HOME || process.env.USERPROFILE || os.homedir()

  // Directory-based rules (read from both .zode and .zodecode)
  const ZODE_RULES_DIRS = [".zode/rules", ".zodecode/rules"]
  const globalRulesDirs = () => [path.join(home(), ".zode", "rules"), path.join(home(), ".zodecode", "rules")]
  ...
  export async function discoverRules(projectDir: string): Promise<RuleFile[]> {
    ...
    // 1. Global rules directories (~/.zode/rules/*.md and ~/.zodecode/rules/*.md)
    ...
    // 2. Project .zode/rules/ and .zodecode/rules/ directories
    const seen = new Set<string>()
    for (const rulesRel of ZODE_RULES_DIRS) {
    ...
    // 3. Legacy .zodecoderules file (only zodecode, not roo/cline)
    ...
    for (const mode of KNOWN_MODES) {
      // Mode-specific directories (.zode/rules-{mode}/*.md and .zodecode/rules-{mode}/*.md)
      const modeSeen = new Set<string>()
      for (const prefix of [".zode", ".zodecode"]) {
      ...
      // Legacy mode-specific file (.zodecoderules-{mode})
      const legacyModeFile = path.join(projectDir, `.zodecoderules-${mode}`)
  ...
  export async function migrate(options: {
    ...
    for (const rule of rules) {
      ...
      if (rule.source === "legacy") {
        warnings.push(
          `Legacy rule file '${path.basename(rule.path)}' found. Consider migrating to .zode/rules/ directory.`,
        )
      }
    }
