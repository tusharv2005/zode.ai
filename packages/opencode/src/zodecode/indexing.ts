...
import { type IndexingTelemetryEvent, type VectorStoreSearchResult } from "@zodecode/zode-indexing/engine"
import { toIndexingConfigInput, type IndexingConfig } from "@zodecode/zode-indexing/config"
import { hasIndexingPlugin } from "@zodecode/zode-indexing/detect"
import { IndexingStatus, disabledIndexingStatus } from "@zodecode/zode-indexing/status"
...
import { LanceDBRuntime } from "./lancedb" // zodecode_change
import { indexingWithZodeDefault, resolveZodeIndexingAuth, type ZodeIndexingAuth } from "./indexing-auth" // zodecode_change

const log = Log.create({ service: "zodecode-indexing" })
...
function isWorktreePath(dir: string): boolean {
  return /(?:\/|\\)\.zode(?:code)?(?:\/|\\)worktrees(?:\/|\\)/.test(dir)
}
...
async function zodeAuth(cfg: Config.Info): Promise<ZodeIndexingAuth> {
  const info = await auth.runPromise((svc) => svc.get("zode"))
  return resolveZodeIndexingAuth({ config: cfg, auth: info })
}

function enrichZode(input: ReturnType<typeof toIndexingConfigInput>, auth: ZodeIndexingAuth) {
  if (input.embedderProvider !== "zode") return input

  return {
    ...input,
    zodeApiKey: input.zodeApiKey ?? auth.apiKey,
    zodeBaseUrl: input.zodeBaseUrl ?? auth.baseUrl,
    zodeOrganizationId: input.zodeOrganizationId ?? auth.organizationId,
  }
}

async function model(input: ReturnType<typeof toIndexingConfigInput>, auth: ZodeIndexingAuth) {
  if (input.embedderProvider !== "zode") return input

  const catalog = await fetchZodeEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey })
  ...
  if (!found) {
    if (input.modelId || input.modelDimension) {
      log.warn("ignoring unsupported Zode embedding model configuration", { model: input.modelId })
    }
    return { ...input, modelId: undefined, modelDimension: undefined }
  }

  if (input.modelId && !chosen) {
    log.warn("using default Zode embedding model instead of unsupported configuration", {
      model: input.modelId,
      fallback: found.id,
    })
  }
...
export namespace ZodeIndexing {
  ...
  const boot = async (hit: Cache): Promise<Entry> => {
    ...
    if (process.env["ZODE_DISABLE_CODEBASE_INDEXING"] === "vscode-no-workspace") {
    ...
    const auth = await zodeAuth(cfg)
    const globalConfig = await AppRuntime.runPromise(Config.Service.use((svc) => svc.getGlobal()))
    const global = globalConfig.indexing
    const merged = indexingWithZodeDefault({ ...global, ...cfg.indexing }, auth)
    const cfgInput = await model(enrichZode(input(merged, global), auth), auth)
