...
/**
 * ZodeClaw SolidJS reactive helpers — Zode Chat protocol.
 *
 * Exposes:
 *   - `createClawStatus`: polled instance status
 *   - `createClawChat`: Zode Chat connection + active-conversation state
 *
...
/**
 * Poll the ZodeClaw instance status every `interval` ms.
 */
export function createClawStatus(sdk: SDK, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.zode.claw.status().catch(() => null)
...
export function createClawChat(sdk: SDK) {
  ...
  onMount(async () => {
    ...
    const statusRes = await sdk.client.zode.claw.status().catch(() => null)
    ...
    const credsRes = await sdk.client.zode.claw.chatCredentials().catch((e: unknown) => {
    ...
    if (!envelope.zodeChatUrl) missing.push("zodeChatUrl")
