...
/**
 * ZodeClaw Zode Chat client wrapper for the TUI.
 *
 * Hosts the HTTP and WebSocket clients, exposes a multi-conversation API
 * (list/create/select/rename/delete), and translates Zode Chat events for
...
import { ZodeChatClient } from "./zode-chat-client"
...
export async function connect(input: ConnectInput): Promise<ClawChatClient> {
  ...
  const chat = new ZodeChatClient({
    baseUrl: input.envelope.zodeChatUrl,
    getToken: async () => input.envelope.token,
  })
  ...
  const sandboxCtx = `/zodeclaw/${input.sandboxId}`
  ...
  function setActive(id: string | null): void {
    ...
    clearTyping()
    if (id) {
      activeCtx = `/zodeclaw/${input.sandboxId}/${id}`
