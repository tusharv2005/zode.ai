/**
 * ZodeClaw full-screen view
 *
 * Main layout component for the /zodeclaw route.
...
export function ZodeClawView() {
  ...
  // Bot display name — sourced from the ZodeClaw platform status (set by the
  // user during onboarding via patchBotIdentity). Falls back to the literal
  // "ZodeClaw" while loading or for instances that skipped onboarding,
  // matching the web UI's fallback chain.
  const botName = createMemo(() => status()?.botName ?? "ZodeClaw")

  // ZodeClaw view commands — single source of truth for both the global
  // command palette / keybinds and the in-chat slash autocomplete.
  // The list is reactive on `chat.connected` so the slash menu only
  // exposes `/new` and `/conversations` once we're connected to zode-chat.
  const zodeCommands = createMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        desc: "Start a new ZodeClaw conversation",
        category: "ZodeClaw",
        namespace: "palette",
        slashName: "new",
        slashAliases: [] as string[],
      ...
      {
        name: "zodeclaw.conversations",
        title: "Conversations",
        desc: "Browse, rename, and delete ZodeClaw conversations",
        category: "ZodeClaw",
        namespace: "palette",
  ...
  useBindings(() => ({
    commands: zodeCommands(),
    bindings: [{ key: "escape", cmd: "zodeclaw.back" }],
  }))
  ...
  const clawSlashes = createMemo<ClawSlashOption[]>(() => {
    const visible = zodeCommands().filter((c) => c.enabled !== false && !c.hidden && c.slashName)
