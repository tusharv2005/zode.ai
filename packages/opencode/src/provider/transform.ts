...
import { zodeProviderOptions } from "@/zodecode/provider-options"
import { isLing } from "@/zodecode/model-match" // zodecode_change
...
export const OUTPUT_TOKEN_MAX = Flag.ZODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
function sdkKey(npm: string): string | undefined {
  switch (npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      return "openrouter"
    case "@zodecode/zode-gateway": // zodecode_change
      return "openrouter"
...
function normalizeMessages(
  ...
  // zodecode_change start - only filter for Claude models on Bedrock, not all Bedrock models
...
export function temperature(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.3 // zodecode_change
  return undefined
}
...
export function topP(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.95 // zodecode_change
  return undefined
}
...
export function topK(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 20 // zodecode_change
  return undefined
}
...
function anthropicAdaptiveEfforts(apiId: string): string[] | null {
  // zodecode_change start - treat opus-4.8 like opus-4.7
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  if (
    ["@zodecode/zode-gateway", "@ai-sdk/openai-compatible"].includes(model.api.npm) &&
    model.variants &&
  ...
  if (
    ...
    id.includes("minimax") ||
    // id.includes("glm") || // zodecode_change
    // id.includes("kimi") || // zodecode_change
  ...
  if (id.includes("grok") && id.includes("grok-3-mini")) {
    if (model.api.npm === "@openrouter/ai-sdk-provider" || model.api.npm === "@zodecode/zode-gateway") {
      // zodecode_change - add Zode Gateway support
      return {
  ...
  switch (model.api.npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      ...
      if (
        ...
        !id.includes("claude") &&
        !model.id.includes("mercury") // zodecode_change
      )
        return {}
      return Object.fromEntries(
        (model.api.npm === "@zodecode/zode-gateway" || !id.includes("gpt") // zodecode_change
          ? OPENAI_EFFORTS
    ...
    case "@ai-sdk/azure":
      ...
      return Object.fromEntries(
        (GPT5_FAMILY_RE.test(id) && gpt5Version(id) === undefined
          ...
          .concat(model.release_date >= OPENAI_XHIGH_EFFORT_RELEASE_DATE ? ["xhigh"] : []) // zodecode_change
          .map((effort) => [
            effort,
            {
              reasoningEffort: effort,
    ...
    case "@ai-sdk/mistral":
      ...
      // https://docs.mistral.ai/studio-api/conversations/reasoning // zodecode_change
      if (!model.capabilities.reasoning) return {}
...
export function options(input: {
  ...
  if (
    ...
    input.model.api.npm === "@zodecode/zode-gateway" // zodecode_change
  ) {
    result["usage"] = {
      include: true,
    }
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    if (!input.model.api.id.includes("gpt-5-pro")) {
      ...
      if (
        ...
        input.model.api.npm === "@ai-sdk/github-copilot" || // zodecode_change
        input.model.api.npm === "@openrouter/ai-sdk-provider" || // zodecode_change
        input.model.api.npm === "@zodecode/zode-gateway" // zodecode_change
      ) {
    ...
    if (
      // zodecode_change start - gate textVerbosity to Responses-API providers
      (input.model.api.npm === "@ai-sdk/openai" ||
        input.model.api.npm === "@ai-sdk/azure" ||
        input.model.api.npm === "@ai-sdk/github-copilot" ||
        input.model.api.npm === "@openrouter/ai-sdk-provider" ||
        input.model.api.npm === "@zodecode/zode-gateway") &&
      // zodecode_change end
...
export function smallOptions(model: Provider.Model) {
  ...
  if (
    ...
    model.api.npm === "@zodecode/zode-gateway" // zodecode_change
  ) {
...
export function providerOptions(model: Provider.Model, options: { [x: string]: any }) {
  ...
  // zodecode_change start
  if (model.api.npm === "@zodecode/zode-gateway") {
    return zodeProviderOptions(options)
  }
  // zodecode_change end
