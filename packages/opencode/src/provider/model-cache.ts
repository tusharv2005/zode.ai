...
import { fetchZodeModels, type ZodeModelsResult } from "@zodecode/zode-gateway"
...
type ZodeOptions = NonNullable<Parameters<typeof fetchZodeModels>[0]>
type Options = { -readonly [K in keyof ZodeOptions]?: ZodeOptions[K] } & { apiKey?: string }
type Failure = NonNullable<ZodeModelsResult["error"]>
type Result = { readonly models: Models; readonly error?: Failure }
type View = { models?: Models; timestamp?: number }

export interface ZodeModels {
  readonly fetch: (options: ZodeOptions) => Effect.Effect<ZodeModelsResult, unknown>
}

export class ZodeModelsService extends Context.Service<ZodeModelsService, ZodeModels>()(
  "@zodecode/ModelCache/ZodeModels",
) {}

export const zodeModelsLayer = Layer.succeed(
  ZodeModelsService,
  ZodeModelsService.of({ fetch: (options) => Effect.tryPromise(() => fetchZodeModels(options)) }),
)
type Cell = {
...
export class Service extends Context.Service<Service, Interface>()("@zodecode/ModelCache") {}
...
export const layer: Layer.Layer<
  Service,
  never,
  Auth.Service | Config.Service | ZodeModelsService | HttpClient.HttpClient
> = Layer.effect(
  Service,
  Effect.gen(function* () {
    const auth = yield* Auth.Service
    const cfg = yield* Config.Service
    const zode = yield* ZodeModelsService
    ...
    const authOptions = Effect.fn("ModelCache.authOptions")(function* (providerID: string) {
      if (providerID !== "zode" && providerID !== "apertis") return {}
      const config = yield* cfg.get()
      const options: Options = {}

      if (providerID === "zode") {
        const item = config.provider?.[providerID]
        if (item?.options?.apiKey) options.zodecodeToken = item.options.apiKey
        if (item?.options?.zodecodeOrganizationId) options.zodecodeOrganizationId = item.options.zodecodeOrganizationId

        const info = yield* auth.get(providerID)
        if (info?.type === "api") options.zodecodeToken = info.key
        if (info?.type === "oauth") {
          options.zodecodeToken = info.access
          if (info.accountId) options.zodecodeOrganizationId = info.accountId
        }

        if (process.env.ZODE_API_KEY) options.zodecodeToken = process.env.ZODE_API_KEY
        if (process.env.ZODE_ORG_ID) options.zodecodeOrganizationId = process.env.ZODE_ORG_ID
        log.debug("auth options resolved", {
          providerID,
          hasToken: !!options.zodecodeToken,
          hasOrganizationId: !!options.zodecodeOrganizationId,
        })
      }
    ...
    const fetchModels = (providerID: string, options: Options): Effect.Effect<Result, unknown> => {
      if (providerID === "zode") return zode.fetch(options)
    ...
    const key = (providerID: string, options?: Options) => {
      if (providerID === "zode") {
        return JSON.stringify([providerID, options?.baseURL, options?.zodecodeOrganizationId, options?.zodecodeToken])
      }
