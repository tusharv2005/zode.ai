...
import {
  ZODE_BUNDLED_PROVIDERS,
  zodeCustomLoaders,
  ZODE_MODEL_SCHEMA_EXTENSIONS,
  patchModelsDevModel as patchZodeModel,
  patchConfigModel as patchZodeConfigModel,
  patchCustomLoaderResult,
  zodeSmallModelPriority,
  buildTimeoutSignal,
  REQUEST_TIMEOUT_MS,
} from "@/zodecode/provider/provider"
// zodecode_change end
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  ...ZODE_BUNDLED_PROVIDERS, // zodecode_change
}
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    azure: Effect.fnUntraced(function* (provider: Info) {
      ...
      // zodecode_change start - prefer explicit Azure endpoint over resource name to avoid conflicting SDK options
      const endpoint = iife(() => {
        return [
      ...
      if (!resource && !endpoint) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              "Azure resource name or endpoint is missing. Set AZURE_RESOURCE_NAME, AZURE_OPENAI_RESOURCE_NAME, AZURE_OPENAI_ENDPOINT, or reconnect the azure provider.", // zodecode_change
            )
          },
        }
      }
      ...
      return {
        ...
        options: {
          ...(endpoint ? { baseURL: endpoint } : { resourceName: resource }), // zodecode_change
        },
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `zode/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`, // zodecode_change
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `zode auth cloudflare-ai-gateway`.", // zodecode_change
        )
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "Zode Code", // zodecode_change
          },
        },
      }),
    zode: () =>
...
export const Model = Schema.Struct({
  ...
  ...ZODE_MODEL_SCHEMA_EXTENSIONS, // zodecode_change
})
...
export const ListResult = Schema.Struct({
  ...
  failed: Schema.Array(Schema.String), // zodecode_change
}).pipe(withStatics((s) => ({ zod: zod(s) })))
...
function fromModelsDevModel(provider: ModelsDev.Provider, model: ModelsDev.Model): Model {
  ...
  Object.assign(base, patchZodeModel(provider.id, model)) // zodecode_change
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          if (!provider) continue // zodecode_change - null entries are transient delete sentinels
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            if (!model) continue // zodecode_change - null entries are transient delete sentinels
            ...
            const parsedModel: Model = {
              ...
              // variants: {}, // zodecode_change, moved into patchZodeConfigModel
              ...patchZodeConfigModel(model, existingModel), // zodecode_change
            }
            const merged = mergeDeep(ProviderTransform.variants(parsedModel), model.variants ?? {})
            parsedModel.variants = mapValues(
              pickBy(merged, (v): v is NonNullable<typeof v> => !!v && !v.disabled), // zodecode_change - drop null delete sentinels
              (v) => omit(v, ["disabled"]),
            )
        ...
        // zodecode_change start - load auths before env so OAuth plugins can override inherited credentials
        ...
        for (const [id, provider] of Object.entries(database)) {
          ...
          if (disabled.has(providerID)) continue
          // zodecode_change start - prefer explicit OAuth auth over inherited env credentials
          if (
        ...
        // zodecode_change start - resolve env once for patchCustomLoaderResult (azure env fallback)
        const zodeEnv = yield* env.all()
        // zodecode_change end
        for (const [id, fn] of Object.entries({ ...custom(dep), ...zodeCustomLoaders(dep) })) {
          // zodecode_change
          const providerID = ProviderID.make(id)
          if (disabled.has(providerID)) continue
          ...
          if (result) patchCustomLoaderResult(id, result, zodeEnv) // zodecode_change
        ...
        for (const [id, provider] of configProviders) {
          if (!provider) continue // zodecode_change - null entries are transient delete sentinels
          const providerID = ProviderID.make(id)
          // zodecode_change start - keep OAuth plugin source when config and Codex auth coexist
          const oauth =
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            if (model.status === "alpha" && !Flag.ZODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          // zodecode_change start - use cancellable timeout for connection phase
          ...
          // zodecode_change start - clear connection-phase timeout once headers arrive
          try {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      // zodecode_change start
      const zodePriority = zodeSmallModelPriority(providerID)
      if (zodePriority) priority = zodePriority
      // zodecode_change end
      ...
      // zodecode_change start - fall back to zode's auto small model
      const zodeFallback = s.providers[ProviderID.make("zode")]
      if (zodeFallback?.models["zode-auto/small"]) {
        return yield* getModel(ProviderID.make("zode"), ModelID.make("zode-auto/small"))
      }
      // zodecode_change end
