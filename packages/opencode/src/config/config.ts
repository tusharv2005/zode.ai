...
import { Env } from "../env"
import { applyEdits, findNodeAtLocation, modify, parseTree } from "jsonc-parser" // zodecode_change - parseTree/findNodeAtLocation used in patchJsonc
...
import { ZodecodeGlobalConfigStamp } from "@/zodecode/config/global-stamp"
import {
  IndexingConfig as ZodeIndexingConfig,
  IndexingSchema as ZodeIndexingSchema,
} from "@zodecode/zode-indexing/config"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  const copy = ZodecodeConfig.retireIndexingFlag({ ...data }, source) // zodecode_change
...
const { caught: caughtWarning } = ZodecodeConfig
// zodecode_change end
...
// zodecode_change start - indexing configuration
export const Indexing = ZodeIndexingConfig
...
const Percent = Schema.Number.check(Schema.isGreaterThan(0), Schema.isLessThanOrEqualTo(100)) // zodecode_change

const IndexingRef = ZodeIndexingSchema.annotate({ [ZodOverride]: ZodeIndexingConfig }) // zodecode_change
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for the zode serve command", // zodecode_change
  }),
  ...
  indexing: Schema.optional(IndexingRef).annotate({ description: "Codebase indexing configuration" }), // zodecode_change
  ...
  // zodecode_change end
  // zodecode_change start - renamed from "build" to "code" + nullable for delete sentinel
  ...
  agent: Schema.optional(
    Schema.StructWithRest(
      Schema.Struct({
        ...
        plan: Schema.optional(ConfigAgent.Info),
        build: Schema.optional(ConfigAgent.Info),
        debug: Schema.optional(ConfigAgent.Info), // zodecode_change
        orchestrator: Schema.optional(ConfigAgent.Info), // zodecode_change
        ask: Schema.optional(ConfigAgent.Info), // zodecode_change
        // subagent
  ...
  provider: Schema.optional(Schema.Record(Schema.String, Schema.NullOr(ConfigProvider.Info))).annotate({
    // zodecode_change - nullable for delete sentinel
  ...
  commit_message: ZodecodeConfig.CommitMessageSchema, // zodecode_change
  ...
  experimental: Schema.optional(
    Schema.Struct({
      ...
      codebase_search: Schema.optional(Schema.Boolean).annotate({ description: "Enable AI-powered codebase search" }), // zodecode_change
      // zodecode_change start
      ...
      // zodecode_change end
      // zodecode_change start - enable telemetry by default
...
type State = {
  ...
  warnings: Warning[] // zodecode_change
...
export interface Interface {
  ...
  ) => Effect.Effect<{ info: Info; changed: boolean }> // zodecode_change
  readonly invalidate: () => Effect.Effect<void>
  readonly directories: () => Effect.Effect<string[]>
  readonly waitForDependencies: () => Effect.Effect<void>
  readonly warnings: () => Effect.Effect<Warning[]> // zodecode_change
}
...
function globalConfigFile() {
  // zodecode_change start
  const candidates = ["zode.jsonc", "zode.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
    // zodecode_change end
...
function patchJsonc(input: string, patch: unknown, path: string[] = []): string {
  if (!isRecord(patch)) {
    // zodecode_change - null means "delete this key" — pass undefined to jsonc-parser's modify()
  ...
  // zodecode_change start — when the existing JSONC node at this path is a
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    let globalStamp = "" // zodecode_change

    const loadGlobal = Effect.fnUntraced(function* () {
      yield* Effect.promise(() => ZodecodeConfig.migrateBashPermission()) // zodecode_change
      globalStamp = yield* ZodecodeGlobalConfigStamp.read(fs, Global.Path.config) // zodecode_change
      let result: Info = {}
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json")))
      // zodecode_change start
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "zode.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "zode.jsonc")))
      // zodecode_change end
    ...
    // zodecode_change start - detect global config edits made by other Zode processes
    const refreshGlobal = Effect.fnUntraced(function* () {
      const stamp = yield* ZodecodeGlobalConfigStamp.read(fs, Global.Path.config)
    ...
    const getGlobal = Effect.fn("Config.getGlobal")(function* () {
      yield* refreshGlobal() // zodecode_change
    ...
    const ensureGitignore = Effect.fn("Config.ensureGitignore")(function* (dir: string) {
      ...
      if (!hasIgnore) {
        yield* fs
          .writeFileString(
            gitignore,
            // zodecode_change start - added pnpm-lock.yaml, yarn.lock, agent-manager.json (not in upstream)
            [
              "node_modules",
              "package.json",
              "package-lock.json",
          ...
          .pipe(
            Effect.catchIf(
              (e) => e.reason._tag === "PermissionDenied" || e.reason._tag === "NotFound", // zodecode_change - also ignore NotFound (broken symlink/junction on Windows)
              () => Effect.void,
            ),
          )
      }
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        // zodecode_change start - warning accumulator and legacy Zode config
        const warnings: Warning[] = []
        const auth = yield* authSvc.all().pipe(Effect.orDie)

        let result: Info = {}
        const legacy = yield* Effect.promise(() =>
          ZodecodeConfig.loadLegacyConfigs({
        ...
        const orgModes = yield* Effect.promise(() => ZodecodeConfig.loadOrganizationModes(auth))
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ZODE_CONFIG_CONTENT") return "local"
        ...
        const merge = Effect.fnUntraced(function* (source: string, next: Info, kind?: ConfigPlugin.Scope) {
          ...
          const scoped = ZodecodeConfig.scopeIndexing(next, scope)
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const source = `${url}/.well-known/opencode` // zodecode_change
            // zodecode_change start - warn instead of fail on wellknown errors
        ...
        // zodecode_change start - capture global config failures as warnings
        ...
        if (Flag.ZODE_CONFIG) {
          // zodecode_change start - capture ZODE_CONFIG failures as warnings
          yield* merge(
            Flag.ZODE_CONFIG,
            yield* loadFile(Flag.ZODE_CONFIG).pipe(
              Effect.catchDefect((err: unknown) => {
                caughtWarning(warnings, Flag.ZODE_CONFIG!, err)
                return Effect.succeed({} as Info)
              }),
            ),
          )
          // zodecode_change end
          log.debug("loaded custom config", { path: Flag.ZODE_CONFIG })
        }

        if (!Flag.ZODE_DISABLE_PROJECT_CONFIG) {
          // zodecode_change start - also discover zode.json project files
          for (const name of ["zode", "opencode"] as const) {
        ...
        if (Flag.ZODE_CONFIG_DIR) {
          log.debug("loading config from ZODE_CONFIG_DIR", { path: Flag.ZODE_CONFIG_DIR })
        }
        ...
        for (const dir of unique(directories)) {
          if (ZodecodeConfig.isConfigDir(dir, Flag.ZODE_CONFIG_DIR)) {
            for (const file of ZodecodeConfig.ALL_CONFIG_FILES) {
          ...
          // zodecode_change start - propagate parse errors to the Warning accumulator
        ...
        if (process.env.ZODE_CONFIG_CONTENT) {
          // zodecode_change start - capture ZODE_CONFIG_CONTENT parse failures as warnings
          const source = "ZODE_CONFIG_CONTENT"
          yield* merge(
            source,
            yield* loadConfig(process.env.ZODE_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            }).pipe(
              Effect.tap(() => Effect.sync(() => log.debug("loaded custom config from ZODE_CONFIG_CONTENT"))),
              Effect.catchDefect((err: unknown) => {
                caughtWarning(warnings, source, err)
                return Effect.succeed({} as Info)
              }),
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ZODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ZODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        // zodecode_change start - include zode.json/zode.jsonc in managed dir loading
        ...
        if (Flag.ZODE_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ZODE_PERMISSION))
        }
        ...
        if (Flag.ZODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ZODE_DISABLE_PRUNE) {
          result.compaction = { ...result.compaction, prune: false }
        }
        // zodecode_change start — inject Zode default plugins into both plugin list and origins
        ZodecodeDefaultPlugins.apply(result, { disabled: Flag.ZODE_DISABLE_DEFAULT_PLUGINS, log })
        // zodecode_change end
    ...
    const get = Effect.fn("Config.get")(function* () {
      // zodecode_change start - reload instance config when global config changed elsewhere
    ...
    const update = Effect.fn("Config.update")(function* (config: Info) {
      // zodecode_change start - delegate Zode project config update behavior.
      const ctx = yield* InstanceState.context
      yield* ZodecodeConfig.updateProjectConfig({
        fs,
    ...
    // zodecode_change start - add dispose option to skip Instance.disposeAll for permission-only changes
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (config: Info, options?: { dispose?: boolean }) {
      ...
      if (!file.endsWith(".jsonc")) {
        ...
        // zodecode_change - use `patch` (writableGlobal) so empty-string sentinels are stripped via undefined
        const merged = ZodecodeConfig.mergeConfig(writable(existing), patch)
      ...
      // zodecode_change start - skip dispose when caller opts out
      if (!dispose) {
        yield* invalidateGlobal
        yield* InstanceState.invalidate(state).pipe(Effect.catchCause(() => Effect.void))
        yield* Effect.sync(() =>
      ...
      // zodecode_change start - hot-reload global config changes in the active instance
      if (changed) {
        yield* InstanceState.invalidate(state).pipe(Effect.catchCause(() => Effect.void))
        yield* Effect.sync(() =>
          GlobalBus.emit("event", {
