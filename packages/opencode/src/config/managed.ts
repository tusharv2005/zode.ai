...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/zode" // zodecode_change
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "zode") // zodecode_change
    default:
      return "/etc/zode" // zodecode_change
  }
}

export function managedConfigDir() {
  return process.env.ZODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
