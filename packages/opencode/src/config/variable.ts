...
type SubstituteInput = ParseSource & {
  text: string
  missing?: "error" | "empty"
  escapeJson?: boolean // zodecode_change
}
...
export async function substitute(input: SubstituteInput) {
  ...
  const escape = input.escapeJson ?? true // zodecode_change
  ...
  for (const match of fileMatches) {
    ...
    out += escape ? JSON.stringify(fileContent).slice(1, -1) : fileContent // zodecode_change
    cursor = index + token.length
  }
