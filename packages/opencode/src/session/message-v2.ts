...
import { SessionNetwork } from "./network" // zodecode_change
import { CodexAuthExpiredError } from "@/zodecode/provider/codex-refresh" // zodecode_change
...
// zodecode_change start - shared editor context schema (used by MessageV2.User and SessionPrompt.PromptInput)
...
// zodecode_change start - strip bloated metadata fields from stored parts to prevent multi-MB payloads
...
export function stripPartMetadata(part: Part): Part {
  // zodecode_change - exported for testing
...
// zodecode_change - apply stripping inside helpers so all read paths are covered
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      for (const part of msg.parts) {
        // zodecode_change - !part.ignored keeps local UI warnings out of future prompts
...
export function parts(message_id: MessageID) {
  ...
  return rows.map(
    (row) =>
      // zodecode_change - apply stripping to parts fetched individually as well to cover all read paths
      stripPartMetadata({
        ...row.data,
        id: row.id,
        sessionID: row.session_id,
...
export function filterCompacted(msgs: Iterable<WithParts>) {
  ...
  result.reverse()
  ZodeSessionMessageOrder.annotate(result) // zodecode_change - preserve chronology before retained-tail projection
...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof CodexAuthExpiredError: // zodecode_change start
      return new AuthError(
        {
          providerID: "openai",
          message: e.message,
        ...
        { cause: e },
      ).toObject() // zodecode_change end
    case SessionNetwork.disconnected(e): // zodecode_change start
      return new APIError(
        {
          message: SessionNetwork.message(e), // zodecode_change end
          isRetryable: true,
          metadata: {
