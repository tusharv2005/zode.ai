...
import { isZodeError } from "@/zodecode/zode-errors" // zodecode_change
import { SessionNetwork } from "./network" // zodecode_change
import { iife } from "@/util/iife"

export type Err = ReturnType<NamedError["toObject"]>

export type RetryReason = string & {} // zodecode_change - Zode does not support OpenCode Go upsell reasons
...
// zodecode_change - Zode does not emit OpenCode Go actions
export function retryable(error: Err, _provider?: string): Retryable | undefined {
  // context overflow errors should not be retried
  if (MessageV2.ContextOverflowError.isInstance(error)) return undefined
  if (MessageV2.APIError.isInstance(error)) {
    const status = error.data.statusCode
    // zodecode_change start - Current Zode errors require user action (login/signup), don't retry
    if (isZodeError(error)) return undefined
    // zodecode_change end
    ...
    // zodecode_change start - Zode does not support OpenCode Go upsells. FreeUsageLimitError is not retryable: retrying
...
export function policy(opts: {
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      // zodecode_change start — enforce retry limit
      ...
      return Effect.gen(function* () {
        // zodecode_change start — handle network disconnect via offline handler
