...
import { DEFAULT_HEADERS } from "@/zodecode/const"
import { getZodeProjectId } from "@/zodecode/project-id"
import {
  HEADER_FEATURE,
...
import { ZodeSession } from "@/zodecode/session"
import { ZodeLLM } from "@/zodecode/session/llm"
import { SessionExport } from "@/zodecode/session-export"
import { getActiveOrg } from "@/zodecode/session-export/eligibility"
// zodecode_change end
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // zodecode_change start - attribute Zode gateway usage to the root product session
      const attr = ZodeSession.attribution(input.sessionID)
      // zodecode_change end
      ...
      system.push(
        [
          // zodecode_change start - soul defines core identity and personality
      ...
      if (isOpenaiOauth) {
        // zodecode_change start - prepend soul to instructions
      ...
      const params = yield* plugin.trigger(
        ...
        },
        {
          ...
          // zodecode_change start - gpt-5 via @ai-sdk/openai-compatible proxies (e.g. LiteLLM)
      ...
      // zodecode_change start - resolve project ID and machine ID for zode provider
      const isZode = input.model.api.npm === "@zodecode/zode-gateway"
      const zodeProjectId = yield* isZode
        ? Effect.promise(() => getZodeProjectId().catch(() => undefined))
        : Effect.succeed(undefined)
      const machineId = yield* isZode
        ? Effect.promise(() => Identity.getMachineId().catch(() => undefined))
        : Effect.succeed(undefined)
      // zodecode_change end

      const tools = resolveTools(input)
      // zodecode_change start - cap maxOutputTokens to fit within context after estimating real input size
      params.maxOutputTokens = ZodeLLM.capOutputTokens({
        model: input.model,
        messages,
        tools,
      ...
      // zodecode_change start - capture eligible session export request start
      const exporting = SessionExport.enabled
      const org = yield* exporting && isZode && input.model.isFree === true
        ? Effect.promise(() => getActiveOrg())
        : Effect.succeed({ type: "unknown" as const })
      const started = Date.now()
      const parent = input.parentSessionID ?? ZodeSession.resolveParent(input.sessionID)
      const found = ZodeSession.resolveRoot(input.sessionID)
      const root = parent ? (found === input.sessionID ? parent : found) : input.sessionID
      const exportable =
        exporting && isZode && input.model.isFree === true && org.type === "personal" && input.agent.name !== "title"
      if (exportable) {
      ...
      const result = streamText({
        ...
        abortSignal: input.abort,
        headers: {
          ...(input.model.providerID.startsWith("zode") // zodecode_change
            ? {
                "x-zode-project": opencodeProjectID,
                "x-zode-session": input.sessionID,
                "x-zode-request": input.user.id,
                "x-zode-client": Flag.ZODE_CLIENT,
              }
            : {
                "x-session-affinity": input.sessionID,
                ...(input.parentSessionID ? { "x-parent-session-id": input.parentSessionID } : {}),
                "User-Agent": `opencode/${InstallationVersion}`,
                ...(input.model.providerID !== "anthropic" ? DEFAULT_HEADERS : undefined), // zodecode_change
              }),
          // zodecode_change start - headers for zode provider
          ...(isZode && input.agent.name ? { "x-zodecode-mode": input.agent.name.toLowerCase() } : {}),
          ...(isZode && zodeProjectId ? { [HEADER_PROJECTID]: zodeProjectId } : {}),
          ...(isZode && machineId ? { [HEADER_MACHINEID]: machineId } : {}),
          ...(isZode ? { [HEADER_TASKID]: input.sessionID } : {}),
          ...(isZode && parent ? { [HEADER_PARENT_TASKID]: parent } : {}),
          ...(isZode && attr.feature ? { [HEADER_FEATURE]: attr.feature } : {}),
          // zodecode_change end
          ...input.model.headers,
          ...headers,
        },
        ...
        // zodecode_change start - disable AI SDK span recording (ai.* / gen_ai.*)
        experimental_telemetry: { isEnabled: false },
      })
      // zodecode_change end
      // zodecode_change start - capture eligible session export request completion off the stream path
...
// zodecode_change start - session export stream observer
