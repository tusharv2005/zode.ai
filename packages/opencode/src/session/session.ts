...
// zodecode_change - drop unused inArray/lt (listGlobal delegated to ZodeSession)
import { eq, and, gte, isNull, desc, like, or } from "drizzle-orm"
import { SyncEvent } from "../sync"
import { PartTable, SessionTable } from "./session.sql"
// zodecode_change - ProjectTable removed (unused)
import { Storage } from "@/storage/storage"
import * as Log from "@opencode-ai/core/util/log"
import { MessageV2 } from "./message-v2"
import type { InstanceContext } from "../project/instance"
import { InstanceState } from "@/effect/instance-state"
import { Instance } from "@/project/instance" // zodecode_change - children() uses Instance.current to scope by project_id
...
// zodecode_change start - Zode session behavior extensions
import { BackgroundProcess } from "@/zodecode/background-process"
import { ZodeSession, zodeSessionFork } from "@/zodecode/session"
...
const Summary = Schema.Struct({
  ...
  files: Schema.Finite,
  diffs: optionalOmitUndefined(Schema.Array(Snapshot.SummaryFileDiff)), // zodecode_change - lightweight diff without patch
})
...
export const GlobalInfo = Schema.Struct({
  ...
  worktreeName: Schema.optional(Schema.String), // zodecode_change - basename of the specific worktree directory
})
...
export const CreateInput = Schema.optional(
  Schema.Struct({
    ...
    platform: Schema.optional(Schema.String), // zodecode_change - per-session platform override for telemetry attribution
...
export const Event = {
  ...
  TurnOpen: ZodeSession.Event.TurnOpen,
  TurnClose: ZodeSession.Event.TurnClose,
  // zodecode_change end
}

export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".zode", "plans") // zodecode_change
...
export const getUsage = (input: {
  ...
  provider?: Provider.Info // zodecode_change
}) => {
  ...
  // zodecode_change start - Use provider-reported cost when available for OpenRouter/Zode
  const reported = ZodeSession.providerCost({
...
export interface Interface {
  ...
  readonly create: (input?: {
    ...
    permission?: Permission.Ruleset
    platform?: string // zodecode_change - per-session platform override for telemetry attribution
    workspaceID?: WorkspaceID
  }) => Effect.Effect<Info>
...
export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service | SyncEvent.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      // zodecode_change start - register attribution before session.created subscribers run
      ZodeSession.register({ id: result.id, parentID: result.parentID, platform: input.platform })
      // zodecode_change end
    ...
    // zodecode_change start - scope by project_id when instance context is available
    ...
    const remove: Interface["remove"] = Effect.fnUntraced(function* (sessionID: SessionID) {
      ...
      try {
        ...
        // zodecode_change start
        yield* Effect.promise(() => ZodeSession.removeSession(sessionID)).pipe(Effect.ignore)
        ZodeSession.clearPlatformOverride(sessionID)
        if (hasInstance) {
        ...
        // zodecode_change end
        yield* sync.run(Event.Deleted, { sessionID, info: session }, { publish: hasInstance }) // zodecode_change
        // zodecode_change - capture final session-export workspace delta on close/delete
        const workspaceKey = hasInstance ? yield* InstanceState.directory : undefined // zodecode_change
        yield* Effect.promise(() => SessionExport.onSessionClose(sessionID, workspaceKey)) // zodecode_change
    ...
    const updateMessage = <T extends MessageV2.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        // zodecode_change start - ignore FK errors when session was deleted while processor was still running
        yield* Effect.sync(() =>
          ZodeSession.runSyncSafe(
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      const session = yield* createNext({
        ...
        model: input?.model,
        permission: input?.permission,
        platform: input?.platform, // zodecode_change
        workspaceID: input?.workspaceID ?? workspace, // zodecode_change - allow explicit override
      })
      return session
    })
...
function* listByProject(
  ...
  // zodecode_change start - ZodeSession.filters keeps sessions visible across project_id changes
  ...
  const conditions =
    input.path !== undefined
      ...
      : ZodeSession.filters({ projectID: input.projectID, directory: input.directory })
  // zodecode_change end
...
// zodecode_change start - delegate to ZodeSession.listGlobal (adds projectID worktree family + directories[])
export function* listGlobal(input?: {
  ...
  yield* ZodeSession.listGlobal<GlobalInfo>({ ...input, fromRow })
}
// zodecode_change end

// zodecode_change - preserve Zode recursive fork/remap behavior without a Session service-local Promise runtime
export const fork = zodeSessionFork
