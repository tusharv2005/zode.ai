...
export namespace SessionNetwork {
  ...
  export function disconnected(err: unknown) {
    ...
    // zodecode_change - recognize AbortSignal.timeout() errors
    for (const item of chain(err)) {
  ...
  export function message(err: unknown) {
    // zodecode_change - check for timeout first
    for (const item of chain(err)) {
  ...
  export const reply = fn(
    ...
    async (input) => {
      ...
      // zodecode_change start - reconnect failed remote MCP servers after network recovery
