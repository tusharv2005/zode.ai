...
import { WarpGrepClient } from "@morphllm/morphsdk/tools/warp-grep/client" // zodecode_change
import { Telemetry } from "@zodecode/zode-telemetry" // zodecode_change
...
const Parameters = Schema.Struct({
  query: Schema.String.annotate({
    description: "Search query describing what code you are looking for. Be specific and descriptive for best results.", // zodecode_change
  }),
})
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          Telemetry.trackToolUsed("codebase_search", ctx.sessionID) // zodecode_change
          ...
          const client = new WarpGrepClient({
            morphApiKey: apiKey ?? "zode-free",
            ...(apiKey ? {} : { morphApiUrl: ZODE_WARPGREP_PROXY_URL }),
            timeout: 60_000,
          })
          ...
          if (!result.success || !result.contexts?.length) {
            ...
            const isAuthOrRateLimit =
              result.error && /401|402|429|rate.limit|free.period|unauthorized/i.test(result.error) // zodecode_change
            const apiKeyMsg =
