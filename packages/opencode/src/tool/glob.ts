...
// zodecode_change start — support absolute glob patterns (e.g. ~/.config/zode/command/*.md)
    ...
    return {
      ...
      execute: (params: { pattern: string; path?: string }, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const absolute = split(params.pattern) // zodecode_change
          yield* ctx.ask({
            permission: "glob",
          ...
          const base = absolute?.dir ?? params.path ?? ins.directory // zodecode_change
          ...
          const files = yield* rg
            .files({ cwd: search, glob: [absolute?.pattern ?? params.pattern], signal: ctx.abort }) // zodecode_change
            .pipe(
