import { lstat } from "fs/promises" // zodecode_change
import { Effect, Option, Schema, Scope } from "effect"
import { NonNegativeInt } from "@opencode-ai/core/schema"
import * as path from "path"
import type { Readable } from "stream" // zodecode_change
...
import * as Encoding from "../zodecode/encoding"
import * as TextStream from "../zodecode/text-stream"
import * as Extract from "../zodecode/tool/read-extract"
// zodecode_change end
...
const DIRECTORY_CONCURRENCY = 8 // zodecode_change
...
export const ReadTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const isBinaryFile = (filepath: string, bytes: Uint8Array) => {
      ...
      // zodecode_change start - UTF-16/32 BOM: NUL bytes are legitimate, skip the NUL/control-char heuristic
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      // zodecode_change start - route extractable binary documents through lines()
...
// zodecode_change start - exported (so readDirectoryFiles can reuse it) and
...
export async function lines(filepath: string, opts: { limit: number; offset: number }) {
  const extracted = await Extract.open(filepath) // zodecode_change - extract supported document contents before paging
  if (extracted) return readLines(extracted, opts) // zodecode_change
