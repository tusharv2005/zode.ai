...
const MAX_DIFF_CONTENT = 500_000 // zodecode_change
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          let contentOld = ""
          let contentNew = ""
          let cachedFilediff: Snapshot.FileDiff | undefined // zodecode_change
          yield* lock(filePath).withPermits(1)(
            Effect.gen(function* () {
              if (params.oldString === "") {
                ...
                cachedFilediff = buildFileDiff(filePath, contentOld, contentNew) // zodecode_change
                yield* ctx.ask({
                  permission: "edit",
                  ...
                  metadata: {
                    filepath: filePath,
                    diff,
                    filediff: cachedFilediff, // zodecode_change
                  },
                })
                yield* EncodedIO.write(filePath, Bom.join(contentNew, desiredBom), source.encoding) // zodecode_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
              ...
              yield* EncodedIO.write(filePath, Bom.join(contentNew, desiredBom), source.encoding) // zodecode_change - encoding-aware write replaces afs.writeWithDirs
              if (yield* format.file(filePath)) {
                contentNew = yield* Bom.syncFile(afs, filePath, desiredBom)
              }
              yield* bus.publish(File.Event.Edited, { file: filePath })
          ...
          const filediff: Snapshot.FileDiff = cachedFilediff ?? buildFileDiff(filePath, contentOld, contentNew) // zodecode_change
