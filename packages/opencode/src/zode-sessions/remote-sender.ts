import { RemoteProtocol } from "@/zode-sessions/remote-protocol"
import type { RemoteWS } from "@/zode-sessions/remote-ws"
...
// zodecode_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | undefined) {
  ...
  return {
    providerID: ProviderID.make("zode"),
    modelID: ModelID.make(model.startsWith("zodecode/") ? model.slice("zodecode/".length) : model),
  }
}
