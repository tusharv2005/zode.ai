...
import { IngestQueue } from "@/zode-sessions/ingest-queue"
import { clearInFlightCache, withInFlightCache } from "@/zode-sessions/inflight-cache"
import type * as SDK from "@zodecode/sdk/v2"
import z from "zod"
import { Context, Effect, Layer, Schema, Stream } from "effect"
import { ZODE_API_BASE } from "@zodecode/zode-gateway"
...
import { RemoteWS } from "@/zode-sessions/remote-ws"
import { RemoteSender } from "@/zode-sessions/remote-sender"
...
export namespace ZodeSessions {
  export const Event = {
    RemoteStatusChanged: BusEvent.define(
      "zode-sessions.remote-status-changed",
      Schema.Struct({
  ...
  export class Service extends Context.Service<Service, Interface>()("@zodecode/ZodeSessions") {}

  const log = Log.create({ service: "zode-sessions" })
  const runtime = makeRuntime(Auth.Service, Auth.defaultLayer)

  const Uuid = z.uuid()
  type Uuid = z.infer<typeof Uuid>

  const tokenValidKeyTemplate = "zode-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "zode-sessions:token"
  const orgKey = "zode-sessions:org"
  const clientKey = "zode-sessions:client"
  const gitUrlKeyPrefix = "zode-sessions:git-url:"
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${ZODE_API_BASE}/api/user`, {
        headers: {
  ...
  async function zodecodeToken() {
    return withInFlightCache(tokenKey, ttlMs, async () => {
      const auth = await runtime.runPromise((svc) => svc.get("zode"))
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await zodecodeToken()
  ...
  const shareDisabled = process.env["ZODE_DISABLE_SHARE"] === "true" || process.env["ZODE_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["ZODE_DISABLE_SESSION_INGEST"] === "true" || process.env["ZODE_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["ZODE_DEBUG_SESSION_INGEST"] === "true" || process.env["ZODE_DEBUG_SESSION_INGEST"] === "1"
  ...
  const remoteEnabled = process.env["ZODE_REMOTE"] === "1"
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("ZodeSessions.state")(function* () {
          ...
          yield* watch(Session.Event.Updated, async (evt) => {
            ...
            await ingest.sync(sessionID, [
              { type: "zode_meta", data: await meta(sessionID) },
      ...
      const init = Effect.fn("ZodeSessions.init")(function* () {
  ...
  export async function enableRemote() {
    ...
    enabling = (async () => {
      const token = await zodecodeToken()
      if (!token) {
        throw new Error("Unable to enable remote: no Zode credentials found. Run `zode auth login`.")
      }

      const valid = await authValid(token)
      if (valid === false) {
        throw new Error("Unable to enable remote: invalid or expired Zode credentials. Run `zode auth login`.")
      }
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (ZODE_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (ZODE_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to share session: no Zode credentials found. Run `zode auth login`.")
    }
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (ZODE_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to unshare session: no Zode credentials found. Run `zode auth login`.")
    }
  ...
  async function meta(sessionId?: string) {
    const override = sessionId ? ZodeSession.resolvePlatform(sessionId) : undefined
    const platform = override || process.env["ZODE_PLATFORM"] || "cli"
