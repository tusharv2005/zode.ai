...
import { existsSync } from "node:fs" // zodecode_change

const subcommand = "pr" // zodecode_change

// zodecode_change start - resolve the currently running CLI instead of hardcoding opencode
export function cliCommand(
  ...
  if (script === subcommand) return [input.execPath] // zodecode_change
...
export const PrCommand = effectCmd({
  command: `${subcommand} <number>`, // zodecode_change
  describe: "fetch and checkout a GitHub PR branch, then run zode", // zodecode_change
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    const cli = cliCommand() // zodecode_change
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        const sessionMatch = prInfo.body.match(/https:\/\/app\.zode\.ai\/s\/([a-zA-Z0-9_-]+)/) // zodecode_change
        if (sessionMatch) {
    ...
    UI.println()
    UI.println("Starting zode...") // zodecode_change
    UI.println()

    const run = sessionId ? [...cli, "-s", sessionId] : cli // zodecode_change
    ...
    if (code !== 0) return yield* Effect.die(new Error(`zode exited with code ${code}`)) // zodecode_change
  }),
})
