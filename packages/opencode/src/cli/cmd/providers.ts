...
import type { Hooks } from "@zodecode/plugin"
...
export const ProvidersCommand = cmd({
  // zodecode_change start - keep "auth" as primary command name
  command: "auth",
  aliases: ["providers"],
  // zodecode_change end
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "zode auth provider", // zodecode_change
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    const options = [
      ...pipe(
        ...
        map((x) => ({
          ...
          value: x.id,
          hint: {
            zode: "recommended", // zodecode_change
            opencode: "recommended",
            openai: "ChatGPT login or API key", // zodecode_change
          }[x.id],
        })),
      ),
    ...
    if (args.provider) {
      ...
      // zodecode_change start - accept codex as an alias for OpenAI ChatGPT auth
