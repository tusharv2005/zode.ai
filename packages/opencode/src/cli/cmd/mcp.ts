...
import { ZodecodeMcpConfig } from "@/zodecode/cli/cmd/mcp" // zodecode_change
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: zode mcp add") // zodecode_change
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in zode.json:") // zodecode_change
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // zodecode_change start - prefer zode.json/.zode over opencode.json/.opencode
  // Check for existing config files (prefer .jsonc over .json, check .zode/ and .opencode/ subdirectory too)
  const candidates = [
    path.join(baseDir, "zode.json"),
    path.join(baseDir, "zode.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".zode", "zode.json"),
      path.join(baseDir, ".zode", "zode.jsonc"),
      path.join(baseDir, ".zode", "opencode.json"),
      path.join(baseDir, ".zode", "opencode.jsonc"),
  ...
  // Default to zode.json if none exist
  return path.join(baseDir, "zode.json")
  // zodecode_change end
}
...
async function addMcpToConfig(name: string, mcpConfig: ConfigMCP.Info, configPath: string) {
  ...
  const result = ZodecodeMcpConfig.format(configPath, applyEdits(text, edits)) // zodecode_change
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., zode x @modelcontextprotocol/server-filesystem", // zodecode_change
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "zode-debug", version: InstallationVersion }, // zodecode_change
            },
            id: 1,
          }),
        })
        ...
        if (response.status === 401) {
          ...
          try {
            const client = new Client({
              name: "zode-debug", // zodecode_change
