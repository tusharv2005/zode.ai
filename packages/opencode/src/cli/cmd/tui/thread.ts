...
import type { GlobalEvent } from "@zodecode/sdk/v2"
import type { EventSource } from "./context/sdk"
import { win32DisableProcessedInput, win32InstallCtrlCGuard } from "./win32"
import { importCloudSession, validateCloudFork } from "@/zodecode/cloud-session" // zodecode_change
import { createZodeClient } from "@zodecode/sdk/v2" // zodecode_change
import { writeHeapSnapshot } from "v8"
import { TuiConfig } from "./config/tui"
import { ZodeTuiThreadDaemon } from "@/zodecode/cli/cmd/tui/thread" // zodecode_change
import {
  ZODE_PROCESS_ROLE,
  ZODE_RUN_ID,
  ensureRunID,
...
declare global {
  const ZODE_WORKER_PATH: string
}

type RpcClient = ReturnType<typeof Rpc.client<typeof rpc>>

// zodecode_change start - lazy-load the TUI app after daemon attach in source mode
...
async function target() {
  if (typeof ZODE_WORKER_PATH !== "undefined") return ZODE_WORKER_PATH
...
export function resolveThreadDirectory(project?: string, envPWD = process.env.PWD, cwd = process.cwd()) {
  // zodecode_change start - ignore stale PWD from wrappers such as `bun --cwd`
  ...
  return real // zodecode_change
}

export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start zode tui", // zodecode_change
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start zode in", // zodecode_change
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // zodecode_change start - default TUI sessions attach to the daemon unless explicitly disabled
      if (await ZodeTuiThreadDaemon.attach({ args, cwd, input: () => input(args.prompt), start })) return
      // zodecode_change end
      const env = sanitizedProcessEnv({
        [ZODE_PROCESS_ROLE]: "worker",
        [ZODE_RUN_ID]: ensureRunID(),
        ZODE_BACKGROUND_PROCESS_PORTS: "true", // zodecode_change - TUI surfaces inferred background process ports
      })
      ...
      // zodecode_change start - graceful shutdown on external signals
      ...
      try {
        await validateSession({
          url: transport.url, // zodecode_change
      ...
      try {
        // zodecode_change start - import cloud session before TUI renders
        ...
        await start({
          // zodecode_change - shared lazy loader also supports daemon attach
          url: transport.url,
          async onSnapshot() {
