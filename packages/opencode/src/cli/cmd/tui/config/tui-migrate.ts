...
import { access, constants } from "fs/promises" // zodecode_change
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  // zodecode_change start: use zode directory everywhere
  const project = Flag.ZODE_DISABLE_PROJECT_CONFIG
    ? []
    : await Filesystem.findUp(["zode.json", "zode.jsonc"], input.cwd, undefined, { rootFirst: true })
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "zode")]
  // zodecode_change end
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "zode"))
  }
  if (Flag.ZODE_CONFIG) files.push(Flag.ZODE_CONFIG)
