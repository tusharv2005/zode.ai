...
import { ZodecodeDefaultPlugins } from "@/zodecode/config/default-plugins" // zodecode_change
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and ZODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ZODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ZODE_TUI_CONFIG override, if set.
  if (Flag.ZODE_TUI_CONFIG) {
    const configFile = Flag.ZODE_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and ZODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  // zodecode_change start - also load tui.json from .zode/.zodecode
  const dirs = unique(directories).filter(
    (dir) =>
      dir.endsWith(".zode") || dir.endsWith(".zodecode") || dir.endsWith(".opencode") || dir === Flag.ZODE_CONFIG_DIR,
  )
  // zodecode_change end

  for (const dir of dirs) {
    // if (!dir.endsWith(".opencode") && dir !== Flag.ZODE_CONFIG_DIR) continue // zodecode_change
  ...
  // zodecode_change start — inject Zode default plugins to keep TUI aligned with server config
