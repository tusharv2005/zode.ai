import type {
  ...
  PermissionRequest,
  QuestionRequest,
  SuggestionRequest, // zodecode_change
  SessionNetworkWait, // zodecode_change
  LspStatus,
  McpStatus,
  McpResource,
  FormatterStatus,
  ...
  ProviderAuthMethod,
  VcsInfo,
  BackgroundProcessInfo, // zodecode_change
} from "@zodecode/sdk/v2"
...
import { batch, createEffect, on, onMount } from "solid-js" // zodecode_change - add createEffect/on for workspace re-bootstrap
import { handleSuggestionEvent } from "@/zodecode/suggestion/tui/sync" // zodecode_change
import { useToast } from "@tui/ui/toast" // zodecode_change
import * as Log from "@opencode-ai/core/util/log"
import { emptyConsoleState, type ConsoleState } from "@/config/console-state"
import type { IndexingStatus } from "@zodecode/zode-indexing/status" // zodecode_change
import { ZodeIndexing } from "@/zodecode/indexing" // zodecode_change
...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    const [store, setStore] = createStore<{
      ...
      // zodecode_change end
      config: Config
      globalConfig: Config // zodecode_change
      session: Session[]
      session_status: {
        [sessionID: string]: SessionStatus
      }
      session_diff: {
        [sessionID: string]: Omit<Snapshot.FileDiff, "before" | "after">[] // zodecode_change
      }
      todo: {
      ...
      indexing: IndexingStatus // zodecode_change
    }>({
      provider_next: {
        all: [],
        default: {},
        connected: [],
        failed: [], // zodecode_change
      },
      console_state: emptyConsoleState,
      provider_auth: {},
      config: {},
      globalConfig: {}, // zodecode_change
      status: "loading",
      agent: [],
      permission: {},
      question: {},
      ...
      session_diff: {},
      todo: {},
      background_process: {}, // zodecode_change
      message: {},
      part: {},
      lsp: [],
      mcp: {},
      ...
      formatter: [],
      vcs: undefined,
      indexing: { state: "Disabled", message: "Indexing disabled.", processedFiles: 0, totalFiles: 0, percent: 0 }, // zodecode_change
    })

    const event = useEvent()
    const project = useProject()
    const sdk = useSDK()
    const toast = useToast() // zodecode_change
    const kv = useKV()
    ...
    function evict(sessionID: string) {
      ...
      setStore(
        produce((draft) => {
          ...
          delete draft.background_process[sessionID] // zodecode_change
    ...
    const deleted = new Set<string>() // zodecode_change
      ...
      switch (event.type) {
        ...
        case "session.network.restored": {
          ...
          if (match.found) {
            ...
            setStore("network", event.properties.sessionID, match.index, "time", "restored", event.properties.time) // zodecode_change
          }
          break
        }
    ...
    async function bootstrap(input: { fatal?: boolean } = {}) {
      ...
      const globalConfigPromise = sdk.client.global.config.get({ throwOnError: true }) // zodecode_change
      const blockingRequests: { name: string; promise: Promise<unknown> }[] = [
        ...
        { name: "global.config", promise: globalConfigPromise }, // zodecode_change
      ...
      await Promise.allSettled(blockingRequests.map((r) => r.promise))
        ...
        .then(async () => {
          ...
          const globalConfigResponse = globalConfigPromise.then((x) => x.data!) // zodecode_change
          ...
          return Promise.all([
            ...
            agentsResponse,
            configResponse,
            globalConfigResponse, // zodecode_change
            ...
            const globalConfig = responses[5] // zodecode_change
            const sessions = responses[6] // zodecode_change
            ...
            batch(() => {
              ...
              setStore("globalConfig", reconcile(globalConfig)) // zodecode_change
        ...
        .then(() => {
          ...
          void Promise.all([
            ...
            // zodecode_change end
            sdk.client.formatter.status({ workspace }).then((x) => setStore("formatter", reconcile(x.data!))), // zodecode_change
            // zodecode_change start
            ...
            // zodecode_change start - show config warnings as persistent toast
            sdk.client.config
              .warnings()
              .then((x) => {
                const list = x.data ?? []
              ...
              .catch(() => {}),
            ZodeIndexing.current().then((x) => setStore("indexing", reconcile(x))),
            // zodecode_change end
          ]).then(() => {
    ...
    // zodecode_change start - re-bootstrap when workspace changes (Agent Manager)
    createEffect(
      on(
    ...
    const result = {
      ...
      get ready() {
        // return true // zodecode_change - upstream #23037 left this debug path enabled; keep it commented so future merges do not restore eager ready state.
        if (process.env.ZODE_FAST_BOOT) return true
          ...
          setStore(
            produce((draft) => {
              ...
              for (const message of messages.data ?? []) {
                infos.push(strip(message.info)) // zodecode_change
