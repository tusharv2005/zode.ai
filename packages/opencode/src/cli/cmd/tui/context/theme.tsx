...
import zode from "./theme/zode.json" with { type: "json" } // zodecode_change
import zode1 from "./theme/zode-v1.json" with { type: "json" } // zodecode_change
...
import colorblind from "./theme/colorblind.json" with { type: "json" } // zodecode_change
...
import type { TuiThemeCurrent } from "@zodecode/plugin/tui"
...
export const DEFAULT_THEMES: Record<string, ThemeJson> = {
  ...
  gruvbox,
  kanagawa,
  zode, // zodecode_change
  ["zode-v1"]: zode1, // zodecode_change
  material,
  matrix,
  mercury,
  monokai,
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "zode") // zodecode_change
        draft.active = typeof active === "string" ? active : "zode" // zodecode_change
        draft.ready = false
      }),
    )
    ...
    function init() {
      void Promise.allSettled([
        ...
        getCustomThemes()
          ...
          .catch(() => {
            setStore("active", "zode") // zodecode_change
          }),
      ]).finally(() => {
        setStore("ready", true)
      })
    ...
    // zodecode_change start - safe fallback to zode import if store lookup fails
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.zode, store.mode) // zodecode_change
    })
    // zodecode_change end
    ...
    // zodecode_change - use empty object as proxy target; all reads go through the getter
    return {
...
async function getCustomThemes() {
  const directories = [
    ...
    ...(await Array.fromAsync(
      Filesystem.up({
        targets: [".zode", ".opencode"], // zodecode_change
        start: process.cwd(),
      }),
    )),
  ]
  ...
  for (const dir of directories) {
    for (const item of await Glob.scan("themes/*.json", {
      ...
      // zodecode_change start - validate custom theme JSON and protect built-in keys
