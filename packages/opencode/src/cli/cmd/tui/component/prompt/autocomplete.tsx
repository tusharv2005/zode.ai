...
import { slashDisplay } from "@/zodecode/cli/cmd/command-display" // zodecode_change
...
export type AutocompleteRef = {
  ...
  // zodecode_change start - validate cursor moves and close overlays without mutating draft text
  onCursorChange: () => void
  dismiss: () => void
  // zodecode_change end
  visible: false | "@" | "/"
...
export function Autocomplete(props: {
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    for (const serverCommand of sync.data.command) {
      // zodecode_change start - preserve suffixes like :skill when inserting selected slash commands
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      // zodecode_change start - close stale suggestions while allowing normal cursor movement
      {
        key: "right",
        fallthrough: true,
        cmd: () => {
  ...
  // zodecode_change start - keep slash text intact when overlays hide the prompt,
  ...
  onMount(() => {
    ...
    props.ref({
      ...
      // zodecode_change start - dismiss stale popup after cursor leaves active filter region
      onCursorChange() {
        if (!store.visible) return
