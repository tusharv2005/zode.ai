...
import type { AssistantMessage, FilePart, UserMessage } from "@zodecode/sdk/v2"
...
import { ZodeSessionTuiSync } from "@/zodecode/session/tui-sync" // zodecode_change
import { slashMatches } from "@/zodecode/cli/cmd/command-display" // zodecode_change
...
export function Prompt(props: PromptProps) {
  ...
  const [store, setStore] = createStore<{
    ...
    interrupt: number
    exitPress: number // zodecode_change - track double ctrl+c to exit
    placeholder: number
  }>({
  ...
  // zodecode_change start - sync local agent/model whenever newest user message changes
  let syncedKey: string | undefined
  createEffect(() => {
    const sessionID = props.sessionID
    const msg = lastUserMessage()
    if (!sessionID || !msg) return
    // zodecode_change start - skip compaction messages while syncing local agent/model
    const parts = sync.data.part[msg.id]
    if (!parts) return
    if (!ZodeSessionTuiSync.model({ role: msg.role, parts })) return
    // zodecode_change end
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.ZODE_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
  ...
  // zodecode_change start - close autocomplete while blocking overlays hide the prompt
  createEffect(() => {
  ...
  // zodecode_change start - require a double Ctrl+C to exit from an empty focused prompt
  useBindings(() => ({
    target: inputTarget,
    enabled: inputTarget() !== undefined && !props.disabled && store.prompt.input === "",
    bindings: [
  ...
  async function submit() {
    ...
    if (store.mode === "shell") {
      void sdk.client.session.shell({
        sessionID,
        agent: local.agent.current()?.name ?? "", // zodecode_change
        model: {
          providerID: selectedModel.providerID,
      ...
      iife(() => {
        ...
        return sync.data.command.some((x) => slashMatches(x, command)) // zodecode_change
      })
    ) {
    ...
    toast.dismiss() // zodecode_change - dismiss persistent config warning on first submit
    history.append({
      ...store.prompt,
      mode: currentMode,
    })
  ...
  async function pasteInputText(text: string) {
    ...
    if (
      (lineCount >= 5 || pastedContent.length > 800) && // zodecode_change #7252 delay paste summary
  ...
  const highlight = createMemo(() => {
    ...
    return local.agent.color(agent.name ?? "") // zodecode_change
  })
  ...
  const spinnerDef = createMemo(() => {
    const agent = local.agent.current()
    const color = agent ? local.agent.color(agent.name ?? "") : theme.border // zodecode_change
    return {
      frames: createFrames({
        color,
        style: "blocks",
  ...
  return (
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false}>
        ...
        >
          ...
          >
            <textarea
              ...
              onCursorChange={() => {
                setCursorVersion((value) => value + 1)
                // zodecode_change start - dismiss autocomplete when cursor movement invalidates its filter
        ...
        <box
          height={1}
          flexShrink={0} // zodecode_change - prevent border box from shrinking in narrow terminals (#6309)
          border={["left"]}
          borderColor={borderHighlight()}
          customBorderChars={{
            ...EmptyBorder,
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          ...
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              {/* zodecode_change start - show "ctrl+c again to exit" hint */}
