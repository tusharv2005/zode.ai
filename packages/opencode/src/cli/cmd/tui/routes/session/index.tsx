...
import type { KeyEvent } from "@opentui/core" // zodecode_change
import type { CommandContext } from "@opentui/keymap" // zodecode_change
import { Prompt, type PromptRef } from "@tui/component/prompt"
// zodecode_change start
import type { AssistantMessage, Part, Provider, ToolPart, UserMessage, TextPart, ReasoningPart } from "@zodecode/sdk/v2"
...
// zodecode_change start
import type { BackgroundProcessTool } from "@/zodecode/tool/background-process"
import type { SemanticSearchTool } from "@/zodecode/tool/semantic-search"
// zodecode_change end
...
import { ZodeErrorBlock } from "@/zodecode/components/zode-error-display" // zodecode_change
...
import { Suggest } from "@/zodecode/suggestion/tui/render" // zodecode_change
import { SuggestPrompt } from "@/zodecode/suggestion/tui/prompt" // zodecode_change
import { NetworkPrompt } from "./network" // zodecode_change
import { DialogExportOptions } from "../../ui/dialog-export-options"
import * as Model from "../../util/model"
import { formatTranscript } from "../../util/transcript"
import { UI } from "@/cli/ui.ts"
import { useTuiConfig } from "../../context/tui-config"
import { splitDiffHunks } from "@/zodecode/tui/diff" // zodecode_change
import { session as banner } from "@/zodecode/cli/logo" // zodecode_change

import { formatMarkdownTables } from "../../util/markdown" // zodecode_change
import { bell } from "@/zodecode/bell" // zodecode_change
import { SessionIndexing } from "@/zodecode/components/session-indexing" // zodecode_change
import { submitFeedback } from "@/zodecode/cli/cmd/tui/feedback" // zodecode_change
...
const sessionBindingCommands = [
  ...
  "messages.copy",
  // zodecode_change start - message feedback
...
export function Session() {
  ...
  // zodecode_change start - footer overlay only hosts blocking suggestions now;
  ...
  // zodecode_change start - ring terminal bell on task completion
  createEffect(
    on(
  ...
  // zodecode_change start - ring terminal bell when input is needed
  createEffect(
    on(
  ...
  createEffect(
    on(
      () => [route.sessionID, suggestions().length + network().length] as const, // zodecode_change
      ([id, len], prev) => {
        if (!prev || prev[0] !== id) return
        if (len > prev[1] && bellEnabled()) bell()
      },
  ...
  const [bellEnabled, _setBellEnabled] = kv.signal("bell_enabled", true) // zodecode_change - terminal bell toggle (toggled via kv.set in app.tsx command)
  ...
  // zodecode_change start - background processes are scoped to the visible session
  ...
  event.on("message.part.updated", (evt) => {
    ...
    // zodecode_change - plan_exit no longer switches agent; PlanFollowup handles it
  ...
  // zodecode_change start - double ctrl+c to exit for child sessions
    ...
    // zodecode_change end
    {
      ...
      run: async () => {
        try {
          ...
          // zodecode_change start - fetch all messages from server instead of truncated sync store
          ...
          const transcript = formatTranscript(
            sessionData,
            sessionMessages, // zodecode_change
            {
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  // zodecode_change start - format markdown tables with fixed-width columns
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.ZODE_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
...
function Edit(props: ToolProps<typeof EditTool>) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diffContent() ?? "")) // zodecode_change
