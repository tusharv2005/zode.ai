...
const AGENT_USERNAME = "zodeconnect[bot]" // zodecode_change
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/zode.yml" // zodecode_change
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              step2,
              "",
              "    3. Go to a GitHub issue and comment `/zode summarize` to see the agent in action", // zodecode_change
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/zodeconnect" // zodecode_change
          const command =
        ...
        async function addWorkflowFiles() {
          // zodecode_change start - updated workflow template with Zode branding and gateway secrets
          const providerEnvStr =
            provider === "amazon-bedrock"
              ? ""
              : providers[provider].env.map((e) => `\n          ${e}: \${{ secrets.${e} }}`).join("")

          const zodeGatewayEnv =
            provider === "zode"
              ? `\n          ZODE_API_KEY: \${{ secrets.ZODE_API_KEY }}\n          ZODE_ORG_ID: \${{ secrets.ZODE_ORG_ID }}`
              : ""

          const envStr = providerEnvStr || zodeGatewayEnv ? `\n        env:${providerEnvStr}${zodeGatewayEnv}` : ""
...
jobs:
  zode:
    if: |
      ...
      startsWith(github.event.comment.body, '/zode')
    ...
    steps:
      ...
      - name: Run Zode
        uses: Zode-Org/zodecode/github@latest${envStr}
        with:
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      try {
        ...
        // Setup zode session // zodecode_change
        ...
        console.log("zode session", session.id) // zodecode_change
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/zode,/kc") // zodecode_change
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to zode...") // zodecode_change
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("zode-github-action") // zodecode_change
        } catch (error) {
      ...
      async function exchangeForAppToken(token: string) {
        // zodecode_change start - updated endpoint URLs per new API structure
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `zode/${type}-${hex}-${timestamp}` // zodecode_change
        }
        return `zode/${type}${issueId}-${timestamp}` // zodecode_change
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the zode infrastructure after your response", // zodecode_change
