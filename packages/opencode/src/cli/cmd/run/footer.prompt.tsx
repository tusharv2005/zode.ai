...
import { slashDisplay, slashMatches } from "@/zodecode/cli/cmd/command-display" // zodecode_change
...
function parseSlashCommand(text: string, commands: RunCommand[] | undefined) {
  ...
  if (!commands.some((item) => slashMatches(item, head.name))) { // zodecode_change
    return { type: "none" as const }
  }
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const slashOptions = createMemo<SlashOption[]>(() => {
    ...
    return [
      ...(input.commands() ?? [])
        .filter((item) => !hidden.has(item.name)) // zodecode_change - suggest skills as slash commands
        .map(
          (item) =>
            ({
              kind: "slash",
              name: item.name,
              display: slashDisplay(item), // zodecode_change
              description: item.description,
  ...
  const select = (item?: PromptOption) => {
    ...
    if (next.kind === "slash") {
      const text = `${next.display} ` // zodecode_change
      const cursor = area.cursorOffset
