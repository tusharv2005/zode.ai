...
//
// Enable with ZODE_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/zode/log/direct/<timestamp>-<pid>.jsonl. Also writes // zodecode_change
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.ZODE_DIRECT_TRACE) {
    state = false
    return undefined
  }
