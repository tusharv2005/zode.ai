...
import * as Config from "@/config/config" // zodecode_change
...
import os from "os"
import z from "zod" // zodecode_change
import { evaluate as evalRule } from "./evaluate"
import { PermissionID } from "./schema"
import { ConfigProtection } from "@/zodecode/permission/config-paths" // zodecode_change
import { Identifier } from "@/id/id" // zodecode_change
import { drainCovered } from "@/zodecode/permission/drain" // zodecode_change
import { ReadPermission } from "@/zodecode/permission/read" // zodecode_change
import { ExternalDirectoryPermission } from "@/zodecode/permission/external-directory" // zodecode_change
...
export const AskInput = Schema.Struct({
  ...
  ruleset: Ruleset,
  hardRuleset: Schema.optional(Ruleset), // zodecode_change
})
...
export interface Interface {
  ...
  readonly reply: (input: ReplyInput) => Effect.Effect<boolean> // zodecode_change
  readonly list: () => Effect.Effect<ReadonlyArray<Request>>
  readonly saveAlwaysRules: (input: z.infer<typeof SaveAlwaysRulesInput>) => Effect.Effect<boolean> // zodecode_change
  readonly allowEverything: (input: z.infer<typeof AllowEverythingInput>) => Effect.Effect<void> // zodecode_change
  readonly pending: (id: string) => Effect.Effect<Request | undefined> // zodecode_change
}

interface PendingEntry {
  info: Request
  ruleset: Ruleset // zodecode_change
  hardRuleset?: Ruleset // zodecode_change
  saved?: boolean // zodecode_change
...
interface State {
  ...
  approved: Ruleset
  session: Record<string, Ruleset> // zodecode_change
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const config = yield* Config.Service // zodecode_change
    const state = yield* InstanceState.make<State>(
      Effect.fn("Permission.state")(function* (ctx) {
        ...
        const state = {
          ...
          approved: row?.data ?? [],
          session: {} as Record<string, Ruleset>, // zodecode_change
        }
    ...
    const ask = Effect.fn("Permission.ask")(function* (input: AskInput) {
      ...
      const { ruleset, hardRuleset, ...request } = input // zodecode_change
      const s = yield* InstanceState.get(state) // zodecode_change
      const local = s.session[request.sessionID] ?? [] // zodecode_change
      let needsAsk = false

      // zodecode_change start — force "ask" for config file edits
      const isProtected = ConfigProtection.isRequest(request)
      // zodecode_change end

      for (const pattern of request.patterns) {
        const rule = resolve(request.permission, pattern, ruleset, approved, local) // zodecode_change — include session-scoped rules
        log.info("evaluated", { permission: request.permission, pattern, action: rule })
        // zodecode_change start — saved/session approvals cannot override hard Ask/Plan denials
        ...
        if (rule.action === "deny") {
          return yield* new DeniedError({
            ruleset: subset(request.permission, ruleset), // zodecode_change
          })
        }
        // zodecode_change start — override "allow" to "ask" for config paths
      ...
      // zodecode_change start — inject disableAlways metadata for config paths
      ...
      pending.set(id, { info, ruleset, hardRuleset, deferred }) // zodecode_change
    ...
    const reply = Effect.fn("Permission.reply")(function* (input: ReplyInput) {
      ...
      if (!existing) return false // zodecode_change
      ...
      if (input.reply === "reject") {
        ...
        return true // zodecode_change
      }

      yield* Deferred.succeed(existing.deferred, undefined)
      if (input.reply === "once") return true // zodecode_change

      // zodecode_change start — downgrade "always" to "once" for config file edits
      if (ConfigProtection.isRequest(existing.info)) return true // zodecode_change
      // zodecode_change end

      for (const pattern of existing.info.always) {
        // zodecode_change start — saveAlwaysRules may have already persisted selected always-rules
        if (!existing.saved) {
          approved.push({
      ...
      // zodecode_change start — drain covered permissions across sibling Agent Manager sessions
      yield* drainCovered(pending as unknown as Map<string, PendingEntry>, approved, DeniedError)
      // zodecode_change end

      // zodecode_change start — persist always-rules to global config
      if (!existing.saved) {
    ...
    const saveAlwaysRules = Effect.fn("Permission.saveAlwaysRules")(function* (
      ...
      existing.saved = true // zodecode_change
    ...
    const allowEverything = Effect.fn("Permission.allowEverything")(function* (
      ...
      if (input.requestID) {
        ...
        const ok = entry ? covered(entry, s.approved, s.session[entry.info.sessionID] ?? []) : false // zodecode_change
      ...
      for (const [id, entry] of s.pending) {
        ...
        if (!covered(entry, s.approved, s.session[entry.info.sessionID] ?? [])) continue // zodecode_change
        s.pending.delete(id)
        yield* bus.publish(Event.Replied, {
          sessionID: entry.info.sessionID,
          requestID: entry.info.id,
...
export function fromConfig(permission: ConfigPermission.Info) {
  ...
  for (const [key, value] of Object.entries(permission)) {
    ...
    if (value === null) continue // zodecode_change — null is a delete sentinel
    ruleset.push(
      // zodecode_change start — filter out null entries (delete sentinels)
...
export const defaultLayer = layer.pipe(Layer.provide(Bus.layer), Layer.provide(Config.defaultLayer)) // zodecode_change

// zodecode_change start — inverse of fromConfig: convert rules back to config format
