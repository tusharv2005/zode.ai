...
import type { Hooks, PluginInput } from "@zodecode/plugin" // zodecode_change
...
function authHeaders() {
  return {
    ...
    "User-Agent": `zodecode/${InstallationVersion}`,
  }
}
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=zodecode` lets xAI attribute zodecode-originated
...
<html>
  <head>
    <title>Zodecode - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to Zodecode.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>Zodecode - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire Zodecode process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `zodecode/${InstallationVersion}`)
