...
import { ZodecodePaths } from "@/zodecode/paths" // zodecode_change
...
import * as ZodeAgent from "@/zodecode/agent" // zodecode_change
...
export const Info = Schema.Struct({
  name: Schema.String,
  displayName: Schema.optional(Schema.String), // zodecode_change - human-readable name for org modes
  description: Schema.optional(Schema.String),
  deprecated: Schema.optional(Schema.Boolean), // zodecode_change
...
type State = Omit<Interface, "generate"> & { version: string } // zodecode_change
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Agent.state")(function* (ctx) {
        ...
        // zodecode_change start - include global config dirs so agents can read them without prompting
        const whitelistedDirs = [
          Truncate.GLOB,
          path.join(Global.Path.tmp, "*"),
          ...skillDirs.map((dir) => path.join(dir, "*")),
          path.join(Global.Path.config, "*"),
          ...ZodecodePaths.globalDirs().map((dir) => path.join(dir, "*")),
        ]
        // zodecode_change end
        ...
        const baseDefaults = Permission.fromConfig({
          // zodecode_change: renamed from defaults
          "*": "allow",
          doom_loop: "ask",
          external_directory: {
            "*": "ask",
        ...
        // zodecode_change start - patch defaults with bash allowlist and recall permission
        const zode = ZodeAgent.prepare(cfg)
        const defaults = Permission.merge(baseDefaults, zode.defaultsPatch)
        // zodecode_change end
        ...
        // zodecode_change start - rename build→code, add debug/orchestrator/ask, patch plan/explore
        ZodeAgent.patchAgents(agents, defaults, user, cfg, zode, ctx.worktree, whitelistedDirs)
        // zodecode_change end

        // zodecode_change start - preprocess config to remap "build" key → "code"
        const agentConfigs = ZodeAgent.preprocessConfig(cfg.agent ?? {})
        for (const [key, value] of Object.entries(agentConfigs)) {
          ...
          ZodeAgent.processConfigItem(item) // zodecode_change - populate displayName from options
        }
        ...
        if (Flag.ZODE_EXPERIMENTAL_SCOUT) {
        ...
        const get = Effect.fnUntraced(function* (agent: string) {
          return agents[ZodeAgent.resolveKey(agent)] // zodecode_change - treat "build" as "code"
        })
        ...
        const list = Effect.fnUntraced(function* () {
          ...
          return pipe(
            ...
            values(),
            sortBy(
              [(x) => (cfg.default_agent ? x.name === cfg.default_agent : x.name === "code"), "desc"], // zodecode_change - renamed from "build" to "code"
              [(x) => x.name, "asc"],
            ),
          )
        })
        ...
        const defaultAgent = Effect.fnUntraced(function* () {
          ...
          if (c.default_agent) {
            const effective = ZodeAgent.resolveKey(c.default_agent) // zodecode_change - treat "build" as "code"
            const agent = agents[effective] // zodecode_change
            if (!agent) throw new Error(`default agent "${c.default_agent}" not found`)
            if (agent.mode === "subagent") throw new Error(`default agent "${c.default_agent}" is a subagent`)
            if (agent.hidden === true) throw new Error(`default agent "${c.default_agent}" is hidden`)
            return agent.name
          }
          // zodecode_change start - prefer "code" as default agent (key order changes after rename from "build")
        ...
        return {
          version: ZodeAgent.cacheKey(cfg), // zodecode_change
          get,
          list,
          defaultAgent,
        } satisfies State
    ...
    // zodecode_change start - rebuild cached agents when permission-relevant config changes
    const current = Effect.fnUntraced(function* <A>(select: (s: State) => Effect.Effect<A>) {
      const cfg = yield* config.get()
      const s = yield* InstanceState.get(state)
      if (s.version === ZodeAgent.cacheKey(cfg)) return yield* select(s)
    ...
    return Service.of({
      get: Effect.fn("Agent.get")(function* (agent: string) {
        return yield* current((s) => s.get(agent)) // zodecode_change
      }),
      list: Effect.fn("Agent.list")(function* () {
        return yield* current((s) => s.list()) // zodecode_change
      }),
      defaultAgent: Effect.fn("Agent.defaultAgent")(function* () {
        return yield* current((s) => s.defaultAgent()) // zodecode_change
      }),
      generate: Effect.fn("Agent.generate")(function* (input: {
        ...
        const params = {
          // zodecode_change start - enable telemetry with custom PostHog tracer
          experimental_telemetry: ZodeAgent.telemetryOptions(cfg),
          // zodecode_change end
          temperature: 0.3,
          messages: [
            ...(isOpenaiOauth
