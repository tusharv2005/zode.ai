...
- Author package entrypoint is `@zodecode/plugin/tui`.
...
- Import types from `@zodecode/plugin/tui`.
- `@zodecode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `ZODE_CONFIG_DIR`, OpenCode installs `@zodecode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@zodecode/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@zodecode/plugin/tui`.
...
- `--pure` / `ZODE_PURE` skips external TUI plugins only.
