...
const reg = process.env.REGISTRY ?? "ghcr.io/zode-org" // zodecode_change
...
const setup = async () => {
  ...
  if (list.includes("zode")) {
    await $`docker buildx use zode`
    return
  }
  await $`docker buildx create --name zode --use`
  // zodecode_change end
}
