...
describe("CodeIndexConfigManager", () => {
  ...
  test("configures Zode with hosted auth options and explicit model metadata", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "zode",
        openAiKey: undefined,
        zodeApiKey: "zode-token",
        zodeBaseUrl: "https://example.test/api/gateway/",
    ...
    expect(cfg.isFeatureConfigured).toBe(true)
    expect(cfg.getConfig().zodeOptions).toEqual({
      apiKey: "zode-token",
  ...
  test("requires Zode model metadata from Cloud config", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "zode",
        openAiKey: undefined,
  ...
  test("uses configured dimension for Zode models outside the fallback catalog", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "zode",
        openAiKey: undefined,
  ...
  describe("loadConfiguration restart checks", () => {
    ...
    test("requires restart when Zode auth changes", () => {
      const cfg = new CodeIndexConfigManager(
        createInput({
          embedderProvider: "zode",
          openAiKey: undefined,
          zodeApiKey: "old-token",
          modelId: "mistralai/mistral-embed-2312",
          modelDimension: 1024,
        }),
      )
