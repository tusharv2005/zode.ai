...
describe("loadIgnore", () => {
  ...
  test("loads root .zodecodeignore in addition to root .gitignore", async () => {
    await writeFile(path.join(root, ".gitignore"), "dist/\n")
    await writeFile(path.join(root, ".zodecodeignore"), "secret/\n")
  ...
  test("preserves existing .gitignore-only behavior when .zodecodeignore is absent", async () => {
  ...
  test("ignores the ignore files themselves", async () => {
    ...
    expect(ig.ignores(".zodecodeignore")).toBe(true)
  })
})
