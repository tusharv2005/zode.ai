...
import { ZodeEmbedder } from "./embedders/zode"
...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(): IEmbedder {
    ...
    if (provider === "zode") {
      if (!config.zodeOptions?.apiKey) throw new Error("Zode API key is required for embedding.")
      if (!config.modelId) throw new Error("Zode embedding model is required.")
      return new ZodeEmbedder({
        apiKey: config.zodeOptions.apiKey,
        baseUrl: config.zodeOptions.baseUrl,
        organizationId: config.zodeOptions.organizationId,
