...
export class CodeIndexConfigManager {
  ...
  private zodeOptions?: { apiKey: string; baseUrl?: string; organizationId?: string }
  ...
  private applyInput(input: IndexingConfigInput): void {
    ...
    this.zodeOptions = input.zodeApiKey
      ? { apiKey: input.zodeApiKey, baseUrl: input.zodeBaseUrl, organizationId: input.zodeOrganizationId }
      : undefined
  ...
  private captureSnapshot(): PreviousConfigSnapshot {
    return {
      ...
      zodeApiKey: this.zodeOptions?.apiKey ?? "",
      zodeBaseUrl: this.zodeOptions?.baseUrl ?? "",
      zodeOrganizationId: this.zodeOptions?.organizationId ?? "",
  ...
  public isConfigured(): boolean {
    ...
    if (provider === "zode")
      return !!(this.zodeOptions?.apiKey && this.modelId && this.currentModelDimension && hasStore)
  ...
  doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
    ...
    // Auth changes
    if ((prev.zodeApiKey ?? "") !== (this.zodeOptions?.apiKey ?? "")) return true
    if ((prev.zodeBaseUrl ?? "") !== (this.zodeOptions?.baseUrl ?? "")) return true
    if ((prev.zodeOrganizationId ?? "") !== (this.zodeOptions?.organizationId ?? "")) return true
