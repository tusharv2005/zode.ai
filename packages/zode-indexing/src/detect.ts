export const INDEXING_PLUGIN_NAMES = ["zode-indexing", "@zodecode/zode-indexing"] as const
...
function isPathSpecifier(value: string): boolean {
  ...
  if (normalized.includes("/.opencode/") || normalized.includes("/.zode/") || normalized.includes("/.zodecode/")) {
    return true
  }
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@zodecode" && parts[i + 1] === "zode-indexing")
  if (scoped >= 0) return "@zodecode/zode-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "zode-indexing")
  if (workspace >= 0) return "@zodecode/zode-indexing"
