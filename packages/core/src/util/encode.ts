export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // zodecode_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // zodecode_change - encode as UTF-8 bytes
