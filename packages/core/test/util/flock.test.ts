...
// zodecode_change start - make worker finalization await the process close event without a Windows race
...
describe("util.flock", () => {
  ...
  test("times out while waiting when lock is still healthy", async () => {
    ...
    try {
      ...
      await stopWorker(proc).catch(() => undefined) // zodecode_change - stopWorker now awaits close before returning
    }
  }, 15_000)
  ...
  test("recovers after a crashed lock owner", async () => {
    ...
    await wait(ready, 5_000)
    await stopWorker(proc) // zodecode_change - stopWorker now awaits close before returning
