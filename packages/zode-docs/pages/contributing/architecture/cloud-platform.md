...
description: "Architecture overview for Zode Cloud services and hosted runtimes"
---

# Cloud Platform Architecture

Zode Cloud is hosted platform layer for authentication, model routing, billing, product configuration, automation, and scoped execution services. Cloud implementation lives in open-source [`Zode-Org/cloud`](https://github.com/Zode-Org/cloud) repository.

{% callout type="info" title="Static source scope" %}
This page describes Worker surfaces, bindings, routes, and code paths present in `Zode-Org/cloud`. Static source shows deployable architecture, not live production enablement, rollout percentages, retention configuration, or vendor settings. Validate live environment before making production or compliance claims. Use [Zode Cloud Security Architecture](/docs/contributing/architecture/cloud-security) for trust boundaries and data flows.
{% /callout %}
...
| Shared cloud services | Model routing, asynchronous orchestration, real-time delivery, persistence adapters, and operational services | Zode Gateway, Workers, queues, Durable Objects, R2, KV, Hyperdrive |
| Scoped execution | Runs code or owner-scoped runtime workloads | Cloud Agent, App Builder preview sandbox, deployment builder sandbox, ZodeClaw runtime, Gas Town container |
| External providers | Services outside Zode Cloud trust boundary | Model providers, source-control providers, messaging providers, telemetry providers |
...
flowchart LR
  ...
  claw["ZodeClaw"]
  chat["Zode Chat / Event Service / Notifications"]
  town["Gas Town"]
  wasteland["Wasteland"]
...
| ZodeClaw | `zodeclaw`{% linebreak /%}`zodeclaw-billing`{% linebreak /%}`gmail-push`{% linebreak /%}`zodeclaw-inbound-email` | Owner-scoped assistant runtime coordination, billing, and external ingress |
| Real-time chat | `zode-chat`{% linebreak /%}`event-service`{% linebreak /%}`notifications` | Conversation state, WebSocket delivery, and mobile push |
| Multi-agent orchestration | `gastown`{% linebreak /%}`wasteland` | Town execution and collaborative commons |
| Evaluation and operations | `o11y`{% linebreak /%}`zode-ops`{% linebreak /%}`model-eval-ingest` | Metrics, alerts, operations, and model-evaluation ingestion |
| Attribution | `ai-attribution` | AI-edit attribution events |

## Zode Gateway

Gateway consists of cloud API routes plus `packages/zode-gateway/` client integration in `Zode-Org/zodecode`. It handles account-aware and anonymous-free model access. See [Cloud Security](/docs/contributing/architecture/cloud-security#model-request-gateway) for request branches and endpoint families.
...
Auto Model clients send stable `zode-auto/*` tier IDs. Gateway resolves tiers server-side before provider routing so mappings can change without client releases. See [Models and Providers](/docs/gateway/models-and-providers#auto-models) for current tier behavior.
...
|---|---|
| Zode Bot | Launches Cloud Agent for requested repository work |
...
| Webhook Agent Ingest | Delivers configured prompt to Cloud Agent or Zode Chat destination |
...
`services/webhook-agent-ingest/` is configured-trigger boundary. It accepts HTTP webhooks and scheduled alarms, then dispatches selected Cloud Agent or Zode Chat destination. [Automation Services](/docs/contributing/architecture/automation-services#webhook-agent-ingest) owns activation, authentication, queue, and alarm details.
...
sequenceDiagram
  ...
  participant Chat as Zode Chat conversation-state DOs
...
Zode Chat stores conversation state in Durable Objects and fans events out through Event Service. Notifications checks Event Service presence context before selected pushes and processes Expo receipts asynchronously. See [Cloud Security](/docs/contributing/architecture/cloud-security#chat-events-and-notifications) for ticket and push-delivery trust boundaries.

## ZodeClaw

ZodeClaw is owner-scoped hosted OpenClaw runtime coordination. Durable Objects track instance lifecycle, routing, configuration, and reconciliation. Runtime provider support includes Fly, docker-local development, and Northflank paths; source support does not prove active provider rollout. See [Cloud Security](/docs/contributing/architecture/cloud-security#zodeclaw-ingress) for ingress controls.

| Ingress path | Auth or validation | Entry boundary | Async handoff | Target |
|---|---|---|---|---|
| Browser request | JWT auth | ZodeClaw proxy | None | Owner-scoped runtime |
| One-time access code | Redeemed code and auth cookie | Access gateway | None | Owner-scoped OpenClaw UI |
| Controller machine check-in | Machine API key and derived gateway token | ZodeClaw controller route | None | Owner-scoped runtime controller |
| Zode Chat RPC | Service binding | ZodeClaw binding | None | Owner-scoped runtime |
| Cloudflare Email Routing | Alias lookup and bounded parse | `zodeclaw-inbound-email` | Queue | ZodeClaw platform service |
| Gmail Pub/Sub push | Google OIDC validation | `gmail-push` | Queue | Owner-scoped runtime controller |

ZodeClaw resolves owner or instance scope before runtime delivery. Table compares ingress boundaries; it does not describe global shared destinations.
...
flowchart TB
  subgraph worker ["Cloudflare Worker"]
    direction LR
    auth["JWT auth<br/>tied to Zode user"]
    instanceDO["Per-instance Durable Object"]
    dbConnection["Zode database connection"]
  end

  db["Zode database<br/>Instances<br/>Short-lived access codes<br/>Image catalog<br/>Billing and user preferences"]
  ...
  subgraph flyInstance ["Fly instance: owner-scoped runtime"]
    direction TB
    subgraph container ["ZodeClaw container"]
      direction TB
      controller["ZodeClaw controller<br/>Supervises OpenClaw gateway<br/>Exposes control endpoints<br/>Proxies HTTP and WebSocket traffic"]
...
Paths below are relative to [`Zode-Org/cloud`](https://github.com/Zode-Org/cloud).
...
| ZodeClaw | `services/zodeclaw/`{% linebreak /%}`services/zodeclaw-billing/`{% linebreak /%}`services/gmail-push/`{% linebreak /%}`services/zodeclaw-inbound-email/` |
| Chat, events, and notifications | `services/zode-chat/`{% linebreak /%}`services/event-service/`{% linebreak /%}`services/notifications/` |
| Multi-agent orchestration | `services/gastown/`{% linebreak /%}`services/wasteland/` |
| Observability and operations | `services/o11y/`{% linebreak /%}`services/zode-ops/`{% linebreak /%}`services/model-eval-ingest/` |
