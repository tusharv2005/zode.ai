...
description: "Overview of the Zode Code platform architecture"
---

# Architecture Overview

This page maps Zode Code's repository-defined architecture. It introduces the local runtime, editor clients, cloud service boundaries, and hosted execution products before the subsystem pages add implementation detail.
...
| Repository | Contents |
|---|---|
| [Zode&#8209;Org/zodecode](https://github.com/Zode-Org/zodecode) | Zode CLI runtime, local daemon, Zode Console, VS Code extension, JetBrains plugin, JavaScript SDK, codebase indexing, Zode Gateway client, telemetry, docs, and shared UI packages |
| [Zode&#8209;Org/cloud](https://github.com/Zode-Org/cloud) | Web control plane, Zode Gateway routes, Cloud Agent session runtime, automation, generated-application preview and deployment services, ZodeClaw, Gas Town, billing, and supporting Workers |

## Three architecture layers

| Layer | Responsibility | Typical boundaries |
|---|---|---|
| Local runtime and clients | Runs local coding sessions and connects editor surfaces to one local agent engine | Zode CLI runtime, `zode serve` server, local daemon, Zode Console, VS Code extension, JetBrains plugin |
| Zode Cloud shared services | Handles hosted identity, authorization, model routing, billing, orchestration, and shared product services | Web control plane, Zode Gateway, Workers, queues, Durable Objects, persistence |
| Hosted product runtimes and automation | Runs scoped cloud work for coding, app generation, assistants, security analysis, and multi-agent orchestration | Cloud Agent, Automation Services, App Builder, Security Agent, ZodeClaw, Gas Town, Wasteland |

Local execution and hosted execution are separate boundaries. Editor clients use a local `zode serve` server. Hosted automation can launch Cloud Agent execution sessions when cloud coding work is required.

## Terms used throughout

| Term | Meaning |
|---|---|
| Zode Code | Umbrella product across local clients, Zode CLI runtime, and Zode Cloud services |
| Zode CLI runtime | Local agent engine in `packages/opencode/`; owns tools, sessions, config, persistence, and provider routing |
...
| Web control plane | Hosted Zode Cloud application layer for identity, organization authorization, billing, product configuration, and API orchestration |
| Zode Gateway | First-party hosted model-routing boundary |
...
|---|---|---|---|
| Local coding | Zode CLI, Zode Console, VS Code, or JetBrains | Zode CLI runtime on developer machine | Editor clients talk to local `zode serve` server. Local runtime owns coding session and sends model requests directly or through Zode Gateway. |
| Hosted work | Webhook, source-control event, command, schedule, or hosted product | Zode Cloud services; Cloud Agent when coding is required | Cloud services coordinate work. Only flows that need repository changes launch Cloud Agent execution session. |
...
flowchart LR
  ...
  chat["Zode Chat, events, and notifications"]
  claw["ZodeClaw"]
  town["Gas Town"]
  wasteland["Wasteland"]
...
| ZodeClaw | Coordinates owner-scoped hosted assistant runtimes | [Cloud Platform](/docs/contributing/architecture/cloud-platform#zodeclaw) | [ZodeClaw ingress](/docs/contributing/architecture/cloud-security#zodeclaw-ingress) |
...
These local surfaces live in [`Zode-Org/zodecode`](https://github.com/Zode-Org/zodecode). Package paths below are relative to that repository root.

| Surface | Package in `Zode-Org/zodecode` | Runtime model |
|---|---|---|
| Zode CLI TUI | `packages/opencode/` | Interactive local client with daemon attach and worker-backed fallback paths |
| `zode run` | `packages/opencode/` | Headless prompt execution through explicit attach, daemon attach, or embedded fallback |
| `zode serve` | `packages/opencode/` | Local HTTP + SSE server for local clients |
| Zode Console | `packages/zode-console/`{% linebreak /%}`packages/opencode/` | Browser UI served at `/console` by a started or reused local daemon |
| VS Code extension | `packages/zode-vscode/` | Extension host starts one shared editor-owned `zode serve` server and routes webviews through HTTP + global SSE; SDK directory selects local runtime instance |
| JetBrains plugin | `packages/zode-jetbrains/` | Split-mode Swing plugin; backend module starts one editor-owned `zode serve` server and caches workspace clients by directory |

## Cloud service families

Hosted service families live in [`Zode-Org/cloud`](https://github.com/Zode-Org/cloud). Paths below are relative to that repository root unless another repository is named.

| Boundary | Primary source paths | Role |
|---|---|---|
| Zode Cloud | `apps/web/`{% linebreak /%}`services/` | Hosted platform repository for identity, billing, routing, product configuration, automation, and scoped execution services |
| Web control plane | `apps/web/` | Hosted application layer for authorization, configuration, and API orchestration |
| Zode Gateway | `apps/web/src/app/api/gateway/`{% linebreak /%}`apps/web/src/lib/ai-gateway/`{% linebreak /%}Local integration: `Zode-Org/zodecode/packages/zode-gateway/` | First-party model-routing boundary and local client integration |
...
| Adjacent hosted boundaries | `services/app-builder/`{% linebreak /%}`services/zodeclaw/`{% linebreak /%}`services/gastown/`{% linebreak /%}`services/wasteland/`{% linebreak /%}Supporting services | App Builder, ZodeClaw, Gas Town, Wasteland, chat, notifications, and supporting services |

## Supporting packages

These supporting packages also live in [`Zode-Org/zodecode`](https://github.com/Zode-Org/zodecode). Package paths below are relative to that repository root.

| Package in `Zode-Org/zodecode` | Role |
|---|---|
| `packages/zode-indexing/` | Per-directory asynchronous codebase indexing engine behind Zode CLI bridge |
| `packages/sdk/js/` | Generated JavaScript client and handwritten wrapper for local server APIs |
| `packages/zode-gateway/` | Local Zode Gateway client integration used by Zode CLI runtime |
| `packages/zode-console/` | Browser UI served by local daemon at `/console` |

## Architecture pages

| Page | What it covers |
|---|---|
| [CLI Runtime](/docs/contributing/architecture/cli-runtime) | Local execution modes, daemon, server authentication, routing, persistence, snapshots, SDK, config, SSE, Zode Console, and indexing |
| [VS Code Extension](/docs/contributing/architecture/vscode-extension) | Shared local `zode serve` ownership, webview bridge, Agent Manager, PTYs, recovery, bundled resources, and build outputs |
| [JetBrains Plugin](/docs/contributing/architecture/jetbrains-plugin) | Split-mode modules, RPC, bundled local `zode serve` lifecycle, Kotlin SDK, recovery, and remote-development constraints |
...
- [ZodeClaw Overview](/docs/zodeclaw/overview) - customer-facing ZodeClaw docs
