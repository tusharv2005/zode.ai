...
{% callout type="info" %}
**New versions of the VS Code extension and CLI are being developed in [Zode-Org/zodecode](https://github.com/Zode-Org/zodecode)** (extension at `packages/zode-vscode`, CLI at `packages/opencode`). For extension and CLI development, please head over to that repository.
{% /callout %}
...
4. **Java 21** - Required only when running JetBrains plugin checks or repo-level checks that include `@zodecode/zode-jetbrains`
...
1. **Fork and Clone the Repository**:
   - **Fork the Repository**:
     - Visit the [Zode Code GitHub repository](https://github.com/Zode-Org/zodecode)
     - Click the "Fork" button in the top-right corner to create your own copy.
   - **Clone Your Fork**:
     ```bash
     git clone https://github.com/[YOUR-USERNAME]/zodecode.git
     cd zodecode
     ```
...
- **`packages/opencode/`** - Zode CLI, agent runtime, local HTTP server, session management, and TUI
- **`packages/zode-vscode/`** - VS Code extension, webview UI, Agent Manager, and extension packaging
- **`packages/sdk/js/`** - Generated TypeScript SDK for the local server API
- **`packages/zode-docs/`** - Documentation site
- **`packages/zode-jetbrains/`** - JetBrains plugin
...
`bun dev` and `bun run dev` are equivalent. Both run the local source in `packages/opencode/`; they do not use a globally installed `zode` binary.

### Backend/API Validation

For backend and API validation, use the root [TESTING.md](https://github.com/Zode-Org/zodecode/blob/main/TESTING.md) guide. It covers starting the local backend with:
...
Zode Code uses several types of tests to ensure quality:
...
Use the root [TESTING.md](https://github.com/Zode-Org/zodecode/blob/main/TESTING.md) guide for backend/API checks that require `bun dev serve` and `curl`-based requests.
...
- When editing shared `packages/opencode/` files, keep changes small and mark Zode-only edits with `// zodecode_change` for a single line or `// zodecode_change start` / `// zodecode_change end` for a block. Do not add these markers inside `zodecode`-named paths.
...
- Check the Output panel in VSCode (View > Output) and select "Zode Code" from the dropdown
