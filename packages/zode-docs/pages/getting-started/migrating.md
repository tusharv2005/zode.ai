...
description: "Guide for migrating to Zode Code from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to Zode Code. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

Zode Code supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why Zode Code's Rules System?

Zode Code simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in Zode Code

| Cursor | Zode Code | Key Difference |
|---|---|---|
| `.cursor/rules/*.mdc` with YAML frontmatter | `.zodecode/rules/*.md` plain Markdown | No YAML metadata required |
...
| Global rules in UI settings | `~/.zodecode/rules/*.md` files | Global rules stored as files in home folder |
...
**2. Create Zode Code directory:**

```bash
mkdir -p .zodecode/rules
```
...
for file in .cursor/rules/*.mdc; do
  basename="${file##*/}"
  cp "$file" ".zodecode/rules/${basename%.mdc}.md"
done
...
- Save to `~/.zodecode/rules/cursor-global.md`

**6. Handle legacy `.cursorrules`:**

```bash
cp .cursorrules .zodecode/rules/legacy-rules.md
```

### Converting Cursor's `globs` Patterns

Cursor's `globs` field specifies which files a rule applies to. Zode Code handles this through **mode-specific directories** instead.
...
**Zode Code approach (Option 1 - Mode-specific directory):**

```bash
mkdir -p .zodecode/rules-code
# Save TypeScript-specific rules here
```

**Zode Code approach (Option 2 - Custom mode):**

```yaml
# .zodecodemodes (at project root)
- slug: typescript
  name: TypeScript
...
Then place rules in `.zodecode/rules-typescript/`

### Flattening Nested Cursor Rules

Cursor supports nested `.cursor/rules/` directories. Zode Code uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# Zode Code: .zodecode/rules/backend-server-api-rules.md
```
...
| Windsurf | Zode Code | Key Difference |
|---|---|---|
| `.windsurf/rules/*.md` | `.zodecode/rules/*.md` | Same Markdown format |
| GUI configuration for activation modes | File location determines scope | Scope controlled by directory structure |
| "Always On" mode (GUI) | Place in `.zodecode/rules/` | Rules stored as files, not GUI settings |
...
| Global rules in `~/.codeium/windsurf/memories/global_rules.md` | `~/.zodecode/rules/*.md` | Global rules in home folder, multiple files |
...
```bash
cp .windsurf/rules/*.md .zodecode/rules/
```

**4. Migrate global rules:**

```bash
cp ~/.codeium/windsurf/memories/global_rules.md ~/.zodecode/rules/global-rules.md
```

**5. Handle legacy `.windsurfrules`:**

```bash
cp .windsurfrules .zodecode/rules/legacy-rules.md
```
...
# .zodecode/rules/api-conventions.md
# .zodecode/rules/testing-standards.md
# .zodecode/rules/code-style.md
```

### Converting Windsurf's Activation Modes

Windsurf configures activation through the GUI. In Zode Code, file organization replaces GUI configuration:

| Windsurf GUI Mode | Zode Code Equivalent |
|---|---|
| **Always On** | Place in `.zodecode/rules/` (default) |
...
Then place the rule in `.zodecode/rules-test/`

## AGENTS.md Support

All three tools support the `AGENTS.md` standard. If you have one, it works in Zode Code automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - Zode Code loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). Zode Code also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in Zode Code and require user approval to modify.

## Understanding Mode-Specific Rules

This is Zode Code's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.

### Directory Structure

```bash
.zodecode/rules/              # Apply to ALL modes
.zodecode/rules-code/         # Only in Code mode
.zodecode/rules-debug/        # Only in Debug mode
.zodecode/rules-ask/          # Only in Ask mode
.zodecode/rules-{custom}/     # Only in your custom mode
```
...
mkdir -p .zodecode/rules-test

# 2. Save rule as plain Markdown
cat > .zodecode/rules-test/testing-standards.md << 'EOF'
# Testing Rules
- Write tests for all features
- Maintain >80% coverage
EOF
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in Zode Code panel
- [ ] **Test rule application:** Ask Zode Code to perform tasks following your rules
- [ ] **Organize rules:** Split large files, use clear names
- [ ] **Set up mode-specific rules:** Create directories for specialized workflows
- [ ] **Update team docs:** Document new `.zodecode/rules/` location
- [ ] **Commit to version control:** `git add .zodecode/`
...
```bash
ls -la .zodecode/rules/      # Project rules
ls -la ~/.zodecode/rules/    # Global rules
```
...
- **For always-on rules:** Place in `.zodecode/rules/`
...
- **After migrating:** Organize files accordingly in Zode Code

### Nested Rules Flattened

Cursor's nested directories don't map to Zode Code. Flatten with descriptive names:

```bash
# Bad: .cursor/rules/backend/api/rules.mdc
# Good: .zodecode/rules/backend-api-rules.md
```
...
- **Check setting:** Verify "Use Agent Rules" is enabled in Zode Code settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

Zode Code supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
```bash
mkdir -p .zodecode/rules-review
mkdir -p .zodecode/rules-docs
```

**Note:** `.zodecodemodes` can be in YAML (preferred) or JSON format. For global modes, edit the `custom_modes.yaml` file via Settings > Edit Global Modes.
