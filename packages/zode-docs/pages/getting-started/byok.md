...
description: "Use your own API keys with Zode Gateway while retaining platform features"
---

# Bring Your Own Key (BYOK)

Bring Your Own Key (BYOK) lets you use your own API keys when using the Zode Gateway, while retaining Zode platform features like Code Reviews and Cloud Agents.

A user or organization may want to use BYOK to:

- Utilize new models quickly, Zode Gateway supports most new models in minutes
...
Zode Gateway supports BYOK keys for these providers.
...
These providers offer coding-focused subscriptions or dedicated endpoints. Bring the API key issued by your plan to use its included models through the Zode Gateway:
...
- When you use the **Zode Gateway** provider, Zode checks if there's a BYOK key for the selected model's provider.
- If a matching BYOK key exists, the request is routed using your key.
- If the key is invalid, the request fails. It does not fall back to using Zode's keys.
...
- BYOK works with the Zode Gateway provider. Users should ensure that is set as the active [provider](/docs/ai-providers).
