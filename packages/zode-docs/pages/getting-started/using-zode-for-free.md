---
title: "Using Zode for Free"
description: "How to use Zode Code for free — Auto Free, finding free models, free autocomplete, and free background tasks"
---

# Using Zode for Free

Zode Code can be used completely free of charge. There are three places where Zode uses AI model inference, and each can be configured to use free models.

## Where Zode Uses Models
...
Each of these consumes credits by default. **To use Zode entirely for free, configure all three to use free models.**

## Free Agentic Usage

Zode provides free models for coding tasks through the Zode Gateway and partner providers.

### Auto Free

The easiest way to get started is [**Auto Free**](/docs/code-with-ai/agents/auto-model) (`zode-auto/free`). This is a Zode-provided model tier that automatically routes your requests to the best available free models — no configuration needed.
...
Zode's autocomplete feature provides AI-powered code completions as you type in IDE extensions.

By default, autocomplete routes through the Zode provider and uses credits. If you run out of credits without a free alternative configured, autocomplete stops working — but your main coding workflow is unaffected.

### How to Get It Free

Add your own Mistral AI (Codestral) API key via **BYOK (Bring Your Own Key)** on the Zode Gateway. Mistral offers a free tier for Codestral. When you configure a BYOK key, autocomplete requests use your key directly — at no cost on your Zode balance.
...
Zode uses a small model in the background for tasks like session titling. By default this is Auto Small, which consumes credits. If the small model is unavailable, Zode falls back to your primary model — which may also consume credits if it's a paid model.
...
**In the CLI:** Set the `small_model` parameter in `~/.config/zode/config.json`:
