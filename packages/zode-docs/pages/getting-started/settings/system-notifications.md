...
description: "Configure native OS notifications for Zode Code"
platform: legacy
---
...
Zode Code uses system notifications to inform you about:
...
Zode Code's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. Zode Code uses macOS's built-in command to display notifications.
...
**How it works:** Zode Code first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
