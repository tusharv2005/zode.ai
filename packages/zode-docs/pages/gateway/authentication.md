...
description: "Learn how to authenticate with the Zode AI Gateway using API keys, session tokens, and Bring Your Own Key (BYOK)."
---

# Authentication

The Zode AI Gateway supports multiple authentication methods depending on your use case.
...
API keys are JWT tokens tied to your Zode account. See [how to get your API key](/docs/getting-started/setup-authentication#zode-gateway-api-key) for step-by-step instructions.
...
```
X-ZodeCode-OrganizationId: your_org_id
```
...
BYOK lets you use your own provider API keys with the Zode AI Gateway. When a BYOK key is configured, requests are sent to the provider using your key. You are billed directly by the provider -- Zode does not add any markup.
...
1. Add your provider API key in the [Zode dashboard](https://app.__PRESERVE_ZODE_AI__) or through your Zode Code extension settings
...
4. Usage is tracked but not billed to your Zode balance (cost is set to $0)
5. If your BYOK key fails, the request will not automatically fall back to Zode's keys
...
| `X-ZodeCode-OrganizationId` | No | Organization context for org-scoped requests |
| `X-ZodeCode-TaskId` | No | Task identifier for prompt cache keying |
| `X-ZodeCode-Version` | No | Client version string |
| `x-zodecode-mode` | No | Mode hint for `zode-auto` model routing |
