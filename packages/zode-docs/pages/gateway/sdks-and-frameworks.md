...
description: "Integrate with the Zode AI Gateway using the Vercel AI SDK, OpenAI SDK, Python, cURL, or any OpenAI-compatible client."
---

# SDKs & Frameworks

The Zode AI Gateway is OpenAI-compatible, meaning any SDK or framework that works with the OpenAI API can work with the Zode Gateway by changing the base URL.
...
const result = streamText({
  model: zode.chat("anthropic/claude-sonnet-4.5"),
...
The official OpenAI SDKs work with the Zode Gateway by setting the base URL.
...
request['Authorization'] = "Bearer #{ENV['ZODE_API_KEY']}"
...
The Zode AI Gateway works with any framework that supports OpenAI-compatible APIs:

| Framework | Integration |
|---|---|
| [Vercel AI SDK](https://ai-sdk.dev) | Use `createOpenAI` with Zode base URL |
...
| [Pi](https://pi.dev) | Install the [Zode provider extension](https://github.com/Zode-Org/zode-pi-provider) |

### Pi coding agent

Use the Zode-maintained [Pi provider extension](https://github.com/Zode-Org/zode-pi-provider) to access Zode Gateway models from the [Pi coding agent](https://pi.dev).

```bash
pi install git:github.com/Zode-Org/zode-pi-provider
```

Run `/login zode` in Pi to connect your account, or use supported free models without signing in. See the provider repository for organization configuration and model-specific behavior.
