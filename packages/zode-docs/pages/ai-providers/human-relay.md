---
title: "Using Human Relay with Zode Code | No API Key Needed"
description: "Use Zode Code with web-based AI chats like ChatGPT and Claude without an API key by manually relaying messages via Human Relay."
sidebar_label: Human Relay
---

# Human Relay Provider

The Human Relay provider allows you to use Zode Code with web-based AI models like ChatGPT or Claude without needing an API key. Instead, it relies on you to manually relay messages between Zode Code and the AI's web interface.

## How it Works

1.  **Select Human Relay**: Choose "Human Relay" as your API provider in Zode Code's settings. No API key is required.
2.  **Initiate a Request**: Start a chat or task with Zode Code as usual.
...
6.  **Paste Back to Zode Code**: Return to the dialog box in VS Code, paste the AI's response into the designated field, and click "Confirm".
7.  **Continue**: Zode Code will process the response as if it came directly from an API.
