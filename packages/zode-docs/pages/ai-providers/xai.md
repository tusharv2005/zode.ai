---
title: "Using xAI Grok with Zode Code"
description: "Connect xAI's Grok models to Zode Code. Use a SuperGrok or X Premium subscription via OAuth or a paid API key. Guide to setup in VS Code and the CLI."
sidebar_label: xAI (Grok)
---

# Using xAI (Grok) With Zode Code
...
Zode Code supports two ways to connect xAI:
...
- **OAuth login — no API keys:** Sign in through your browser and Zode Code handles token management automatically.
- **Automatic token refresh:** Zode Code refreshes your access token in the background so long-running sessions stay authenticated.

{% callout type="note" %}
SuperGrok and X Premium subscription access works with Zode Code's core functionality (VS Code extension and CLI). For cloud features such as Cloud Agents or ZodeClaw, use the [Zode Gateway](/docs/gateway) — the Gateway supports xAI via [BYOK](/docs/getting-started/byok) with an API key (OAuth and subscription-based access are not supported through the Gateway).
{% /callout %}
...
5. Back in Zode Code settings, select your desired Grok model.
6. Save.
...
```bash
zode auth login --provider xai
```

Zode Code offers three methods at the prompt:
...
- **Sign out:** To disconnect in VS Code, use the "Disconnect" button in the provider settings. In the CLI, run `zode auth logout` and choose xAI.
...
- **Token rotation:** xAI rotates refresh tokens on each use. Zode Code persists the latest tokens automatically. If you run Zode Code from multiple processes simultaneously, the first refresh can invalidate the other process's token — re-run `zode auth login --provider xai` to restore the session.
...
3. **Create a Key:** Click to create a new API key. Give your key a descriptive name (e.g., "Zode Code").
