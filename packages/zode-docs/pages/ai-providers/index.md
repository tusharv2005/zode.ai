---
title: "AI Providers"
description: "Configure and connect different AI model providers to Zode Code"
---

# AI Providers

Zode Code supports a wide variety of AI providers, giving you flexibility in how you power your AI-assisted development workflow. Choose from cloud providers, local models, or AI gateways based on your needs.

## Getting Started

The fastest way to get started is with **Zode Code's built-in provider**, which requires no configuration. Simply sign in and start coding.
...
- **[Atomic Chat](/docs/ai-providers/atomic-chat)** - Local models with TurboQuant inference and auto-discovery in Zode Code
...
|---|---|
| Ease of use | [Zode Code (built-in)](/docs/ai-providers/zodecode) |
...
{% callout type="note" %}
In the **VSCode (Legacy)** version, API keys use VS Code's Secret Storage. In the current **VSCode & CLI** version, keys are set via environment variables or referenced in `zode.json` config files. See individual provider pages for setup instructions for each platform.
{% /callout %}

## Disabling Built-in Providers

You can prevent specific providers from loading using `disabled_providers` in your `zode.json` (or `zode.jsonc`). This is useful to hide models from built-in or detected providers that you don't intend to use.
...
Both fields accept provider IDs — the lowercase identifier used in the `provider/model` format (e.g. `zode`, `anthropic`, `openai`, `google`, `groq`).

## Next Steps

- **New to Zode Code?** Start with the [Zode Code provider](/docs/ai-providers/zodecode) - no setup required
