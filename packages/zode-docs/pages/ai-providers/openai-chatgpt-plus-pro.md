---
title: "Using ChatGPT Plus/Pro with Zode Code"
description: "Use your ChatGPT Plus or Pro subscription with Zode Code. Flat-rate access to OpenAI Codex models with OAuth login — no separate API key required."
sidebar_label: ChatGPT Plus/Pro
---

# Using ChatGPT Subscriptions With Zode Code

If you already pay for ChatGPT Plus or Pro, you can use that subscription to run OpenAI's top coding models directly inside Zode Code — with no extra API charges beyond your subscription.
...
- **Full agentic workflows:** Generate, refactor, debug, edit files, and run terminal commands inside Zode Code.
- **Multiple AI modes:** Switch between Code, Plan, Debug, and Ask modes for different tasks.

{% callout type="note" %}
Your ChatGPT subscription works with Zode Code's core functionality (VS Code extension and CLI), but does **not** include cloud features such as Cloud Agents, Zode Deploy, or ZodeClaw. To use GPT models in those features, use the [Zode Gateway](/docs/gateway).
{% /callout %}
...
5. Back in Zode Code settings, pick a model from the dropdown.
6. Save.
...
If OpenAI is already connected from an API key, environment variable, or `zode.json` config, you can still sign in with ChatGPT from the OpenAI provider row. Zode Code uses the ChatGPT sign-in for Codex models until you disconnect it, then falls back to your existing OpenAI API configuration.
...
```bash
zode auth login --provider codex
```
...
Then set your default model to one of the OpenAI Codex models available in Zode Code:
...
- **Sign Out:** To disconnect in VS Code, use the "Disconnect" button in the provider settings. In the CLI, run `zode auth logout` and choose OpenAI.
- **Switching providers:** You can switch to Claude, Gemini, or local models at any time — this provider is optional.

## Limitations

- **Codex catalog models only.** This provider only exposes the models listed in Zode Code's Codex model catalog. It does not give access to every model available through the OpenAI API.
- **OAuth tokens can't be exported with settings.** Tokens are stored in VS Code SecretStorage, which isn't included in Zode Code's settings export.
- **Cloud features not included.** Cloud Agents, Zode Deploy, and ZodeClaw require the [Zode Gateway](/docs/gateway).
...
**Can I use my ChatGPT subscription in ZodeClaw?**
No. For ZodeClaw and other cloud features, use GPT models through the [Zode Gateway](/docs/gateway), which gives access to 500+ models including the latest GPT releases.
