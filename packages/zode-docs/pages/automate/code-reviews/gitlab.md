...
Zode's Code Reviews integrate with GitLab to automatically review merge requests with AI. When an MR is opened, updated, or reopened, the Review Agent analyzes the changes and posts feedback directly on the merge request — as summary notes and inline discussion comments.
...
> **Why Maintainer role?** Zode creates a bot account (Project Access Token) on each project so that review comments appear from a bot, not your personal account. This requires Maintainer access.
...
When you select repositories, Zode **automatically creates webhooks** on each project.
...
1. A 👀 reaction appears on the MR — this means Zode is reviewing
...
Review comments are posted by a **Zode Code Review Bot** — not by your personal GitLab account. This bot is created automatically as a Project Access Token on each project.

- Created automatically the first time a project is reviewed
- Valid for 365 days and rotated automatically before expiry
- If you manually revoke the bot token in GitLab, Zode creates a new one on the next review
- Requires **Maintainer role** on the project

## Webhooks

Zode manages webhooks automatically:
...
- Ensure you have sufficient Zode Code credits
