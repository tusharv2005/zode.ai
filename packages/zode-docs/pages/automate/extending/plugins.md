---
title: "Plugins"
description: "Extend the Zode CLI with custom hooks, tools, auth providers, and more"
platform: new
---

# Plugins

Plugins extend Zode by hooking into events, adding custom tools, registering auth or model providers, and customizing runtime behavior. They are TypeScript or JavaScript modules loaded at startup, and work in both the Zode CLI and the VS Code extension.
...
- Global: `~/.config/zode/plugin/`
- Project: `.zode/plugin/`, `.zodecode/plugin/`, or `.opencode/plugin/`
...
### From the `zode plugin` command

Install an npm plugin and patch your config in one step:

```bash
# Install into the current project's config
zode plugin my-plugin

# Install into your global config
zode plugin my-plugin --global

# Replace an existing entry
zode plugin my-plugin --force
```

The command resolves the package, reads its `package.json` for plugin entrypoints, and writes the entry into the appropriate config file (currently `.opencode/opencode.jsonc` / `.opencode/tui.jsonc` for local installs, or `~/.config/zode/opencode.jsonc` / `~/.config/zode/tui.jsonc` for `--global`) while preserving JSONC comments.
...
- **Install scripts are disabled** for npm plugins. Zode installs packages with lifecycle scripts such as `install` and `postinstall` blocked.
- **Local plugins** are loaded directly from the plugin directory. If your plugin imports external packages, add a `package.json` to your config directory (see [Dependencies](#dependencies)) — Zode runs `bun install` on startup so imports resolve.

### Load order

Plugins from all sources run on every session. They load in this order:

1. Internal built-ins (Zode Gateway auth, Codex auth, Copilot auth, Cloudflare, etc.)
2. Global config plugin array (`~/.config/zode/zode.json`)
3. Global plugin directory (`~/.config/zode/plugin/`)
4. Project config plugin array (`zode.json` / `opencode.json`)
5. Project plugin directory (`.zode/plugin/` and friends)
...
Set the `ZODE_PURE=1` environment variable to skip all external plugins — only built-in plugins will load. Useful for reproducible CI runs or debugging.
...
| `client` | A Zode SDK client (`@zodecode/sdk`) for calling the local server. |
| `$` | [Bun's shell API](https://bun.com/docs/runtime/shell). |
| `serverUrl` | URL of the local Zode server. |
...
Workspace adaptors let plugins add custom workspace targets to Zode's workspace creation flow. This API is experimental and may change.
...
const WorkspacePlugin: Plugin = async ({ experimental_workspace }) => {
  experimental_workspace.register("folder", {
    ...
    configure(config) {
      return { ...config, directory: `/tmp/zode-${Date.now()}` }
    },
    async create(config) {
...
Published npm plugins should declare separate package entrypoints for each runtime they support. Zode detects install targets from `package.json`:
...
```bash
bun add -d @zodecode/plugin
```

```ts
import type { Plugin } from "@zodecode/plugin"
import { tool } from "@zodecode/plugin/tool"
```

Zode automatically creates a `package.json` in config directories that contain a `plugin/` folder and installs `@zodecode/plugin` so types resolve out of the box.
...
Zode runs `bun install` at startup so your plugins can import the packages:

```ts
// .zode/plugin/escape-bash.ts
...
| `shell.env` | Inject environment variables into every shell command Zode runs. |
...
Zode fills provider/model IDs from the returned catalog and uses the returned models in the picker and provider router.
...
The `event` hook fires for every event on Zode's internal bus. Common event types include:
...
For tools that don't need the full plugin context, drop them in a `tool/` or `tools/` folder inside any config directory — for example `.zode/tool/database.ts` or `~/.config/zode/tool/database.ts`. The filename becomes the tool name, and each file exports a `tool()` definition directly. The layout is identical to the [OpenCode custom tools guide](https://opencode.ai/docs/custom-tools); substitute `.zode/` (or `.zodecode/` / `.opencode/`) for `.opencode/`.
...
```ts
// .zode/plugin/notify.ts
import type { Plugin } from "@zodecode/plugin"

const Notify: Plugin = async ({ $ }) => ({
  event: async ({ event }) => {
    if (event.type === "session.idle") {
      await $`osascript -e 'display notification "Session complete!" with title "Zode"'`
    }
  },
})
...
Prefer `client.app.log()` over `console.log` so entries land in Zode's log pipeline:
...
By default, Zode sends a synthetic "continue" turn after compaction so the agent resumes the interrupted task. Use `experimental.compaction.autocontinue` to disable that turn for specific sessions or providers:
...
Plugins can also target the Zode TUI itself — registering slash commands, routes, slots, dialogs, and keybinds. TUI plugins are SolidJS modules exported from `"./tui"` in your plugin package, or theme-only packages declared with `oc-themes`.

TUI plugins live in a separate module namespace (`@zodecode/plugin/tui`) and have their own API surface (`TuiPluginApi`). Because the TUI API is larger and still evolving, this guide doesn't cover it exhaustively — use the types in `@zodecode/plugin/tui` as the reference, and look at the built-in TUI plugins under `packages/opencode/src/cli/cmd/tui/feature-plugins/` for working examples.
...
- **Plugin failed to load** — check the CLI logs with `zode --print-logs --log-level DEBUG`. Load failures are also surfaced as session errors in the TUI and VS Code extension.
...
- **Plugin loads in dev but not in CI** — verify `ZODE_PURE` is not set, and that npm-installed plugins are cached under `packages/` in the current CLI XDG cache directory (`~/.cache/opencode/packages/` by default, or `$XDG_CACHE_HOME/opencode/packages/` when `XDG_CACHE_HOME` is set). Run with `--log-level DEBUG` to see install output.
- **Reset the plugin cache** — delete the plugin package folder under the CLI's `packages/` cache directory (or the `node_modules` cache under your config directory) and restart Zode.

---

## Reference

- Types: [`@zodecode/plugin`](https://github.com/Zode-Org/zodecode/tree/main/packages/plugin) — `Plugin`, `Hooks`, `PluginInput`, `ToolDefinition`, `AuthHook`, `ProviderHook`.
- Example plugin: [`packages/plugin/src/example.ts`](https://github.com/Zode-Org/zodecode/blob/main/packages/plugin/src/example.ts)
- CLI command: [`zode plugin`](/docs/code-with-ai/platforms/cli-reference#zode-plugin)
