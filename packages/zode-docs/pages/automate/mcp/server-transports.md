...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Zode Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Zode Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Zode Code
...
- Single-client scenarios (one Zode Code instance per server)
...
1. The client (Zode Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Zode Code
...
STDIO servers run locally on the same machine as Zode Code, which has several important implications:
...
- **Execution**: Starts and stops with Zode Code (child process lifecycle)
...
- Start when needed by Zode Code
- Not require network configuration
- Need to be installed alongside Zode Code or via a package manager
...
## Configuring Transports in Zode Code

For detailed information on configuring STDIO and SSE transports in Zode Code, including example configurations, see the [Understanding Transport Types](using-in-zode-code#understanding-transport-types) section in the Using MCP in Zode Code guide.
