...
description: "How to configure and use MCP servers in the Zode CLI"
---

# Using MCP in the CLI

The Zode CLI supports both local and remote MCP servers. Once added, MCP tools are automatically available to the LLM alongside built-in tools.
...
|---|---|---|
| **Global** | `~/.config/zode/zode.json` | `zode.jsonc`, `config.json` |
| **Project** | `./zode.json` or `./.zode/zode.json` | `zode.jsonc` |
...
Browse community-contributed MCP server configurations and agent skills in the [Zode Marketplace](https://github.com/Zode-Org/zode-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
