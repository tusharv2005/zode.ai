---
title: Apply Diff Tool | Zode Code Docs
...
- When Zode Code needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.zodecodeignore` rules.
...
2.  **ZodeCodeIgnore Check**: Validates if the target file path is allowed by `.zodecodeignore` rules.
...
Zode Code uses this strategy for applying diffs:
