---
title: Ask Followup Question Tool | Zode Code Docs
description: Learn how the ask_followup_question tool enables Zode Code to gather additional information needed to complete tasks through interactive communication.
---
...
This tool creates a conversational interface between Zode Code and the user, allowing for gathering clarification, additional details, or user preferences when facing ambiguities or decision points. Each question can include suggested responses to streamline the interaction.

## When is it used?

- When critical information is missing from the original request
- When Zode Code needs to choose between multiple valid implementation approaches
- When technical details or preferences are required to proceed
- When Zode Code encounters ambiguities that need resolution
...
1. **Information Gap Recognition**: Zode Code identifies missing information needed to proceed
2. **Specific Question Creation**: Zode Code formulates a clear, targeted question
3. **Suggestion Development**: Zode Code creates relevant suggested answers (optional but recommended)
...
10. **Task Continuation**: Zode Code proceeds with the task using the new information

## Examples When Used

- When developing a web application, Zode Code might ask about preferred styling frameworks (Bootstrap, Tailwind, custom CSS)
- When creating an API, Zode Code might ask about authentication methods (JWT, OAuth, API keys)
- When refactoring code, Zode Code might ask about prioritizing performance vs. readability
- When setting up a database, Zode Code might ask about specific schema design preferences
- When creating a custom feature, Zode Code might ask about specific behavior expectations
- When troubleshooting errors, Zode Code might ask about specific environment details
