---
title: List Files Tool | Zode Code Docs
description: Learn how the list_files tool displays files and directories within a specified location to help Zode Code understand your project structure.
---

# list_files

The `list_files` tool displays the files and directories within a specified location. It helps Zode Code understand your project structure and navigate your codebase effectively.
...
- When Zode Code needs to understand your project structure
- When Zode Code explores what files are available before reading specific ones
- When Zode Code maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Zode Code needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.zodecodeignore` with a lock symbol (🔒) when `showZodeCodeIgnoredFiles` is enabled
...
5. **Result Filtering**:
   ...
   - Respects `.gitignore` rules when in recursive mode
   - Handles `.zodecodeignore` patterns, either hiding files or marking them with a lock symbol
6. **Formatting**:
   - Marks directories with a trailing slash (`/`)
   - Sorts results to show directories before their contents for logical hierarchy
   - Marks ignored files with a lock symbol (🔒) when `showZodeCodeIgnored` is enabled
...
- Files ignored by `.zodecodeignore` are marked with a lock symbol (🔒) when `showZodeCodeIgnored` is enabled
...
When `.zodecodeignore` files are used and `showZodeCodeIgnored` is enabled:
...
- When starting a new task, Zode Code may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Zode Code first lists directories to know where to look.
- When providing recommendations for code organization, Zode Code examines the current project structure first.
- When setting up a new feature, Zode Code lists related directories to understand the project conventions.
