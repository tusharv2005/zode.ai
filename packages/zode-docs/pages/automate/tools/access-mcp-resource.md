---
title: Access MCP Resource Tool | Zode Code Docs
description: Learn how the access_mcp_resource tool retrieves data from Model Context Protocol servers to provide additional context for Zode Code tasks.
---

# access_mcp_resource

The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows Zode Code to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When Zode Code needs additional context from external systems
- When Zode Code needs to access domain-specific data from specialized MCP servers
- When Zode Code needs to retrieve reference documentation hosted by MCP servers
- When Zode Code needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
   ...
   - Returns the processed resource data to Zode Code for use in the current task
...
- When helping with API development, Zode Code retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, Zode Code accesses current data samples from connected MCP servers.
- When working in specialized domains, Zode Code retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, Zode Code references compliance requirements from documentation resources.
