---
title: Use MCP Tool | Zode Code Docs
description: Learn how the use_mcp_tool tool enables Zode Code to interact with external tools provided by Model Context Protocol servers for extended capabilities.
---

# use_mcp_tool

The `use_mcp_tool` tool enables interaction with external tools provided by connected Model Context Protocol (MCP) servers. It extends Zode Code's capabilities with domain-specific functionality through a standardized protocol.
...
This tool allows Zode Code to access specialized functionality provided by external MCP servers. Each MCP server can offer multiple tools with unique capabilities, extending Zode Code beyond its built-in functionality. The system validates arguments against schemas, manages server connections, and processes responses of various content types (text, image, resource).
...
- **Global Configuration**: Managed through the Zode Code extension settings in VS Code. These apply across all projects unless overridden.
- **Project-level Configuration**: Defined in a `.zodecode/mcp.json` file within your project's root directory.
...
- Since `.zodecode/mcp.json` can be committed to version control, it simplifies sharing configurations with your team.
...
3. **Response Processing**:
   ...
   - Results are formatted for display in the Zode Code interface
