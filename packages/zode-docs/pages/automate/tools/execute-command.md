---
title: Execute Command Tool | Zode Code Docs
...
The `execute_command` tool runs CLI commands on the user's system. It allows Zode Code to perform system operations, install dependencies, build projects, start servers, and execute other terminal-based tasks needed to accomplish user objectives.
...
- Integrates with ZodeCodeIgnore system for file access control
- Handles terminal escape sequences for clean output

## Limitations

- Command access may be restricted by ZodeCodeIgnore rules and security validations
...
1. **Command Validation and Security Checks**:
   ...
   - Checks against ZodeCodeIgnore rules for file access permissions
...
2. **Second Priority: Security Validation**
   ...
   - Commands are checked against ZodeCodeIgnore rules for file access control
...
- When setting up a new project, Zode Code runs initialization commands like `npm init -y` followed by installing dependencies.
- When building a web application, Zode Code executes build commands like `npm run build` to compile assets.
- When deploying code, Zode Code runs git commands to commit and push changes to a repository.
- When troubleshooting, Zode Code executes diagnostic commands to gather system information.
- When starting a development server, Zode Code launches the appropriate server command (e.g., `npm start`).
- When running tests, Zode Code executes the test runner command for the project's testing framework.
