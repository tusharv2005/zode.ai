---
title: Read File Tool | Zode Code Docs
description: Learn how the read_file tool examines file contents with line number support to help Zode Code understand code, configuration, and documentation.
---

# read_file

The `read_file` tool examines the contents of files in a project. It allows Zode Code to understand code, configuration files, and documentation to provide better assistance.
...
- When Zode Code needs to understand existing code structure
- When Zode Code needs to analyze configuration files
- When Zode Code needs to extract information from text files
- When Zode Code needs to see code before suggesting changes
...
- When asked to explain or improve code, Zode Code first reads the relevant files to understand the current implementation.
- When troubleshooting configuration issues, Zode Code reads config files to identify potential problems.
- When working with documentation, Zode Code reads existing docs to understand the current content before suggesting improvements.
...
If the file is excluded by rules in a `.zodecodeignore` file:
...
```
Error: Access denied to file '.env' due to .zodecodeignore rules.
```
