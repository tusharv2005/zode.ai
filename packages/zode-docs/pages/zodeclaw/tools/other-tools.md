...
description: "Configure your ZodeClaw agent to use third-party tools and services that aren't pre-installed"
---

# Setting Up Other Services

While ZodeClaw comes with a set of [pre-configured tool integrations](/docs/zodeclaw/tools), your agent isn't limited to just those. ZodeClaw can be configured to use virtually any third-party integration as a tool — as long as it has a CLI or an API, you can teach your agent to work with it.
...
1. Tell ZodeClaw to install the CLI.
...
The instructions above will get your ZodeClaw started with using the tool, but it will have to read the documentation every time and may fumble to use the CLI or API in question. 

As you use the CLI or API, instruct ZodeClaw to do the following to make usage more reliable and less token-intensive:
