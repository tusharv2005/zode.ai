...
Codebase Indexing enables semantic code search across your entire project using AI embeddings. Instead of searching for exact text matches, it understands the _meaning_ of your queries, helping Zode Code find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`semantic_search`](/docs/automate/tools/semantic-search) tool** to Zode Code for intelligent code discovery
...
- **Enhanced AI Understanding**: Zode Code can better comprehend and work with your codebase
...
1. Open Zode Code **Settings** → **Indexing**, or click the indexing indicator at the bottom of the prompt input panel.
...
You can also edit the `indexing` section in `zode.jsonc` directly:
...
- **LanceDB** — embedded, file-based. No server to run. Stores data under your Zode data directory by default.
...
Open a Zode TUI session and run:
...
All changes are written to your `zode.jsonc` config and take effect immediately.
...
- `lancedb` — `{ directory? }` — embedded, file-based. No server to run. Uses a default Zode data directory when omitted.
...
3. If you don't see the icon, open Zode Code settings ({% codicon name="gear" /%}) and search for **Codebase Indexing**.
...
- Files matching `.gitignore` and [`.zodecodeignore`](/docs/customize/context/zodecodeignore) patterns
...
- **API Keys**: Stored in your `zode.jsonc` config. Treat that file as a secret in shared environments.
...
- Check that `indexing.enabled` is `true` in your `zode.jsonc`
...
Once indexed, Zode Code can use the [`semantic_search`](/docs/automate/tools/semantic-search) tool to find relevant code:
...
The tool provides Zode Code with:
...
- **Secure storage**: API keys are stored in your local `zode.jsonc` configuration
- **Fully local option**: Use **Ollama + LanceDB** for completely local processing
- **Access control**: Respects existing file permissions and `.zodecodeignore`
