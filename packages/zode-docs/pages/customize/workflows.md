---
title: "Workflows"
description: "Create automated workflows with Zode Code"
platform: new
---

# Workflows

Workflows (also called **slash commands** in the new extension) automate repetitive tasks by defining step-by-step instructions for Zode Code to execute.

{% image src="/docs/img/screenshot-tests/zode-vscode/visual-regression/settings/agent-behaviour-workflows-chromium-linux.png" alt="Workflows tab in Zode Code" width="420" caption="Workflows tab in Zode Code" /%}
...
Workflows are Markdown files stored as **slash commands** in `.zode/commands/`:

- **Global commands**: `~/.config/zode/commands/` (available in all projects)
- **Project commands**: `[project]/.zode/commands/` (project-specific)
...
For example, a file at `.zode/commands/submit-pr.md` is invoked with `/submit-pr`.
...
The new extension automatically migrates legacy workflows from `.zodecode/workflows/` to the new command format on startup. You can also manually move files and remove the `.md` extension from invocations.

{% /tab %}
{% tab label="VSCode (Legacy)" %}

Workflows are markdown files stored in `.zodecode/workflows/`:

- **Global workflows**: `~/.zodecode/workflows/` (available in all projects)
- **Project workflows**: `[project]/.zodecode/workflows/` (project-specific)
...
Create a file called `submit-pr.md` in your `.zode/commands` directory:
...
Create a file called `submit-pr.md` in your `.zodecode/workflows` directory:
