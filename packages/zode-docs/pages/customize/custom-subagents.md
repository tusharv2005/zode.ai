...
Zode Code includes two built-in subagents:
...
Add agents to the `agent` section of your `zode.jsonc` config file. Any key that doesn't match a built-in agent name creates a new custom agent.
...
- **Global**: `~/.config/zode/agents/`
- **Project-specific**: `.zode/agents/`
...
2. **Global config** (`~/.config/zode/config.json`)
3. **Project config** (`zode.jsonc` in the project root)
4. **Global agent markdown files** (`~/.config/zode/agents/*.md`)
5. **Project agent markdown files** (`.zode/agents/*.md`)
