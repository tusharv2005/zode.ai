...
description: "Provide custom instructions to guide Zode Code"
---

# Custom Instructions

Custom Instructions allow you to personalize how Zode Code behaves, providing specific guidance that shapes responses, coding style, and decision-making processes. Both the **VSCode** and **CLI** versions support custom instructions, though the mechanisms differ.

## What Are Custom Instructions?

Custom Instructions define specific Extension behaviors, preferences, and constraints beyond Zode's basic role definition. Examples include coding style, documentation standards, testing requirements, and workflow guidelines.
...
Zode automatically discovers instruction files at your project root and in parent directories (via `findUp`). The following filenames are recognized:

- **`AGENTS.md`** — The primary instruction file for Zode
...
The `instructions` key in `zode.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. You can manage these in **Settings → Agent Behaviour → Rules** subtab.
...
## Legacy `.zodecoderules` Support

If your project contains `.zodecoderules` files from the VSCode extension, these are still loaded via auto-migration. However, migrating to `AGENTS.md` is recommended for new projects.
...
Each agent can have its own custom prompt defined in its `.md` file (the markdown body) or via the `agent.<name>.prompt` key in `zode.jsonc`:
...
Or as the markdown body in `.zode/agents/code.md`:
...
The `instructions` key in `zode.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. Configure these in your `zode.jsonc`:
...
{% image src="/docs/img/custom-instructions/custom-instructions.png" alt="Zode Code Modes tab showing global custom instructions interface" width="600" caption="Zode Code Modes tab showing global custom instructions interface" /%}

1.  **Open Modes Tab:** Click the <Codicon name="notebook" /> icon in the Zode Code top menu bar
...
Mode-specific instructions can be set using the Modes Tab
    ...
    {% image src="/docs/img/custom-instructions/custom-instructions-3.png" alt="Zode Code Modes tab showing mode-specific custom instructions interface" width="600" caption="Zode Code Modes tab showing mode-specific custom instructions interface" /%}
    * **Open Tab:** Click the <Codicon name="notebook" /> icon in the Zode Code top menu bar
...
- Preferred: `.zode/rules-{mode-slug}/` (directory)
- Fallback: `.zodecoderules-{mode-slug}` (single file)

{% callout type="info" title="Legacy Naming Note" %}
Only `.zodecoderules-{mode-slug}` is recognized as the legacy fallback. Older naming like `.clinerules-{mode-slug}` is not supported.
{% /callout %}
