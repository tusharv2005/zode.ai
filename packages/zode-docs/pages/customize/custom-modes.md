---
title: "Custom Modes"
description: "Create and configure custom modes in Zode Code"
---

# Custom Modes

Zode Code allows you to create **custom modes** (also called **agents**) to tailor Zode's behavior to specific tasks or workflows. Custom modes can be either **global** (available across all projects) or **project-specific** (defined within a single project).

{% callout type="info" %}
The current VS Code extension (built on the Zode CLI) uses **agent Markdown files** to define custom modes. The legacy extension used `custom_modes.yaml` / `.zodecodemodes`. See the tabs below for the relevant approach.
{% /callout %}
...
### 1. Ask Zode! (Recommended)

Ask Zode to create an agent for you:

```
Create a new agent called "docs-writer" that can only read files and edit Markdown files.
```

Zode will generate the agent definition and write it to `.zode/agent/` in your project.
...
```
.zode/agents/my-agent.md
.zode/agent/my-agent.md
...
**Example agent file** (`.zode/agents/docs-writer.md`):
...
### 4. Config File (`zode.jsonc`)

Define agents under the `agent` key in your project's `zode.jsonc`:
...
2. Global config (`~/.config/zode/zode.jsonc`)
3. Project config (`zode.jsonc` at project root)
4. `.zode/` / `.opencode/` directory configs and agent `.md` files
5. Environment variable overrides (`ZODE_CONFIG_CONTENT`)
...
```jsonc
// zode.jsonc — override the built-in "code" agent
{
  "agent": {
    "code": {
      "model": "openai/gpt-4o",
...
Or as a `.md` file (`.zode/agents/code.md`):
...
If you have existing `.zodecodemodes` or `custom_modes.yaml` files from the VSCode extension, the extension automatically migrates them on startup. The migration converts:
...
**Legacy `custom_modes.yaml` is not loaded from `~/.config/zode/`.** If you're migrating from the legacy VSCode extension, global custom modes are read from `~/.zodecode/cli/global/settings/custom_modes.yaml` (not from the CLI's XDG config directory). The recommended approach is to convert legacy modes to agent `.md` files and place them in `~/.config/zode/agent/` instead — see [Markdown files](#3-markdown-files-with-yaml-frontmatter) and [Migration](#migration-from-vscode-extension-modes) below.
{% /callout %}
...
The TUI also **remembers the last model you picked for each agent** across sessions. A config-pinned `model` acts as the default when no manual pick exists. To reset a pick and let the config take over, use the model picker (`Ctrl+X m`) and select a different model, or remove the saved pick from `~/.local/state/zode/model.json`.
...
If you have existing `.zodecodemodes` or `custom_modes.yaml` files from the VSCode extension, the CLI automatically migrates them on startup. The migration converts:
...
| 2 | `~/.zodecode/cli/global/settings/custom_modes.yaml` | YAML | Global |
| 3 | `~/.zodecodemodes` | YAML | Global |
| 4 | `<project>/.zodecodemodes` | YAML | Project (wins on conflict) |

{% callout type="info" %}
`~/.config/zode/` is the XDG config directory for the new agent format — legacy `custom_modes.yaml` placed there will **not** be loaded. Use `~/.config/zode/agent/*.md` or `~/.config/zode/zode.jsonc` for new agent definitions instead.
{% /callout %}
...
Each mode—including custom ones—features **Sticky Models**. This means Zode Code automatically remembers and selects the last model you used with a particular mode. This lets you assign different preferred models to different tasks without constant reconfiguration, as Zode switches between models when you change modes.
...
{% image src="/docs/img/custom-modes/custom-modes-2.png" alt="Custom mode creation interface in Zode Code" width="600" caption="Custom mode creation interface in Zode Code" /%}

_Zode Code's interface for creating and managing custom modes._

## What's Included in a Custom Mode?

Custom modes are defined by several key properties. Understanding these concepts will help you tailor Zode's behavior effectively.

| UI Field / YAML Property | Conceptual Description |
|---|---|
| **Slug** (`slug`) | A unique internal identifier for the mode. Used by Zode Code to reference the mode, especially for associating mode-specific instruction files. |
| **Name** (`name`) | The display name for the mode as it appears in the Zode Code user interface. Should be human-readable and descriptive. |
...
| **Role Definition** (`roleDefinition`) | Defines the core identity and expertise of the mode. This text is placed at the beginning of the system prompt and defines Zode's personality and behavior when this mode is active. |
...
| **When to Use** (`whenToUse`) | _(Optional)_ Provides guidance for Zode's automated decision-making, particularly for mode selection and task orchestration. Used by the Orchestrator mode for task coordination. |
| **Custom Instructions** (`customInstructions`) | _(Optional)_ Specific behavioral guidelines or rules for the mode. Added near the end of the system prompt to further refine Zode's behavior. |
...
Modes are managed from the Modes area in Zode Code. Depending on your UI layout, you can open this from the mode selector in the chat panel or from the notebook icon.
...
5. Zode packages the mode's configuration and any rules into the YAML file
...
4. Choose the import level:
   - **Project:** Available only in current workspace (saved to `.zodecodemodes` file)
...
- **Project agents:** `.zode/agents/*.md` (or `.opencode/agents/*.md`)
- **Global agents:** `~/.config/zode/agents/*.md`
...
### Ask Zode! (Recommended)

You can also have Zode create an agent file for you. For example:

```
Create a new agent called "Documentation Writer". It should only be able to read files and write Markdown files.
```

Zode will create the appropriate `.zode/agents/docs-writer.md` file with the right frontmatter.
...
You can quickly create a basic custom mode by asking Zode Code to do it for you. For example:

```
Create a new mode called "Documentation Writer". It should only be able to read files and write Markdown files.
```

Zode Code will guide you through the process, prompting for necessary information and creating the mode using the preferred YAML format.
...
The interface provides fields for Name, Slug, Description, Save Location, Role Definition, When to Use (optional), Available Tools, and Custom Instructions. After filling these, click the "Create Mode" button. Zode Code will save the new mode in YAML format.

### 3. Manual Configuration (YAML & JSON)

You can directly edit the configuration files to create or modify custom modes. This method offers the most control over all properties. Zode Code now supports both YAML (preferred) and JSON formats.

- **Global Modes:** Edit `custom_modes.yaml` (primary). `custom_modes.json` is a legacy fallback and may still exist in older setups.
- **Project Modes:** Edit `.zodecodemodes` in your project root (YAML preferred; JSON still supported for compatibility).
...
If you see both YAML and JSON mode files, this is usually from legacy configuration. Zode Code reads YAML first and does not keep both files synchronized line-by-line. In practice, edit YAML unless you have a specific reason to stay on JSON.
{% /callout %}
...
- **Usage:** Used internally and in file/directory names for mode-specific rules (e.g., `.zode/rules-{slug}/`)
...
- **Purpose:** The display name shown in the Zode Code UI
...
- **Purpose:** Provides guidance for Zode's automated decision-making, particularly for mode selection and task orchestration
- **Format:** A string describing ideal scenarios or task types for this mode
- **Usage:** Used by Zode for automated decisions and not displayed in the mode selector UI
...
While JSON is still fully supported, new modes created via the UI or by asking Zode will default to YAML.
...
### Project Modes (`.zodecodemodes`)

- No automatic startup migration occurs for project-specific files
- Zode Code can read `.zodecodemodes` files in either YAML or JSON format
- When editing through the UI, JSON files will be converted to YAML format
- For manual conversion, you can ask Zode to help reformat configurations
...
### Preferred Method: Directory (`.zode/rules-{mode-slug}/`)
...
### Fallback Method: Single File (`.zoderules-{mode-slug}`)

```
.
├── .zoderules-docs-writer  # Example for mode slug "docs-writer"
└── ... (other project files)
```

**Rules Directory Scope:**

- **Global modes:** Rules are stored in `~/.zode/rules-{slug}/`
- **Project modes:** Rules are stored in `{workspace}/.zode/rules-{slug}/`
...
1. **Project-level mode configurations** (from `.zodecodemodes` - YAML or JSON)
2. **Global mode configurations** (from `custom_modes.yaml`, then `custom_modes.json` if YAML not found)
3. **Default mode configurations**

**Important:** When modes with the same slug exist in both `.zodecodemodes` and global settings, the `.zodecodemodes` version completely overrides the global one for ALL properties.

## Overriding Default Modes

You can override Zode Code's built-in modes (like 💻 Code, 🪲 Debug, ❓ Ask, 🏗️ Architect, 🪃 Orchestrator) by creating a custom mode with the same slug.
...
**Let Zode Build Your Regex Patterns**

Instead of writing complex regex manually, ask Zode:

```
Create a regex pattern that matches JavaScript files but excludes test files
```

Zode will generate the pattern. Remember to adapt it for YAML (usually single backslashes) or JSON (double backslashes).
...
### Basic Documentation Writer (`.zode/agents/docs-writer.md`)
...
### Test Engineer (`.zode/agents/test-engineer.md`)
...
### Security Reviewer (`.zode/agents/security-review.md`)
...
### Config File Example (`zode.jsonc`)
...
- **Agent not appearing:** Ensure the `.md` file is in a recognized directory (`.zode/agents/`, `.zode/agent/`, `.opencode/agents/`). Check that the `mode` property is `primary` or `all` if you expect it in the agent picker.
...
- **Legacy modes are auto-migrated:** If you have `.zodecodemodes` files, they'll be converted on startup — no manual migration needed
...
- **Keep prompts focused:** The markdown body is your system prompt — write it as if briefing a colleague
- **Use `mode: subagent`** for helper agents that shouldn't be directly selectable by users
- **Test with `zode agent create`** to see how the CLI generates agent definitions, then customize from there
- **Legacy modes are auto-migrated:** If you have `.zodecodemodes` files, they'll be converted on startup — no manual migration needed
...
Ready to explore more? Check out the [Show and Tell](https://github.com/Zode-Org/zodecode/discussions/categories/show-and-tell) to discover and share custom modes and agents created by the community!
