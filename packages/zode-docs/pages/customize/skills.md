---
title: "Skills"
description: "Extend Zode Code capabilities with skills"
---

# Skills

Zode Code implements [Agent Skills](https://agentskills.io/home), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when Zode Code initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.zode` directory within your Home directory:

- Mac and Linux: `~/.zode/skills/`
- Windows: `\Users\<yourUser>\.zode\skills\`
...
Located in `.zode/skills/` within your project:
...
For interoperability with other tools, Zode Code also loads skills from:
...
You can configure extra skill locations and remote skill URLs in your `zode.jsonc` config (project or global):
...
Global skills are located in the `.zodecode` directory within your Home directory.

- Mac and Linux: `~/.zodecode/skills/`
- Windows: `\Users\<yourUser>\.zodecode\`
...
Located in `.zodecode/skills/` within your project:
...
```bash
# For Code mode only
mkdir -p ~/.zodecode/skills-code/typescript-patterns

# For Architect mode only
mkdir -p ~/.zodecode/skills-architect/microservices
```
...
When multiple skills share the same name, project-level skills (`.zode/skills/`) take precedence over global skills (`~/.zode/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
When multiple skills share the same name, Zode Code uses these priority rules:
...
- In the **CLI**: Skills are loaded when you start a new session or run `zode run`
...
Skills are discovered when Zode Code initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the Zode Code extension.
...
In Zode Code, the `name` field **must match** the parent directory name:
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.zode/skills/api-design
   ```
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.zodecode/skills/api-design
   ```
...
- **[Zode Marketplace repository](https://github.com/Zode-Org/zode-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
- **Zode Marketplace** — Browse skills directly in the Zode Code extension via the Marketplace tab, or explore the [Zode Marketplace repository](https://github.com/Zode-Org/zode-marketplace) on GitHub
- [Agent Skills Specification](https://agentskills.io/home) — The open specification that skills follow, enabling interoperability across different AI agents
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.zode/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `zode.jsonc`.
...
1. **Check the Output panel**: Open `View` → `Output` → Select "Zode Code" from dropdown. Look for skill-related errors.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [Zode Marketplace](https://github.com/Zode-Org/zode-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [Zode Marketplace repository](https://github.com/Zode-Org/zode-marketplace) can be manually downloaded into your `.zode/skills/` directory or loaded via `skills.urls` in config.
...
3. **Fork the marketplace repository**: Visit [github.com/Zode-Org/zode-marketplace](https://github.com/Zode-Org/zode-marketplace) and create a fork
...
- Follow the [contribution guidelines](https://github.com/Zode-Org/zode-marketplace/blob/main/CONTRIBUTING.md) in the marketplace repository

For more details on contributing to Zode Code, see the [Contributing Guide](/docs/contributing).
