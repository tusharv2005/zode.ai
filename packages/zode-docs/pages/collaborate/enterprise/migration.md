---
title: "Migration"
description: "Migrate your team to Zode Code Enterprise"
---

# Migration

Switch to **Zode Teams** or **Zode Enterprise** from other AI coding tools and experience transparent pricing, no vendor lock-in, and superior team management capabilities.

## Why Teams Switch to Zode
...
**Zode Teams** and **Zode Enterprise** show you exactly what each AI request costs - no markup, no hidden fees, complete transparency.

### No Rate Limiting

**Other tools** slow you down with rate limits and model switching when you need AI most.

**Zode Teams** and **Zode Enterprise** never limit your usage - pay for what you use, use what you need.

### True Team Management

**Other solutions** offer basic user management with limited visibility.

**Zode Teams** provides comprehensive team analytics, role-based permissions, and detailed usage insights, while **Zode Enterprise** adds advanced governance, audit logging, and enterprise-level security controls.
...
### What You Gain with Zode Teams or Zode Enterprise
...
3. **Calculate current costs** (if visible) vs. Zode pricing
...
1. **Invite team members** to Zode
2. **Install Zode Code extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on Zode Code features and workflows

**Step 4: Full Transition**

1. **Monitor usage patterns** in Zode dashboard
...
| Cursor Feature | Zode Equivalent |
|---|---|
...
2. **Install Zode Code** extension for team members
3. **Start with simple tasks** in Zode Code
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use Zode Code** for new projects
...
Zode Code uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.zodecode/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.zodecode/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Zode Code (no conversion needed)
- **Global rules**: Recreate in `~/.zodecode/rules/*.md` directory

Zode Code also supports mode-specific rules (`.zodecode/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- Zode Code extension installation and setup
