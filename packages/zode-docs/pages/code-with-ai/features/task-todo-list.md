...
{% image src="/docs/img/screenshot-tests/zode-vscode/visual-regression/composite-webview/todo-write-docs-overview-chromium-linux.png" alt="Task Todo List overview showing interactive checklist in Zode Code" width="420" /%}
...
- Ask Zode to "use the [update_todo_list tool](/docs/automate/tools/update-todo-list)"
- Say "create a todo list"

**The bottom line**: Zode decides what goes in the list, but you can provide feedback during approval dialogs.

---

## How todo lists are updated

Todo lists are managed with the [`update_todo_list` tool](/docs/automate/tools/update-todo-list). Each time Zode updates the list, it replaces the entire checklist with the latest view of the task.

Zode updates the list when:
...
**Now**: Zode creates structured checklists that update automatically as work progresses. You see exactly where you are and what's coming up.
...
{% image src="/docs/img/screenshot-tests/zode-vscode/visual-regression/chat/task-header-with-todos-chromium-linux.png" alt="Task header summary showing todo list progress" width="420" /%}
...
- Edit descriptions when Zode asks for approval
- Stage changes using the "Edit" button

**3. Environment Details**
Background "REMINDERS" table that keeps Zode informed about current progress
...
When Zode proposes a todo list update, you can edit the list before approving. Use the "Edit" button in the tool block to update item text, add or remove steps, or adjust status. Once approved, Zode continues with the updated list.

## Common questions

**"Can I create my own todo lists?"**
Yes, just ask Zode to use the update_todo_list tool. But Zode stays in control of the content and workflow.

**"What about simple tasks?"**
Zode typically skips todo lists for simple tasks. The overhead isn't worth it.

**"Why can't I directly edit the list?"**
Design choice. Zode maintains authority over task management to ensure consistent progress tracking. You provide input, Zode executes.

---

## Settings

You can disable todo lists in Settings -> Advanced -> **Enable todo list tool**. When disabled, Zode won't create or update todo lists, and the REMINDERS table won't appear in Environment Details.
