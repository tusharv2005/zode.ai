...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Zode Code. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.
...
Once your BYOK key is saved, Zode Code's autocomplete will automatically use your Codestral key through the Zode Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Zode Code extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("Zode Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your Zode balance.
...
```
Your Editor → Zode Gateway (with your key) → Mistral
```

- The Zode Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — Zode does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to Zode's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in Zode Code settings (it is on by default). Also verify you are signed into Zode Code in the extension.
...
- **Seeing charges on your Zode balance?** If you haven't configured BYOK, autocomplete defaults to using your Zode credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
...
{% youtube url="https://www.youtube.com/embed/0aqBbB8fPho" caption="Setting up Mistral for free autocomplete in Zode Code" /%}

## Step 1: Open Zode Code Settings

In VS Code, open the Zode Code panel and click the **Settings** icon (gear) in the top-right corner.

![Open Zode Code Settings](/docs/img/mistral-setup/01-open-zode-code-settings.png)
...
{% callout type="note" %}
When creating an autocomplete profile, you don't need to select a specific model—Zode Code will automatically use the appropriate Codestral model optimized for code completions.
{% /callout %}
...
## Step 9: Paste API Key in Zode Code

Return to Zode Code settings and paste your API key into the **Mistral API Key** field.
