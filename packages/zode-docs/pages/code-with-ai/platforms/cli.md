---
title: "Zode Code CLI: Run the AI Coding Agent from Your Terminal"
description: "Using Zode Code from the command line"
platform: new
---

{% callout type="warning" title="Version Notice" %}
This documentation applies only to Zode version 1.0 and later. Users running versions below 1.0 should upgrade before proceeding.
{% /callout %}

# Zode Code CLI: AI Coding Agent in Your Terminal

Orchestrate agents from your terminal. Plan, debug, and code fast with keyboard-first navigation on the command line.

The Zode Code CLI uses the same underlying technology that powers the IDE extensions, so you can expect the same workflow to handle agentic coding tasks from start to finish.

**Source code & issues (Zode CLI 1.0):** [Zode-Org/zodecode](https://github.com/Zode-Org/zodecode) · [Report an issue](https://github.com/Zode-Org/zodecode/issues)
...
Change directory to where you want to work and run zode:
...
After installation, run `zode` and use the `/connect` command to add your first provider credentials. This is the interactive way to configure API keys for model providers.

## Update

Upgrade the Zode CLI:

`zode upgrade`

Or use npm:

`npm update -g @zodecode/cli`

## What you can do with Zode Code CLI
...
- **Switch between hundreds of LLMs without constraints.** Other CLI tools only work with one model or curate opinionated lists. With Zode, you can switch models without booting up another tool.
...
#### Zode Gateway Commands (when connected)

| Command | Aliases | Description |
|---|---|---|
| `/profile` | `/me`, `/whoami` | View your Zode Gateway profile |
| `/teams` | `/team`, `/org`, `/orgs` | Switch between Zode Gateway teams |
...
- Config files in **`~/.config/zode/`**: use **`zode.jsonc`** for provider, model, permission, and **MCP** settings. Restart the CLI after editing. See [Using MCP in Zode Code](/docs/automate/mcp/using-in-zode-code) for MCP config format.
- `zode auth` for credential management
...
Zode Code uses the permission config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Zode was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many bash commands).
...
The Zode CLI is a fork of [OpenCode](https://opencode.ai) and supports the same configuration options. The CLI you install with `npm install -g @zodecode/cli` (Zode CLI 1.0) is built from [Zode-Org/zodecode](https://github.com/Zode-Org/zodecode). For comprehensive configuration documentation, see the [OpenCode Config documentation](https://opencode.ai/docs/config).

### Config File Location (Zode CLI 1.0)

| Scope | Path |
|---|---|
| **Global** | `~/.config/zode/opencode.json` or `opencode.jsonc` (Windows: config dir may vary; same filenames) |
...
Some terminals don't send modifier keys with Enter by default. Windows Terminal requires a one-time configuration to forward `Shift+Enter` as an escape sequence that Zode can read.
...
Save the file and restart Windows Terminal or open a new tab. `Shift+Enter` will now insert a newline in the Zode prompt instead of submitting the message.

### OpenTelemetry Export

Zode telemetry is enabled by default and can be disabled with `experimental.openTelemetry = false`:
...
Interactive mode is the default mode when running Zode Code without the `--auto` flag, designed to work interactively with a user through the console.

In interactive mode Zode Code will request approval for operations which have not been auto-approved, allowing the user to review and approve operations before they are executed, and optionally add them to the auto-approval list.
...
Zode only saves the pattern you select. Approving a specific command does not approve redirected variants or broader command patterns unless that broader option is shown and selected.

## Autonomous Mode (Non-Interactive)

Autonomous mode allows Zode Code to run in automated environments like CI/CD pipelines without requiring user interaction.

```bash
# Run in autonomous mode with a message
zode run --auto "Implement feature X"
```
...
- name: Run Zode Code
  run: |
    zode run "Implement the new feature" --auto
```
...
Remote Connections let you access your local CLI sessions from the Cloud Agents web interface. Requires [Zode Gateway](/docs/gateway) connection.
...
Requires connection to Zode Gateway. The `/remote` command appears only when authenticated.

**Enable by default:**

Add to `~/.config/zode/config.json`:
...
- Connection to Zode Gateway
- Same Zode account on CLI and Cloud Agent
- CLI must remain running with internet connection

{% callout type="warning" title="Security Warning" %}
Anyone with access to your Zode account can send messages to your computer when remote mode is enabled.
{% /callout %}
...
- `ZODE_PROVIDER`: Override the active provider ID
- For `zodecode` provider: `ZODECODE_<FIELD_NAME>` (e.g., `ZODECODE_MODEL` → `zodecodeModel`)
- For other providers: `ZODE_<FIELD_NAME>` (e.g., `ZODE_API_KEY` → `apiKey`)

## Using the CLI in an Organization

If you belong to a Zode organization (Team or Enterprise), you can route CLI requests through that organization. The process differs slightly between interactive and non-interactive usage.
...
### Non-Interactive Usage (`zode run`)

There is no `--org` or `--team` flag on `zode run`. Instead, the organization is determined from the following sources, in order of priority (highest first):

1. **`ZODE_ORG_ID` environment variable** — Best for non-interactive and CI environments.
