---
title: "Zode Code for VS Code: Free Open-Source AI Coding Extension"
description: "Using Zode Code in Visual Studio Code"
---

# Zode Code for VS Code

Zode Code is available as two VS Code extensions: the **VSCode (Legacy)** extension and the current **VSCode** version built on Zode's shared agent runtime.
...
The extension includes its own embedded runtime. No separate Zode CLI installation is required.
...
- **[JSONC config files](/docs/getting-started/settings)** — Portable settings in `zode.jsonc` instead of VS Code settings
- **[Granular permissions](/docs/getting-started/settings/auto-approving-actions)** — Per-tool permission rules with glob patterns
- **[Agents](/docs/code-with-ai/agents/using-agents)** — Customizable agents (`.zode/agents/*.md`) replacing the modes system
...
Settings apply across extension surfaces, including the sidebar and Agent Manager. The standalone CLI uses the same `~/.config/zode/zode.jsonc` (global) and `./zode.jsonc` (project) files when used directly.

## Proxy and Certificate Troubleshooting

Zode Code for VS Code starts its embedded runtime from the extension and applies the relevant VS Code network settings to that runtime. On managed networks, configure proxy and certificate trust in VS Code settings rather than in a separate CLI install.
...
- Leave `http.proxySupport` enabled unless you intentionally want VS Code and Zode Code to ignore proxy settings.
...
- If the operating system trust store is not enough, set `zode-code.new.extraCaCerts` to the absolute path of a PEM file that contains the additional certificate authority certificates.
...
```json
{
  ...
  "zode-code.new.extraCaCerts": "/absolute/path/to/corporate-ca.pem"
}
```
...
- **[Custom Modes](/docs/customize/custom-modes)** — Define custom modes with `.zodecodemodes` YAML files
