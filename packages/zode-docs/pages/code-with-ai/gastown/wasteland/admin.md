...
### Zode Cloud hosted option

<!-- TODO: verify — confirm whether Zode Cloud offers a hosted wasteland creation flow or if self-serve is CLI/dashboard only -->

Zode Cloud provides the managed infrastructure (Cloudflare containers, encryption, billing) but the upstream DoltHub database is always user-owned. There is no fully-hosted wasteland where Zode manages the DoltHub repo on your behalf — you always create and own the upstream. The wasteland container runs on Zode's infrastructure, but data lives in your DoltHub account.
...
1. Click **Add member**
2. Enter the user's Zode user ID
