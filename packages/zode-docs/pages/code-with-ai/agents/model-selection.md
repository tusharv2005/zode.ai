...
Instead of maintaining a static list that's perpetually behind, we built something better — a real-time leaderboard showing which models Zode Code users are actually having success with right now.
...
- Set per-agent defaults and a global default in the **Settings** panel (Models tab), or directly in the `zode.jsonc` config file.
...
- For non-interactive use, pass `--model` flag to `zode run` (e.g., `zode run --model claude-sonnet-4-20250514`).
- Set the global default with the `model` key in `zode.jsonc`, or configure per-agent models in the `agent` section.
...
- **Via config file** — edit `zode.jsonc`:
