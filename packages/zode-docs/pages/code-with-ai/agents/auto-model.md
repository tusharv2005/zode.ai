...
|---|---|---|
| `zode-auto/frontier` | Maximum capability with the best available models | Paid |
| `zode-auto/balanced` | Strong performance at a lower cost | Paid |
| `zode-auto/free` | The best free models available | Free |

## How It Works

1. Select an Auto Model tier (e.g. `zode-auto/frontier`) in the model dropdown
...
You can see which underlying models are used, as well as the cost, in the expanded model picker. Model mapping information is also available on the [Gateway Model page](/docs/gateway/models-and-providers#zode-autofrontier).
...
Select an Auto Model tier from the model dropdown in the Zode Code chat interface. That's all you need to do.
{% /callout %}

1. Open Zode Code in VS Code or JetBrains
2. Click the model selector dropdown
3. Choose an Auto Model such as `zode-auto/frontier` or `zode-auto/balanced`
...
- **New Zode Code users** - Great defaults without needing to understand model differences
...
- [Using Agents](/docs/code-with-ai/agents/using-agents) - Learn about different Zode Code agents
- [Using Zode for Free](/docs/getting-started/using-zode-for-free) - Cost-effective alternatives
