...
# Migrating from Cline to Zode

A practical guide for developers switching from Cline to Zode.

## Why Switch?

**Speed.** The Zode Platform is designed to reduce friction across the entire development workflow. Beyond feature parity with Cline, Zode includes Code Reviews, App Builder, Cloud Agents, and one-click Deploy.

**Multi-platform.** Work in VS Code, JetBrains IDEs, CLI, or the web. Your Sessions sync across all of them automatically.

**Specialized modes.** Instead of one agent doing everything, Zode has five specialized modes optimized for different parts of your workflow.

**500+ models.** More providers, more options, better pricing flexibility. Switch models mid-conversation if you want.

---

## Understanding Zode's Mode System

Cline uses a single agent with a Plan/Act toggle. Zode takes a different approach: five specialized modes, each optimized for a specific part of development. You select modes from a dropdown in the interface in the Zode Extension, or with slash commands in the CLI.
...
**Zode:** Select **Ask** from the dropdown
...
**Why this matters:** In Cline, you might accidentally make changes while exploring. In Zode, Ask and Architect modes can't write files, so you're safe to explore without worry. Zode's **Orchestrator** mode lets you intelligently generate and execute subtasks that are optimized for each mode.
...
2. Search "Zode Code"
3. Click Install
4. Find the Zode icon in your sidebar
...
4. Find Zode icon in right sidebar
...
- **Zode Deploy:** One-click deployments
...
**Option 1: Use Zode Gateway (easiest)**

1. Open settings (⚙️ icon)
2. Select "Zode Gateway" as provider
...
## Beyond the IDE: Zode's Platform Features

One of the biggest differences from Cline is that Zode isn't just an IDE extension. It's a platform with multiple interfaces that can all share your Sessions and context.
...
Build live apps and sites directly from the web with a real-time preview. Similar to Lovable, but integrated with your Zode Sessions. Deploy with one click when you're ready.

### Zode Deploy

One-click deployments from directly within Zode. Go from code to production without leaving your workflow.
...
Semantic search across your repositories using cloud-hosted embeddings. Zode indexes your codebase to deliver more relevant, context-aware responses.
...
| Cline Feature | Zode Equivalent | Notes |
|---|---|---|
...
- **One-click Deploy:** Ship directly from Zode
- **Code Reviews:** AI-powered PR analysis
...
**Q: Can I use both Cline and Zode side-by-side?** Yes. They're separate extensions.
...
1. Install Zode in your primary IDE
