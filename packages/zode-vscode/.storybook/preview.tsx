...
// Reference zode-ui stories helpers directly — not exported via package.json
import { applyZodeTheme, applyVscodeTheme, clearVscodeTheme } from "../../zode-ui/src/stories/theme-decorator"
import "../../zode-ui/.storybook/fonts.css"
import "@zodecode/zode-ui/styles"
import "../webview-ui/src/styles/chat.css"

// Make the Zode logo available in Storybook (normally injected by the extension host)
;(window as { ICONS_BASE_URI?: string }).ICONS_BASE_URI = "/icons"

const themeDecorator: DecoratorFunction<SolidRenderer> = (Story, context) => {
  const themeId = (context.globals["theme"] as string) ?? "zode-vscode"
  const vscodeThemeId = (context.globals["vscodeTheme"] as string) ?? "dark-modern"

  const colorScheme = (() => {
    if (themeId === "zode-vscode") return applyVscodeTheme(vscodeThemeId)
    clearVscodeTheme()
    return (context.globals["colorScheme"] as "light" | "dark") ?? "dark"
  })()

  applyZodeTheme(themeId, colorScheme)
  ...
  globalTypes: {
    theme: {
      ...
      toolbar: {
        ...
        icon: "paintbrush",
        items: [
          { value: "zode-vscode", title: "Zode VSCode" },
          { value: "zode", title: "Zode" },
        ],
        dynamicTitle: true,
      },
    },
