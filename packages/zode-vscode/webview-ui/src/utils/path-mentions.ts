...
export function convertToMentionPath(path: string, cwd: string): string {
  ...
  try {
    ...
    console.error("[Zode New] Failed to decode dropped URI:", err, cleaned)
  }
 ...
 */
export const ZODE_FILE_PATH_MIME = "application/x-zode-file-path"

/**
 * Extract file paths from a drop's DataTransfer.
 * Checks (in order):
 * 1. Internal relative-path drag (application/x-zode-file-path)
...
export function extractDropPaths(dt: DataTransfer): string[] | null {
  ...
  const zode = dt.getData(ZODE_FILE_PATH_MIME)
  if (zode) {
    const paths = zode.split(/\r?\n/).filter((line) => line.trim() !== "")
