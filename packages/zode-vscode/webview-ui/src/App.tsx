...
import type { Message as SDKMessage, Part as SDKPart } from "@zodecode/sdk/v2"
...
export const DataBridge: Component<{ children: any }> = (props) => {
  ...
  return (
    <DataProvider
      data={data}
      directory={directory()}
      // @ts-expect-error — onPermissionRespond/onQuestion* are extension-specific props not yet in zode-ui's DataProvider types
...
export const MermaidDownloadBridge: Component = () => {
  ...
  onMount(() => {
    ...
    window.addEventListener("zode:save-image", save)
    onCleanup(() => {
      window.removeEventListener("zode:save-image", save)
    })
  })
  ...
  onMount(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (message?.type === "action" && message.action) {
        console.log("[Zode New] App: 🎬 action:", message.action)
        handleViewAction(message.action)
      }
      if (message?.type === "navigate" && message.view && VALID_VIEWS.has(message.view)) {
        console.log("[Zode New] App: 🧭 navigate:", message.view, message.tab ? `tab=${message.tab}` : "")
      ...
      if (message?.type === "openCloudSession" && message.sessionId) {
        console.log("[Zode New] App: ☁️ openCloudSession:", message.sessionId)
        session.selectCloudSession(message.sessionId)
        setCurrentView("newTask")
      }
      handleForked(message)
      if (message?.type === "viewSubAgentSession" && message.sessionID) {
        console.log("[Zode New] App: 🔍 viewSubAgentSession:", message.sessionID)
      ...
      if (message?.type === "migrationState") {
        console.log("[Zode New] App: 🔄 migrationState:", message.needed)
