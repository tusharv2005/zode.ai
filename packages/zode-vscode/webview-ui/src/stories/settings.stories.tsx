...
import { ZodeEmbeddingModelsContext } from "../context/zode-embedding-models"
...
export const IndexingZodeModelPreset: Story = {
  name: "IndexingTab - Zode stale custom model fallback",
  render: () => {
    const cfg: Config = {
      indexing: {
        provider: "zode",
    ...
    return (
      <StoryProviders config={cfg}>
        <ZodeEmbeddingModelsContext.Provider value={{ catalog: () => catalog }}>
          <div style={{ "max-height": "700px", overflow: "auto" }}>
            <IndexingTab />
          </div>
        </ZodeEmbeddingModelsContext.Provider>
      </StoryProviders>
    )
  },
}

export const IndexingZodeCatalogLoading: Story = {
  name: "IndexingTab - Zode catalog loading",
  render: () => {
    const [saved, setSaved] = createSignal<Record<string, unknown>>({})
    const cfg: Config = {
      indexing: {},
    ...
    return (
      <>
        ...
        <pre data-testid="indexing-zode-loading-save">{JSON.stringify(saved(), null, 2)}</pre>
      </>
    )
  },
}
