...
import { DataProvider } from "@zodecode/zode-ui/context/data"
import { DiffComponentProvider } from "@zodecode/zode-ui/context/diff"
import { CodeComponentProvider } from "@zodecode/zode-ui/context/code"
import { FileComponentProvider } from "@zodecode/zode-ui/context/file"
...
import { Diff } from "@zodecode/zode-ui/diff"
import { Code } from "@zodecode/zode-ui/code"
import { File } from "@zodecode/zode-ui/file"
import { SessionContext } from "../context/session"
import { NotificationsContext } from "../context/notifications"
import { LanguageContext } from "../context/language"
import { IndexingProvider } from "../context/indexing"
import { ZodeEmbeddingModelsProvider } from "../context/zode-embedding-models"
...
const dict: Record<string, string> = { ...appEn, ...amEn, ...uiEn, ...zodeEn }
...
// ---------------------------------------------------------------------------
// Mock providers — pre-loaded Zode Gateway model for stories
// ---------------------------------------------------------------------------
...
const MockProviderProvider: ParentComponent<{ zodeAuth?: boolean }> = (props) => {
  const value = {
    providers: () => MOCK_PROVIDERS as any,
    connected: () => ["zode"],
    defaults: () => ({}),
    defaultSelection: () => ({ providerID: "zode", modelID: "anthropic/claude-sonnet-4-6" }),
    models: () => MOCK_MODELS,
    findModel: (sel: any) => _findModel(MOCK_MODELS, sel),
    authMethods: () => ({}),
    authStates: () => (props.zodeAuth ? { zode: "oauth" } : {}) as Record<string, ProviderAuthState>,
    isModelValid: () => true,
  }
...
function mockNotificationsValue(items: ZodecodeNotification[] = []) {
  return {
    notifications: () => items,
...
export function mockSessionValue(overrides?: {
  ...
  return {
    ...
    selected: () => ({ providerID: "zode", modelID: "anthropic/claude-sonnet-4-6" }),
    selectModel: noop,
    ...
    getSessionModel: () => ({ providerID: "zode", modelID: "anthropic/claude-sonnet-4-6" }),
...
interface StoryProvidersProps {
  ...
  questions?: QuestionRequest[]
  suggestions?: SuggestionRequest[]
  notifications?: ZodecodeNotification[]
  status?: string
  sessionID?: string
      ...
      <ServerProvider>
        <FeedbackProvider>
          <ConfigWrapper config={props.config} onConfigChange={props.onConfigChange}>
            <DisplayProvider>
              <MockProviderProvider zodeAuth={props.zodeAuth}>
                <DialogProvider>
