...
import { Card } from "@zodecode/zode-ui/card"
import { formatZodeEmbeddingModelLabel, getZodeEmbeddingModel } from "@zodecode/zode-indexing/embedding-models"
import { Select } from "@zodecode/zode-ui/select"
import { Switch } from "@zodecode/zode-ui/switch"
import { TextField } from "@zodecode/zode-ui/text-field"
import { Tooltip } from "@zodecode/zode-ui/tooltip"
import { useConfig } from "../../context/config"
import { formatIndexingLabel, useIndexing } from "../../context/indexing"
import { useZodeEmbeddingModels } from "../../context/zode-embedding-models"
...
const IndexingTab: Component = () => {
  ...
  const embeds = useZodeEmbeddingModels()
  ...
  const zodeDefault = () =>
    getZodeEmbeddingModel(embeds.catalog().defaultModel, embeds.catalog())?.id ?? embeds.catalog().defaultModel
  const zodeModels = createMemo(() =>
    embeds.catalog().models.map((model) => ({
      value: model.id,
      label: formatZodeEmbeddingModelLabel(model),
    })),
  )
  const knownZodeModel = (model: string | null | undefined) =>
    getZodeEmbeddingModel(model ?? undefined, embeds.catalog())?.id
  const zodeValue = () => knownZodeModel(cfg().model) ?? zodeDefault()
  const zodeAvailable = () => !!server.profileData() || provider.authStates()[ZODE_PROVIDER_ID] !== undefined
  const selectedProvider = () => cfg().provider ?? (zodeAvailable() ? "zode" : undefined)
  const staleZodeModel = () => selectedProvider() === "zode" && !!cfg().model && !knownZodeModel(cfg().model)
  const providers = createMemo(() =>
    allProviders.filter((item) => item.value !== "zode" || zodeAvailable() || selectedProvider() === "zode"),
  )
  const fields = createMemo(() => providerFields(selectedProvider()))

  const saveProvider = (next: ProviderId | undefined) => {
    if (next === "zode") {
      const model = knownZodeModel(cfg().model) ?? (zodeDefault() || null)
      updateIndexing({
        provider: next,
        model,
        dimension: null,
  ...
  const saveEnabled = (enabled: boolean) => {
    if (enabled && !cfg().provider && zodeAvailable()) {
      updateIndexing({
        enabled,
        provider: "zode",
        model: knownZodeModel(cfg().model) ?? (zodeDefault() || null),
        dimension: null,
      })
      return
    }
  ...
  const saveGlobalEnabled = (enabled: boolean) => {
    if (enabled && !globalCfg().provider && !cfg().provider && zodeAvailable()) {
      updateGlobalConfig({
        indexing: {
          enabled,
          provider: "zode",
  ...
  const saveModel = (value: string) => {
    if (selectedProvider() === "zode") return
  ...
  return (
    <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
      ...
      <Card>
        ...
        <Show when={selectedProvider() === "zode"}>
          <Show when={zodeModels().length > 0}>
            <SettingsRow
              title={language.t("settings.indexing.zodeModel.title")}
              description={language.t("settings.indexing.zodeModel.description")}
            >
              <Select
                options={zodeModels()}
                current={zodeModels().find((item) => item.value === zodeValue())}
                value={(item) => item.value}
                label={(item) => item.label}
                onSelect={(item) => updateIndexing({ model: item?.value ?? zodeDefault(), dimension: null })}
                variant="secondary"
                size="small"
                triggerVariant="settings"
        ...
        <SettingsRow
          ...
          last={!selectedProvider() || (fields().length === 0 && !(selectedProvider() === "zode" && !zodeAvailable()))}
        >
          <TextField
            value={
              staleZodeModel() || cfg().dimension === undefined || cfg().dimension === null
                ? ""
        ...
        <Show when={selectedProvider() === "zode" && !zodeAvailable()}>
          <SettingsRow
            title={language.t("settings.indexing.zodeSignIn.title")}
            description={language.t("settings.indexing.zodeSignIn.description")}
            last
          >
            <span />
          </SettingsRow>
