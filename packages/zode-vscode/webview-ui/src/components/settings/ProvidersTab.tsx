...
import { disabledProviderOptions, providersWithZodeFallback, visibleConnectedIds } from "./provider-visibility"
import { ZODE_PROVIDER_ID, CUSTOM_PROVIDER_PACKAGE } from "../../../../src/shared/provider-model"
...
const ProvidersTab: Component = () => {
  ...
  const providers = createMemo(() => providersWithZodeFallback(provider.providers()))
  ...
  function connectProvider(item: Provider) {
    if (item.id === ZODE_PROVIDER_ID) {
      // Route Zode Gateway sign-in through the Profile view so the user sees
  ...
  return (
    <div>
      <Show when={!disabledIds().has(ZODE_PROVIDER_ID)}>
        {/* Zode Gateway — always at the top, not editable */}
        <Card>
          <div
            style={{
              display: "flex",
      ...
      {/* Connected providers (excluding Zode) */}
