...
export function providerIcon(providerID: string): IconName {
  if (providerID === ZODE_PROVIDER_ID) return "synthetic"
  if (iconNames.includes(providerID as IconName)) return providerID as IconName
  return "synthetic"
}

export function zodeFallbackProvider(): Provider {
  return createZodeFallbackProvider()
}

export function providerNoteKey(providerID: string) {
  if (providerID === "zode") return "dialog.provider.zode.note"
