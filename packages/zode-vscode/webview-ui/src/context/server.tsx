...
export const ServerProvider: ParentComponent = (props) => {
  ...
  onMount(() => {
    const unsubscribe = vscode.onMessage((message: ExtensionMessage) => {
      switch (message.type) {
        case "ready":
          console.log("[Zode New] Server ready:", message.serverInfo)
        ...
        case "connectionState":
          console.log("[Zode New] Connection state changed:", message.state)
        ...
        case "error":
          console.error("[Zode New] Server error:", message.message)
          setErrorMessage(message.message)
          setErrorDetails(message.message)
          break

        case "profileData":
          console.log("[Zode New] Profile data:", message.data ? "received" : "null")
          setProfileData(message.data)
          break

        case "deviceAuthStarted":
          console.log("[Zode New] Device auth started")
          setDeviceAuth({
            status: "pending",
            code: message.code,
        ...
        case "deviceAuthComplete":
          console.log("[Zode New] Device auth complete")
        ...
        case "deviceAuthFailed":
          console.log("[Zode New] Device auth failed:", message.error)
          setDeviceAuth({ status: "error", error: message.error })
          break

        case "deviceAuthCancelled":
          console.log("[Zode New] Device auth cancelled")
    ...
    console.log("[Zode New] Webview ready")
   ...
   * outside the Profile page — e.g. the Zode Gateway card in the Providers
