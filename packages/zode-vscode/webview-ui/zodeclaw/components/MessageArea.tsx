// ZodeClaw active-conversation view — message list + composer.
// Mirrors cloud/apps/web/src/app/(app)/claw/zode-chat/components/MessageArea.tsx

import { For, Show, createEffect, createMemo, createSignal, on, onCleanup } from "solid-js"
import { Button } from "@zodecode/zode-ui/button"
import { Spinner } from "@zodecode/zode-ui/spinner"
import { createAutoScroll } from "@zodecode/zode-ui/hooks"
...
export function MessageArea() {
  ...
  //
  // Uses @zodecode/zode-ui's createAutoScroll (the same one MessageList.tsx
  ...
  const sendDisabledReason = createMemo(() => {
    ...
    const st = botDisplay().state
    if (st === "unknown") return t("zodeClaw.chat.waitingBotStatus")
    return t("zodeClaw.chat.botOffline")
  })

  // Render the typing banner with friendly names. Bot members come in as
  // `bot:zodeclaw:{sandboxId}` — the bot is the only non-self member in
  // 1:1 chats, so we resolve bot ids to the user-configured `botName`
  // (falls back to the default "ZodeClaw" label) and render any human
  // collaborators by their raw memberId.
  const typingNames = createMemo(() => {
    const activeId = claw.activeConversationId()
    if (!activeId) return []
    const assistant = claw.assistantName() ?? t("zodeClaw.message.bot")
  ...
  return (
    <Show
      ...
      fallback={
        <div class="zodeclaw-empty-area">
          <p>{t("zodeClaw.conversations.selectOne")}</p>
        </div>
      }
    >
      <div class="zodeclaw-area">
        {/* Messages */}
        <div class="zodeclaw-area-messages-wrap">
          <div
            class="zodeclaw-area-messages"
            ref={(el) => {
              scrollEl = el
              auto.scrollRef(el)
            }}
          ...
          >
            <div ref={auto.contentRef}>
              <Show when={claw.messages().length === 0}>
                <div class="zodeclaw-empty">
                  {claw.assistantName()
                    ? t("zodeClaw.chat.emptyWithBot").replace("{bot}", claw.assistantName()!)
                    : t("zodeClaw.chat.empty")}
                </div>
              </Show>
        ...
        <Show when={typingNames().length > 0}>
          <div class="zodeclaw-typing">
            <Spinner />
            <span>
              {typingNames().length === 1
                ? t("zodeClaw.typing.one").replace("{name}", typingNames()[0])
                : t("zodeClaw.typing.many").replace("{count}", String(typingNames().length))}
            </span>
          </div>
        </Show>
        ...
        <Show when={replyingTo()}>
          {(r) => (
            <div class="zodeclaw-reply-preview">
              <span class="zodeclaw-reply-preview-label">{t("zodeClaw.message.replyTo")}</span>
              <span class="zodeclaw-reply-preview-text">{replyText(r())}</span>
              <button
                type="button"
                class="zodeclaw-iconbtn-sm"
                onClick={() => setReplyingTo(null)}
                aria-label={t("zodeClaw.message.cancelReply")}
              >
                ×
              </button>
            </div>
        ...
        <div class="zodeclaw-input-wrap">
          <textarea
            ref={input}
            class="zodeclaw-input"
            placeholder={sendDisabledReason() ?? t("zodeClaw.chat.placeholder")}
            disabled={!canSend()}
            value={text()}
            onInput={onInput}
            onKeyDown={onKeyDown}
            rows={1}
            aria-label={t("zodeClaw.chat.placeholder")}
          />
          <Button variant="primary" disabled={!canSend() || !text().trim()} onClick={submit}>
            {t("zodeClaw.chat.send")}
          </Button>
        </div>
      </div>
    </Show>
