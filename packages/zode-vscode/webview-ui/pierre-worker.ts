// Zode-specific replacement for the shared `@opencode-ai/ui/pierre/worker`
...
function uri(): string | undefined {
  ...
  return (window as { ZODE_SHIKI_WORKER_URI?: string }).ZODE_SHIKI_WORKER_URI
}

export function workerFactory(): Worker {
  const url = uri()
  if (!url) throw new Error("ZODE_SHIKI_WORKER_URI is not set")
...
function createPool(lineDiffType: "none" | "word-alt") {
  const pool = new WorkerPoolManager(
    ...
    { theme: "Zode", lineDiffType, preferredHighlighter: ENGINE },
  )
