...
const GLOBALS = "colorScheme:dark;theme:zode-vscode;vscodeTheme:dark-modern"
const STORY_ID = "settings--indexing-provider-blur-race"
const ZODE_STORY_ID = "settings--indexing-zode-model-preset"
const ZODE_LOADING_STORY_ID = "settings--indexing-zode-catalog-loading"
...
test("Zode exposes only supported embedding model presets", async ({ page }) => {
  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(ZODE_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })

  await expect(page.getByText("Zode model preset", { exact: true })).toBeVisible()
...
test("enabling Zode before its catalog loads does not store an empty model", async ({ page }) => {
  const saved = page.getByTestId("indexing-zode-loading-save")
  const cfg = async () => JSON.parse(((await saved.textContent()) ?? "{}").trim()) as Saved
  const verify = async () => {
    await expect.poll(async () => (await cfg()).provider).toBe("zode")
    expect((await cfg()).model).toBeNull()
    expect((await cfg()).dimension).toBeNull()
  }

  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(ZODE_LOADING_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })
