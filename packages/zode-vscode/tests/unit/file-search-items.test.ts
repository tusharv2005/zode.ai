...
import { mergeFileSearchItems } from "../../src/zode-provider/file-search-items"
...
describe("mergeFileSearchItems", () => {
  ...
  it("normalizes Windows separators for matching and output", () => {
    const result = mergeFileSearchItems({
      query: "zode-vscode",
      files: ["packages\\zode-vscode\\src\\ZodeProvider.ts"],
      folders: ["packages\\zode-vscode\\"],
    })
    expect(result).toEqual([
      { path: "packages/zode-vscode/", type: "folder" },
      { path: "packages/zode-vscode/src/ZodeProvider.ts", type: "file" },
    ])
  })
  ...
  it("keeps active and open file results before prefix folder matches", () => {
    const result = mergeFileSearchItems({
      query: "e",
      files: ["packages/zode-vscode/src/extension.ts"],
      folders: ["packages/extensions/", "packages/example/", "packages/core/src/effect/"],
      open: new Set(["packages/zode-vscode/src/extension.ts"]),
    })
    expect(result).toEqual([
      { path: "packages/zode-vscode/src/extension.ts", type: "opened-file" },
