...
async function createTempWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zode-ignore-"))
  tempDirs.push(dir)
  return dir
}

describe("FileIgnoreController", () => {
  describe("when .zodecodeignore exists", () => {
    it("applies only .zodecodeignore patterns", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".zodecodeignore"), "secret/**\n*.snap\n")
      ...
      expect(controller.getInstructions()).toContain(".zodecodeignore")
    })

    it("does NOT block .env files unless explicitly listed", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".zodecodeignore"), "dist/\n")
    ...
    it("does NOT apply .gitignore patterns", async () => {
      ...
      // .zodecodeignore pattern should apply
      expect(controller.validateAccess(path.join(workspace, "dist", "bundle.js"))).toBe(false)
    })
  })

  describe("when no .zodecodeignore exists (fallback)", () => {
    ...
    it("blocks .env files via hardcoded sensitive patterns", async () => {
      const workspace = await createTempWorkspace()
      // No .zodecodeignore, no .gitignore
  ...
  describe("Windows cross-drive paths", () => {
    it("does not throw for a Windows-style absolute path from another drive", async () => {
      ...
      const cross =
        "c:/Users/User/AppData/Roaming/Code/User/globalStorage/zodecode.zode-code/settings/mcp_settings.json"
    ...
    it("does not throw for file:// URIs with Windows drive letters", async () => {
      ...
      const uri =
        "file:///c:/Users/User/AppData/Roaming/Code/User/globalStorage/zodecode.zode-code/settings/mcp_settings.json"
