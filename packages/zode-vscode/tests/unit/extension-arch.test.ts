...
const ZODE_PROVIDER_FILE = path.join(ROOT, "src/ZodeProvider.ts")
...
describe("Extension — package.json command sync", () => {
  ...
  it("every contributes.commands entry has a registerCommand() call", () => {
    ...
    const dynamic: Record<string, string> = {
      "zode-code.new.agentManager.jumpTo": "registerCommand(`zode-code.new.agentManager.jumpTo${",
    }
  ...
  /**
   * All declared commands must use the zode-code.new. prefix.
   * The legacy zode-code.* namespace (without .new.) belongs to the old
   * extension and must not be reintroduced.
   */
  it("all declared commands use the zode-code.new. prefix", () => {
    const bad = declared.filter((cmd) => !cmd.startsWith("zode-code.new."))
    expect(
      bad,
      `Commands without "zode-code.new." prefix — use the namespaced form:\n` + bad.map((b) => `  - ${b}`).join("\n"),
    ).toEqual([])
  })
})

// ---------------------------------------------------------------------------
// ZodeProvider handler wiring — every new ZodeProvider() must get
// setContinueInWorktreeHandler() called before resolving its webview.
//
// Regression: tab panels created via openZodeInNewTab() and the TabPanel
...
describe("Extension — ZodeProvider handler wiring", () => {
  const ext = fs.readFileSync(EXTENSION_FILE, "utf-8")

  /**
   * Every `new ZodeProvider(` in extension.ts must be followed (before the
   * next `new ZodeProvider(`) by a `setContinueInWorktreeHandler` call.
   * This prevents future tab/panel additions from silently missing the handler.
   */
  it("every ZodeProvider instance gets setContinueInWorktreeHandler wired", () => {
    const pattern = /new ZodeProvider\(/g
    ...
    expect(
      instances.length,
      "expected at least 3 ZodeProvider instances (sidebar, tab, deserializer)",
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      if (!region.includes("setContinueInWorktreeHandler")) {
        ...
        missing.push(`ZodeProvider at line ${line}`)
      }
    }

    expect(
      missing,
      `These ZodeProvider instances are missing setContinueInWorktreeHandler.\n` +
  ...
  it("openZodeInNewTab wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const fn = ext.indexOf("function openZodeInNewTab")
    expect(fn, "openZodeInNewTab must exist").toBeGreaterThan(-1)
  ...
  it("TabPanel deserializer wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const serializer = ext.indexOf('"zode-code.new.TabPanel"')
...
// ---------------------------------------------------------------------------
// ZodeProvider — continueInWorktree error fallback
//
...
describe("ZodeProvider — continueInWorktree error fallback", () => {
  const helper = fs.readFileSync(path.join(ROOT, "src/zode-provider/continue-worktree.ts"), "utf-8")
