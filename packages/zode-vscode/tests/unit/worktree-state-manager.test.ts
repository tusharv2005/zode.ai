...
describe("WorktreeStateManager", () => {
  ...
  beforeEach(() => {
    ...
    // Pre-create .zode dir so fire-and-forget saves don't race on mkdir
    fs.mkdirSync(path.join(root, ".zode"), { recursive: true })
    logs.length = 0
  ...
  describe("session CRUD", () => {
    ...
    it("drops obsolete session prefs while loading state", async () => {
      const file = path.join(root, ".zode", "agent-manager.json")
      fs.writeFileSync(
        file,
        JSON.stringify({
          worktrees: {},
  ...
  describe("persistence", () => {
    ...
    it("creates .zode directory if missing", async () => {
      ...
      expect(fs.existsSync(path.join(fresh, ".zode", "agent-manager.json"))).toBe(true)
    })
    ...
    it("backs up a corrupt state file before recovery saves", async () => {
      ...
      const files = fs.readdirSync(path.join(root, ".zode"))
      expect(result.status).toBe("failed")
  ...
  describe("tab order", () => {
    ...
    it("does not persist empty tab order", async () => {
      ...
      const content = fs.readFileSync(path.join(root, ".zode", "agent-manager.json"), "utf-8")
  ...
  describe("validate", () => {
    ...
    it("resolves relative paths against root", async () => {
      const relative = ".zode/worktrees/test-branch"
  ...
  describe("legacy .zodecode migration", () => {
    it("migrates and loads state from .zodecode when .zode is absent", async () => {
      // Remove the .zode dir created in beforeEach
      fs.rmSync(path.join(root, ".zode"), { recursive: true, force: true })

      // Write state to legacy .zodecode dir (migration will move it to .zode)
      const legacyDir = path.join(root, ".zodecode")
    ...
    it("skips migration when .zode state already exists", async () => {
      // .zode state already present — migration should skip agent-manager.json
      fs.writeFileSync(
        path.join(root, ".zode", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-new": {
              branch: "new-branch",
    ...
    it("rewrites stale .zodecode paths in worktree entries (unix)", async () => {
      fs.writeFileSync(
        path.join(root, ".zode", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-stale": {
              branch: "fix",
              path: "/repo/.zodecode/worktrees/fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe(`/repo/.zode/worktrees/fix`)
    })

    it("rewrites stale .zodecode paths with backslashes (windows)", async () => {
      fs.writeFileSync(
        path.join(root, ".zode", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
      ...
      expect(manager.getWorktrees()[0].path).toBe("C:\\.zode\\worktrees\\fix")
    })
  })
})
