...
import { globalFiles, localFiles } from "../../src/zode-provider/config-file"
import { openConfig } from "../../src/zode-provider/open-config"
...
const labels = {
  ...
  sourceHomeZode: "Home Zode",
  sourceHomeZodecode: "Home Zodecode",
  sourceHomeOpencode: "Home Opencode",
  sourceProjectZode: "Project Zode",
  sourceProjectZodecode: "Project Zodecode",
...
async function temp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "zode-config-"))
  dirs.push(dir)
  return dir
}
...
describe("config file discovery", () => {
  it("discovers global, env, legacy, and virtual config sources", async () => {
    ...
    process.env.ZODE_CONFIG_DIR = extra
    process.env.ZODE_CONFIG_CONTENT = "{}"
    await file(path.join(xdg, "zode", "zode.json"))
    await file(path.join(home, ".zodecode", "opencode.json"))
    await file(path.join(home, ".opencode", "zode.jsonc"))
    await file(envfile)
    ...
    expect(sources).toContain("sourceHomeZodecode")
  ...
  it("marks project files unloaded when project config is disabled", async () => {
    ...
    await file(path.join(root, "zode.json"))
    ...
    expect(list.find((item) => item.recommended)?.file).toBe(path.join(root, ".zode", "zode.jsonc"))
  })
})
...
describe("openConfig", () => {
  ...
  it("opens the only editable config without showing the picker", async () => {
    reset()
    const root = await temp()
    const cfg = path.join(root, ".zode", "zode.jsonc")
    await file(cfg)
