 ...
 * 2. Every WebviewMessage type handled in ZodeProvider has a corresponding member in the WebviewMessage union
...
const ZODE_PROVIDER_UTILS_FILE = path.join(ROOT, "src/zode-provider-utils.ts")
...
describe("ZodeProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in ZodeProvider exist in WebviewMessage union", () => {
    const providerContent = readFile(ZODE_PROVIDER_FILE)
    ...
    expect(
      unrecognized,
      `ZodeProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
...
describe("mapSSEEventToWebviewMessage output types", () => {
  it("all output types from mapSSEEventToWebviewMessage exist in ExtensionMessage", () => {
    const utilsContent = readFile(ZODE_PROVIDER_UTILS_FILE)
    const messagesContent = readMessageTypeSources()
