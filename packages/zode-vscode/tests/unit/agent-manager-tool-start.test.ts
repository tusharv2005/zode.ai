...
function result(path: string): CreateWorktreeResult {
  return { path, branch: "zode/test", parentBranch: "main", startPointSource: "fallback" } as CreateWorktreeResult
}
...
function deps(overrides: Partial<ToolDeps> = {}): ToolDeps {
  ...
  return {
    ...
    createWorktree: mock(async () => ({ worktree: { id: "wt-1" }, result: result("/repo/.zode/worktrees/wt-1") })),
...
describe("agent manager tool start", () => {
  ...
  it("starts worktree sessions through existing hooks", async () => {
    ...
    expect(c.registerWorktreeSession).toHaveBeenCalledWith("s-wt", "/repo/.zode/worktrees/wt-1")
