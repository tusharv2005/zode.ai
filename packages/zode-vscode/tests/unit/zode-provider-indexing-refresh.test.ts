...
import type { Config } from "@zodecode/sdk/v2/client"

// vscode mock is provided by the shared preload (tests/setup/vscode-mock.ts)
const { ZodeProvider } = await import("../../src/ZodeProvider")
...
describe("ZodeProvider indexing refresh", () => {
  it("reloadAfterAuthChange fetches config first, then indexing status", async () => {
    const provider = new ZodeProvider({} as never, {} as never)
  ...
  it("handleUpdateConfig no longer eagerly fetches indexing status", async () => {
    ...
    const provider = new ZodeProvider({} as never, conn.service as never)
    const internal = provider as unknown as Internals
  ...
  it("fetchAndSendIndexingStatus uses current session directory header", async () => {
    const worktree = "/repo/.zode/.zodecode/worktrees/feature"
    ...
    try {
      const provider = new ZodeProvider(
        {} as never,
        {
          getClient: () => ({}) as never,
      ...
      const auth = Buffer.from("zode:secret").toString("base64")
      expect(headers.get("Authorization")).toBe(`Basic ${auth}`)
      expect(headers.get("x-zode-directory")).toBe(worktree)
    } finally {
