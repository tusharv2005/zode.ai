...
describe("isModelValid", () => {
  ...
  it("rejects a disconnected non-zode provider", () => {
    expect(isModelValid(providers, [], { providerID: "openai", modelID: "gpt-4o" })).toBe(false)
  })

  it("accepts zode models when present in the catalog", () => {
    expect(isModelValid(providers, [], { providerID: "zode", modelID: "zode-auto/free" })).toBe(true)
  })
