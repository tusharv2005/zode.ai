...
import { mapSSEEventToWebviewMessage } from "../../src/zode-provider-utils"
import { configFeatures } from "../../src/features"
import type { EventIndexingStatus, IndexingStatus } from "@zodecode/sdk/v2/client"
...
describe("indexing feature detection", () => {
  it("enables indexing settings when the indexing plugin is present", () => {
    expect(configFeatures({ plugin: ["zode-indexing"] }).indexing).toBe(true)
  })

  it("detects supported indexing plugin specifiers", () => {
    expect(configFeatures({ plugin: ["zode-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["zode-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@zodecode/zode-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@zodecode/zode-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/.opencode/plugin/zode-indexing.js"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/node_modules/@zodecode/zode-indexing/index.js"] }).indexing).toBe(
      true,
    )
  })

  it("ignores unrelated plugin lists", () => {
    expect(configFeatures({ plugin: ["@zodecode/zode-gateway"] }).indexing).toBe(false)
