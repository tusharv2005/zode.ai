...
} from "../../src/zode-provider/handlers/permission-handler"
...
describe("recoveryDirs", () => {
  ...
  it("returns workspace root plus each unique worktree directory", () => {
    const dirs = new Map([
      ["s1", "/workspace/.zode/worktrees/alpha"],
      ["s2", "/workspace/.zode/worktrees/beta"],
      ["s3", "/workspace/.zode/worktrees/alpha"],
    ])
    expect(recoveryDirs("/workspace", dirs)).toEqual([
      "/workspace",
      "/workspace/.zode/worktrees/alpha",
      "/workspace/.zode/worktrees/beta",
    ])
  })
})
...
describe("fetchAndSendPendingPermissions", () => {
  ...
  it("queries workspace root plus each unique worktree directory", async () => {
    ...
    expect(queries).toContain("/workspace/.zode/worktrees/alpha")
    expect(queries).toContain("/workspace/.zode/worktrees/beta")
  ...
  it("deduplicates directories", async () => {
    const dirs = new Map([
      ["s1", "/workspace/.zode/worktrees/alpha"],
      ["s2", "/workspace/.zode/worktrees/alpha"],
    ])
    const { fake, queries } = ctx({ tracked: ["s1", "s2"], dirs })
    await fetchAndSendPendingPermissions(fake)
    expect(queries.filter((d) => d === "/workspace/.zode/worktrees/alpha")).toHaveLength(1)
  })
