...
describe("Agent Manager CSS Prefix", () => {
  it("all class selectors should use am- prefix", () => {
    ...
    // - `zode-diff-theme` is the shared Pierre diff theme utility defined
    //   in webview-ui/src/styles/diff.css and reused across webviews.
    // - `css` is matched from `@import "./diff.css"` file extension, not a
    //   class selector.
    const host = new Set(["vscode-high-contrast", "vscode-high-contrast-light", "zode-diff-theme", "css"])
  ...
  it("all CSS custom properties should use am- prefix", () => {
    ...
    // Allow zode-ui design tokens, vscode theme variables, and third-party
    // library tokens (@pierre/diffs, zode-ui sticky-accordion) used as fallbacks
...
describe("Agent Manager Webview — non-git sessionsLoaded fix", () => {
  ...
  /**
   * Regression: when isGitRepo is false, the Zode server never sends a
...
// ---------------------------------------------------------------------------
// ZodeProvider — pendingSessionRefresh race condition fix
// ---------------------------------------------------------------------------

describe("ZodeProvider — pending session refresh on reconnect", () => {
  const provider = fs.readFileSync(ZODE_PROVIDER_FILE, "utf-8")
  const utils = fs.readFileSync(path.join(ROOT, "src/zode-provider-utils.ts"), "utf-8")
   ...
   * zode-provider-utils) sets it when httpClient is unavailable, and
  ...
  it("loadSessions sets pendingSessionRefresh when client is null", () => {
    ...
    expect(start, "loadSessions must exist in zode-provider-utils").toBeGreaterThan(-1)
...
//
// Regression: PR #7473 moved ZodeNotifications into MessageList. It calls
