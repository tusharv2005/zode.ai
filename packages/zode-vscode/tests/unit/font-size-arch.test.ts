 ...
 *
 * Zode's VS Code webviews use the `zode-code.new.fontSize` setting, not
 * VS Code editor font-size or raw pixel declarations. This keeps the Zode UI
 * independently scalable across sidebar, settings, Agent Manager, ZodeClaw,
 * diff viewers, code blocks, and shared zode-ui controls.
 */
...
describe("webview font-size architecture", () => {
  it("does not use raw font-size pixels or VS Code editor font-size variables in runtime webview UI", () => {
    ...
    expect(
      violations,
      `Use the Zode webview font-size tokens instead of raw pixels or VS Code editor font-size variables.\n` +
        `Preferred tokens: var(--font-size-base), var(--font-size-small), or var(--zode-font-size-N).\n\n` +
  ...
  it("injects and live-broadcasts the webview font-size setting to all webview providers", () => {
    ...
    expect(util, "buildWebviewHtml must define scaled Zode font tokens").toContain("--zode-font-size-")

    const missing = WATCHED_PROVIDERS.filter((file) => !fs.readFileSync(file, "utf-8").includes("watchFontSizeConfig"))
    expect(
      missing.map(rel),
      `Webview providers that stay open must broadcast fontSizeChanged when zode-code.new.fontSize changes.\n` +
