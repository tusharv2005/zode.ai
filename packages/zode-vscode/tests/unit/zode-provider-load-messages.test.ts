...
function makeProvider(client: ReturnType<typeof createClient>) {
  ...
  const provider = new ZodeProvider({} as never, connection as never)
...
describe("ZodeProvider.handleLoadMessages / focus mode freshness", () => {
...
describe("ZodeProvider.handleDeleteSession / background processes", () => {
...
describe("ZodeProvider.handleLoadMessages / slim payload", () => {
...
describe("ZodeProvider.loadMessages / sub-agent viewer full history", () => {
...
describe("ZodeProvider.handleLoadMessages / prepend into deleted session", () => {
