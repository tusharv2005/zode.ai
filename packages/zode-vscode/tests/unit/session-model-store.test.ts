...
const ZODE_AUTO: ModelSelection = { providerID: "zode", modelID: "zode-auto/free" }

const providers: Record<string, Provider> = {
  zode: makeProvider("zode", ["zode-auto/free"]),
...
function env(): ResolveEnv {
  return {
    providers,
    connected: ["zode", "anthropic", "openai"],
    fallback: ZODE_AUTO,
    getModeModel: () => null,
    getGlobalModel: () => null,
  }
...
describe("per-session model selection", () => {
  it("selecting a model in session A does not write per-mode globally", () => {
    ...
    expect(sessionB).toEqual(ZODE_AUTO)
  })
...
describe("per-mode model memory", () => {
  ...
  it("switching modes falls back to default after session override is cleared", () => {
    ...
    expect(getSelected(cleared, e, "session-a", "code")).toEqual(ZODE_AUTO)
  })
