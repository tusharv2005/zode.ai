...
describe("parseServerPort", () => {
  it("parses port from standard CLI startup message", () => {
    expect(parseServerPort("zode server listening on http://127.0.0.1:12345")).toBe(12345)
  })
  ...
  it("parses port when embedded in longer output", () => {
    const output = "[INFO] 2024-01-01 zode server listening on http://127.0.0.1:54321\n[INFO] ready"
...
describe("cli tree-sitter resources", () => {
  it("resolves resources next to the VS Code bundled CLI", () => {
    const root = "/Users/test/.vscode/extensions/zodecode.zode-code-7.2.50-darwin-arm64"
    const bin = `${root}/bin/zode`
    ...
    expect(resolveTreeSitterEnv(root)).toEqual({ ZODE_TREE_SITTER_WASM_DIR: `${root}/bin/tree-sitter` })
  })

  it("resolves resources next to a Windows packaged CLI", () => {
    const root = String.raw`C:\Users\test\.vscode\extensions\zodecode.zode-code-7.2.50-win32-x64`
    const bin = String.raw`${root}\bin\zode.exe`
    ...
    expect(resolveTreeSitterEnv(root)).toEqual({
      ZODE_TREE_SITTER_WASM_DIR: String.raw`${root}\bin\tree-sitter`,
    })
  })

  it("copies runtime and language WASMs with the packaged CLI binary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zode-vscode-tree-sitter-"))
    try {
      const source = path.join(root, "dist", "@zodecode", "cli-darwin-arm64", "bin", "zode")
      const target = path.join(root, "extension", "bin", "zode")
...
describe("toErrorMessage", () => {
  ...
  it("strips ANSI codes before matching Error:", () => {
    const ansiError = "\x1b[91m\x1b[1mError: \x1b[0mConfig file at /path/zode.json is not valid JSON(C):"
    const result = toErrorMessage("startup failed", [ansiError])
    expect(result.userMessage).toBe("Config file at /path/zode.json is not valid JSON(C):")
  })
  ...
  it("includes CLI path in userDetails when provided", () => {
    const result = toErrorMessage("startup failed", [], "/usr/local/bin/zode")
    expect(result.userDetails).toContain("CLI path: /usr/local/bin/zode")
  })
...
describe("server workspace helpers", () => {
  ...
  it("disables codebase indexing only when no workspace folder is open", () => {
    expect(resolveIndexingEnv(undefined)).toEqual({ ZODE_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
    expect(resolveIndexingEnv([])).toEqual({ ZODE_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
