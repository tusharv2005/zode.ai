...
import { mergeFileSearchResults } from "../../src/zode-provider/file-search-results"
...
describe("mergeFileSearchResults", () => {
  ...
  it("preserves camel-case scoring for acronym-style queries", () => {
    const result = mergeFileSearchResults({
      query: "amprov",
      backend: [
        "packages/zode-docs/pages/contributing/architecture/onboarding-improvements.md",
        "packages/zode-vscode/src/agent-manager/AgentManagerProvider.ts",
      ],
      open: new Set(),
    })
    expect(result[0]).toBe("packages/zode-vscode/src/agent-manager/AgentManagerProvider.ts")
  })
})
