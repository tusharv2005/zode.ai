/**
 * Runtime contract tests for zode-vscode's dependencies on @zodecode/zode-ui.
 *
 * These tests import the upstream UI modules directly and verify at runtime
 * that the exports zode-vscode depends on still exist with the expected shape.
 *
 * Because the upstream modules use SolidJS JSX (jsxImportSource: "solid-js"),
 * they must be loaded from within packages/zode-ui/ where bun picks up the
...
const ZODE_UI_DIR = path.join(MONOREPO_ROOT, "packages/zode-ui")
...
const ZODE_MESSAGE_PART_FILE = path.join(MONOREPO_ROOT, "packages/zode-ui/src/components/message-part.tsx")
...
/**
 * Tool names that zode-vscode overrides or uses directly.
 * Sources:
...
describe("ToolRegistry tool name contract (runtime)", () => {
  it("all tools used by zode-vscode are registered in ToolRegistry", () => {
...
describe("DataProvider contract (runtime)", () => {
  ...
  it("DataProvider accepts onOpenFile prop and exports OpenFileFn (source)", () => {
    // onOpenFile and OpenFileFn are `zodecode_change` additions — TypeScript types
  ...
  it("DataProvider accepts onOpenDiff prop and exports OpenDiffFn (source)", () => {
    // onOpenDiff and OpenDiffFn are `zodecode_change` additions — TypeScript types
    // erased at runtime, so we verify via source analysis
    const src = fs.readFileSync(DATA_CONTEXT_FILE, "utf-8")
...
describe("Assistant Markdown streaming contract (source)", () => {
  const src = fs.readFileSync(ZODE_MESSAGE_PART_FILE, "utf-8")
  const block =
    src.match(
...
describe("Collapsed deferred tool details contract (source)", () => {
  ...
  const message = fs.readFileSync(ZODE_MESSAGE_PART_FILE, "utf-8")
