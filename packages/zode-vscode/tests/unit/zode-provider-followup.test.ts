...
import type { Event, Session } from "@zodecode/sdk/v2/client"
...
describe("ZodeProvider follow-up sessions", () => {
  it("adopts pending follow-up sessions for single-session views", async () => {
    const service = connection()
    const provider = new ZodeProvider({} as never, service as never)
  ...
  it("calls onFollowupAdopted listeners with session and directory", async () => {
    ...
    internal.pendingFollowup = { dir: "/repo/.zode/worktrees/feat", time: Date.now() }
    service.emit(created({ id: "ses-wt", directory: "/repo/.zode/worktrees/feat" }))
    await Promise.resolve()

    expect(adopted).toEqual([{ id: "ses-wt", dir: "/repo/.zode/worktrees/feat" }])
  })
})
