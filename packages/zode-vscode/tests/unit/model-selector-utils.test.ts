...
describe("providerSortKey", () => {
  it("returns 0 for zode gateway", () => {
    expect(providerSortKey(ZODE_GATEWAY_ID)).toBe(0)
  })
  ...
  it("sorts providers correctly when used with sort", () => {
    const ids = ["google", "anthropic", "zode", "openai", "deepseek"]
    const sorted = ids.slice().sort((a, b) => providerSortKey(a) - providerSortKey(b))
    expect(sorted).toEqual(["zode", "anthropic", "deepseek", "openai", "google"])
  })
})
...
describe("stripSubProviderPrefix", () => {
  ...
  it("does not strip 'Zode: ' prefix", () => {
    expect(stripSubProviderPrefix("Zode: Auto")).toBe("Zode: Auto")
    expect(stripSubProviderPrefix("zode: Auto")).toBe("zode: Auto")
  })
})
...
describe("sanitizeName", () => {
  ...
  it("preserves bare trailing Free in names like 'Zode Auto Free'", () => {
    expect(sanitizeName("Zode Auto Free")).toBe("Zode Auto Free")
...
describe("isDataCollectedModel", () => {
  it("only marks free Zode Gateway models with the training disclosure", () => {
    expect(isDataCollectedModel({ providerID: ZODE_GATEWAY_ID, isFree: true })).toBe(true)
    expect(isDataCollectedModel({ providerID: "openrouter", isFree: true })).toBe(false)
    expect(isDataCollectedModel({ providerID: ZODE_GATEWAY_ID, isFree: false })).toBe(false)
  })
})

describe("buildTriggerLabel", () => {
  it("returns resolved model name for non-zode provider unchanged", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", undefined, null, false, "", true, labels)).toBe("GPT-4o")
  })

  it("strips sub-provider prefix from resolved name for zode gateway models", () => {
    expect(
      buildTriggerLabel("Anthropic: Claude Sonnet", ZODE_GATEWAY_ID, undefined, null, false, "", true, labels),
    ).toBe("Claude Sonnet")
  })

  it("does not strip prefix for non-zode provider even if name contains ': '", () => {
  ...
  it("returns providerName / resolvedName for non-zode provider with providerName", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", "OpenAI", null, false, "", true, labels)).toBe("OpenAI / GPT-4o")
  })

  it("returns modelID for zode gateway raw selection", () => {
    const raw = { providerID: "zode", modelID: "zode-auto/frontier" }
    expect(buildTriggerLabel(undefined, undefined, undefined, raw, false, "", true, labels)).toBe("zode-auto/frontier")
  })

  it("returns providerID / modelID for non-zode raw selection", () => {
