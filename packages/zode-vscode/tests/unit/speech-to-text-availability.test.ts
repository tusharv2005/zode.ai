...
describe("speech-to-text availability", () => {
  const providers = ["zode"]
  const profile = {}

  it("shows speech input by default when Zode access exists", () => {
    expect(canUseSpeechToText({}, providers, profile)).toBe(true)
  })

  it("hides speech input without a signed-in, enabled Zode provider", () => {
    expect(canUseSpeechToText({}, [], profile)).toBe(false)
    expect(canUseSpeechToText({}, providers, null)).toBe(false)
    expect(canUseSpeechToText({ disabled_providers: ["zode"] }, providers, profile)).toBe(false)
  })
