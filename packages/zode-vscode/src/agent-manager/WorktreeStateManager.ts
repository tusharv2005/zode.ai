 ...
 *
 * Persists to `.zode/agent-manager.json`. Decouples worktrees from sessions
...
import { ZODE_DIR, migrateAgentManagerData, type MigrationResult } from "./constants"
...
export class WorktreeStateManager {
  ...
  constructor(root: string, log: (msg: string) => void) {
    this.root = root
    this.file = path.join(root, ZODE_DIR, STATE_FILE)
    this.log = log
  }
  ...
  async load(): Promise<StateLoadResult> {
    // Migrate Agent Manager data from .zodecode → .zode before first read
  ...
  private apply(content: string): void {
    ...
    for (const [id, wt] of Object.entries(data.worktrees ?? {})) {
      // Rewrite stale .zodecode paths while preserving the separator style already stored.
      const fixed =
        wt.path?.replace(/([/\\])\.zodecode([/\\])/g, (_match, leadingSep, trailingSep) => {
          return `${leadingSep}.zode${trailingSep}`
        }) ?? wt.path
