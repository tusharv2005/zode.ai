 ...
 *
 * Ported from zodecode/src/core/zodecode/agent-manager/WorktreeManager.ts.
 * Handles creation, discovery, and cleanup of worktrees stored in
 * {projectRoot}/.zode/worktrees/
 */
...
import { type GitOps, isZodeOwnedSshCommand, nonInteractiveEnv } from "./GitOps"
...
import { ZODE_DIR, LEGACY_DIR, migrateAgentManagerData } from "./constants"

const SESSION_ID_FILE = "session-id"
const METADATA_FILE = "metadata.json"
const GIT_METADATA_FILE = "zode-agent-manager-metadata.json"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps) {
    this.root = root
    this.dir = path.join(root, ZODE_DIR, "worktrees")
    this.git = simpleGit(root)
    this.ops = ops
    this.log = log
  }

  /** Run once before first read/write to migrate Agent Manager data from .zodecode → .zode. */
  ...
  private async removeWorktreeImpl(worktreePath: string, branch?: string): Promise<void> {
    ...
    const temp = path.join(path.dirname(worktreePath), `.zode-delete-${randomUUID()}`)
    try {
  ...
  /** Remove orphaned .zode-delete-* temp dirs left by interrupted deletions. */
  ...
  async readMetadata(
    ...
    // Check .zode/ first, then legacy .zodecode/
    for (const dirName of [ZODE_DIR, LEGACY_DIR]) {
  ...
  async ensureGitExclude(): Promise<void> {
    ...
    const items = [
      [".zode/worktrees/", "Zode Code agent worktrees"],
      [".zode/agent-manager.json", "Zode Agent Manager state"],
      [".zode/setup-script", "Zode Code worktree setup script"],
      [".zode/setup-script.sh", "Zode Code worktree setup script"],
      [".zode/setup-script.ps1", "Zode Code worktree setup script"],
      [".zode/setup-script.cmd", "Zode Code worktree setup script"],
      [".zode/setup-script.bat", "Zode Code worktree setup script"],
      [".zodecode/worktrees/", "Zode Code legacy agent worktrees"],
      [".zodecode/agent-manager.json", "Zode Agent Manager legacy state"],
      [".zodecode/setup-script", "Zode Code legacy worktree setup script"],
      [".zodecode/setup-script.sh", "Zode Code legacy worktree setup script"],
      [".zodecode/setup-script.ps1", "Zode Code legacy worktree setup script"],
      [".zodecode/setup-script.cmd", "Zode Code legacy worktree setup script"],
      [".zodecode/setup-script.bat", "Zode Code legacy worktree setup script"],
    ] as const
  ...
  async resolveStartPoint(
    ...
    if (remote) {
      ...
      try {
        ...
        // is the fixed value Zode injects — never for an inherited one, which
        // could be attacker-controlled.
        const env = nonInteractiveEnv()
        await simpleGit(this.root, { unsafe: { allowUnsafeSshCommand: isZodeOwnedSshCommand(env) } })
          .env(env)
