...
// TODO: Remove the legacy .zodecode -> .zode migration helpers below after the
// GA release cleanup tracked in https://github.com/Zode-Org/zodecode/issues/6986.
...
/** Zode config directory name (project-level and inside worktrees). */
export const ZODE_DIR = ".zode"

/** Legacy config directory name for backward compatibility reads. */
export const LEGACY_DIR = ".zodecode"

/** Agent Manager files that should be migrated from .zodecode/ to .zode/. */
...
export interface MigrationResult {
  /** Number of git worktree refs that were rewritten from .zodecode → .zode. */
  refsFixed: number
}

/**
 * Migrate Agent Manager data from .zodecode/ to .zode/.
 *
 * Moves individual Agent Manager files/directories (worktrees, state,
 * setup scripts) from the legacy .zodecode/ into .zode/. Skips items
 * that already exist in .zode/ (the new location wins). This is safe
 * because Agent Manager exclusively owns these files.
 *
 * Fixes git worktree internal references (.git/worktrees/{name}/gitdir)
 * whenever .zode/worktrees/ exists so partially migrated repos recover too.
 *
...
export async function migrateAgentManagerData(root: string, log: (msg: string) => void): Promise<MigrationResult> {
  ...
  const target = path.join(root, ZODE_DIR)
  ...
  if (await isDirectory(legacy)) {
    ...
    for (const item of AGENT_MANAGER_ITEMS) {
      ...
      if (await exists(dst)) {
        log(`Skipping ${item}: already exists in ${ZODE_DIR}`)
        continue
      }

      try {
        await fs.promises.rename(src, dst)
        log(`Migrated ${item} from ${LEGACY_DIR} to ${ZODE_DIR}`)
      } catch (err) {
...
/**
 * After moving worktrees from .zodecode/ to .zode/, fix git internal refs.
 *
 ...
 * gitdir files that reference the old .zodecode path.
 *
...
async function fixGitWorktreeRefs(root: string, log: (msg: string) => void): Promise<number> {
  ...
  const newSegment = path.join(root, ZODE_DIR) + path.sep
  let fixed = 0
