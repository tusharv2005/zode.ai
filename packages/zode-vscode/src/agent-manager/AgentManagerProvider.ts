 ...
 * sessions are stored in `.zode/agent-manager.json`. The UI shows two
...
export class AgentManagerProvider implements Disposable {
  public static readonly viewType = "zode-code.new.AgentManagerPanel"
  ...
  constructor(
    ...
    this.outputChannel = host.createOutput("Zode Agent Manager")
    ...
    this.unsubTool = this.connectionService.onEventFiltered(
      (event) => (event as { type?: string }).type === "zodecode.agent_manager.start",
  ...
  private async initializeState(): Promise<void> {
    ...
    // When the .zodecode → .zode migration rewrote git worktree refs, nudge
  ...
  private openWorktreeFile(sessionId: string, filePath: string, line?: number, column?: number): void {
    ...
    try {
      ...
      console.error("[Zode New] AgentManagerProvider: Cannot resolve file path:", err)
      return
    }
   ...
   * Called from ZodeProvider when the sidebar sends "continueInWorktree".
   */
