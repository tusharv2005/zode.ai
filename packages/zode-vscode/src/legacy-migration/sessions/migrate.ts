...
export async function migrate(
  ...
  try {
    ...
    const project = await client.zodecode.sessionImport.project(payload.project, { throwOnError: true })
    const projectID = project.data?.id ?? payload.project.id
    const session = await client.zodecode.sessionImport.session(
      {
        ...payload.session,
        projectID,
    ...
    for (const msg of payload.messages) {
      await client.zodecode.sessionImport.message(msg, { throwOnError: true })
    }

    for (const part of payload.parts) {
      await client.zodecode.sessionImport.part(part, { throwOnError: true })
    }
