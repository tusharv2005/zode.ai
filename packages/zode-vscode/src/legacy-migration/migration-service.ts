 ...
 *
 * Reads legacy Zode Code v5.x data from VS Code SecretStorage and the extension's
...
const MIGRATION_STATUS_KEY = "zode.legacyMigrationStatus"
...
export async function clearLegacyData(context: vscode.ExtensionContext): Promise<void> {
  ...
  const legacyStateKeys = [
    "zode-code.allowedCommands",
    "zode-code.deniedCommands",
    "zode-code.autoApprovalEnabled",
    "zode-code.fuzzyMatchThreshold",
    "zode-code.diffEnabled",
    "zode-code.language",
    "zode-code.customModes",
    "zode-code.firstInstallCompleted",
    "zode-code.telemetrySetting",
  ...
  // Clear legacy VS Code settings registered under the "zode-code" configuration scope.
  ...
  const cfg = vscode.workspace.getConfiguration("zode-code")
...
async function migrateProvider(
  ...
  // The profile endpoint requires type:"oauth". The legacy extension stored the same Zode
  ...
  // For providers that support an organization ID (e.g. Zode Gateway), migrate using OAuth
...
async function migrateDefaultModel(settings: LegacyProviderSettings, client: ZodeClient): Promise<MigrationResultItem> {
  const provider = settings.apiProvider
  if (!provider) {
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("zode-code.new.autocomplete")
...
function readLegacySettings(context: vscode.ExtensionContext): LegacySettings {
  ...
  return {
    autoApprovalEnabled: context.globalState.get<boolean>("zode-code.autoApprovalEnabled"),
    allowedCommands: context.globalState.get<string[]>("zode-code.allowedCommands"),
    deniedCommands: context.globalState.get<string[]>("zode-code.deniedCommands"),
    ...
    language: context.globalState.get<string>("zode-code.language"),
