/**
 * Permission handlers — extracted from ZodeProvider.
 *
...
import type { ZodeClient, PermissionRequest } from "@zodecode/sdk/v2/client"
...
export async function handlePermissionResponse(
  ...
  if (!target) {
    console.error("[Zode New] ZodeProvider: No sessionID for permission response")
    ctx.postMessage({ type: "permissionError", permissionID: permissionId })
    return
  }
  ...
  if (approvedAlways.length > 0 || deniedAlways.length > 0) {
    const saveResult = await ctx.client.permission
      ...
      .catch((error: unknown) => {
        ...
        console.error("[Zode New] ZodeProvider: Failed to save always-rules:", error)
        ctx.postMessage({ type: "permissionError", permissionID: permissionId })
  ...
  const replyResult = await ctx.client.permission
    ...
    .catch((error: unknown) => {
      if (isNotFoundError(error)) return "stale" as const
      console.error("[Zode New] ZodeProvider: Failed to respond to permission:", error)
      ctx.postMessage({ type: "permissionError", permissionID: permissionId })
      return "error" as const
    })
...
export async function fetchAndSendPendingPermissions(ctx: PermissionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Zode New] ZodeProvider: Failed to fetch pending permissions:", error)
  }
}
