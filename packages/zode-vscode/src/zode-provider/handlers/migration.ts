/**
 * Legacy migration handlers — extracted from ZodeProvider.
 *
 * Manages the migration wizard for users upgrading from Zode Code v5.x.
...
export async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[Zode New] ZodeProvider: 🔄 Legacy data detected, showing migration wizard")
  ctx.postMessage({
    type: "migrationState",
    needed: true,
    data: {
...
export async function handleStartLegacyMigration(
  ...
  try {
    ...
    console.error("[Zode New] ZodeProvider: ❌ Migration failed", error)
    ctx.postMessage({
