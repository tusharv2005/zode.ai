/**
 * Question handlers — extracted from ZodeProvider.
 *
...
export async function fetchAndSendPendingQuestions(ctx: QuestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Zode New] ZodeProvider: Failed to fetch pending questions:", error)
  }
}
...
export async function handleQuestionReply(
  ...
  try {
    ...
    console.error("[Zode New] ZodeProvider: Failed to reply to question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
...
export async function handleQuestionReject(
  ...
  try {
    ...
    console.error("[Zode New] ZodeProvider: Failed to reject question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
