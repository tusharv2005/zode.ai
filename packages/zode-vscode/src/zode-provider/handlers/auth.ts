/**
 * Authentication handlers — extracted from ZodeProvider.
 *
...
import { getErrorMessage } from "../../zode-provider-utils"

export interface AuthContext {
  readonly client: ZodeClient | null
...
export async function handleLogin(ctx: AuthContext, attempt: number, getAttempt: () => number): Promise<void> {
  ...
  console.log("[Zode New] ZodeProvider: 🔐 Starting login flow...")
  ...
  try {
    ...
    const { data: auth } = await ctx.client.provider.oauth.authorize(
      { providerID: "zode", method: 0, directory: dir },
      { throwOnError: true },
    )
    console.log("[Zode New] ZodeProvider: 🔐 Got auth URL:", auth.url)
    ...
    await ctx.client.provider.oauth.callback({ providerID: "zode", method: 0, directory: dir }, { throwOnError: true })

    // Check if this attempt was cancelled
    if (attempt !== getAttempt()) return

    console.log("[Zode New] ZodeProvider: 🔐 Login successful")

    await ctx.disposeGlobal()

    // Step 3: Fetch profile and push to webview
    const { data: profile } = await ctx.client.zode.profile(undefined, { throwOnError: true })
...
export async function handleLogout(ctx: AuthContext): Promise<void> {
  ...
  try {
    console.log("[Zode New] ZodeProvider: 🚪 Logging out...")
    await ctx.client.auth.remove({ providerID: "zode" }, { throwOnError: true })
    console.log("[Zode New] ZodeProvider: 🚪 Logged out successfully")
    ...
    console.error("[Zode New] ZodeProvider: ❌ Logout failed:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleSetOrganization(ctx: AuthContext, organizationId: string | null): Promise<void> {
  ...
  console.log("[Zode New] ZodeProvider: Switching organization:", organizationId ?? "personal")
  try {
    await ctx.client.zode.organization.set({ organizationId }, { throwOnError: true })
  } catch (error) {
    console.error("[Zode New] ZodeProvider: Failed to switch organization:", error)
    // Re-fetch current profile to reset webview state — best-effort
    try {
      const result = await ctx.client.zode.profile()
      ctx.postMessage({ type: "profileData", data: result.data ?? null })
    } catch (profileError) {
      console.error("[Zode New] ZodeProvider: Failed to refresh profile after org switch error:", profileError)
    }
    return
  }
  ...
  try {
    ...
    console.error("[Zode New] ZodeProvider: Failed to refresh agents after org switch:", error)
  }
}
...
export async function handleRefreshProfile(ctx: AuthContext): Promise<void> {
  ...
  console.log("[Zode New] ZodeProvider: 🔄 Refreshing profile...")
  const result = await ctx.client.zode.profile().catch(() => ({ data: null }))
  ctx.postMessage({ type: "profileData", data: result.data ?? null })
}
