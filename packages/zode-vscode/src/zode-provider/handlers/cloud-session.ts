/**
 * Cloud session handlers — extracted from ZodeProvider.
 *
...
import type { ZodeClient, Session, TextPartInput, FilePartInput } from "@zodecode/sdk/v2/client"
import type { CloudSessionData, EditorContext } from "../../services/cli-backend/types"
import { getErrorMessage, sessionToWebview, mapCloudSessionMessageToWebviewMessage } from "../../zode-provider-utils"
...
export async function handleRequestCloudSessions(
  ...
  try {
    const result = await ctx.client.zode.cloudSessions({
    ...
    console.error("[Zode New] ZodeProvider: Failed to fetch cloud sessions:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleRequestCloudSessionData(ctx: CloudSessionContext, sessionId: string): Promise<void> {
  ...
  try {
    const result = await ctx.client.zode.cloud.session.get({ id: sessionId }, { signal: AbortSignal.timeout(TIMEOUT) })
    ...
    console.error("[Zode New] Failed to load cloud session data:", err)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId: sessionId,
...
export async function handleImportAndSend(
  ...
  try {
    const result = await ctx.client.zode.cloud.session.import(
      {
    ...
    console.error("[Zode New] ZodeProvider: ❌ Cloud session import failed:", error)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId,
  ...
  try {
    ...
    console.error("[Zode New] Failed to send message after cloud import:", err)
    ctx.postMessage({
