import type { Session, SessionStatus } from "@zodecode/sdk/v2/client"
...
export async function handleForkSession(ctx: ForkContext, sessionId: string, messageId?: string): Promise<void> {
  const status =
    ...
    (await Promise.resolve()
      ...
      .catch((e) => {
        console.error("[Zode New] refreshForkStatus failed:", e)
  ...
  await forkSession(
    {
      ...
      registerSession: () => {},
      log: (...args) => console.log("[Zode New] ZodeProvider:", ...args),
    },
    sessionId,
    undefined,
    messageId,
