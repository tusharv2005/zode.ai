...
function previewImage(dir: vscode.Uri | undefined, dataUrl: string, filename: string): void {
        ...
        return Promise.all(
          stale.map((name) =>
            Promise.resolve(vscode.workspace.fs.delete(vscode.Uri.joinPath(root, name), { recursive: true })).then(
              undefined,
              (err: unknown) => {
                console.warn("[Zode New] ZodeProvider: Failed to delete stale preview:", err)
              },
            ),
          ),
        )
  ...
  void vscode.workspace.fs
    ...
    .then(() => clean())
    .then(open, (err) => console.error("[Zode New] ZodeProvider: Failed to preview image:", err))
}
...
function openContent(content: string, language?: string): void {
  vscode.workspace.openTextDocument({ content, language: language || "log" }).then(
    ...
    (err) => console.error("[Zode New] ZodeProvider: Failed to open content:", err),
  )
}
  ...
  vscode.workspace.fs.stat(uri).then(
    (stat) => {
      ...
      vscode.workspace.openTextDocument(uri).then(
        ...
        (err) => console.error("[Zode New] ZodeProvider: Failed to open file:", uri.fsPath, err),
      )
    },
    (err) => console.error("[Zode New] ZodeProvider: Path does not exist:", uri.fsPath, err),
  )
}
