...
import { ZodeProvider } from "./ZodeProvider"
import { AgentManagerProvider } from "./agent-manager/AgentManagerProvider"
import { VscodeHost } from "./agent-manager/vscode-host"
import { ZodeClawProvider } from "./zodeclaw/ZodeClawProvider"
...
import { ZodeConnectionService } from "./services/cli-backend"
...
import { registerCodeActions, registerTerminalActions, ZodeCodeActionProvider } from "./services/code-actions"
...
const RESTORE_KEY = "zode.workbench.restore"
...
// requiring the user to open a Zode sidebar or panel first. The CLI backend is NOT spawned here;
// it starts lazily when a webview connects or when ensureBackendForAutocomplete() triggers it.
export function activate(context: vscode.ExtensionContext) {
  console.log("Zode Code extension is now active")
  shuttingDown = false

  const telemetry = TelemetryProxy.getInstance()

  // Create shared connection service (one server for all webviews)
  const connectionService = new ZodeConnectionService(context)
  ...
  const unsubscribeStateChange = connectionService.onStateChange((state) => {
    if (state === "connected") {
      ...
      if (config) {
        ...
        // CLI reads ZODE_TELEMETRY_LEVEL once at spawn, so without this call
      ...
      try {
        ...
        console.log("[Zode New] CLI connected, calling remoteService.refresh()")
        remoteService.refresh().catch((err) => console.warn("[Zode New] initial remote refresh failed:", err))
      } catch {
  ...
  for (const folder of vscode.workspace.workspaceFolders ?? []) {
    void markWorkspace(folder.uri.fsPath, (msg) => console.warn(`[Zode New] ${msg}`))
  }
  ...
  const tabPanels = new Map<vscode.WebviewPanel, ZodeProvider>()
  ...
  const provider = new ZodeProvider(context.extensionUri, connectionService, context)
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(ZodeProvider.viewType, provider, {
  ...
  // commands by default.
  const skip = ["zode-code.new.agentManagerOpen", "zode-code.new.agentManager.showTerminal"]
  if (process.platform === "darwin") skip.push("zode-code.new.agentManager.runScript")
  ensureCommandsSkipShell(skip)

  // Create ZodeClaw chat provider for editor panel
  const zodeClawProvider = new ZodeClawProvider(context.extensionUri, connectionService)
  context.subscriptions.push(zodeClawProvider)
  ...
  // Register serializer so ZodeClaw panel restores when VS Code restarts
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer(ZodeClawProvider.viewType, {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        zodeClawProvider.restorePanel(panel)
        return Promise.resolve()
      },
    }),
  )
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("zode-code.new.TabPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        const tabProvider = new ZodeProvider(context.extensionUri, connectionService, context, {
        ...
        panel.onDidDispose(
          () => {
            console.log("[Zode New] Tab panel restored from restart disposed")
  ...
  for (const suffix of settingsViews) {
    context.subscriptions.push(
      vscode.window.registerWebviewPanelSerializer(`zode-code.new.${suffix}`, {
        deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("zode-code.new.SubAgentViewerPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("zode-code.new.sidebarTitle.plusButtonClicked", () => {
      track("new_task", "zode-code.new.plusButtonClicked")
    }),
    vscode.commands.registerCommand("zode-code.new.sidebarTitle.historyButtonClicked", () => {
      track("history", "zode-code.new.historyButtonClicked")
    }),
    vscode.commands.registerCommand("zode-code.new.sidebarTitle.agentManagerOpen", () => {
      track("agent_manager", "zode-code.new.agentManagerOpen")
    }),
    vscode.commands.registerCommand("zode-code.new.sidebarTitle.zodeClawOpen", () => {
      track("zodeclaw", "zode-code.new.zodeClawOpen")
    }),
    vscode.commands.registerCommand("zode-code.new.sidebarTitle.marketplaceButtonClicked", () => {
      track("marketplace", "zode-code.new.marketplaceButtonClicked")
    }),
    vscode.commands.registerCommand("zode-code.new.sidebarTitle.profileButtonClicked", () => {
      track("profile", "zode-code.new.profileButtonClicked")
    }),
    vscode.commands.registerCommand("zode-code.new.sidebarTitle.settingsButtonClicked", () => {
      track("settings", "zode-code.new.settingsButtonClicked")
    }),
    vscode.commands.registerCommand("zode-code.new.plusButtonClicked", () => {
    ...
    vscode.commands.registerCommand("zode-code.new.agentManagerOpen", () => {
      agentManagerProvider.openPanel()
    }),
    vscode.commands.registerCommand("zode-code.new.marketplaceButtonClicked", (directory?: string | null) => {
      marketplacePanelProvider.openPanel(directory)
    }),
    vscode.commands.registerCommand("zode-code.new.zodeClawOpen", () => {
      zodeClawProvider.openPanel()
    }),
    vscode.commands.registerCommand("zode-code.new.historyButtonClicked", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("zode-code.new.cycleAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("zode-code.new.cyclePreviousAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("zode-code.new.profileButtonClicked", () => {
      settingsEditorProvider.openPanel("profile")
    }),
    vscode.commands.registerCommand("zode-code.new.settingsButtonClicked", (tab?: string) => {
      settingsEditorProvider.openPanel("settings", tab)
    }),
    vscode.commands.registerCommand("zode-code.new.openIndexingSettings", () => {
      settingsEditorProvider.openPanel("settings", "indexing")
    }),
    // legacy-migration start
    vscode.commands.registerCommand("zode-code.new.openMigrationWizard", () => {
      provider.postMessage({ type: "migrationState", needed: true })
    }),
    // legacy-migration end
    vscode.commands.registerCommand("zode-code.new.generateTerminalCommand", async () => {
    ...
    vscode.commands.registerCommand("zode-code.new.toggleRemote", () => {
      remoteService.toggle().catch((err) => console.error("[Zode New] toggleRemote command failed:", err))
    }),
    vscode.commands.registerCommand("zode-code.new.openInTab", () => {
      return openZodeInNewTab(
        context,
        connectionService,
    ...
    vscode.commands.registerCommand(
      "zode-code.new.showChanges",
      (arg?: { sessionId?: string; turnId?: string; initialSourceId?: string }) => {
        diffViewerProvider.openFromCommand(arg)
      },
    ),
    vscode.commands.registerCommand("zode-code.new.openSubAgentViewer", (sessionID: string, title?: string) => {
      subAgentViewerProvider.openPanel(sessionID, title)
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.previousSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionPrevious" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.nextSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionNext" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.previousTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabPrevious" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.nextTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabNext" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.showTerminal", () => {
    ...
    vscode.commands.registerCommand("zode-code.new.agentManager.runScript", () => {
      agentManagerProvider.postMessage({ type: "action", action: "runScript" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.toggleDiff", () => {
      agentManagerProvider.postMessage({ type: "action", action: "toggleDiff" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.showShortcuts", () => {
      agentManagerProvider.postMessage({ type: "action", action: "showShortcuts" })
    }),

    vscode.commands.registerCommand("zode-code.new.agentManager.newTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTab" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.newTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTerminal" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.closeTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeTab" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.newWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newWorktree" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.openWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openWorktree" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.closeWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeWorktree" })
    }),
    vscode.commands.registerCommand("zode-code.new.agentManager.advancedWorktree", () =>
      agentManagerProvider.openAdvancedWorktree(),
    ),
    ...Array.from({ length: 9 }, (_, i) =>
      vscode.commands.registerCommand(`zode-code.new.agentManager.jumpTo${i + 1}`, () => {
        agentManagerProvider.postMessage({ type: "action", action: `jumpTo${i + 1}` })
      }),
    ),
  )

  // Register URI handler for session imports (vscode://zodecode.zode-code/zodecode/s/{sessionId})
  context.subscriptions.push(
    vscode.window.registerUriHandler({
      async handleUri(uri: vscode.Uri) {
        const match = uri.path.match(/^\/zodecode\/s\/([a-zA-Z0-9_-]+)$/)
        if (!match) return
        const sessionId = match[1]
        if (!sessionId) return
        console.log("[Zode New] URI handler: opening cloud session:", sessionId)
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      { scheme: "file" },
      new ZodeCodeActionProvider(),
      ZodeCodeActionProvider.metadata,
    ),
  )
...
async function openZodeInNewTab(
  context: vscode.ExtensionContext,
  connectionService: ZodeConnectionService,
  agentManagerProvider: AgentManagerProvider,
  tabPanels: Map<vscode.WebviewPanel, ZodeProvider>,
  ...
  const panel = vscode.window.createWebviewPanel("zode-code.new.TabPanel", EXTENSION_DISPLAY_NAME, targetCol, {
    enableScripts: true,
  ...
  panel.iconPath = {
    light: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "zode-light.svg"),
    dark: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "zode-dark.svg"),
  }
  ...
  panel.onDidDispose(
    () => {
      console.log("[Zode New] Tab panel disposed")
      tabPanels.delete(panel)
      tabProvider.dispose()
    },
    null,
