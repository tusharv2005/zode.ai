...
import type { ZodeClient, Event } from "@zodecode/sdk/v2/client"
import type { ZodeConnectionService } from "../services/cli-backend/connection-service"
...
const CONFIG = "zode-code.new.autoApprove"
const KEY = "enabled"
...
export function registerToggleAutoApprove(
      ...
      try {
        ...
        for (const req of pending) {
          ...
          await client.permission.reply({ requestID: req.id, directory: dir, reply: "once" }).catch((err) => {
            console.error("[Zode New] toggleAutoApprove: failed to drain pending:", err)
          })
        }
      } catch (err) {
        console.error("[Zode New] toggleAutoApprove: failed to list pending permissions:", err)
      }
    }
  ...
  const unsubscribe = connectionService.onEvent((event: Event) => {
    ...
    client.permission.reply({ requestID: event.properties.id, directory: dir, reply: "once" }).catch((err) => {
      console.error("[Zode New] toggleAutoApprove: failed to auto-reply:", err)
    })
  })
  ...
  context.subscriptions.push(vscode.commands.registerCommand("zode-code.new.toggleAutoApprove", toggle))
...
function tryGetClient(connectionService: ZodeConnectionService): ZodeClient | undefined {
  try {
