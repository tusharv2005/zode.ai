...
export class DiffViewerProvider implements vscode.Disposable {
  public static readonly viewType = "zode-code.new.DiffViewerPanel"
  ...
  constructor(
    ...
    this.output = vscode.window.createOutputChannel("Zode Diff Panel")
  }
  ...
  /**
   * Entry point for the `zode-code.new.showChanges` command. Composes the
  ...
  private onWebviewReady(): void {
    ...
    void this.panel.webview.postMessage({
      ...
      languageOverride: vscode.workspace.getConfiguration("zode-code.new").get<string>("language"),
