...
const PANEL_TITLES: Record<PanelView, string> = {
  settings: "Zode Settings",
  profile: "Zode Profile",
 ...
 *
 * Uses a full ZodeProvider under the hood so each panel has
...
export class SettingsEditorProvider implements vscode.Disposable {
  ...
  private providers = new Map<PanelView, ZodeProvider>()
  ...
  /** Extract the PanelView from a viewType string like "zode-code.new.settingsPanel". */
  static viewFromType(type: string): PanelView | undefined {
    const match = type.match(/^zode-code\.new\.(\w+)Panel$/)
  ...
  private wirePanel(panel: vscode.WebviewPanel, view: PanelView, projectDirectory: string | null): void {
    ...
    // Create a dedicated ZodeProvider for this panel so it has full
    // backend connectivity (config, providers, agents, profile, auth).
    const provider = new ZodeProvider(this.extensionUri, this.connectionService, this.context, {
      projectDirectory,
    })
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "webviewReady") {
        // Small delay to let ZodeProvider's own webviewReady handler finish first
        setTimeout(() => {
    ...
    panel.onDidDispose(() => {
      console.log(`[Zode New] ${title} panel disposed`)
