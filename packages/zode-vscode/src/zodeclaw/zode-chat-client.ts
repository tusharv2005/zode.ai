/**
 * HTTP client for the zode-chat Cloudflare Worker.
 *
 * Minimal inline port of `@zodecode/zode-chat/client` (cloud monorepo) tailored
 ...
 * the zode-chat worker is the source of truth and validates at its edge.
 */
...
export type ZodeChatClientConfig = {
  baseUrl: string
  getToken: () => Promise<string>
  onUnauthorized?: () => void
}

export class ZodeChatApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: unknown,
  ) {
    super(`ZodeChat request failed: ${status}${formatBodyDetail(body)}`)
    this.name = "ZodeChatApiError"
  }
}
...
export class ZodeChatClient {
  ...
  private async request<T>(path: string, opts: HttpOpts = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new ZodeChatApiError(res.status, body)
    }
