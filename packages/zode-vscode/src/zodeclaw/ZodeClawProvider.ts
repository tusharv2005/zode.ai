/**
 * ZodeClaw panel provider for the VS Code extension.
 *
 * Owns the Zode Chat HTTP client + event-service WebSocket connection
...
import type { ZodeConnectionService } from "../services/cli-backend"
import type { ZodeClient } from "@zodecode/sdk/v2/client"
import { buildWebviewHtml } from "../utils"
import { watchFontSizeConfig } from "../zode-provider/font-size"
import { TokenManager } from "./token-manager"
import { ZodeChatApiError, ZodeChatClient } from "./zode-chat-client"
...
export class ZodeClawProvider implements vscode.Disposable {
  static readonly viewType = "zode-code.new.ZodeClawPanel"
  ...
  private chat: ZodeChatClient | null = null
  ...
  constructor(
    ...
    private readonly connection: ZodeConnectionService,
  ) {}
  ...
  openPanel(): void {
    ...
    const panel = vscode.window.createWebviewPanel(ZodeClawProvider.viewType, "ZodeClaw", vscode.ViewColumn.One, {
      enableScripts: true,
  ...
  private attach(panel: vscode.WebviewPanel): void {
    ...
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.uri, "assets", "icons", "zode-light.svg"),
      dark: vscode.Uri.joinPath(this.uri, "assets", "icons", "zode-dark.svg"),
    }
    ...
    panel.webview.html = buildWebviewHtml(panel.webview, {
      scriptUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "zodeclaw.js")),
      styleUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "zodeclaw.css")),
    ...
    const msgSub = panel.webview.onDidReceiveMessage((msg: ZodeClawInMessage) => this.onMessage(msg))
    ...
    const unsub = this.connection.onLanguageChanged((locale) => {
      this.post({ type: "zodeclaw.locale", locale })
    })
  ...
  private post(msg: ZodeClawOutMessage): void {
    this.panel?.webview.postMessage(msg)
  }

  private async onMessage(msg: ZodeClawInMessage): Promise<void> {
    switch (msg.type) {
      case "zodeclaw.ready":
        await this.init()
        return
      case "zodeclaw.openExternal": {
  ...
  private get locale(): string {
    const override = vscode.workspace.getConfiguration("zode-code.new").get<string>("language")
  ...
  private async init(): Promise<void> {
    ...
    try {
      this.post({ type: "zodeclaw.state", state: { phase: "loading", locale: this.locale } })
      ...
      this.post({ type: "zodeclaw.state", state })
      this.startPolling()
      this.startBotNudge()
    } finally {
   ...
   */
  private async bootstrap(client: ZodeClient, gen: number): Promise<boolean> {
    ...
    if (!this.sandboxId) {
      this.post({ type: "zodeclaw.state", state: { phase: "noInstance", locale: this.locale } })
      return false
    }
  ...
  private async resolveStatus(client: ZodeClient, gen: number): Promise<boolean> {
    const statusRes = await client.zode.claw.status().catch(() => null)
    if (this.stale(gen)) return false
  ...
  private async fetchChatToken(gen: number): Promise<ChatToken | null> {
    ...
    try {
      ...
      console.error("[Zode New] ZodeClaw chat token fetch failed:", message)
      // Token fetch typically fails when the instance hasn't been upgraded
      // to support zode-chat — surface that as the upgrade prompt.
      this.post({ type: "zodeclaw.state", state: { phase: "needsUpgrade", locale: this.locale } })
      return null
    }
  }
  ...
  private async openChatStream(envelope: ChatToken, gen: number): Promise<boolean> {
    ...
    const events = new EventServiceClient({
      ...
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "zodeclaw.error", error: "Authentication expired" })
      },
    })
    this.events = events

    const chat = new ZodeChatClient({
      baseUrl: envelope.zodeChatUrl,
      getToken: () => tokens.get(),
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "zodeclaw.error", error: "Authentication expired" })
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.error("[Zode New] ZodeClaw event-service connect failed:", message)
      this.post({
        type: "zodeclaw.state",
  ...
  private async loadInitialSnapshots(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Zode New] ZodeClaw listConversations failed:", message)
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Zode New] ZodeClaw getBotStatus failed:", message)
    }
    ...
    if (!this.activeConversationId && this.conversations.length > 0) {
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err)
        console.warn("[Zode New] ZodeClaw getConversationStatus failed:", message)
      }
  ...
  private async resolveClient(): Promise<ZodeClient | null> {
    if (this.connection.getConnectionState() !== "connected") {
      try {
        ...
        console.debug("[Zode New] ZodeClaw connect deferred:", (err as Error)?.message ?? err)
        return null
      }
    }
    try {
      ...
      console.debug("[Zode New] ZodeClaw getClient deferred:", (err as Error)?.message ?? err)
      return null
    }
  }
  ...
  private subscribeSandboxContext(): void {
    ...
    const ctx = `/zodeclaw/${this.sandboxId}`
  ...
  private subscribeConversationContext(conversationId: string): void {
    ...
    const ctx = `/zodeclaw/${this.sandboxId}/${conversationId}`
    this.events.subscribe([ctx])
  ...
  private attachEventHandlers(events: EventServiceClient, _chat: ZodeChatClient): void {
    ...
    this.chatSubs.push(
      events.on("conversation.created", (ctx, e: ConversationCreatedEvent) => {
        if (!this.sandboxId || ctx !== `/zodeclaw/${this.sandboxId}`) return
    ...
    this.chatSubs.push(
      events.on("conversation.left", (ctx, e: ConversationLeftEvent) => {
        ...
        if (this.activeConversationId === e.conversationId) {
          ...
          this.post({ type: "zodeclaw.activeConversation", conversationId: null })
          this.post({
            type: "zodeclaw.messages",
            conversationId: e.conversationId,
            messages: [],
            hasMore: false,
            replace: true,
          })
          this.post({ type: "zodeclaw.conversationStatus", status: null })
        }
        this.broadcastConversations({ replace: true })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("bot.status", (ctx, e: BotStatusEvent) => {
        ...
        this.post({ type: "zodeclaw.botStatus", status: this.botStatus })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("action.delivery_failed", (ctx, e: ActionDeliveryFailedEvent) => {
        ...
        this.broadcastMessages({ replace: true })
        this.post({ type: "zodeclaw.error", error: "Couldn't reach the bot — please try again" })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("conversation.status", (ctx, e: ConversationStatusEvent) => {
        ...
        this.post({ type: "zodeclaw.conversationStatus", status: this.conversationStatus })
      }),
    )
  }
  ...
  private async selectConversation(conversationId: string): Promise<void> {
    ...
    this.post({ type: "zodeclaw.activeConversation", conversationId })
    this.typingMembers = []
  ...
  private async createConversation(title?: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to create conversation") })
    }
  }
  ...
  private async renameConversation(conversationId: string, title: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to rename conversation") })
      void this.refreshConversations()
    }
  }
  ...
  private async leaveConversation(conversationId: string): Promise<void> {
    ...
    try {
      ...
      if (this.activeConversationId === conversationId) {
        ...
        this.messages = []
        this.post({ type: "zodeclaw.activeConversation", conversationId: null })
        this.post({ type: "zodeclaw.messages", conversationId, messages: [], hasMore: false, replace: true })
      }
      this.broadcastConversations({ replace: true })
    } catch (err) {
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to leave conversation") })
    }
  }
  ...
  private async loadMoreConversations(): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to load conversations") })
    }
  }
  ...
  private async sendMessage(
    ...
    // zode-chat validates clientId as a ULID (Crockford Base32); generate
    ...
    if (conversationId === this.activeConversationId) {
      ...
      this.post({ type: "zodeclaw.messageOptimistic", conversationId, message: optimistic })
    }
    ...
    try {
      ...
      console.error("[Zode New] ZodeClaw sendMessage failed:", err instanceof Error ? err.message : err)
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to send message") })
      if (conversationId === this.activeConversationId) {
        this.messages = this.messages.filter((m) => m.id !== pendingId)
        this.post({ type: "zodeclaw.messageRemoved", conversationId, messageId: pendingId })
      }
    }
  }
  ...
  private async editMessage(conversationId: string, messageId: string, content: ContentBlock[]): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to edit message") })
      if (snapshot && conversationId === this.activeConversationId) {
  ...
  private async deleteMessage(conversationId: string, messageId: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to delete message") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async loadMoreMessages(conversationId: string, before: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to load messages") })
    }
  }
  ...
  private async addReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to add reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async removeReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to remove reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async executeAction(
    ...
    try {
      ...
      this.post({ type: "zodeclaw.error", error: this.formatError(err, "Failed to execute action") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async refreshConversations(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Zode New] ZodeClaw refreshConversations failed:", message)
    }
  }
  ...
  private async refreshActiveMessages(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Zode New] ZodeClaw refreshActiveMessages failed:", message)
    }
  }
  ...
  private upsertTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "zodeclaw.typing", conversationId: this.activeConversationId, memberId })
    }
  ...
  private removeTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "zodeclaw.typingStop", conversationId: this.activeConversationId, memberId })
    }
  }
  ...
  private formatError(err: unknown, fallback: string): string {
    if (err instanceof ZodeChatApiError) {
  ...
  private startBotNudge(): void {
    ...
    this.botNudge = setInterval(() => {
      ...
      this.chat.requestBotStatus(this.sandboxId).catch((err) => {
        console.debug("[Zode New] ZodeClaw requestBotStatus failed:", (err as Error)?.message ?? err)
      })
    }, BOT_STATUS_NUDGE_MS)
  }
  ...
  private async poll(): Promise<void> {
    try {
      ...
      const res = await client.zode.claw.status()
      if (res?.data) {
        this.status = res.data as ClawStatus
        this.post({ type: "zodeclaw.status", data: this.status })
      }
    } catch (err) {
      console.debug("[Zode New] ZodeClaw poll failed:", (err as Error)?.message ?? err)
    }
  }
