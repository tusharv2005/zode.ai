 ...
 *
 * Minimal inline port of `@zodecode/event-service` (cloud monorepo). Connects
 * to the zode events Cloudflare Worker using a two-step ticket flow:
...
import type { ZodeChatEventMap, ZodeChatEventName } from "./types"

const WS_SUBPROTOCOL = "zode.events.v1"
...
export class EventServiceClient {
  ...
  on<N extends ZodeChatEventName>(event: N, handler: (ctx: string, payload: ZodeChatEventMap[N]) => void): () => void {
    ...
    // zode-chat worker edge before broadcast.
    const wrapped: EventHandler = (ctx, payload) => handler(ctx, payload as ZodeChatEventMap[N])
    set.add(wrapped)
  ...
  private handleMessage(data: string): void {
    ...
    if (m.type === "error") {
      console.warn("[Zode New] event-service server error", m)
    }
  }
