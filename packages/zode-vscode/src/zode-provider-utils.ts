import type { Session, Agent, Event, ProviderListResponse } from "@zodecode/sdk/v2/client"
import { prettifyError } from "zod/v4"
import type { CloudSessionMessage, IndexingStatus } from "./services/cli-backend/types"
import type { PartBatch, PartUpdate } from "./zode-provider/session-stream-scheduler"
import type { PartRemove } from "./shared/stream-messages"
import * as path from "path"

export { SessionStreamScheduler } from "./zode-provider/session-stream-scheduler"
...
function safeStringify(value: unknown): string | undefined {
  try {
    ...
    console.warn("[Zode New] getErrorMessage: JSON.stringify failed", err)
  }
  return undefined
}
...
export async function runWithMessageConfirmation<T>(
  ...
  try {
    ...
    if (await state.wait(id)) {
      console.warn(`[Zode New] ${label} ended after server accepted it; ignoring transport error`, {
...
/**
 * Shared interface for the subset of ZodeProvider state needed by session-refresh helpers.
 * Extracted here so the logic can be tested without importing ZodeProvider (and vscode).
 */
  ...
  const extra = await Promise.all(
    [...worktreeDirs].map((dir) =>
      list(dir).catch((err: unknown) => {
        console.error(`[Zode] Failed to list sessions for ${dir}:`, err)
        failed.add(dir)
        return [] as Session[]
      }),
    ),
