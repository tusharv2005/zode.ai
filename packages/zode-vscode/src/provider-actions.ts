/**
 * Provider action handlers extracted from ZodeProvider to stay under max-lines.
 * These are pure async functions that operate on the SDK client — no vscode dependency.
 */
import type { Config, ZodeClient } from "@zodecode/sdk/v2"
...
import { CUSTOM_PROVIDER_PACKAGE, ZODE_AUTO, parseModelString } from "./shared/provider-model"
...
export async function fetchProviderData(client: ZodeClient, dir: string) {
  const authRequest =
  ...
  return {
    ...
    disposeGlobal: async (reason: string) => {
      ...
      await client.global.dispose().catch((error: unknown) => {
        console.warn(`[Zode New] ZodeProvider: global.dispose() after ${reason} failed:`, error)
      })
    },
...
async function removeAuth(ctx: ActionContext, id: string, configured: boolean) {
  try {
    ...
    console.warn(`[Zode New] auth.remove failed for configured provider ${id} (non-fatal):`, err)
  }
}
