...
// equivalents in @zodecode/sdk. All API types (Session, Event, Agent,
// McpStatus, Config, etc.) should be imported from "@zodecode/sdk/v2/client".

import type { IndexingStatus as SdkIndexingStatus } from "@zodecode/sdk/v2/client"
...
export interface ZodecodeOrganization {
  id: string
  name: string
  role: string
}

export interface ZodecodeProfile {
  email: string
  name?: string
  organizations?: ZodecodeOrganization[]
}
...
// Cloud session from the Zode cloud API (cli_sessions_v2)
...
// Full cloud session data for preview (from /zode/cloud/session/:id)
