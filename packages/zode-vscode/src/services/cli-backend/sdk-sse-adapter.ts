import type { ZodeClient, GlobalEvent, Event } from "@zodecode/sdk/v2/client"
...
export class SdkSSEAdapter {
  ...
  constructor(private readonly client: ZodeClient) {}
  ...
  connect(): void {
    if (this.abortController) {
      console.log("[Zode New] SSE: ⚠️ Already connected, skipping")
      return
    }

    console.log("[Zode New] SSE: 🔌 connect() called")
    this.abortController = new AbortController()
    console.log('[Zode New] SSE: 🔄 Setting state to "connecting"')
    this.notifyState("connecting")
    void this.consumeLoop(this.abortController.signal).catch((err) => {
      console.error("[Zode New] SSE: Unhandled error in consumeLoop:", err)
   ...
   */
  disconnect(): void {
    console.log("[Zode New] SSE: 🔌 disconnect() called")
  ...
  reconnect(): void {
    if (!this.attemptController) {
      console.log("[Zode New] SSE: ⚠️ reconnect() called but no active attempt")
      return
    }
    console.log("[Zode New] SSE: 🔄 reconnect() — aborting current attempt")
  ...
  private async consumeLoop(signal: AbortSignal): Promise<void> {
    ...
    while (!signal.aborted) {
      ...
      try {
        console.log("[Zode New] SSE: 🎬 Calling SDK global.event()...")
        const events = await this.client.global.event({
          ...
          onSseError: (error) => {
            ...
            console.error("[Zode New] SSE: ❌ SDK SSE error callback:", error)
            this.notifyError(error instanceof Error ? error : new Error(String(error)))
          },
        })

        console.log("[Zode New] SSE: ⏳ Waiting for first stream event")
        ...
        for await (const event of events.stream) {
          ...
          if (!ready) {
            ...
            console.log("[Zode New] SSE: ✅ Stream opened successfully")
          ...
          if (type !== "server.heartbeat") {
            console.log("[Zode New] SSE: 📨 Event:", type)
          }
          this.notifyEvent(globalEvent.payload as Event, globalEvent.directory)
        }

        console.log(
          ready ? "[Zode New] SSE: 📭 Stream ended normally" : "[Zode New] SSE: 📭 Stream ended before first event",
        )
      } catch (error) {
        ...
        if (!aborted) {
          console.error("[Zode New] SSE: ❌ Stream error:", error)
          this.notifyError(error instanceof Error ? error : new Error(String(error)))
        }
      } finally {
      ...
      console.log(`[Zode New] SSE: 🔄 Reconnecting in ${wait}ms...`)
      this.notifyState("connecting")
  ...
  private resetHeartbeat(attempt: AbortController): void {
    ...
    this.heartbeatTimer = setTimeout(() => {
      console.log("[Zode New] SSE: ⏰ Heartbeat timeout — aborting stale connection")
      attempt.abort()
  ...
  private notifyEvent(event: Event, directory?: string): void {
    for (const handler of this.handlers) {
      try {
        ...
        console.error("[Zode New] SSE: Error in event handler:", error)
      }
    }
  }
  ...
  private notifyError(error: Error): void {
    for (const handler of this.errorHandlers) {
      try {
        ...
        console.error("[Zode New] SSE: Error in error handler:", err)
      }
    }
  }
  ...
  private notifyState(state: "connecting" | "connected" | "disconnected"): void {
    for (const handler of this.stateHandlers) {
      try {
        ...
        console.error("[Zode New] SSE: Error in state handler:", error)
      }
    }
  }
}
