...
export function resolveIndexingEnv(folders: readonly WorkspaceFolderLike[] | undefined): Record<string, string> {
  ...
  return { ZODE_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" }
}
...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    console.log("[Zode New] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[Zode New] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[Zode New] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[Zode New] ServerManager: 🚀 Starting new server instance...")
    this.startupPromise = this.startServer()
    try {
      this.instance = await this.startupPromise
      console.log("[Zode New] ServerManager: ✅ Server started successfully:", { port: this.instance.port })
      return this.instance
    } finally {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[Zode New] ServerManager: 📍 CLI path:", cliPath)
    console.log("[Zode New] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    console.log("[Zode New] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[Zode New] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))

    return new Promise((resolve, reject) => {
      console.log("[Zode New] ServerManager: 🎬 Spawning CLI process:", cliPath, ["serve", "--port", "0"])
      const cfg = vscode.workspace.getConfiguration("zode-code.new")
      ...
      //     `zode-code.new.extraCaCerts` (NODE_EXTRA_CA_CERTS).
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          ZODE_SERVER_PASSWORD: password,
          ZODE_CLIENT: "vscode",
          ZODE_ENABLE_QUESTION_TOOL: "true",
          ZODECODE_FEATURE: "vscode-extension",
          ...indexingEnv,
          ZODE_TELEMETRY_LEVEL: vscode.env.isTelemetryEnabled ? "all" : "off",
          ZODE_APP_NAME: "zode-code",
          ZODE_EDITOR_NAME: vscode.env.appName,
          ZODE_PLATFORM: "vscode",
          ZODE_MACHINE_ID: vscode.env.machineId,
          ZODE_APP_VERSION: this.context.extension.packageJSON.version,
          ZODE_VSCODE_VERSION: vscode.version,
          ZODECODE_EDITOR_NAME: `${vscode.env.appName} ${vscode.version}`,
          ...(!claudeCompat && { ZODE_DISABLE_CLAUDE_CODE: "true" }),
          ...resolveTreeSitterEnv(this.context.extensionPath),
        },
        stdio: ["ignore", "pipe", "pipe"],
        detached: true,
      })
      console.log("[Zode New] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[Zode New] ServerManager: 📥 CLI Server stdout:", output)

        const port = parseServerPort(output)
        if (port !== null && !resolved) {
          resolved = true
          console.log("[Zode New] ServerManager: 🎯 Port detected:", port)
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[Zode New] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
        stderrLines.push(errorOutput)
      })

      serverProcess.on("error", (error) => {
        console.error("[Zode New] ServerManager: ❌ Process error:", error)
        if (!resolved) {
          reject(error)
        }
      })
      ...
      serverProcess.on("exit", (code) => {
        console.log("[Zode New] ServerManager: 🛑 Process exited with code:", code)
      ...
      setTimeout(() => {
        if (!resolved) {
          console.error(`[Zode New] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
  ...
  private getCliPath(): string {
    ...
    console.log("[Zode New] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  dispose(): void {
    ...
    console.log("[Zode New] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[Zode New] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
