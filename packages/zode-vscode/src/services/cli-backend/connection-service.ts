...
import { createZodeClient, type ZodeClient, type Event } from "@zodecode/sdk/v2/client"
...
async function drainNetworkWaits(client: ZodeClient, dir: string) {
...
/**
 * Shared connection service that owns the single ServerManager, ZodeClient (SDK), and SdkSSEAdapter.
 * Multiple ZodeProvider instances subscribe to it for SSE events and state changes.
 */
export class ZodeConnectionService {
  private readonly serverManager: ServerManager
  private client: ZodeClient | null = null
   ...
   */
  async getClientAsync(dir?: string): Promise<ZodeClient> {
  ...
  /**
   * Subscribe to notification dismiss events broadcast from any ZodeProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a notification dismiss event to all subscribed ZodeProvider instances.
   */
  ...
  /**
   * Subscribe to language change events broadcast from any ZodeProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a language change event to all subscribed ZodeProvider instances.
   */
  ...
  /**
   * Subscribe to profile change events broadcast from any ZodeProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a profile change event to all subscribed ZodeProvider instances.
   */
  ...
  /**
   * Subscribe to migration-complete events broadcast from any ZodeProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a migration-complete event to all subscribed ZodeProvider instances.
   */
  ...
  /**
   * Subscribe to favorites change events broadcast from any ZodeProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a favorites change event to all subscribed ZodeProvider instances.
   */
   ...
   * ZodeProvider (root + worktree dirs). Used by drainPendingPrompts() to
   ...
   * directory known to any currently-mounted ZodeProvider.
   *
  ...
  flushViewed(): void {
    ...
    this.debounceTimer = setTimeout(() => {
      ...
      this.client?.session
        ...
        .catch((err) => console.warn("[Zode New] ConnectionService: viewed flush failed:", err))
    }, 150)
  }
  ...
  private startHealthPoll(baseUrl: string, password: string): void {
    ...
    this.healthPollTimer = setInterval(async () => {
      ...
      if (!healthy && this.state === "connected") {
        console.warn("[Zode New] ConnectionService: ❤️‍🩹 Health check failed — forcing SSE reconnect")
  ...
  private async checkHealth(baseUrl: string, password: string): Promise<boolean> {
    try {
      ...
      const res = await fetch(`${baseUrl}/global/health`, {
        headers: { Authorization: `Basic ${Buffer.from(`zode:${password}`).toString("base64")}` },
  ...
  private handleServerExit(code: number | null): void {
    console.warn("[Zode New] ConnectionService: CLI background process exited:", code)
  ...
  private async doConnect(workspaceDir: string): Promise<void> {
    ...
    const authHeader = `Basic ${Buffer.from(`zode:${server.password}`).toString("base64")}`
...
async function drainSuggestions(client: ZodeClient, directory: string): Promise<void> {
