...
import type { ZodeClient, McpStatus } from "@zodecode/sdk/v2/client"
...
export class BrowserAutomationService implements vscode.Disposable {
  ...
  private static readonly MCP_SERVER_NAME = "zode-playwright"

  constructor(private readonly connectionService: ZodeConnectionService) {
    // Listen for settings changes
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("zode-code.new.browserAutomation")) {
  ...
  private async register(): Promise<void> {
    ...
    if (!client) {
      console.error("[Zode New] BrowserAutomationService: No SDK client available")
    ...
    try {
      ...
      if (serverStatus?.status === "connected") {
        ...
        console.error(
          "[Zode New] BrowserAutomationService: MCP server failed:",
      ...
      console.error("[Zode New] BrowserAutomationService: Failed to register MCP server:", error)
      this.setState("failed")
    }
  }
  ...
  private async unregister(): Promise<void> {
    ...
    if (client) {
      try {
        ...
        console.error("[Zode New] BrowserAutomationService: Failed to disconnect MCP server:", error)
      }
    }
  ...
  private getClient(): ZodeClient | null {
    try {
  ...
  private setState(state: BrowserAutomationState): void {
    ...
    console.log(`[Zode New] BrowserAutomationService: State ${this.state} → ${state}`)
    this.state = state
