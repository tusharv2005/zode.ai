...
const ZODECODEIGNORE = ".zodecodeignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .zodecodeignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .zodecodeignore first — if it exists, use only that.
    ...
    const zodecodeignorePath = path.join(this.workspacePath, ZODECODEIGNORE)
    if (fs.existsSync(zodecodeignorePath)) {
      const zodecodeignoreContent = fs.readFileSync(zodecodeignorePath, "utf-8")
      if (zodecodeignoreContent.trim()) {
        this.ignoreInstance.add(zodecodeignoreContent)
        this.ignoreInstance.add(ZODECODEIGNORE)
        this.loadedContents.push({ file: ZODECODEIGNORE, content: zodecodeignoreContent })
        return
      }
    }
