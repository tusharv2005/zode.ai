 ...
 * Uses the SDK's zode.fim() SSE endpoint which handles auth and streaming.
 *
...
export async function generateFim(
  ...
  console.info(`[FIM] request provider=${info.providerID} model=${info.requestModel} url=/zode/fim`)

  const { stream } = await client.zode.fim(
    {
      prefix,
      suffix,
 ...
 */
export function hasValidCredentials(connectionService: ZodeConnectionService): boolean {
