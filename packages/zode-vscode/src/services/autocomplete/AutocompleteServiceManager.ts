...
const CONFIG_SECTION = "zode-code.new.autocomplete"
...
export class AutocompleteServiceManager {
  ...
  constructor(context: vscode.ExtensionContext, connectionService: ZodeConnectionService) {
  ...
  private async updateGlobalContext() {
    await vscode.commands.executeCommand(
      "setContext",
      "zodecode.autocomplete.enableSmartInlineTaskKeybinding",
  ...
  private handleFatalAutocompleteError(status: number | null): void {
    const msg =
      status === 402
        ? t("zodecode:autocomplete.creditsExhausted.message")
        : t("zodecode:autocomplete.authError.message")
  ...
  public async showIncompatibilityExtensionPopup() {
    const message = t("zodecode:autocomplete.incompatibilityExtensionPopup.message")
    const disableCopilot = t("zodecode:autocomplete.incompatibilityExtensionPopup.disableCopilot")
    const disableInlineAssist = t("zodecode:autocomplete.incompatibilityExtensionPopup.disableInlineAssist")
    ...
    if (response === disableCopilot) {
      ...
      await vscode.commands.executeCommand("zode-code.new.autocomplete.disable")
    }
  }
