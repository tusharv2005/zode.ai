 ...
 *
 * Uses a full ZodeProvider so the viewer has backend connectivity
...
export class SubAgentViewerProvider implements vscode.Disposable {
  ...
  private providers = new Map<string, ZodeProvider>()
  ...
  openPanel(sessionID: string, title?: string): void {
    ...
    const panel = vscode.window.createWebviewPanel("zode-code.new.SubAgentViewerPanel", label, vscode.ViewColumn.One, {
      enableScripts: true,
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage(async (msg) => {
      ...
      try {
        ...
        console.error("[Zode New] SubAgentViewerProvider: Failed to load session:", err)
      }
    })
    ...
    panel.onDidDispose(() => {
      console.log("[Zode New] Sub-agent viewer panel disposed:", sessionID)
