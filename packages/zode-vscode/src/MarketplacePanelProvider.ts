...
import type { Event, SessionStatus } from "@zodecode/sdk/v2/client"
import { buildWebviewHtml, getWebviewFontSize } from "./utils"
import { watchFontSizeConfig } from "./zode-provider/font-size"
import { mapSSEEventToWebviewMessage } from "./zode-provider-utils"
...
export class MarketplacePanelProvider implements vscode.Disposable {
  public static readonly viewType = "zode-code.new.marketplacePanel"
  ...
  private attach(panel: vscode.WebviewPanel, project: string | null): void {
    ...
    this.ready = false
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "zode-light.svg"),
      dark: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "zode-dark.svg"),
    }
  ...
  private async sync(reconcile: boolean): Promise<void> {
    ...
    try {
      ...
      console.warn("[Zode New] Marketplace session status sync failed:", err)
    }
  }
  ...
  private async handle(msg: MarketplaceMessage): Promise<void> {
    switch (msg.type) {
      ...
      case "dismissAgentMigrationBanner":
        await this.context.globalState.update("zode.agentMigrationBannerDismissed", true)
        return
      case "openExternal":
  ...
  private async fetchData(): Promise<void> {
    try {
      ...
      const dismissed = this.context.globalState.get<boolean>("zode.agentMigrationBannerDismissed") ?? false
      this.post({ type: "marketplaceData", ...data, showAgentMigrationBanner: !dismissed })
    } catch (err) {
      const error = err instanceof Error ? err.message : String(err)
      console.warn("[Zode New] Marketplace data fetch failed:", err)
      this.post({
  ...
  private post(msg: unknown): void {
    ...
    void this.panel.webview.postMessage(msg).then(undefined, (err) => {
      console.warn("[Zode New] Marketplace panel postMessage failed:", err)
    })
  }
