...
| Custom Instructions (per-mode) | `<TextArea>` (10 rows) + link to `.zodecode/rules-{slug}/rules.md` | Not available as separate field (merged into system prompt) |
...
| Override System Prompt | Advanced disclosure with link to `.zodecode/system-prompt-{slug}` | Not available |
...
| Link to global rules file | Link to `.zodecode/rules/rules.md` | Not available |

### Organization Features

| Feature | Legacy | New |
|---|---|---|
| Share modes banner | `ZodeShareModesBanner` component | Not available |
