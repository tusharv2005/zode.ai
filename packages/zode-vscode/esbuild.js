...
/**
 * Force all solid-js imports (from zode-ui and the webview) to resolve to
...
const solidDedupePlugin = {
  name: "solid-dedupe",
  setup(build) {
    // Resolve these bare specifiers to the zode-vscode-local copy
 ...
 * variants) to the Zode implementation in `webview-ui/pierre-worker.ts`.
 *
 * The upstream module loads Pierre's Shiki worker via a Vite-only
 * `?worker&url` import that esbuild can't resolve. The Zode replacement loads
...
/**
 * Resolve the synthetic `zode-shiki-worker` entry point to Pierre's Shiki worker
...
const shikiWorkerEntryPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^zode-shiki-worker$/ }, async () => {
    ...
    build.onLoad({ filter: /sprite\.svg$/ }, (args) => {
      ...
      return {
        contents: `
          ...
          const inject = () => {
            if (!document.getElementById("zode-sprite")) {
...
async function main() {
  ...
  // Build ZodeClaw webview (SolidJS, standalone chat panel)
  const zodeClawCtx = await createBrowserWebviewContext("webview-ui/zodeclaw/index.tsx", "dist/zodeclaw.js")
