export type AutocompleteProviderID = "zode" | "mistral" | "inception"
export type DirectAutocompleteProviderID = Exclude<AutocompleteProviderID, "zode">
...
export interface AutocompleteModelDef {
  ...
  /** Provider key to use for direct BYOK. Empty means Zode Gateway. */
   ...
   * through `/zode/edit` and use Mercury's Next Edit pipeline.
   */
...
const models: AutocompleteModelDef[] = [
  {
    id: "zode/mistralai/codestral-2508",
    modelID: "mistralai/codestral-2508",
    label: "Codestral",
    providerID: "zode",
    provider: "Zode Gateway",
    requestModel: "mistralai/codestral-2508",
    temperature: 0.2,
  },
  {
    id: "zode/inception/mercury-edit-2",
  ...
  {
    // Same wire-level model as `zode/inception/mercury-edit-2`, but routed
    // through the Zode Gateway's Next Edit endpoint instead of FIM. Picked by
    // users who want multi-line next-edit predictions with the jump-to-edit UX.
    id: "zode/inception/mercury-next-edit",
...
export const DEFAULT_AUTOCOMPLETE_PROVIDER_ID: AutocompleteProviderID = "zode"
...
export function getAutocompleteModel(provider?: string, model?: string): AutocompleteModelDef {
  // When provider is unset, always default to Zode Gateway. Direct-provider
