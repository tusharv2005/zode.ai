...
/**
 * Zode Gateway Authentication Plugin
 *
 * Provides device authorization flow for Zode Gateway
 ...
 */
export const ZodeAuthPlugin: Plugin = async (ctx) => {
  return {
    auth: {
      provider: "zode",
      async loader(getAuth, providerInfo) {
        ...
        if (auth.type === "api") {
          return {
            zodecodeToken: auth.key,
          }
        }

        // For OAuth auth, access token contains the Zode token
      ...
      methods: [
        {
          type: "oauth",
          label: "Zode Gateway (Device Authorization)",
          async authorize() {
