...
import { getZodeProfile, getZodeDefaultModel, promptOrganizationSelection } from "../api/profile.js"
import { ZODE_API_BASE, POLL_INTERVAL_MS } from "../api/constants.js"
...
async function initiateDeviceAuth(): Promise<DeviceAuthInitiateResponse> {
  const response = await fetch(`${ZODE_API_BASE}/api/device-auth/codes`, {
    method: "POST",
    headers: {
...
async function pollDeviceAuth(code: string): Promise<DeviceAuthPollResponse> {
  const response = await fetch(`${ZODE_API_BASE}/api/device-auth/codes/${code}`)
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getZodeProfile(token)
  ...
  const model = await getZodeDefaultModel(token, organizationId)
