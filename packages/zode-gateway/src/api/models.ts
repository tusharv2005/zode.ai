...
import { getDefaultHeaders, buildZodeHeaders } from "../headers.js"
import { ZODE_API_BASE, ZODE_OPENROUTER_BASE, MODELS_FETCH_TIMEOUT_MS, PROMPTS, AI_SDK_PROVIDERS } from "./constants.js"

export type ZodeModelsResult = {
...
/**
 * Fetch models from Zode API (OpenRouter-compatible endpoint)
 *
 * @param options - Configuration options
 * @returns Typed result with models and optional error info
 */
export async function fetchZodeModels(options?: {
  zodecodeToken?: string
  zodecodeOrganizationId?: string
  baseURL?: string
}): Promise<ZodeModelsResult> {
  const token = options?.zodecodeToken
  const organizationId = options?.zodecodeOrganizationId

  // Construct base URL
  const defaultBaseURL = organizationId ? `${ZODE_API_BASE}/api/organizations/${organizationId}` : ZODE_OPENROUTER_BASE

  const baseURL = options?.baseURL ?? defaultBaseURL

  // Transform URL with token if available
  const finalBaseURL = token ? getZodeUrlFromToken(baseURL, token) : baseURL
  ...
  const response = await fetch(modelsURL, {
    headers: {
      ...getDefaultHeaders(),
      ...buildZodeHeaders(undefined, { zodecodeOrganizationId: organizationId }),
  ...
  for (const model of result.data.data) {
    ...
    // Skip models that don't support tools — Zode requires tool calling
