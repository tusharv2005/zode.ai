...
import type { ZodecodeProfile, Organization, ZodecodeBalance } from "../types.js"
import { ZODE_API_BASE, DEFAULT_MODEL, DEFAULT_FREE_MODEL } from "./constants.js"

/**
 * Fetch user profile from Zode API
 */
export async function fetchProfile(token: string): Promise<ZodecodeProfile> {
  const response = await fetch(`${ZODE_API_BASE}/api/profile`, {
    headers: {
  ...
  // Transform to flat ZodecodeProfile structure
  return {
 ...
 */
export const getZodeProfile = fetchProfile

/**
 * Fetch user balance from Zode API
 * @param token - Authentication token
 * @param organizationId - Optional organization ID for team balance
 */
export async function fetchBalance(token: string, organizationId?: string): Promise<ZodecodeBalance | null> {
  try {
    ...
    if (organizationId) {
      headers["x-zodecode-organizationid"] = organizationId
    }

    const response = await fetch(`${ZODE_API_BASE}/api/profile/balance`, { headers })
 ...
 * Alias for compatibility with existing code
 */
export const getZodeBalance = fetchBalance
 ...
 * Alias for compatibility with existing code
 */
export const getZodeDefaultModel = fetchDefaultModel
