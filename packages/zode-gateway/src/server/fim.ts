    ...
    return fetch(url, {
      ...
      headers: {
        ...
        ...(target.provider === "zode"
          ? buildZodeHeaders(undefined, { zodecodeOrganizationId: input.organizationId })
          : {}),
...
export function createFimHandler(Auth: Auth) {
  return async (c: any) => {
    ...
    const proxy = target.provider === "zode" ? await getProxyAuth(Auth) : undefined
    const token = target.provider === "zode" ? proxy?.token : await getProviderKey(Auth, target.provider)

    if (target.provider === "zode" && !proxy?.auth) {
      return c.json({ error: "Not authenticated with Zode Gateway" }, 401)
    }

    if (target.provider === "zode" && !token) {
