// Verifies fetchZodeModels typed result and 401 fallback behaviour.

import { test, expect } from "bun:test"
import { fetchZodeModels } from "../../src/api/models.js"
...
test("returns empty models and error when both auth and public requests return 401", async () => {
  ...
  const result = await fetchZodeModels({ zodecodeToken: "bad-token" })
...
test("falls back to public endpoint on 401 and returns models", async () => {
  ...
  const result = await fetchZodeModels({
    zodecodeToken: "expired-token",
