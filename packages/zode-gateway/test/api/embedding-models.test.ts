...
import { EMPTY_ZODE_EMBEDDING_MODEL_CATALOG, fetchZodeEmbeddingModelCatalog } from "../../src/api/embedding-models"

describe("fetchZodeEmbeddingModelCatalog", () => {
  test("fetches catalog from Zode Gateway", async () => {
    ...
    try {
      const catalog = await fetchZodeEmbeddingModelCatalog({ baseURL: "https://example.test" })
  ...
  test("falls back when the request fails", async () => {
    ...
    try {
      await expect(fetchZodeEmbeddingModelCatalog({ baseURL: "https://example.test" })).resolves.toEqual(
  ...
  test("fallback catalog is empty so Cloud owns model metadata", () => {
    expect(EMPTY_ZODE_EMBEDDING_MODEL_CATALOG).toEqual({
      defaultModel: "",
      models: [],
      aliases: {},
    })
