...
describe("Edit target resolution", () => {
  ...
  test("does NOT route the FIM Mercury model to the edit endpoint", () => {
    // `mercury-edit-2` (kind: fim) must fall through to the zode placeholder,
    // not the edit endpoint — only `mercury-next-edit` (kind: edit) is NES.
    expect(resolveEditTarget("inception", "mercury-edit-2").provider).toBe("zode")
  })

  test("routes the Zode Gateway next-edit model to the gateway proxy", () => {
    const target = resolveEditTarget("zode", "inception/mercury-next-edit")
    expect(target.provider).toBe("zode")
    expect(target.model).toBe("inception/mercury-edit-2")
    expect(target.url).toMatch(/\/api\/edit\/completions$/)
  })

  test("falls back to a zode placeholder (no upstream) for non-edit models", () => {
    expect(resolveEditTarget("zode", "mistralai/codestral-2508")).toEqual({
      provider: "zode",
      model: "mistralai/codestral-2508",
      url: "",
    })
    expect(resolveEditTarget()).toMatchObject({ provider: "zode", url: "" })
  })
})
