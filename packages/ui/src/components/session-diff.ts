...
import type { SnapshotFileDiff, VcsFileDiff } from "@zodecode/sdk/v2"
...
// zodecode_change start - expose patch text extraction without building FileDiffMetadata on the UI thread
...
export type ViewDiff = {
  file: string
  patch: string
  before: string // zodecode_change
  after: string // zodecode_change
  additions: number
  deletions: number
  status?: "added" | "deleted" | "modified"
...
export function normalize(diff: ReviewDiff): ViewDiff {
  const next = contents(diff) // zodecode_change
  return {
    file: diff.file, // zodecode_change
    patch: next.patch, // zodecode_change
    before: next.before, // zodecode_change
    after: next.after, // zodecode_change
