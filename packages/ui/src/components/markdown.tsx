...
import { tryFastRender } from "../zodecode/markdown-fast-path" // zodecode_change
import { hasMermaid, preserveMermaid, renderMermaid, type MermaidLabels } from "../zodecode/markdown-mermaid" // zodecode_change
import { preserveStreamingHighlight } from "../zodecode/markdown-stream-highlight" // zodecode_change
...
export function Markdown(
  ...
  // zodecode_change start: generation counter prevents stale deferredHighlight
  ...
  // zodecode_change start: Mermaid diagram rendering
  const mermaidState = { gen: 0, signal: { aborted: false } }
  // zodecode_change end

  // zodecode_change start: rAF-coalesced morphdom render.
  ...
  createEffect(() => {
    ...
    if (!content) {
      // zodecode_change start: cancel any in-flight coalesced render so a
    ...
    // zodecode_change start: queue the latest content for a single rAF tick.
    ...
    pendingFrame = requestAnimationFrame(() => {
      ...
      // zodecode_change start: morphdom guard for highlighted blocks (issue #6221)
      ...
      morphdom(container, temp, {
        ...
        onBeforeElUpdated: (fromEl, toEl) => {
          ...
          // zodecode_change start: preserve rendered Mermaid diagrams across
          ...
          if (
            ...
            if (preserveStreamingHighlight(fromEl, toEl, local.streaming ?? false)) return false // zodecode_change
            // Source changed during streaming — fall through so morphdom replaces // zodecode_change
      ...
      kickMermaid(container, local.streaming ?? false, mermaid) // zodecode_change
      kickHighlight(container, nextLabels)
    })
    // zodecode_change end
  })

  // zodecode_change start: progressive Shiki highlighting (issue #6221, PR #7102).
  ...
  onCleanup(() => {
    // zodecode_change: cancel any in-flight deferredHighlight pass so its
    ...
    mermaidState.gen++
    // zodecode_change end
    // zodecode_change: cancel any queued rAF parse so it doesn't touch the
