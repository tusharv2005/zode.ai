...
// zodecode_change: marked-shiki highlighted code blocks synchronously during
...
// zodecode_change start: import types for double-dollar math extension
import type { MarkedExtension, TokenizerAndRendererExtension } from "marked"
// zodecode_change end
import { bundledLanguages, type BundledLanguage } from "shiki"
import { parseFilePath } from "../file-path" // zodecode_change
import { createSimpleContext } from "./helper"
import { getSharedHighlighter, registerCustomTheme, ThemeRegistrationResolved } from "@pierre/diffs"

registerCustomTheme("Zode", () => {
...
// zodecode_change start: double-dollar-only math rules for marked.
const BLOCK = /^\$\$\n((?:\\[^]|[^\\])+?)\n\$\$(?:\n|$)/
const INLINE = /^\$\$(?!\$)((?:\\.|[^\\\n])*?(?:\\.|[^\\\n$]))\$\$/
// zodecode_change end
...
function renderMathInText(text: string): string {
  ...
  // zodecode_change: removed single-dollar inline math ($...$) rendering.
...
// zodecode_change: parseFilePath imported from ../file-path

// zodecode_change start: highlight cache for deferred highlighting
...
export async function deferredHighlight(
  ...
  const highlighter = await getSharedHighlighter({ themes: ["Zode"], langs: [] })
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const highlight = () => {
            ...
            const html = highlighter.codeToHtml(code, { lang: language, theme: "Zode", tabindex: false })
...
export const { use: useMarked, provider: MarkedProvider } = createSimpleContext({
  ...
  init: (props: { nativeParser?: NativeMarkdownParser }) => {
    // zodecode_change start: two-pass parser — first pass skips Shiki highlighting
    ...
    const parser = marked.use(
      ...
      // zodecode_change start: enable only double-dollar math.
      // Single $ is far more common as a currency symbol in agent responses
      ...
      // zodecode_change end
      // zodecode_change: markedShiki removed — the custom `code` renderer
