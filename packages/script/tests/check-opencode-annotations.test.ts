...
function isExempt(file: string) {
  ...
  if (norm.split("/").some((part) => part.includes("zodecode") || part.startsWith("zode-"))) return true
...
const MARKER_PREFIX = /(?:\/\/|\{?\s*\/\*|#)\s*zodecode_change\b/
...
function coveredLines(text: string): Set<number> {
  ...
  if (first?.match(/(?:\/\/|\{?\s*\/\*|#)\s*zodecode_change\s*-\s*new\s*file\b/)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/(?:\/\/|\{?\s*\/\*|#)\s*zodecode_change\s+start\b/)) {
      block = true
      covered.add(n)
      continue
    }

    if (line.match(/(?:\/\/|\{?\s*\/\*|#)\s*zodecode_change\s+end\b/)) {
      covered.add(n)
      block = false
      continue
    }
...
describe("hasMarker", () => {
  const cases: Array<[string, boolean]> = [
    // JS-style inline
    ["// zodecode_change", true],
    ["  // zodecode_change", true],
    ["const x = 1 // zodecode_change", true],
    ["// zodecode_change start", true],
    ["// zodecode_change end", true],
    ["// zodecode_change - new file", true],
    ["//   zodecode_change", true],
    ["// zodecode_change  ", true],
    ...
    ["{/* zodecode_change end */}", true],
    ["{/* zodecode_change - new file */}", true],
    ["{/* zodecode_change - ZodeNews added */}", true],
    ["{/*   zodecode_change */}", true],
    ["{/* zodecode_change  */}", true],
    ...
    ["# zodecode_change", true],
    ["  # zodecode_change", true],
    ["name: test # zodecode_change", true],
    ['name = "zed" # zodecode_change', true],
    ['export FOO="bar" # zodecode_change', true],
    ["# zodecode_change start", true],
    ["# zodecode_change end", true],
    ...
    ["// zodecode_changes", false],
    ["// zodecode_changelog", false],
    ["/* zodecode_change_log */", false],
    ["{/* zodecode_changes */}", false],
    ["// zodecode_changeable", false],
    ["", false],
    ["  ", false],
  ]
...
describe("isExempt", () => {
  const cases: Array<[string, boolean]> = [
    // exempt — "zodecode" in path
    ["packages/opencode/src/zodecode/foo.ts", true],
    ["packages/opencode/test/zodecode/bar.test.ts", true],
    ["packages/opencode/src/some/zodecode/deep/path.ts", true],
    ["packages/opencode/src/zodecode/deep/nested/file.tsx", true],
    ["packages/opencode/src/zode-sessions/session.ts", true],
    ["packages/zode-ui/src/components/icon.tsx", true],
    ["packages/zode-vscode/src/extension.ts", true],
    ...
    // exempt — "zodecode" in filename
    ["packages/opencode/src/foo/zodecode.ts", true],
    ["packages/opencode/src/bar/zodecode.test.ts", true],
    ["packages/opencode/src/file.zodecode.ts", true],
    // exempt — case-insensitive
    ["packages/opencode/src/ZodeCode/foo.ts", true],
    ["packages/opencode/src/ZODECODE/bar.ts", true],
    // NOT exempt
    ...
    // zodecode_change is not the same as zodecode
...
describe("isChecked", () => {
  const cases: Array<[string, boolean]> = [
    ...
    ["packages/zode-ui/src/components/icon.tsx", false],
    ["packages/zode-vscode/src/extension.ts", false],
...
describe("coveredLines", () => {
  ...
  test("whole-file JS annotation", () => {
    const covered = coveredLines("// zodecode_change - new file\nexport const x = 1\nexport const y = 2")
  ...
  test("bare /* */ block markers", () => {
    const text = ["/* zodecode_change start */", "const b = 2", "/* zodecode_change end */"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("YAML block markers", () => {
    const text = ["# zodecode_change start", "name: test", "# zodecode_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("TOML block markers", () => {
    const text = ["# zodecode_change start", 'id = "opencode"', "# zodecode_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("shell block markers", () => {
    const text = ["# zodecode_change start", "set -euo pipefail", "# zodecode_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("inline JS marker covers only that line", () => {
    const text = ["const a = 1", "const b = 2 // zodecode_change", "const c = 3"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })

  test("inline JSX marker covers only that line", () => {
    const text = ["const a = 1", "{/* zodecode_change */}", "const c = 3"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })

  test("inline JS marker with code on same line", () => {
    const text = "const url = Flag.ZODE_MODELS_URL || 'https://models.dev' // zodecode_change\n"
    const covered = coveredLines(text)
  ...
  test("JSX block marker with descriptive suffix", () => {
    const text = [
      "{/* zodecode_change start - Zode-specific error display */}",
      "<ErrorDisplay />",
      "{/* zodecode_change end */}",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("marker line with extra text after marker is still covered", () => {
    const text = [
      "const a = 1",
      "// zodecode_change start - this is zode specific",
      "const b = 2",
      "// zodecode_change end",
    ].join("\n")
    const covered = coveredLines(text)
...
describe("checkLine (main loop simulation)", () => {
  ...
  test("covered line reports no violation", () => {
    const text = ["// zodecode_change start", "const zode = 1", "// zodecode_change end"].join("\n")
  ...
  test("marker lines are skipped even if uncovered", () => {
    ...
    const text = ["{/* zodecode_change */}", "{/* zodecode_change start */}"].join("\n")
  ...
  test("real-world TSX permission.tsx inline pattern", () => {
    const text = [
      ...
      "{/* zodecode_change */}",
      "<AnotherZodeComponent />",
    ].join("\n")
    expect(check(text, [2, 4])).toEqual(["line 2: <PermissionDeniedCard />", "line 4: <AnotherZodeComponent />"])
    expect(check(text, [1, 3])).toEqual([])
  })

  test("JS-style session/index.tsx pattern (from existing codebase)", () => {
    const text = ["const foo = 1", "<Toast />", "{/* zodecode_change */}", "<Footer />", "</box>"].join("\n")
  ...
  test("whole-file annotated file — no violations even for unmarked lines", () => {
    const text = [
      "// zodecode_change - new file",
      "export const zodeFeature = true",
      "export const alsoZode = 123",
...
// and a flag indicating whether the diff removes any zodecode_change marker
// (i.e. the change is reverting Zode modifications back to upstream).
...
describe("parseDiff (revert detection)", () => {
  ...
  test("revert: hunk removes zodecode_change marker block and adds upstream original", () => {
    ...
    const diff = [
      ...
      "+++ b/test.ts",
      "@@ -16,3 +16 @@ describe(...)",
      "-  // zodecode_change start - TODO: skip flaky test",
      "-  test.skip('foo', async () => {",
      "-    // zodecode_change end",
      "+  test('foo', async () => {",
    ].join("\n")
  ...
  test("revert: inline marker removed, upstream original added", () => {
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -5 +5 @@",
      "-const url = Flag.X || 'fallback' // zodecode_change",
      "+const url = Flag.X",
    ].join("\n")
    const out = parseDiff(diff)
  ...
  test("file-level revert: marker removed in one hunk covers other hunks", () => {
    // Mirrors the prompt.test.ts case from PR #9908: zodecode_change marker
    // is removed in hunk A, while a separate hunk B replaces references that
    // depended on the removed Zode construct.
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -218 +217,0 @@",
      "-const unixSkip = it.live.skip // zodecode_change - skip flaky tests",
      "@@ -1589 +1583 @@ unixSkip(",
      "-unixSkip(",
      "+unix(",
    ].join("\n")
  ...
  test("multiple zodecode_change start/end markers removed across hunks", () => {
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -1432,2 +1431 @@",
      "-// zodecode_change start - flaky on Linux CI",
      "-unixSkip(",
      "+unix(",
      "@@ -1469 +1466,0 @@",
      "-// zodecode_change end",
  ...
  test("YAML/shell marker removal also triggers revert", () => {
    const diff = [
      ...
      "@@ -10 +10 @@",
      "-      - uses: actions/checkout@v6 # zodecode_change",
...
describe("MARKER_PREFIX regex edge cases", () => {
  test("handles { followed immediately by /*", () => {
    expect(hasMarker("{/* zodecode_change */}")).toBe(true)
  })
  ...
  test("does not match {/* without zodecode_change", () => {
  ...
  test("does not match zodecode_changes (word boundary)", () => {
    expect(hasMarker("// zodecode_changes")).toBe(false)
    expect(hasMarker("// zodecode_changelog")).toBe(false)
    expect(hasMarker("{/* zodecode_changes */}")).toBe(false)
    expect(hasMarker("// zodecode_changeable")).toBe(false)
  })
})
...
describe("isExempt — Windows backslash paths", () => {
  test("Windows paths with backslashes", () => {
    expect(isExempt("packages\\opencode\\src\\zodecode\\foo.ts")).toBe(true)
    expect(isExempt("packages\\opencode\\test\\zodecode\\bar.test.ts")).toBe(true)
...
describe("coveredLines — additional patterns", () => {
  test("block with descriptive suffix is still recognized", () => {
    const text = [
      "{/* zodecode_change start - Zode-specific indexing display */}",
      "<IndexingStatus />",
      "{/* zodecode_change end */}",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("empty file content", () => {
    const covered = coveredLines("// zodecode_change start\n  \n// zodecode_change end")
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("multiple separate JS inline markers", () => {
    const text = [
      "const a = 1 // zodecode_change",
      "const b = 2",
      "const c = 3 // zodecode_change",
      "const d = 4",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("consecutive block markers (no content)", () => {
    const text = ["// zodecode_change start", "// zodecode_change end"].join("\n")
    const covered = coveredLines(text)
  ...
  test("trailing empty line after block end is not covered", () => {
    const text = "// zodecode_change start\nconst a = 1\n// zodecode_change end\n\n"
    const covered = coveredLines(text)
...
describe("checkLine — additional patterns", () => {
  ...
  test("real-world TUI routes — line between marker and code should be uncovered", () => {
    ...
    const text = ["{/* zodecode_change start */}", "", "<ZodeIndexing />", "", "{/* zodecode_change end */}"].join("\n")
  ...
  test("end marker on same line as content is covered", () => {
    const text = "const a = 1\n{/* zodecode_change end */} // block already closed, still covered\n"
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })
