 ...
 * or worktree). Each call routes to a [ZodeBackendWorkspace]
 * via the workspace manager.
 */
@Rpc
interface ZodeWorkspaceRpcApi : RemoteApi<Unit> {
    companion object {
        suspend fun getInstance(): ZodeWorkspaceRpcApi {
            return RemoteApiProviderService.resolve(remoteApiDescriptor<ZodeWorkspaceRpcApi>())
        }
    }
    ...
    suspend fun state(directory: String): Flow<ZodeWorkspaceStateDto>
