...
/**
 * Logging interface for the Zode JetBrains plugin.
 *
 ...
 * system property), output is written only to a `zode-dev.log` file inside the IDE log directory. RC plugin builds
 * write to both IntelliJ's log and `zode-dev.log`.
 *
 * Usage:
 * ```kotlin
 * private val log = ZodeLog.create(this::class.java)
 *
 ...
 * ```
 *
 * The log level for the sandbox file can be controlled via the `zode.dev.log.level` system property
...
interface ZodeLog {
    ...
    companion object {
        fun create(cls: Class<*>): ZodeLog {
            if (sandbox()) return FileLog(cls)
            val intellij = IntellijLog(cls)
            if (!runCatching { ZodePlugin.isRc() }.getOrDefault(false)) return intellij
        ...
        fun payload(log: ZodeLog? = null): Map<String, String> = buildMap {
...
internal class IntellijLog(cls: Class<*>) : ZodeLog {
...
internal class FileLog(cls: Class<*>) : ZodeLog {
    ...
    companion object {
        ...
        private val root: java.util.logging.Logger by lazy {
            val logger = java.util.logging.Logger.getLogger("ai.zodecode")
            val payload = ZodeLog.payload().entries.joinToString(" ") { "${it.key}=${it.value}" }
        ...
        private val handler: FileHandler by lazy {
            ...
            val path = dir.resolve("zode-dev.log")
            val h = FileHandler(path.toString(), true)
            h.formatter = ZodeFormatter()
            h
        }
        ...
        private fun resolveLogDir(): Path {
            ...
            while (current.parent != null) {
                ...
                if (name.startsWith("log_run")) {
                    side = if (name.lowercase().contains("frontend")) "zode-frontend" else "zode-backend"
                }
                if (name == "zode.jetbrains" && side != null) {
        ...
        private fun resolveLevel(): Level {
            val prop = System.getProperty("zode.dev.log.level") ?: return Level.INFO
...
internal class ZodeFormatter : Formatter() {
    ...
    override fun format(record: LogRecord): String {
        ...
        val category = record.sourceClassName ?: record.loggerName ?: "zode.dev"
...
internal class CompositeLog(vararg val delegates: ZodeLog) : ZodeLog {
    override val isDebugEnabled: Boolean
