# AGENTS.md — Zode JetBrains Plugin

## Package Overview

- **Split-mode plugin** with three Gradle modules: `shared/`, `frontend/`, `backend/`. The module descriptors are `zode.jetbrains.shared.xml`, `zode.jetbrains.frontend.xml`, `zode.jetbrains.backend.xml` — these must stay in sync with `plugin.xml`'s `<content>` block.
...
- Kotlin source goes under `{module}/src/main/kotlin/ai/zodecode/jetbrains/`. Package name is `ai.zodecode.jetbrains` (matches `group` in root `build.gradle.kts`).
...
- `plugin.xml` `<content>` entries ↔ module XML descriptors (`zode.jetbrains.{shared,frontend,backend}.xml`)
...
- Non-light services that need XML registration go in `zode.jetbrains.backend.xml` under `<extensions defaultExtensionNs="com.intellij"><applicationService>` (or `<projectService>`).
...
- The plugin spawns `zode serve --port 0` (OS assigns random port) and reads stdout for `listening on http://...:(\d+)` to discover the port.
- A random 32-byte hex password is passed via `ZODE_SERVER_PASSWORD` env var for Basic Auth.
- Fixed env vars set on every spawn: `ZODE_CLIENT=jetbrains`, `ZODE_PLATFORM=jetbrains`, `ZODE_APP_NAME=zode-code`, `ZODE_ENABLE_QUESTION_TOOL=true`, `ZODE_DISABLE_CLAUDE_CODE=true`, `ZODECODE_FEATURE=jetbrains-plugin`.
- Unless already provided by the base environment, the backend sets `ZODE_CONFIG_CONTENT` to make `edit` and `bash` permissions ask by default for JetBrains-launched CLI processes.
- This is the same protocol used by the VS Code extension (`packages/zode-vscode/src/services/cli-backend/server-manager.ts`).

### Dev Storage Isolation

- In development (`runIdeBackend` / `runIde`), the Gradle property `zode.dev.storage.isolated=true` makes the backend set `XDG_DATA_HOME`, `XDG_CONFIG_HOME`, `XDG_STATE_HOME`, and `XDG_CACHE_HOME` to `<worktree>/.zode-dev/{data,config,state,cache}` before spawning the CLI. The worktree root comes from the `zode.dev.worktree.root` JVM system property (auto-set by Gradle from the project directory).
- The checked-in `Run IDE (Backend)` run configuration enables isolation by default (`-Pzode.dev.storage.isolated=true`). Developers can disable it by passing `-Pzode.dev.storage.isolated=false`.
- Use standard `XDG_*_HOME` env vars for this isolation. Do not introduce custom `ZODE_DATA_DIR`, `ZODE_GLOBAL_CONFIG_DIR`, `ZODE_STATE_DIR`, or `ZODE_CACHE_DIR` env vars — the CLI core already respects `XDG_*_HOME` via `xdg-basedir`.
- The `.zode-dev/` directory is gitignored and created automatically on first run.
- The implementation lives in `ZodeBackendCliManager.buildEnv()` / `devStorageEnv()`. Tests: `ZodeBackendCliManagerEnvTest`.

### Debugging Session Event Logs

- Use `script/dev/part-update.sh client <session-id>` from `packages/zode-jetbrains/` to print frontend `message.part.delta` text by part id.
- Use `script/dev/part-update.sh backend <session-id>` from `packages/zode-jetbrains/` for backend sandbox events.
- Append with `>> file.txt` when you need to keep the output.
- For full chat payload previews in JetBrains dev runs, pass `-Pzode.dev.log.chat.content=<mode>` where `<mode>` is `off` (default, no content), `preview` (cleaned/truncated content), or `full` (cleaned full content).
- `-Pzode.dev.log.chat.preview.max=<n>` controls preview length, clamped from 1 to 2000.

## Build and Verification

- **Marketplace version build**: Use `script/build-version.sh <version>` from `packages/zode-jetbrains/` to clean, prepare production CLI binaries, build, sign, and verify the JetBrains Marketplace plugin ZIP. Pass `--skip-verification` only when explicitly needed.
- **Test version build**: If the user asks for a JetBrains test build, still require a version and use `script/build-version.sh <version> --skip-signing --skip-verification` from `packages/zode-jetbrains/` so no signing secrets are needed. Add `--skip-clean` only when the user wants a faster incremental test build.
- **Typecheck**: `bun run typecheck` or `./gradlew typecheck` from `packages/zode-jetbrains/` — compiles all Kotlin sources including the generated API client. Does NOT require CLI binaries.
- **Full build**: `bun run build` from `packages/zode-jetbrains/` (prepares CLI binaries + runs Gradle `buildPlugin`).
- **Gradle only**: `./gradlew buildPlugin` from `packages/zode-jetbrains/` (requires CLI binaries already present in `backend/build/generated/cli/`; run `bun run build --prepare-cli` first).
- **Via Turbo**: `bun turbo build --filter=@zodecode/zode-jetbrains` from repo root.
...
**Do not use Kotlin UI DSL v2 (`com.intellij.ui.dsl.builder`) in this plugin.** Use standard Swing with IntelliJ Platform components for all UI layout. The JetBrains modular template and some older sections of the codebase reference the DSL, but Zode should not introduce it.

**Do not use Kotlin Compose or `intellij.platform.compose` in this plugin.** The JetBrains modular template uses Compose for its demo tool window, but Zode should use standard Swing with IntelliJ Platform components only. Keep all plugin UI in the existing Swing-based stack.
...
**`UiStyle`** (`frontend/src/main/kotlin/ai/zodecode/client/ui/UiStyle.kt`) is the single source of truth for reusable UI constants in this plugin.
...
**`SessionUiStyle`** (`frontend/src/main/kotlin/ai/zodecode/client/session/ui/style/SessionUiStyle.kt`) owns static tokens specific to the chat/session UI:
...
| One-dimensional multi-component rows/columns | `ai.zodecode.client.ui.layout.Stack` — see section below |
| Fluent platform panels | `JBPanel.withBorder(...)`, `.andTransparent()`, `.andOpaque()`, `.withBackground(...)` |
| Single-component alignment wrapper | `ai.zodecode.client.ui.layout.Align` — see section below |

### Stack — One-Dimensional Multi-Component Layout

Use `Stack` (`ai.zodecode.client.ui.layout.Stack`) when multiple Swing components should be laid out as one vertical column or one horizontal row without visual chrome. It is a transparent, no-border, no-color `JPanel(null)` that lays out visible children in insertion order.
...
- Register declaratively in module XML via `com.intellij.toolWindow` extension point (already done in `zode.jetbrains.frontend.xml`).
...
- Declare in module XML: `<notificationGroup id="Zode Code" displayType="BALLOON"/>`.
- Show: `Notification("Zode Code", "message", NotificationType.INFORMATION).notify(project)`.
...
- Tool-window-bound: `displayType="TOOL_WINDOW" toolWindowId="Zode Code"`.
...
`frontend/src/main/kotlin/ai/zodecode/client/session/`.
...
- After history load, `recoverPending()` seeds state in this priority order: (1) pending permission, (2) pending question, (3) current session status (`busy`/`retry`/`offline` from `ZodeSessionService.statuses`), (4) `Idle`.
