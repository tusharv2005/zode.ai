...
class TurnLifecycleTest : SessionControllerTestBase() {
    ...
    fun `test TurnClose error reason does not clobber Error state`() {
        ...
        assertSession(
            """
            [code] [zode/gpt-5] [error] [Timed out]
            """,
            m,
        )
    }
    ...
    fun `test Error fires StateChanged to Error`() {
        ...
        assertSession(
            """
            [code] [zode/gpt-5] [error] [Bad Request]
            """,
            m,
        )
    }
    ...
    fun `test paid model auth error enters login required state`() {
        ...
        assertSession(
            """
            [code] [zode/gpt-5] [login-required] [Go to User Profile settings to sign in, then continue this session.]
            """,
            m,
        )
    }
    ...
    fun `test login clears paid model gate`() {
        ...
        appRpc.state.value = ZodeAppStateDto(
            ...
            config = ConfigDto(model = "zode/gpt-5"),
    ...
    fun `test login resumes paid model prompt`() {
        ...
        assertEquals("zode/openai", prompt.providerID)
