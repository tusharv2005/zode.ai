...
import ai.zodecode.rpc.dto.PermissionFileDiffDto
...
import ai.zodecode.rpc.dto.QuestionInfoDto
import ai.zodecode.rpc.dto.QuestionOptionDto
import ai.zodecode.rpc.dto.QuestionReplyDto
import ai.zodecode.rpc.dto.QuestionRequestDto
import ai.zodecode.rpc.dto.ToolRefDto
...
class PromptLifecycleTest : SessionControllerTestBase() {
    ...
    override fun setUp() {
        super.setUp()
        edt { ZodePluginSettings.unsetAutoApprove() }
    }
    ...
    fun `test PermissionAsked moves state to AwaitingPermission`() {
        ...
        assertSession(
            ...
            [code] [zode/gpt-5] [awaiting-permission]
            """,
            m,
        )
    }
    ...
    fun `test PermissionReplied resumes Busy state`() {
        ...
        assertSession(
            """
            [code] [zode/gpt-5] [busy] [considering next steps]
            """,
            m,
        )
    }
    ...
    fun `test QuestionAsked moves state to AwaitingQuestion`() {
        ...
        assertSession(
            ...
            [code] [zode/gpt-5] [awaiting-question]
            """,
            m,
        )
    }
    ...
    fun `test auto approve drains pending permissions during recovery`() {
        appRpc.state.value = ai.zodecode.rpc.dto.ZodeAppStateDto(ai.zodecode.rpc.dto.ZodeAppStatusDto.READY, config = ai.zodecode.rpc.dto.ConfigDto(model = "zode/gpt-5"))
        projectRpc.state.value = workspaceReady()
        rpc.pendingPermissionList.add(permission("perm_pending"))
        edt { ZodePluginSettings.setAutoApprove(true) }
    ...
    fun `test auto approve persists in properties`() {
        ...
        assertFalse(ZodePluginSettings.getAutoApprove())
        edt { m.setAutoApprove(true) }

        assertTrue(ZodePluginSettings.getAutoApprove())
    ...
    fun `test child non-permission events do not change root state`() {
        ...
        emit(ChatEventDto.SessionStatusChanged("ses_child", ai.zodecode.rpc.dto.SessionStatusDto("busy")), flush = false)
        ...
        val stateEvents = modelEvents.filterIsInstance<ai.zodecode.client.session.model.SessionModelEvent.StateChanged>()
