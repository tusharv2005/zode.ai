...
class SessionModelTest : BasePlatformTestCase() {
    ...
    fun `test initial app and workspace state`() {
        assertEquals(ZodeAppStatusDto.DISCONNECTED, model.app.status)
        assertEquals(ZodeWorkspaceStatusDto.PENDING, model.workspace.status)
    ...
    fun `test isReady requires app and workspace readiness`() {
        model.app = ZodeAppStateDto(ZodeAppStatusDto.READY)
        assertFalse(model.isReady())

        model.workspace = ZodeWorkspaceStateDto(ZodeWorkspaceStatusDto.READY)
