...
import ai.zodecode.client.session.views.PlanExitView
...
import ai.zodecode.client.session.views.TextView
import ai.zodecode.client.session.views.ToolView
...
class SessionMessageListPanelTest : BasePlatformTestCase() {
    ...
    fun `test text markdown link uses panel url opener`() {
        ...
        model.upsertMessage(msg("a1", "assistant"))
        model.updateContent("a1", part("p1", "a1", "text", text = "[docs](https://zodecode.ai/docs)"))

        val view = item.findMessage("a1")!!.part("p1") as TextView
        view.md.simulateLink("https://zodecode.ai/docs")

        assertEquals(listOf("https://zodecode.ai/docs"), urls)
    }
    ...
    fun `test completed plan update replaces tool view and keeps open file action`() {
        ...
        view.simulateLink(".zode/plans/x.md")

        assertEquals(listOf(".zode/plans/x.md"), opened)
    }
