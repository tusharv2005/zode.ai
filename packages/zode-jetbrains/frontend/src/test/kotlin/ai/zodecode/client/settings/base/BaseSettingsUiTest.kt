...
class BaseSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = ZodeAppService(appScope, FakeAppRpcApi())
        workspaces = ZodeWorkspaceService(appScope, FakeWorkspaceRpcApi())
    }
    ...
    fun `test login banner can be shown`() {
        ...
        edt { assertTrue(text(view).contains("Sign in to Zode Code")) }
    }
    ...
    fun `test login banner can be disabled`() {
        ...
        edt { assertFalse(text(view).contains("Sign in to Zode Code")) }
    }
    ...
    private class FakePanel(
        ...
        override fun draft(state: ZodeAppStateDto): Draft = draft
