...
class ZodeSettingsConfigurableTest : BasePlatformTestCase() {

    fun `test id matches xml registration`() {
        val cfg = ZodeSettingsConfigurable()
        assertEquals("ai.zodecode.jetbrains.settings", cfg.id)
    }

    fun `test child profile id matches xml registration`() {
        // Verify the constants used in XML registrations are stable
        assertEquals("ai.zodecode.jetbrains.settings.profile", UserProfileConfigurable.ID)
    }

    fun `test child models id matches xml registration`() {
        assertEquals("ai.zodecode.jetbrains.settings.models", ModelsConfigurable.ID)
    }
    ...
    fun `test root implements SearchableConfigurable but not Parent`() {
        ...
        val interfaces = ZodeSettingsConfigurable::class.java.interfaces
        assertFalse(
            "ZodeSettingsConfigurable must not implement SearchableConfigurable.Parent",
    ...
    fun `test open invokes select with child found by id`() {
        ...
        assertEquals("ai.zodecode.jetbrains.settings.profile", UserProfileConfigurable.ID)
        assertEquals("ai.zodecode.jetbrains.settings.profile", profile.id)
    }

    fun `test isModified always false`() {
        assertFalse(ZodeSettingsConfigurable().isModified)
    }
