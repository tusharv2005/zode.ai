...
class SessionUiUpdateTest : BasePlatformTestCase() {
    ...
    fun `test tool state transitions are reflected in ToolView`() {
        ...
        val tv = panel.findMessage("a1")!!.part("t1") as ai.zodecode.client.session.views.ToolView
    ...
    fun `test read tool renders as ReadToolView`() {
        ...
        assertTrue(tv is ai.zodecode.client.session.views.ReadToolView)
    }
    ...
    fun `test compaction part appears in panel`() {
        ...
        assertTrue(mv.part("cp1") is ai.zodecode.client.session.views.CompactionView)
    }
    ...
    fun `test unknown part type falls back to GenericView`() {
        ...
        val gv = mv.part("g1")
        assertNotNull(gv)
        assertTrue(gv is ai.zodecode.client.session.views.base.GenericView)
        assertTrue((gv as ai.zodecode.client.session.views.base.GenericView).labelText().contains("snapshot"))
