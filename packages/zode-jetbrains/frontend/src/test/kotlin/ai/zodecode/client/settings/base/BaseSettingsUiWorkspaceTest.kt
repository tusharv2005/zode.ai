...
class BaseSettingsUiWorkspaceTest : BasePlatformTestCase() {
    ...
    fun `test non ready app state calls unavailable hook`() {
        ...
        rpc.state.value = ZodeAppStateDto(ZodeAppStatusDto.DISCONNECTED)
    ...
    fun `test model state updates are delivered on edt`() {
        rpc.models = ModelStateDto(favorite = listOf(ModelSelectionDto("zode", "new")))
        rpc.state.value = state("ready")
        val view = create("/test")
    ...
    private fun state(model: String) = ZodeAppStateDto(
    ...
    private class FakePanel(
        cs: CoroutineScope,
        app: ZodeAppService,
        workspaces: ZodeWorkspaceService,
        hint: String,
        ...
        override fun draft(state: ZodeAppStateDto): Draft = Draft(state.config?.model ?: "none")
