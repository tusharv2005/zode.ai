...
import ai.zodecode.rpc.dto.PartTimeDto
...
class SessionHeaderPanelTest : SessionControllerTestBase() {
    ...
    fun `test starts hidden for empty header`() {
        appRpc.state.value = ai.zodecode.rpc.dto.ZodeAppStateDto(ai.zodecode.rpc.dto.ZodeAppStatusDto.READY)
    ...
    fun `test timeline colors honor semantic named color keys`() {
        val old = UIManager.getColor("Zode.Session.Timeline.Read")
        val color = Color(12, 34, 56)

        try {
            UIManager.put("Zode.Session.Timeline.Read", color)

            assertEquals(color.rgb, SessionUiStyle.Timeline.READ.rgb)
        } finally {
            UIManager.put("Zode.Session.Timeline.Read", old)
        }
    }
    ...
    private fun promptedHeader(): ai.zodecode.client.session.controller.SessionController {
        appRpc.state.value = ai.zodecode.rpc.dto.ZodeAppStateDto(
            ai.zodecode.rpc.dto.ZodeAppStatusDto.READY,
            config = ai.zodecode.rpc.dto.ConfigDto(model = "zode/gpt-5"),
        )
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    ...
                    models = mapOf(
                        "gpt-5" to ModelDto(
                            id = "gpt-5",
                            name = "GPT-5",
                            limit = ai.zodecode.rpc.dto.ModelLimitDto(context = 2_000_000, output = 200_000),
                        ),
                    ),
                ),
            ),
