...
class ModelPickerTest : BasePlatformTestCase() {
    ...
    fun `test section order starts with favorites and recommended`() {
        val rows = modelPickerRows(listOf(
            ...
            item("auto", "Zode Auto", "zode", "Zode", 0.0),
        ...
        assertEquals("zode/auto", rows[1].key)
    ...
    fun `test zode provider group is first and other providers keep source order`() {
        val rows = modelPickerRows(listOf(
            item("gpt", "GPT", "openai", "OpenAI"),
            item("auto", "Auto", "zode", "Zode"),
            item("claude", "Claude", "anthropic", "Anthropic"),
        ), emptyList(), "")

        assertEquals(listOf("Zode", "OpenAI", "Anthropic"), rows.indices.mapNotNull { modelPickerSectionTitle(rows, it) })
    }
    ...
    fun `test small rows are included only when requested`() {
        val items = listOf(
            item("auto-small", "Small", "zode", "Zode"),
            item("auto", "Auto", "zode", "Zode"),
        )
        ...
        assertEquals(listOf("zode/auto"), default.mapNotNull { it.key })
        assertEquals(listOf("zode/auto", "zode/auto-small"), small.mapNotNull { it.key }.sorted())
    }
    ...
    fun `test selected free model indicates data collection`() {
        ...
        picker.setItems(listOf(item("auto", "Auto Free", "zode", "Zode", free = true)))
    ...
    fun `test selected non-zode free model does not indicate data collection`() {
        val picker = ModelPicker()
    ...
    fun `test display parts sanitize free suffix`() {
        val parts = ModelText.parts(item("auto", "Auto Free (free)", "zode", "Zode"))
    ...
    fun `test renderer shows empty favorite star only for selected row`() {
        val row = ModelPickerRow(item("auto", "Auto", "zode", "Zode"), "Zode", false)
    ...
    fun `test renderer keeps favorite star visible`() {
        val row = ModelPickerRow(item("auto", "Auto", "zode", "Zode"), "Zode", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { setOf("zode/auto") })
        val list = JBList(model)
    ...
    fun `test renderer shows free badge for free model`() {
        val row = ModelPickerRow(ModelPicker.Item("auto", "Auto", "zode", "Zode", free = true), "Zode", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer hides data collection warning for non-zode free model`() {
