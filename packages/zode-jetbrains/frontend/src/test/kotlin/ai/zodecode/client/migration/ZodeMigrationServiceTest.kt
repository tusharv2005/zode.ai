...
import ai.zodecode.client.testing.FakeMigrationRpcApi
...
class ZodeMigrationServiceTest : BasePlatformTestCase() {

    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeMigrationRpcApi
    private lateinit var service: ZodeMigrationService
    private lateinit var app: MutableStateFlow<ZodeAppStateDto>
    ...
    override fun setUp() {
        ...
        app = MutableStateFlow(ZodeAppStateDto(ZodeAppStatusDto.DISCONNECTED))
        autocomplete.clear()
        service = ZodeMigrationService(scope, rpc, app) { autocomplete.add(it) }
    }
    ...
    fun `test migration required app state shows needed without polling`() {
        app.value = ZodeAppStateDto(ZodeAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
    ...
    fun `test ready app state hides migration`() {
        app.value = ZodeAppStateDto(ZodeAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
        app.value = ZodeAppStateDto(ZodeAppStatusDto.READY)
        settle()
    ...
    fun `test duplicate migration required does not reset running migration`() {
        ...
        app.value = ZodeAppStateDto(ZodeAppStatusDto.MIGRATION_REQUIRED, migration = detection)
        settle()
    ...
    fun `test complete event without items finalizes pending session progress`() = runBlocking {
        app.value = ZodeAppStateDto(ZodeAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection().copy(
