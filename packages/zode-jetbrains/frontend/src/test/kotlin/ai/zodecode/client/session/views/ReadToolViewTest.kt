...
class ReadToolViewTest : BasePlatformTestCase() {
    ...
    fun `test read file output renders filename hyperlink`() {
        ...
        val path = "/Users/kirillk/work/zodecode/.zode/worktrees/agreeable-marlin/packages/zode-jetbrains/frontend/src/test/kotlin/ai/zodecode/client/session/SessionUiLayoutTest.kt"
        val t = tool().also {
            it.output = """
                <path>$path</path>
                <type>file</type>
    ...
    fun `test read directory output remains plain text`() {
        val path = "/Users/kirillk/work/zodecode/packages/zode-jetbrains"
        val t = tool().also {
            it.output = """
                <path>$path</path>
                <type>directory</type>
