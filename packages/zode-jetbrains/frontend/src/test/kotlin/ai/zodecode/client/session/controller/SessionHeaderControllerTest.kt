...
class SessionHeaderControllerTest : SessionControllerTestBase() {
    ...
    fun `test session updated refreshes model metadata`() {
        ...
        assertTrue(modelEvents.any { it is ai.zodecode.client.session.model.SessionModelEvent.SessionUpdated })
    }
    ...
    fun `test compact calls RPC with selected model when eligible`() {
        ...
        assertEquals("zode", call.third.providerID)
    ...
    fun `test compact is blocked without model`() {
        appRpc.state.value = ai.zodecode.rpc.dto.ZodeAppStateDto(ai.zodecode.rpc.dto.ZodeAppStatusDto.READY, config = ai.zodecode.rpc.dto.ConfigDto(model = null))
