...
import ai.zodecode.client.session.history.CloudHistoryItem
import ai.zodecode.client.session.history.HistoryController
import ai.zodecode.client.session.history.HistoryDataKeys
import ai.zodecode.client.session.history.HistorySelection
import ai.zodecode.client.session.history.HistorySource
...
import ai.zodecode.rpc.dto.CloudSessionDto
...
class HistorySessionActionsTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        val workspaces = ZodeWorkspaceService(scope, FakeWorkspaceRpcApi().also {
            it.state.value = ZodeWorkspaceStateDto(status = ZodeWorkspaceStatusDto.READY)
        })
    ...
    fun `test rename action passes untitled fallback to input`() {
        ...
        assertEquals(listOf(ZodeBundle.message("history.untitled")), prompts)
        assertTrue(rpc.renames.isEmpty())
    }
    ...
    fun `test frontend descriptor registers history actions`() {
        val xml = javaClass.classLoader.getResourceAsStream("zode.jetbrains.frontend.xml")
            ?.bufferedReader()
            ?.use { it.readText() }
            ?: error("missing frontend descriptor")

        assertTrue(xml.contains("id=\"Zode.Session.Open\""))
        assertTrue(xml.contains("id=\"Zode.Session.Rename\""))
        assertTrue(xml.contains("id=\"Zode.Session.Delete\""))
        assertTrue(xml.contains("id=\"Zode.History.ContextMenu\""))
        assertTrue(xml.contains("ref=\"Zode.Session.Open\""))
        assertTrue(xml.contains("ref=\"Zode.Session.Rename\""))
        assertTrue(xml.contains("ref=\"Zode.Session.Delete\""))
    }
