...
class ModelsSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = ZodeAppService(appScope, rpc)
        workspaces = ZodeWorkspaceService(appScope, workspaceRpc)
        val state = ZodeAppStateDto(
            ZodeAppStatusDto.READY,
            config = ConfigDto(model = "zode/old"),
    ...
    fun `test matching app state before save callback keeps pending target`() {
        ...
        rpc.state.value = state("zode/new")
    ...
    fun `test stale app state during pending save is ignored`() {
        ...
        rpc.state.value = state("zode/old")
    ...
    fun `test failed save after dispose shows notification`() {
        ...
        flushUntil {
            notes.any { it.groupId == "Zode Code" && it.type == NotificationType.ERROR }
        }
    ...
    fun `test logged out save keeps login banner stable`() {
        ...
        edt { ui = ModelsSettingsUi(uiScope, app, workspaces, directory = "/test") }
        val panel = requireUi()
        flushUntil { text(panel).contains("Old") && text(panel).contains("Sign in to Zode Code") }
        ...
        edt {
            select(panel, "new")
            panel.applyDraft()
            assertTrue(text(panel).contains("Sign in to Zode Code"))
    ...
    private fun state(model: String): ZodeAppStateDto = ZodeAppStateDto(
        ZodeAppStatusDto.READY,
    ...
    private fun select(panel: ModelsSettingsUi, id: String) {
        ...
        picker.onSelect(ModelPicker.Item(id, id.replaceFirstChar { it.titlecase() }, "zode", "Zode"))
    }
