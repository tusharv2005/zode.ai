...
import ai.zodecode.client.session.history.HistoryPanel
...
class SessionSidePanelManagerTest : BasePlatformTestCase() {
    ...
    fun `test inactive sessions keep queued style updates`() {
        ...
        val style = ai.zodecode.client.session.ui.style.SessionEditorStyle.create(family = "Courier New", size = 24)
    ...
    fun `test history overlays update before hidden timeout`() {
        ...
        assertEquals(ZodeBundle.message("history.badge.question"), history.badgeText(0))
    }
    ...
    private fun setInactiveDisposeTimeout(ms: Int) {
        val key = "zode.session.inactive.disposeTimeoutMs"
    ...
    private fun JPanel.controller(): ai.zodecode.client.session.controller.SessionController {
        val field = SessionUi::class.java.getDeclaredField("controller")
        field.isAccessible = true
        return field.get(this) as ai.zodecode.client.session.controller.SessionController
    }
