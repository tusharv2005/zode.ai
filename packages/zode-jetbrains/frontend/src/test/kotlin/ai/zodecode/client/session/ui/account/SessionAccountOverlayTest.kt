...
import ai.zodecode.client.session.controller.SessionControllerEvent.AccountOverlaySnapshot
...
class SessionAccountOverlayTest : SessionControllerTestBase() {
    ...
    private fun snap(profile: ProfileDto?) =
        AccountOverlaySnapshot(status = ZodeAppStatusDto.READY, profile = profile)
    ...
    fun `test transient null profile keeps logged in balance badge`() {
        ...
        show(AccountOverlaySnapshot(status = ZodeAppStatusDto.READY, profile = null, transient = true))
        edt {
            assertTrue(panel.loggedInVisible())
            assertTrue(panel.balanceVisible())
