...
import ai.zodecode.client.settings.profile.ProfileUi
...
class UserProfileConfigurableTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        rpc = FakeAppRpcApi()
        app = ZodeAppService(scope, rpc)
        app._state.value = ZodeAppStateDto(ZodeAppStatusDto.READY)
        edt {
            panel = ProfileUi(
                profile = null,
    ...
    fun `test login updates profile UI`() {
        ...
        edt {
            ...
            buttons(panel).first { it.text == "Login with Zode Code" }.doClick()
        }
        flush()
    ...
    fun `test logout updates profile UI`() {
        ...
        app._state.value = ZodeAppStateDto(ZodeAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, ZodeAppStatusDto.READY) }
        ...
        edt {
            ...
            assertTrue(buttons(panel).any { it.text == "Login with Zode Code" })
        }
    }
    ...
    fun `test organization switch updates balance UI`() {
        ...
        app._state.value = ZodeAppStateDto(ZodeAppStatusDto.READY, profile = personal)
        edt { panel.update(personal, ZodeAppStatusDto.READY) }
    ...
    fun `test logged in profile uses compact stack and copyable email`() {
        ...
        edt {
            ...
            layout(panel)
            val logo = labelsByName(panel, "zode.profile.logo.loggedIn").single()
    ...
    fun `test logged out update retains login button`() {
        edt {
            val btn = buttons(panel).first { it.text == "Login with Zode Code" }
            panel.update(null, ZodeAppStatusDto.READY)
            val btn2 = buttons(panel).first { it.text == "Login with Zode Code" }
            assertSame(btn, btn2)
        }
    }

    fun `test logged out profile shows zode icon above login content`() {
        edt {
            panel.update(null, ZodeAppStatusDto.READY)
            panel.setSize(800, 600)
            layout(panel)

            val logo = labelsByName(panel, "zode.profile.logo.loggedOut").single()
    ...
    fun `test account update retains name label`() {
        ...
        edt {
            panel.update(alice, ZodeAppStatusDto.READY)
            val lbl = labels(panel).first { it.text == "Alice" }
            panel.update(bob, ZodeAppStatusDto.READY)
    ...
    fun `test organization switch keeps account visible during transient null profile`() {
        ...
        val transientState = ZodeAppStateDto(
            status = ZodeAppStatusDto.READY,
            profile = null,
        ...
        edt {
            panel.update(personal, ZodeAppStatusDto.READY)
    ...
    fun `test login shows device auth card before completion`() {
        ...
        edt {
            val qr = labelsByName(panel, "zode.login.qr").firstOrNull()
            assertNotNull(qr)
    ...
    fun `test auth card retains qr label across sync`() {
        ...
        val qrBefore = edt { labelsByName(panel, "zode.login.qr").firstOrNull() }
        assertNotNull(qrBefore)

        // Force another sync call while still pending
        edt { panel.update(null, ZodeAppStatusDto.READY) }
        flush()

        val qrAfter = edt { labelsByName(panel, "zode.login.qr").firstOrNull() }
    ...
    fun `test url field selects all on click`() {
        ...
        edt {
            val field = fieldsByName(panel, "zode.login.url").firstOrNull()
    ...
    fun `test balance card has card background`() {
        ...
        edt {
            val card = panelsByName(panel, "zode.profile.balanceCard").firstOrNull()
    ...
    fun `test code panel has card background`() {
        ...
        edt {
            val codePanel = panelsByName(panel, "zode.login.codePanel").firstOrNull()
    ...
    fun `test combo model not rebuilt on balance change with same org list`() {
        ...
        edt { panel.update(updated, ZodeAppStatusDto.READY) }
    ...
    fun `test combo model updated in place when org list changes`() {
        ...
        app._state.value = ZodeAppStateDto(ZodeAppStatusDto.READY, profile = profile1)
        edt { panel.update(profile1, ZodeAppStatusDto.READY) }
        ...
        edt { panel.update(profile2, ZodeAppStatusDto.READY) }
    ...
    fun `test profile update does not trigger organization rpc`() {
        ...
        app._state.value = ZodeAppStateDto(ZodeAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, ZodeAppStatusDto.READY) }
        edt { panel.update(profile.copy(currentOrgId = "org_1"), ZodeAppStatusDto.READY) }
        flush()
    ...
    fun `test connecting while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, ZodeAppStatusDto.CONNECTING) }
    ...
    fun `test loading while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, ZodeAppStatusDto.LOADING) }
    ...
    fun `test loading with null profile while logged in does not crash`() {
        ...
        edt { panel.update(ZodeAppStateDto(ZodeAppStatusDto.LOADING)) }
        ...
        edt { panel.update(switched, ZodeAppStatusDto.READY) }
    ...
    fun `test connecting while logged in with org selected keeps combo selection`() {
        ...
        app._state.value = ZodeAppStateDto(ZodeAppStatusDto.READY, profile = profile)
        edt {
            panel.update(profile, ZodeAppStatusDto.READY)
            combos(panel).single().selectedIndex = 1
        }
    ...
    fun `test preferred focus for logged-out is login button`() {
        edt {
            val focus = panel.preferredFocus()
            val loginBtn = buttons(panel).firstOrNull { it.text == "Login with Zode Code" }
