...
class ConnectionDelayTest : SessionControllerTestBase() {
    ...
    fun `test short connecting state does not fire connection banner`() {
        ...
        appRpc.state.value = ZodeAppStateDto(ZodeAppStatusDto.CONNECTING)
        pause(25)
        appRpc.state.value = ZodeAppStateDto(ZodeAppStatusDto.READY)
        pause(150)
    ...
    fun `test changed error restarts delay and shows latest state`() {
        ...
        appRpc.state.value = ZodeAppStateDto(ZodeAppStatusDto.ERROR, error = "first")
        pause(50)
        appRpc.state.value = ZodeAppStateDto(ZodeAppStatusDto.ERROR, error = "second")
        pause(150)
    ...
    fun `test persistent workspace error is delayed`() {
        ...
        projectRpc.state.value = ZodeWorkspaceStateDto(
    ...
    fun `test config warning remains immediate`() {
        ...
        assertEquals(".zode/zode.json: Invalid JSON", event.detail)
    }
