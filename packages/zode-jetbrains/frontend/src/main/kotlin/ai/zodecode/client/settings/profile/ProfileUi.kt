...
internal class ProfileUi(
     ...
     * account content) when [ZodeAppStateDto.progress]`.profile` is [ProfileStatusDto.PENDING],
     ...
     */
    @RequiresEdt
    fun update(state: ZodeAppStateDto) {
        checkEdt()
     ...
     */
    @RequiresEdt
    fun update(profile: ProfileDto?, status: ZodeAppStatusDto) {
        checkEdt()
        this.status = status
    ...
    private fun targetCard(): Card {
        ...
        val transientLoad = s == ZodeAppStatusDto.CONNECTING || s == ZodeAppStatusDto.LOADING || s == ZodeAppStatusDto.MIGRATION_REQUIRED
        if (transientLoad && shown == Card.LOGGED_IN) return Card.LOGGED_IN
        return when {
            s == ZodeAppStatusDto.DISCONNECTED || transientLoad -> Card.LOGGED_OUT
            s == ZodeAppStatusDto.ERROR -> Card.LOGGED_OUT
     ...
     * Called from [ai.zodecode.client.settings.profile.UserProfileConfigurable.disposeUIResources].
     */
    @RequiresEdt
    fun dispose() {
        attempt++
...
internal fun compactLoginError(e: Exception): String {
    val msg = e.message?.trim() ?: return ZodeBundle.message("profile.login.failed")
    val lower = msg.lowercase()
    if (HTML_MARKERS.any { lower.contains(it) }) {
        val status = HTTP_STATUS_RE.find(msg)?.groupValues?.getOrNull(1)
        return if (status != null) "${ZodeBundle.message("profile.login.failed")} ($status)"
        else ZodeBundle.message("profile.login.failed")
    }
    val norm = msg.replace(Regex("\\s+"), " ")
    val summary = norm.take(180)
    return if (summary.isNotBlank()) summary else ZodeBundle.message("profile.login.failed")
}
