...
class ZodeWorkspaceService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: ZodeWorkspaceRpcApi?,
) {
    /** Platform constructor — resolves RPC from the service container. */
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = ZodeLog.create(ZodeWorkspaceService::class.java)
        private val INIT = ZodeWorkspaceStateDto(ZodeWorkspaceStatusDto.PENDING)
    }
    ...
    private suspend fun <T> call(block: suspend ZodeWorkspaceRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(ZodeWorkspaceRpcApi.getInstance()) }
    }

    private fun <T> stream(block: suspend ZodeWorkspaceRpcApi.() -> Flow<T>): Flow<T> = flow {
        val api = rpc
        if (api != null) block(api).collect { emit(it) }
        else durable { block(ZodeWorkspaceRpcApi.getInstance()).collect { emit(it) } }
    }
