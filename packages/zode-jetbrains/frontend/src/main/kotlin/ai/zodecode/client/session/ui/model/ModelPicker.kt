package ai.zodecode.client.session.ui.model

import ai.zodecode.client.plugin.ZodeBundle
import ai.zodecode.client.ui.PickerButton
...
class ModelPicker : PickerButton() {
    ...
    var emptyText: String = ZodeBundle.message("settings.models.notSet")
    ...
    init {
        isEnabled = false
        text = " "
        toolTipText = ZodeBundle.message("model.picker.tooltip")
    ...
    private fun refresh() {
        ...
        toolTipText = if (item?.let(ModelText::collectsData) == true) ModelText.dataCollectedTooltip() else ZodeBundle.message("model.picker.tooltip")
        isEnabled = true
    ...
    private fun showPopup() {
        ...
        val list = JBList(model).apply {
            ...
            isFocusable = false
            emptyText.text = ZodeBundle.message("model.picker.no.matches")
        ...
        val search = SearchTextField(false).apply {
            textEditor.emptyText.text = ZodeBundle.message("model.picker.search")
        }
...
internal object ModelText {
    private val small = setOf("zode-auto/small", "auto-small")
    ...
    fun small(item: ModelPicker.Item): Boolean = item.provider == "zode" && item.id in small

    fun providerSort(id: String): Int = if (id == "zode") 0 else 1

    fun dataCollected(): String = ZodeBundle.message("model.picker.dataCollected")

    fun dataCollectedTooltip(): String = XmlStringUtil.wrapInHtmlLines(
        ZodeBundle.message("model.picker.tooltip"),
        ZodeBundle.message("model.picker.dataCollected.current"),
    )

    fun freeLabel(): String = ZodeBundle.message("model.picker.free")

    fun collectsData(item: ModelPicker.Item): Boolean = item.free && item.provider == "zode"

    fun freeBg(): JBColor = JBColor.namedColor("Zode.ModelPicker.freeBadgeBackground", JBColor(0x95D6AC, 0x7FCA99))
}
