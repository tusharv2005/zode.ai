...
import ai.zodecode.client.ui.HoverIcon
import ai.zodecode.client.ui.RoundedContentPanel
...
internal class LoggedOutProfileUi(
    ...
    // -- retained buttons --
    val loginBtn = JButton(ZodeBundle.message("profile.action.login"))
        .also { it.addActionListener { login() } }

    private val retryBtnConnecting = JButton(ZodeBundle.message("profile.action.retry"))
        .also { it.addActionListener { retry() } }

    private val retryBtnError = JButton(ZodeBundle.message("profile.action.retry"))
        .also { it.addActionListener { retry() } }

    private val authRetryBtn = JButton(ZodeBundle.message("profile.login.tryAgain"))
        .also { it.addActionListener { login() } }

    private val cancelBtn = JButton(ZodeBundle.message("profile.login.cancel"))
        .also { it.addActionListener { cancel() } }

    private val openBtn = JButton(ZodeBundle.message("profile.login.openBrowser"))

    private val copyUrlBtn = HoverIcon().apply {
        icon = AllIcons.Actions.Copy
        toolTipText = ZodeBundle.message("profile.login.copyUrl")
    }
    ...
    val qrLabel = JBLabel().apply {
        ...
        name = "zode.login.qr"
        accessibleContext.accessibleName = ZodeBundle.message("profile.login.qr")
        accessibleContext.accessibleDescription = ZodeBundle.message("profile.login.qr.description")
    }

    private val codePanel = RoundedContentPanel(UiStyle.Gap.sm(), UiStyle.Gap.md()).apply {
        name = "zode.login.codePanel"
        addMouseListener(object : MouseAdapter() {
            override fun mouseClicked(e: MouseEvent) {
                val c = rawCode ?: return
                copyToClipboard(c, ZodeBundle.message("profile.login.codeCopied"), this@LoggedOutProfileUi)
            }
        })
    }
    ...
    private val codeHint = JBLabel(ZodeBundle.message("profile.login.clickToCopy")).apply {
        foreground = UiStyle.Colors.weak()
        horizontalAlignment = SwingConstants.CENTER
    }

    private val initiatingIcon = AsyncProcessIcon("ZodeInitiating").also { it.suspend() }

    private val waitIcon = AsyncProcessIcon("ZodeLogin")

    private val waitLabel = JBLabel().apply {
        foreground = UiStyle.Colors.weak()
    }

    private val logoLabel = JBLabel(IconLoader.getIcon("/icons/zode-profile.svg", LoggedOutProfileUi::class.java)).apply {
        name = "zode.profile.logo.loggedOut"
        horizontalAlignment = SwingConstants.CENTER
        accessibleContext.accessibleName = ZodeBundle.message("settings.zode.displayName")
    }
    ...
    private fun connectingCard(): JPanel {
        val p = padded()
        p.add(JBLabel(ZodeBundle.message("profile.status.connecting")).apply {
            foreground = UiStyle.Colors.weak()
            horizontalAlignment = SwingConstants.CENTER
        }, gbc(0))
    ...
    private fun appErrorCard(): JPanel {
        val p = padded()
        p.add(JBLabel(ZodeBundle.message("profile.status.error")).apply {
            foreground = UiStyle.Colors.errorLabelForeground()
            horizontalAlignment = SwingConstants.CENTER
        }, gbc(0))
    ...
    private fun emptyCard(): JPanel {
        ...
        p.add(JBLabel(ZodeBundle.message("profile.notLoggedIn")).apply {
            foreground = UiStyle.Colors.weak()
            horizontalAlignment = SwingConstants.CENTER
    ...
    private fun initiatingCard(): JPanel {
        ...
        val row = JPanel(FlowLayout(FlowLayout.CENTER, UiStyle.Gap.sm(), 0)).apply {
            isOpaque = false
            add(initiatingIcon)
            add(JBLabel(ZodeBundle.message("profile.login.starting")).apply {
                foreground = UiStyle.Colors.weak()
            })
        }
    ...
    private fun authCard(): JPanel {
        ...
        p.add(JBLabel(ZodeBundle.message("profile.login.title")).apply {
            font = UiStyle.Fonts.heading()
            horizontalAlignment = SwingConstants.CENTER
        }, gbc(row++))

        p.add(stepLabel(ZodeBundle.message("profile.login.step.one"), ZodeBundle.message("profile.login.step.url")),
        ...
        val s2 = stepLabel(ZodeBundle.message("profile.login.step.two"), ZodeBundle.message("profile.login.step.code"))
        step2Label = s2
    ...
    @RequiresEdt
    fun update(status: ZodeAppStatusDto, login: LoginState) {
        ...
        if (target == OutMode.AUTH && login is LoginState.Pending) {
            ...
            if (url != lastPendingUrl) {
                ...
                copyUrlBtn.addActionListener {
                    copyToClipboard(url, ZodeBundle.message("profile.login.urlCopied"), copyUrlBtn)
                }
    ...
    private fun resolveMode(status: ZodeAppStatusDto, login: LoginState): OutMode = when {
        status == ZodeAppStatusDto.DISCONNECTED || status == ZodeAppStatusDto.CONNECTING || status == ZodeAppStatusDto.MIGRATION_REQUIRED -> OutMode.CONNECTING
        status == ZodeAppStatusDto.ERROR -> OutMode.APP_ERROR
    ...
    private fun syncTime() {
        ...
        val min = remain / 60
        val sec = remain % 60
        waitLabel.text = ZodeBundle.message("profile.login.waitingTimed", "$min:${sec.toString().padStart(2, '0')}")
    }
