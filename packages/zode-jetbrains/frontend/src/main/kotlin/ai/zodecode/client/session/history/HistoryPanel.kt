...
class HistoryPanel(
    ...
    private val repoOnly = JBCheckBox(ZodeBundle.message("history.cloud.repo.only"), true).apply {
        isVisible = false
    ...
    private val localInfo = TabInfo(localPanel)
        .setText(ZodeBundle.message("history.tab.local"))
        .setForeSideComponent(back())
    private val cloudInfo = TabInfo(cloudPanel)
        .setText(ZodeBundle.message("history.tab.cloud"))
        .setForeSideComponent(back())
    ...
    private fun search(model: HistoryModel<out HistoryItem>) = SearchTextField(false).apply {
        textEditor.emptyText.text = ZodeBundle.message("history.search.placeholder")
    ...
    private fun back(): BorderLayoutPanel {
        val label = ZodeBundle.message("history.back")
    ...
    private fun localList() = JBList(controller.local).apply {
        ...
        cursor = Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)
        emptyText.text = ZodeBundle.message("history.empty")
    ...
    private fun <T : HistoryItem> syncList(list: JBList<T>, model: HistoryModel<T>) {
        ...
        list.emptyText.text = when {
            model.loading -> ZodeBundle.message("history.loading")
            model.error != null -> model.error.orEmpty()
            else -> ZodeBundle.message("history.empty")
        }
    }
    ...
    private fun installContextMenu(list: JBList<out HistoryItem>) {
        val group = ActionManager.getInstance().getAction("Zode.History.ContextMenu")
    ...
    private fun confirm(item: LocalHistoryItem) {
        ...
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            ZodeBundle.message("history.delete.confirm.message", title(item)),
    ...
    internal fun confirmDelete(items: List<LocalHistoryItem>) {
        ...
        val msg = if (active.size == 1)
            ZodeBundle.message("history.delete.confirm.message", title(active[0]))
        else
            ZodeBundle.message("history.delete.confirm.message.multiple", active.size)
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            msg,
            ZodeBundle.message("history.delete.confirm.title"),
    ...
    private class LoadMoreButton : JButton(ZodeBundle.message("history.cloud.load.more")) {
