package ai.zodecode.client.settings.base
...
internal abstract class BaseSettingsUi<C : BaseContentPanel, D, P, R, W>(
    ...
    protected var appState: ZodeAppStateDto = app.state.value
        private set
    ...
    @RequiresEdt
    private fun updateApp(state: ZodeAppStateDto) {
        appState = state
        if (state.status != ZodeAppStatusDto.READY) {
    ...
    private fun load() {
        ...
        if (appState.status != ZodeAppStatusDto.READY || workspaceLoading || workspaceLoaded) return
    ...
    @RequiresEdt
    protected abstract fun draft(state: ZodeAppStateDto): D
    ...
    @RequiresEdt
    protected open fun onSaveFailedAfterDispose(change: P) = ZodeNotifications.error(failedText())
    ...
    @RequiresEdt
    protected open fun unavailable(state: ZodeAppStateDto) = Unit
