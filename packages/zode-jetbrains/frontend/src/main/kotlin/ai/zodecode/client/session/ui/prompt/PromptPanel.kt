...
import ai.zodecode.client.actions.SendPromptAction
import ai.zodecode.client.actions.StopSessionAction
...
import ai.zodecode.client.ui.iconButton
...
class PromptPanel(
    ...
    companion object {
        private val LOG = ZodeLog.create(PromptPanel::class.java)
    ...
    private val reset = HoverIcon().apply {
        ...
        toolTipText = ZodeBundle.message("model.picker.reset")
        accessibleContext.accessibleName = ZodeBundle.message("model.picker.reset")
        isVisible = false
    ...
    private fun syncAutoApprove() {
        ...
        auto.toolTipText = if (autoApprove) {
            ZodeBundle.message("prompt.action.autoApprove.enabled.tooltip")
        } else {
            ZodeBundle.message("prompt.action.autoApprove.disabled.tooltip")
        }
        auto.accessibleContext.accessibleName = if (autoApprove) {
            ZodeBundle.message("prompt.action.autoApprove.disable")
        } else {
            ZodeBundle.message("prompt.action.autoApprove.enable")
        }
        auto.repaint()
    }
    ...
    private fun tooltip(): String {
        val id = if (busy) StopSessionAction.ID else SendPromptAction.ID
        val text = if (busy) {
            ZodeBundle.message("prompt.button.stop")
        } else {
            ZodeBundle.message("prompt.button.send")
        }
        ...
        return XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(tip) + "<br>" +
                XmlStringUtil.escapeString(ZodeBundle.message("prompt.button.send.tooltip.stop", stop))
        )
    }
    ...
    private fun placeholder(): String {
        ...
        if (send.isNotEmpty() && line.isNotEmpty()) {
            return ZodeBundle.message("prompt.placeholder.with.shortcuts", send, line)
        }
        if (send.isNotEmpty()) return ZodeBundle.message("prompt.placeholder.with.send", send)
        if (line.isNotEmpty()) return ZodeBundle.message("prompt.placeholder.with.newline", line)
        return ZodeBundle.message("prompt.placeholder")
    }
