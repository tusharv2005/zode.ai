package ai.zodecode.client.session.views.question
...
class QuestionView(
    ...
    private val back = HoverIcon().apply {
        ...
        toolTipText = ZodeBundle.message("session.question.back")
    ...
    private val fwd = HoverIcon().apply {
        ...
        icon = ico
        disabledIcon = IconLoader.getDisabledIcon(ico)
        toolTipText = ZodeBundle.message("session.question.next")
    ...
    private fun syncPage() {
        ...
        body.removeAll()
        if (review(q)) {
            card.setHeader(ZodeBundle.message("session.question.review.title"))
            addReview(q)
        } else {
            val item = q.items[idx]
            val hint = ZodeBundle.message(
    ...
    private fun syncHeader(q: Question) {
        ...
        summary.text = ZodeBundle.message("session.question.summary", shown, total)
    ...
    private fun syncFooter(q: Question) {
        ...
        actions.add(BaseQuestionView.Action(ID_DISMISS, ZodeBundle.message("session.question.dismiss"), primary = false) { doReject() })

        if (review(q)) {
            actions.add(BaseQuestionView.Action(ID_BACK, ZodeBundle.message("session.question.back"), primary = false) { goBack() })
            actions.add(BaseQuestionView.Action(ID_MAIN, ZodeBundle.message("session.question.submit"), primary = true) { doReply() })
        } else {
            val label = when {
                direct(q) -> ZodeBundle.message("session.question.submit")
                lastItem(q) -> ZodeBundle.message("session.question.review")
                else -> ZodeBundle.message("session.question.next")
            }
    ...
    private fun reviewRow(item: QuestionItem, i: Int): JPanel {
        ...
        val answer = text(
            joined.ifBlank { ZodeBundle.message("session.question.review.notAnswered") },
            UiStyle.Colors.fg(),
            true,
        )
    ...
    private fun customRow(item: QuestionItem, set: MutableSet<String>): JPanel {
        ...
        val label = text(ZodeBundle.message("session.question.custom.label"), UiStyle.Colors.fg(), true)
    ...
    private fun buildCustomEditor(): SessionEditorTextField {
        ...
        ed.setPlaceholder(ZodeBundle.message("session.question.custom.placeholder"))
