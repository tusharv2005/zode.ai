...
import ai.zodecode.client.session.views.base.SecondarySessionPartView
...
private fun title(tool: Tool) = when (tool.name) {
    "read" -> ZodeBundle.message("session.part.tool.read")
    "bash" -> ZodeBundle.message("session.part.tool.shell")
    else -> toolTitle(tool)
}
...
private fun stateText(tool: Tool) = when (tool.state) {
    ToolExecState.PENDING -> ZodeBundle.message("session.part.tool.pending")
    ToolExecState.RUNNING -> ZodeBundle.message("session.part.tool.running")
    ToolExecState.COMPLETED -> ""
    ToolExecState.ERROR -> ZodeBundle.message("session.part.tool.error")
}
...
private class Preview {
    ...
    fun build(): String {
        ...
        text.append(ZodeBundle.message("session.part.tool.truncated"))
