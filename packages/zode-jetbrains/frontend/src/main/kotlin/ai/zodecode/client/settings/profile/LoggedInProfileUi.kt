...
internal class LoggedInProfileUi(
    ...
    companion object {
        private val LOG = ZodeLog.create(LoggedInProfileUi::class.java)
    }
    ...
    private val logoLabel = JBLabel(IconLoader.getIcon("/icons/zode-profile.svg", LoggedInProfileUi::class.java)).apply {
        name = "zode.profile.logo.loggedIn"
        accessibleContext.accessibleName = ZodeBundle.message("settings.zode.displayName")
    }

    private val titleLabel = JBLabel(ZodeBundle.message("profile.balance.title")).apply {
        foreground = UiStyle.Colors.weak()
    }
    ...
    private val refreshBtn = JButton(ZodeBundle.message("profile.action.refresh"), AllIcons.Actions.Refresh)
        .also {
            it.isOpaque = false
    ...
    val dashboardBtn = JButton(ZodeBundle.message("profile.action.dashboard"))
        .also { it.addActionListener { dashboard() } }
    val logoutBtn = JButton(ZodeBundle.message("profile.action.logout"))
    ...
    fun setRefreshing(refreshing: Boolean) {
        ...
        val text = if (refreshing) ZodeBundle.message("profile.action.refreshing")
        else ZodeBundle.message("profile.action.refresh")
    ...
    private fun applyOrganizations(profile: ProfileDto) {
        ...
        val keys: List<Pair<String?, String>> = listOf(null to ZodeBundle.message("profile.personalAccount")) +
