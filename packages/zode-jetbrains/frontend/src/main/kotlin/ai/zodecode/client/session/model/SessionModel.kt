package ai.zodecode.client.session.model
 ...
 *
 * **EDT-only access** — no synchronization. [ai.zodecode.client.session.controller.SessionController] guarantees all
...
class SessionModel {
    ...
    var app: ZodeAppStateDto = ZodeAppStateDto(ZodeAppStatusDto.DISCONNECTED)
    var version: String? = null

    var workspace: ZodeWorkspaceStateDto = ZodeWorkspaceStateDto(ZodeWorkspaceStatusDto.PENDING)
    ...
    @RequiresEdt
    fun isReady(): Boolean = app.status == ZodeAppStatusDto.READY && workspace.status == ZodeWorkspaceStatusDto.READY
...
private fun ai.zodecode.rpc.dto.PartTimeDto.durationMs(): Long? {
