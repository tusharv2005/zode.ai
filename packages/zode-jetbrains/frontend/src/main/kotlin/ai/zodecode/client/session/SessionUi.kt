...
import ai.zodecode.client.migration.ZodeMigrationService
import ai.zodecode.client.migration.MigrationUiController
import ai.zodecode.client.migration.MigrationUiState
import ai.zodecode.client.migration.ui.MigrationOverlayPanel
import ai.zodecode.client.session.model.SessionModelEvent
import ai.zodecode.client.session.model.SessionState
import ai.zodecode.client.session.scroll.SessionScroll
import ai.zodecode.client.session.ui.ConnectionPanel
import ai.zodecode.client.session.ui.empty.EmptySessionPanel
import ai.zodecode.client.session.ui.LoadingPanel
import ai.zodecode.client.session.ui.ReasoningPicker
import ai.zodecode.client.session.ui.mode.ModePicker
import ai.zodecode.client.session.ui.model.ModelPicker
import ai.zodecode.client.session.ui.prompt.PromptPanel
import ai.zodecode.client.session.ui.account.SessionAccountOverlay
import ai.zodecode.client.session.ui.SessionRootPanel
import ai.zodecode.client.session.ui.SessionMessageListPanel
import ai.zodecode.client.session.ui.header.SessionHeaderPanel
...
import ai.zodecode.client.session.controller.EVENT_FLUSH_MS
...
import ai.zodecode.client.session.views.LoginRequiredView
import ai.zodecode.client.session.views.permission.PermissionView
import ai.zodecode.client.session.views.question.QuestionView
...
class SessionUi(
    ...
    private val migration: MigrationUiController = service<ZodeMigrationService>(),
) : JPanel(BorderLayout()), Disposable, SessionEditorStyleTarget, UiDataProvider {

    companion object {
        private val LOG = ZodeLog.create(SessionUi::class.java)
    }
    ...
    private val flushMs =
        Registry.intValue("zode.session.flushMs", EVENT_FLUSH_MS.toInt())
            .takeIf { it > 0 }
            ?.toLong()
            ?: EVENT_FLUSH_MS
    ...
    private val controller = SessionController(
        ...
        comp = this,
        flushMs = flushMs,
        condense = Registry.`is`("zode.session.condense", true),
        displayMs = displayMs,
...
private fun ai.zodecode.client.session.model.QuestionItem.planFollowup() =
