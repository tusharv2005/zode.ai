...
import ai.zodecode.client.settings.profile.formatBalance
...
internal class SessionAccountOverlay(
    ...
    private val profileBtn = HoverIcon().apply {
        ...
        toolTipText = ZodeBundle.message("action.Zode.ShowProfile.description")
        accessibleContext.accessibleName = ZodeBundle.message("action.Zode.ShowProfile.text")
    ...
    @RequiresEdt
    private fun updateLoggedIn(prof: ai.zodecode.rpc.dto.ProfileDto, switching: Boolean, target: String?): Boolean {
        var layout = false

        val orgs = prof.organizations
        val next = listOf(AccountChoice(null, ZodeBundle.message("profile.personalAccount"))) +
        ...
        val tip = if (switching) {
            ZodeBundle.message("profile.switchingAccount")
        } else {
            ZodeBundle.message("session.account.switcher")
        }
    ...
    @RequiresEdt
    private fun syncBalance(prof: ai.zodecode.rpc.dto.ProfileDto): Boolean {
        var layout = false
        ...
        if (next == null) {
            ...
            val tip = ZodeBundle.message("session.account.balance", next)
