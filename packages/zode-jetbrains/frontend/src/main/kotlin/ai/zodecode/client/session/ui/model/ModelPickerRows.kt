...
internal fun modelPickerRows(
    ...
    emptyText: String = ZodeBundle.message("settings.models.notSet"),
    ...
    if (q.isBlank()) {
        ...
        if (fav.isNotEmpty()) {
            val section = ZodeBundle.message("model.picker.favorites")
            out += fav.map { ModelPickerRow(it, section, favorite = true) }
        }
    }
    if (recommended.isNotEmpty()) {
        val section = ZodeBundle.message("model.picker.recommended")
