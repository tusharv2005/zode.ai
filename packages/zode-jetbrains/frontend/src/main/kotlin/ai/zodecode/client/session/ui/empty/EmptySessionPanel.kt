...
class EmptySessionPanel(
    ...
    init {
        ...
        val logo = JBLabel(
            IconLoader.getIcon("/icons/zode-content.svg", EmptySessionPanel::class.java),
        ).apply {
            horizontalAlignment = JBLabel.CENTER
        }
    ...
    internal fun feedbackText() = ZodeBundle.message("feedback.button")
    ...
    internal fun explanationText() = ZodeBundle.message("session.empty.welcome")
    ...
    internal open class ShowHistoryButton(
        text: String = ZodeBundle.message("session.showHistory"),
    ...
    private fun welcomeHtml() = XmlStringUtil.wrapInHtml(
        "<div style='text-align:center'>${XmlStringUtil.escapeString(ZodeBundle.message("session.empty.welcome"))}</div>"
    )
