...
internal object HistoryTime {
    ...
    fun title(section: HistorySection): String = when (section) {
        HistorySection.TODAY -> ZodeBundle.message("history.group.today")
        HistorySection.YESTERDAY -> ZodeBundle.message("history.group.yesterday")
        HistorySection.WEEK -> ZodeBundle.message("history.group.week")
        HistorySection.MONTH -> ZodeBundle.message("history.group.month")
        HistorySection.OLDER -> ZodeBundle.message("history.group.older")
    }
    ...
    fun relative(item: HistoryItem, now: Long = System.currentTimeMillis()): String {
        ...
        if (diff < MINUTE) return ZodeBundle.message("history.time.moments")
        if (diff < HOUR) return ZodeBundle.message("history.time.minutes", (diff / MINUTE).coerceAtLeast(1))
        if (diff < DAY) return ZodeBundle.message("history.time.hours", (diff / HOUR).coerceAtLeast(1))
        if (diff < 7 * DAY) return ZodeBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        ...
        if (months < 1) return ZodeBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        if (months < 12) return ZodeBundle.message("history.time.months", months)
        return ZodeBundle.message("history.time.years", months / 12)
    }
