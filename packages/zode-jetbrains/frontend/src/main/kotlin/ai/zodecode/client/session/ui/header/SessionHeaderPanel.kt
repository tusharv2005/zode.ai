package ai.zodecode.client.session.ui.header

import ai.zodecode.client.plugin.ZodeBundle
import ai.zodecode.client.session.model.SessionHeaderSnapshot
...
import ai.zodecode.client.session.views.todo.TodoListPanel
import ai.zodecode.client.ui.HoverIcon
import ai.zodecode.client.ui.UiStyle
import ai.zodecode.rpc.dto.TodoDto
import ai.zodecode.rpc.dto.TokensDto
...
class SessionHeaderPanel(
    ...
    companion object {
        ...
        internal const val EXPANDED_KEY = "zode.session.header.expanded"
    }
    ...
    private val compact = HoverIcon().apply {
        icon = COMPRESS_ICON
        toolTipText = ZodeBundle.message("session.header.compact.description")
        accessibleContext.accessibleName = ZodeBundle.message("session.header.compact")
        addActionListener { controller.compact() }
    }
    private val expand = HoverIcon().apply {
        icon = CHEVRON_ICON
        toolTipText = ZodeBundle.message("session.header.expand")
        accessibleContext.accessibleName = ZodeBundle.message("session.header.expand")
    ...
    private val tokenTitle = JBLabel(ZodeBundle.message("session.header.tokens"))
    ...
    private val todoRow = JPanel(FlowLayout(FlowLayout.LEFT, UiStyle.Gap.sm(), 0)).apply {
        ...
        toolTipText = ZodeBundle.message("session.header.todos.toggle")
        accessibleContext.accessibleName = ZodeBundle.message("session.header.todos.toggle")
        add(todoArrow)
        add(todos)
    }
    ...
    private fun setTokens(value: TokensDto?) {
        ...
        tokenTitle.text = ZodeBundle.message("session.header.tokens")
        tokens.toolTipText = ZodeBundle.message("session.header.tokens.description")
        tokenTitle.isVisible = total > 0
        set(input, if (sent > 0) num(sent) else null)
        set(output, if (received > 0) num(received) else null)
        set(cacheWrite, if (write > 0) ZodeBundle.message("session.header.cache.write", num(write)) else null)
        set(cacheRead, if (read > 0) ZodeBundle.message("session.header.cache.read", num(read)) else null)
    ...
    private fun setExpand(expanded: Boolean) {
        ...
        expand.toolTipText = ZodeBundle.message(key)
        expand.accessibleContext.accessibleName = ZodeBundle.message(key)
    }
