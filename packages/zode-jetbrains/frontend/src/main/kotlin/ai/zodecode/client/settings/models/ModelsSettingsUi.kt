...
import ai.zodecode.client.session.ui.ReasoningPicker
import ai.zodecode.client.session.ui.model.ModelPicker
import ai.zodecode.client.session.ui.model.ModelText
import ai.zodecode.client.settings.base.BaseContentPanel
import ai.zodecode.client.settings.base.BaseSettingsUi
import ai.zodecode.client.settings.base.SettingsBannerKind
import ai.zodecode.client.settings.base.SettingsRow
import ai.zodecode.client.settings.base.SettingsRows
...
internal class ModelsSettingsUi(
    cs: CoroutineScope,
    private val app: ZodeAppService = service(),
    private val workspaces: ZodeWorkspaceService = service(),
    directory: String? = null,
) : BaseSettingsUi<ModelsSettingsContent, ModelsDraft, ConfigPatchDto, ZodeAppStateDto, ModelsWorkspaceDto>(
    cs,
    ModelsDraft(),
    app,
    workspaces,
    ...
    companion object {
        private val LOG = ZodeLog.create(ModelsSettingsUi::class.java)
    }
    ...
    override fun save(change: ConfigPatchDto, done: (ZodeAppStateDto?) -> Unit) {
        app.updateConfigAsync(change, done)
    }

    override fun base(result: ZodeAppStateDto): ModelsDraft = modelsDraft(result.config, agents)

    override fun draft(state: ZodeAppStateDto): ModelsDraft = modelsDraft(state.config, agents)

    override fun saved(base: ModelsDraft, draft: ModelsDraft): Boolean = savedMatches(base, draft)

    override fun pendingText(): String = ZodeBundle.message("settings.models.save.pending")

    override fun failedText(): String = ZodeBundle.message("settings.models.save.failed")
    ...
    override fun unavailable(state: ZodeAppStateDto) {
    ...
    override fun syncContent() {
        ...
        val state = modelsStatus(
            ready = appState.status == ZodeAppStatusDto.READY && hasProjectDirectory,
            loading = workspaceLoading || (appState.status == ZodeAppStatusDto.READY && !workspaceLoaded && hasProjectDirectory),
            providers = providers,
            items = allItems.size,
            errors = errors,
            saving = saving,
        ...
        syncLoginBanner(login) {
            ...
            when (state) {
                ModelsStatus.LOAD_FAILED -> top.showBanner(
                    ZodeBundle.message("settings.models.load.failed"),
                    emptyList(),
                    SettingsBannerKind.ERROR,
                )
                ModelsStatus.NO_PROVIDERS -> top.showBanner(ZodeBundle.message("settings.models.noProviders"), emptyList())
                ModelsStatus.MODES_FAILED -> top.showBanner(ZodeBundle.message("settings.models.modes.failed"), emptyList())
                else -> top.hideBanner()
            }
        }
    }
    ...
    private fun items(includeSmall: Boolean): List<ModelPicker.Item> {
        ...
        return cfg.providers
            .filter { it.id == ZODE_PROVIDER || it.id in cfg.connected }
            .flatMap { provider ->
...
internal class ModelsSettingsContent(
    ...
    val variantRow = SettingsRow(
        ZodeBundle.message("settings.models.subagentVariant.title"),
        ZodeBundle.message("settings.models.subagentVariant.description"),
    ...
    init {
        ...
        val rows = section(ZodeBundle.message("settings.models.displayName"))
        rows.row(SettingsRow(
            ZodeBundle.message("settings.models.defaultModel.title"),
            ZodeBundle.message("settings.models.defaultModel.description"),
            defaults,
        ))
        rows.row(SettingsRow(
            ZodeBundle.message("settings.models.smallModel.title"),
            ZodeBundle.message("settings.models.smallModel.description"),
            small,
        ))
        rows.row(SettingsRow(
            ZodeBundle.message("settings.models.subagentModel.title"),
            ZodeBundle.message("settings.models.subagentModel.description"),
            subagent,
        ))
        rows.row(variantRow)
        modes = section(
            ZodeBundle.message("settings.models.modeModels.title"),
            ZodeBundle.message("settings.models.modeModels.description"),
        )
    }
}
