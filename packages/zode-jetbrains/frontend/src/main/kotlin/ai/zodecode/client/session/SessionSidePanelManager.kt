...
class SessionSidePanelManager(
    ...
    private val resolve: (String) -> Workspace = { dir -> service<ZodeWorkspaceService>().workspace(dir) },
    private val status: () -> Map<String, SessionActivityKind> = { project.service<ZodeSessionService>().activity() },
    ...
    private fun schedule(ui: SessionUi) {
        cancel(ui)
        val delay = Registry.intValue("zode.session.inactive.disposeTimeoutMs").coerceAtLeast(0)
