...
import ai.zodecode.client.session.model.AgentItem
import ai.zodecode.client.session.model.ModelLimitItem
import ai.zodecode.client.session.model.ModelItem
import ai.zodecode.client.session.model.SessionModel
...
import ai.zodecode.client.session.model.PermissionMeta
import ai.zodecode.client.session.model.PermissionRequestState
import ai.zodecode.client.session.model.Question
import ai.zodecode.client.session.model.QuestionItem
import ai.zodecode.client.session.model.QuestionOption
import ai.zodecode.client.session.model.Reasoning
import ai.zodecode.client.session.model.ToolCallRef
import ai.zodecode.client.session.model.Text
import ai.zodecode.client.plugin.ZodePluginSettings
import ai.zodecode.client.session.SessionRef
...
import ai.zodecode.rpc.dto.PartDto
...
import ai.zodecode.rpc.dto.MessageDto
...
class SessionController(
  parent: Disposable,
  ref: SessionRef? = null,
  private val sessions: ZodeSessionService,
  private val workspace: Workspace,
  private val app: ZodeAppService,
  ...
  private val telemetry: (String, Map<String, String>) -> Unit = { event, props -> Telemetry.send(event, props) },
    ...
    companion object {
        private val LOG = ZodeLog.create(SessionController::class.java)
    ...
    val autoApprove: Boolean get() = ZodePluginSettings.getAutoApprove()
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    val msg = e.message ?: ZodeBundle.message("session.error.prompt")
                    updateModel {
    ...
    fun setAutoApprove(value: Boolean) {
        assertEdt()
        ZodePluginSettings.setAutoApprove(value)
    ...
    fun compact() {
        ...
        cs.launch {
            try {
                ...
                edt {
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: ZodeBundle.message("session.error.compact")))
                    }
                }
            }
        }
    ...
    fun retryConnection() {
        ...
        if (model.app.status != ZodeAppStatusDto.READY || model.app.status == ZodeAppStatusDto.ERROR) {
            app.retryAsync()
            return
        }
        ...
        if (model.workspace.status == ZodeWorkspaceStatusDto.ERROR) {
            workspace.reload()
        }
    }
            ...
            try {
                ...
                edt {
                    updatePermission(
                        ...
                        e.message ?: ZodeBundle.message("session.permission.error"),
                    )
                }
            }
        }
    ...
    private fun approve(id: String, restore: () -> Permission) {
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    model.setState(SessionState.Busy(ZodeBundle.message("session.status.considering")))
                }
                ...
                edt {
                    ...
                    model.setState(SessionState.AwaitingPermission(restore().copy(
                        ...
                        message = e.message ?: ZodeBundle.message("session.permission.error"),
                    )))
                }
            }
        }
    ...
    init {
        ...
        cs.launch {
            app.state.collect { state ->
                if (state.status == ZodeAppStatusDto.READY) app.fetchVersionAsync()
        ...
        cs.launch {
            workspace.state.collect { state ->
                fire(SessionControllerEvent.WorkspaceChanged) {
                    ...
                    if (state.status != ZodeWorkspaceStatusDto.READY) return@fire
                    ...
                    model.models = state.providers?.let { providers ->
                        providers.providers
                            .filter { it.id == ZODE_PROVIDER || it.id in providers.connected }
                            .flatMap { provider ->
                ...
                if (state.status == ZodeWorkspaceStatusDto.READY) {
                    fire(SessionControllerEvent.WorkspaceReady)
                    edt {
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: ZodeBundle.message("history.error.local")))
                    }
                    showSession()
                    loaded(false)
                }
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: ZodeBundle.message("history.error.cloud")))
                    }
                    showSession()
                    loaded(false)
                }
    ...
    private fun seedStatus(dto: SessionStatusDto) {
        ...
        val state = when (dto.type) {
            "busy" -> SessionState.Busy(ZodeBundle.message("session.status.considering"))
    ...
    private fun error(event: ChatEventDto.Error, reveal: Boolean) {
        ...
        if (isPaidModelAuthRequired(event.error)) {
            ...
            model.setState(SessionState.LoginRequired(ZodeBundle.message("session.login.required.description")))
            return
        }
        val msg = event.error?.message ?: event.error?.type ?: ZodeBundle.message("session.error.unknown")
        ...
        val state = when (dto.type) {
            ...
            "busy" -> {
                ...
                if (current is SessionState.Idle || current is SessionState.Error)
                    SessionState.Busy(ZodeBundle.message("session.status.considering"))
    ...
    private fun configModel(agent: String): String? {
        if (model.app.status != ZodeAppStatusDto.READY) return null
        val cfg = model.app.config
    ...
    private fun status(): String = when (partType) {
        "reasoning" -> ZodeBundle.message("session.status.thinking")
        "text" -> ZodeBundle.message("session.status.writing")
        "tool" -> when (tool) {
            "task" -> ZodeBundle.message("session.status.delegating")
            "todowrite", "todoread" -> ZodeBundle.message("session.status.planning")
            "read" -> ZodeBundle.message("session.status.gathering")
            "glob", "grep", "list" -> ZodeBundle.message("session.status.searching.codebase")
            "webfetch", "websearch", "codesearch" -> ZodeBundle.message("session.status.searching.web")
            "edit", "write" -> ZodeBundle.message("session.status.editing")
            "bash" -> ZodeBundle.message("session.status.commands")
            else -> ZodeBundle.message("session.status.considering")
        }
        else -> ZodeBundle.message("session.status.considering")
    }
    ...
    private fun resolveConnectionState(): SessionControllerEvent.ConnectionChanged {
        ...
        if (app.status == ZodeAppStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                ZodeBundle.message("session.connection.error.app"),
                app.errors.toErrorText() ?: app.error,
            )
        }

        if (workspace.status == ZodeWorkspaceStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                ZodeBundle.message("session.connection.error.workspace"),
                workspace.errors.toErrorText() ?: workspace.error,
                "workspace",
            )
        }

        if (app.status == ZodeAppStatusDto.READY && workspace.status == ZodeWorkspaceStatusDto.READY && app.warnings.isNotEmpty()) {
        ...
        if (app.status == ZodeAppStatusDto.READY && workspace.status == ZodeWorkspaceStatusDto.READY) {
...
private fun summary(count: Int): String {
    val base = ZodeBundle.message("session.connection.warning.config")
...
private const val ZODE_PROVIDER = "zode"
private const val ZODE_AUTO_MODEL = "zode-auto/free"
...
private fun resolveModelSelection(
    ...
    fallback: ModelSelectionDto? = ModelSelectionDto(ZODE_PROVIDER, ZODE_AUTO_MODEL),
): ModelSelectionDto? {
...
private fun valid(providers: ProvidersDto?, item: ModelSelectionDto?): ModelSelectionDto? {
    ...
    if (item.providerID != ZODE_PROVIDER && item.providerID !in providers.connected) return null
