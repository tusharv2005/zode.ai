...
import ai.zodecode.backend.workspace.ZodeBackendWorkspace
...
class ZodeBackendWorkspaceTest {
    ...
    private fun setup(): ZodeBackendAppService =
        ZodeBackendAppService.create(scope, FakeCliServer(mock), log)

    private suspend fun ready(app: ZodeBackendAppService): ZodeBackendWorkspace {
        app.connect()
        withTimeout(10_000) {
            app.appState.first { it is ZodeAppState.Ready }
        }
    ...
    fun `get creates workspace on demand after Ready`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is ZodeWorkspaceState.Ready }
        }

        val state = ws.state.value as ZodeWorkspaceState.Ready
    ...
    fun `same directory returns same workspace instance`() = runBlocking {
        val app = setup()
        app.connect()
        withTimeout(10_000) { app.appState.first { it is ZodeAppState.Ready } }
    ...
    fun `workspaces stopped on app disconnect`() = runBlocking {
        ...
        assertIs<ZodeWorkspaceState.Pending>(ws.state.value)
    ...
    fun `workspace reaches Ready after creation`() = runBlocking {
        ...
        assertIs<ZodeWorkspaceState.Ready>(ws.state.value)
    }
    ...
    fun `providers failure retries then transitions to Error`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is ZodeWorkspaceState.Error }
        }

        val err = ws.state.value as ZodeWorkspaceState.Error
    ...
    // ------ Data mapping ------
    // Detailed provider/command/path parsing correctness is covered in ZodeCliDataParserTest.
    ...
    fun `concurrent load calls on workspace produce valid final state`() = runBlocking {
        ...
        withTimeout(15_000) {
            while (true) {
                ...
                if (state is ZodeWorkspaceState.Ready) {
                    delay(300)
                    if (ws.state.value is ZodeWorkspaceState.Ready) break
                }
                delay(100)
            }
        }
        ...
        withTimeout(15_000) {
            ...
            while (true) {
                ...
                if (state is ZodeAppState.Ready) {
                    delay(300)
                    if (app.appState.value is ZodeAppState.Ready) break
                }
                delay(100)
            }
        }
