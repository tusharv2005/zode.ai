...
import ai.zodecode.backend.app.ConnectionState
import ai.zodecode.backend.app.ZodeConnectionService
...
class ZodeConnectionServiceTest {
    ...
    fun `connect provides API client`() = runBlocking {
        val svc = ZodeConnectionService(scope, fake, {}, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    fun `SSE concurrent callbacks preserve callback order`() = runBlocking {
        ...
        val svc = ZodeConnectionService(scope, fake, {}, blocked)
        val field = ZodeConnectionService::class.java.getDeclaredField("listener")
        ...
        val sourceField = ZodeConnectionService::class.java.getDeclaredField("source")
    ...
    fun `init error transitions to Error state`() = runBlocking {
        ...
        val svc = ZodeConnectionService(scope, failing, {}, log)
        svc.connect()
    ...
    // extractType tests moved to ZodeCliDataParserTest
    ...
    private class BlockingLog : ZodeLog {
