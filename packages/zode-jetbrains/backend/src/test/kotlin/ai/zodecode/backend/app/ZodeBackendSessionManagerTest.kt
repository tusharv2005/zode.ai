...
class ZodeBackendSessionManagerTest {
    ...
    private fun setup(): ZodeBackendAppService {
        return ZodeBackendAppService.create(scope, FakeCliServer(mock), log)
    }

    private suspend fun ready(app: ZodeBackendAppService) {
        app.connect()
        withTimeout(10_000) {
    ...
    fun `cloudSessions passes filters and cursor`() = runBlocking {
        ...
        app.sessions.cloudSessions("/repo path", "cur 1", 25, "git@github.com:Zode-Org/zode.git")

        val path = mock.lastCloudSessionsPath ?: error("missing cloud sessions request")
        assertTrue(path.startsWith("/zode/cloud-sessions?"))
        ...
        assertTrue(decoded.contains("gitUrl=git@github.com:Zode-Org/zode.git"), path)
    }
    ...
    fun `importCloudSession posts cloud id and returns local session`() = runBlocking {
        ...
        assertTrue(path.startsWith("/zode/cloud/session/import?"))
        assertTrue(URLDecoder.decode(path, "UTF-8").contains("directory=/repo path"), path)
