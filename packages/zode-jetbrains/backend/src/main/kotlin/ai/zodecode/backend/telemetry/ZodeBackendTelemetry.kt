...
class ZodeBackendTelemetry(
    private val log: ZodeLog = ZodeLog.create(ZodeBackendTelemetry::class.java),
) {
    companion object {
    ...
    private fun base(): Map<String, JsonPrimitive> =
        ZodeLog.payload(log).mapValues { JsonPrimitive(it.value) }
}
