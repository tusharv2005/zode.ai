...
import ai.zodecode.backend.cli.CliServer
import ai.zodecode.backend.cli.ZodeBackendCliManager
...
import ai.zodecode.jetbrains.api.infrastructure.ClientError
import ai.zodecode.jetbrains.api.infrastructure.ClientException
import ai.zodecode.jetbrains.api.infrastructure.ServerError
import ai.zodecode.jetbrains.api.infrastructure.ServerException
import ai.zodecode.jetbrains.api.model.Config
import ai.zodecode.jetbrains.api.model.ConfigWarnings200ResponseInner
...
import ai.zodecode.jetbrains.api.model.ProviderOauthAuthorizeRequest
import ai.zodecode.jetbrains.api.model.ProviderOauthCallbackRequest
...
class ZodeBackendAppService private constructor(
  ...
  private val loadTimeoutMs: Long,
    ...
    constructor(cs: CoroutineScope) : this(
        cs,
        ZodeBackendCliManager(),
      ZodeLog.create(ZodeBackendAppService::class.java),
        APP_LOAD_TIMEOUT_MS,
    )
    ...
    companion object {
        ...
        ) = ZodeBackendAppService(cs, server, log, loadTimeoutMs)
    }

    private val mutex = Mutex()
    private val connection = ZodeConnectionService(cs, server, onReconnect = {
        cs.launch { reconnect() }
    ...
    private val _appState = MutableStateFlow<ZodeAppState>(ZodeAppState.Disconnected)
    val appState: StateFlow<ZodeAppState> = _appState.asStateFlow()
    ...
    val sessions = ZodeBackendSessionManager(cs, log)
    val chat = ZodeBackendChatManager(cs, log)
    val models = ZodeBackendModelStateManager(log)
    val workspaces = ZodeBackendWorkspaceManager(cs, sessions, log)
    @Volatile var profile: ZodeProfile200Response? = null
        private set

    @Volatile var config: Config? = null
        private set

    @Volatile var notifications: List<ZodeNotifications200ResponseInner> = emptyList()
        private set
    ...
    suspend fun connect() {
        mutex.withLock {
            ...
            if (current is ZodeAppState.Ready || current is ZodeAppState.Connecting || current is ZodeAppState.Loading || current is ZodeAppState.MigrationRequired) return
            ensureWatcher()
            connection.connect()
        }
    }
    ...
    suspend fun retry() {
        mutex.withLock {
            when (val current = _appState.value) {
                ZodeAppState.Disconnected -> {
                    ensureWatcher()
                    connection.connect()
                }
                ZodeAppState.Connecting,
                is ZodeAppState.Loading -> Unit
                is ZodeAppState.MigrationRequired -> {
                    log.info("retry: rerunning migration detection")
                    load()
                }
                is ZodeAppState.Ready -> {
                    if (current.data.warnings.isEmpty()) return
                    log.info("retry: refreshing config warnings")
                    refreshConfigState()
                    val next = _appState.value
                    val warns = (next as? ZodeAppState.Ready)?.data?.warnings
                    if (next is ZodeAppState.Ready && warns.isNullOrEmpty()) return
                    restartConnection("warnings remained after refresh")
                }
                is ZodeAppState.Error -> {
    ...
    fun requireReady() {
        when (_appState.value) {
            is ZodeAppState.Ready -> return
            is ZodeAppState.MigrationRequired -> throw IllegalStateException("Migration required")
            else -> throw IllegalStateException("Zode backend is not ready")
        }
    }

    suspend fun updateConfig(patch: ConfigPatchDto): ZodeAppState {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val current = _appState.value as? ZodeAppState.Ready ?: throw IllegalStateException("Zode backend is not ready")
        val body = ZodeCliDataParser.buildConfigPatch(patch)
        ...
        return (_appState.value as? ZodeAppState.Ready) ?: current
    }

    internal suspend fun resumeAfterMigration() {
        mutex.withLock {
            if (_appState.value !is ZodeAppState.MigrationRequired) return
            load()
        }
    }
    ...
    private suspend fun reconnect() {
        mutex.withLock {
            val current = _appState.value
            if (current is ZodeAppState.Ready || current is ZodeAppState.Connecting || current is ZodeAppState.Loading || current is ZodeAppState.MigrationRequired) {
    ...
    private fun ensureWatcher() {
        ...
        watcher = cs.launch {
            connection.state.collect { next ->
                when (next) {
                    ConnectionState.Disconnected -> _appState.value = ZodeAppState.Disconnected
                    ConnectionState.Connecting -> _appState.value = ZodeAppState.Connecting
     ...
     *
     * Progress is tracked via [LoadProgress] and emitted as [ZodeAppState.Loading].
     * On success, transitions to [ZodeAppState.Ready].
     * On failure of required data, transitions to [ZodeAppState.Error].
     */
    private fun load() {
        synchronized(loadLock) {
            ...
            loader = cs.launch {
                ...
                _appState.value = ZodeAppState.Loading(progress.get())
                ...
                if (migration != null) {
                    ...
                    warnings = emptyList()
                    _appState.value = ZodeAppState.MigrationRequired(migration)
                ...
                var cfg: Config? = null
                var prof: ZodeProfile200Response? = null
                var notifs: List<ZodeNotifications200ResponseInner> = emptyList()
                    ...
                    withTimeout(loadTimeoutMs) {
                        ...
                        launch {
                            ...
                            progress.updateAndGet { it.copy(profile = status) }
                                .also { _appState.value = ZodeAppState.Loading(it) }
                        }
                        launch {
    ...
    private fun captureLoad(event: String, start: Long, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<ZodeBackendTelemetry>().capture(
                    http,
                    port,
                    event,
    ...
    private fun setTelemetry(enabled: Boolean) {
        ...
        cs.launch {
            runCatching {
                service<ZodeBackendTelemetry>().setEnabled(http, port, enabled)
    ...
    private fun captureBackend(event: String, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<ZodeBackendTelemetry>().capture(http, port, event, props)
    ...
    private suspend fun detectMigration(): LegacyMigrationDetection? = withContext(Dispatchers.IO) {
        ...
        val store = ZodeBackendLegacyMigrationStoreService.store(log)
        val status = store.status()
        if (status != null) {
            log.info("Migration check: skipped because status=$status")
            return@withContext null
        }
        val detection = ZodeBackendMigrationManager(http, connection.port).detect(store)
     ...
     * as failures.
     */
    private suspend fun fetchProfile(): FetchResult<ZodeProfile200Response?> {
        val client = connection.appLoadApi
            ?: return FetchResult.ok(null)
        return try {
            val response = client.zodeProfile()
    ...
    private suspend fun fetchNotifications(): FetchResult<List<ZodeNotifications200ResponseInner>> {
        val client = connection.appLoadApi
            ?: return FetchResult.fail("notifications", detail = "Not connected")
        return try {
            FetchResult.ok(client.zodeNotifications())
        } catch (e: Exception) {
    ...
    private suspend fun refreshConfigState() {
        val current = _appState.value as? ZodeAppState.Ready ?: return
        ...
        if (state !is ZodeAppState.Ready || state.data !== current.data) return
    ...
    private fun setAppReady(data: AppData) {
        ...
        _appState.value = ZodeAppState.Ready(data)
    }

    private fun setAppError(message: String, errors: List<LoadError>) {
        val state = ZodeAppState.Error(message, errors)
        warnAppError(state)
        _appState.value = state
    }

    private fun warnAppError(state: ZodeAppState.Error) {
    ...
    private suspend fun awaitLoadResult(prev: ZodeAppState): ZodeAppState {
        val next = appState.first { it !== prev }
        if (next !is ZodeAppState.Loading) return next
        return appState.first { it !is ZodeAppState.Loading }
    }
     ...
     *   Re-fetches config and updates [ZodeAppState.Ready] data in-place.
     *
                ...
                connection.events.collect { event ->
                    when (event.type) {
                        ...
                        "global.disposed" -> {
                            ...
                            val current = _appState.value
                            if (current is ZodeAppState.Ready) load()
                        }
    ...
    private fun clear() {
        ...
        notifications = emptyList()
        warnings = emptyList()
        _appState.value = ZodeAppState.Disconnected
    }
     ...
     * Updates the current [ZodeAppState.Ready] profile in-place if the app is ready.
     */
    suspend fun refreshProfile(): ZodeProfile200Response? {
        val result = fetchProfile()
        val fresh = result.value
        val current = _appState.value
        if (current is ZodeAppState.Ready) {
    ...
    /**
     * Start the Zode device auth login flow.
    ...
    suspend fun startLogin(directory: String?): DeviceAuthDto {
        ...
        val response = client.providerOauthAuthorize(providerID = "zode", directory = directory, providerOauthAuthorizeRequest = body)
    ...
    /**
     * Complete the Zode device auth login flow.
     ...
     */
    suspend fun completeLogin(directory: String?): ZodeProfile200Response? {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        client.providerOauthCallback(providerID = "zode", directory = directory, providerOauthCallbackRequest = ProviderOauthCallbackRequest(method = 0.0))
    ...
    suspend fun logout(): Boolean {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        val result = client.authRemove(providerID = "zode")
        val current = _appState.value
        if (current is ZodeAppState.Ready) {
            profile = null
     ...
     */
    suspend fun setOrganization(organizationId: String?): ZodeProfile200Response? {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val body = JsonObject(
            mapOf("organizationId" to (organizationId?.let { JsonPrimitive(it) } ?: JsonNull)),
        ).toString()
        val request = Request.Builder()
            .url("http://127.0.0.1:$port/zode/organization")
            .header("Accept", "application/json")
