...
import ai.zodecode.backend.app.ZodeBackendChatManager
...
/**
 * Backend implementation of [ZodeSessionRpcApi].
 *
 * Session CRUD routes through the [ZodeBackendWorkspaceManager] to
 * get the correct workspace for a directory. Status tracking and
 * worktree directory management go directly to the
 * [ZodeBackendSessionManager]. Chat operations delegate to
 * [ZodeBackendChatManager].
 */
class ZodeSessionRpcApiImpl : ZodeSessionRpcApi {
    companion object {
        private val LOG = ZodeLog.create(ZodeSessionRpcApiImpl::class.java)
    }

    private val workspaces: ZodeBackendWorkspaceManager
        get() = app.workspaces

    private val sessions: ZodeBackendSessionManager
        get() = app.sessions

    private val chat: ZodeBackendChatManager
        get() = app.chat

    private val app: ZodeBackendAppService
        get() = service()
    ...
    override suspend fun rename(id: String, directory: String, title: String): ai.zodecode.rpc.dto.SessionDto {
        app.requireReady()
        val dir = sessions.getDirectory(id, directory)
