...
/**
 * Backend adapter for writing migrated data to the CLI (zode serve).
 *
...
interface LegacyMigrationBackend {
    ...
    /** POST /zodecode/session-import/project — returns the project ID */
    fun importProject(project: JsonObject): String

    /** POST /zodecode/session-import/session */
    fun importSession(session: JsonObject): LegacyImportResult

    /** POST /zodecode/session-import/message */
    fun importMessage(message: JsonObject)

    /** POST /zodecode/session-import/part */
