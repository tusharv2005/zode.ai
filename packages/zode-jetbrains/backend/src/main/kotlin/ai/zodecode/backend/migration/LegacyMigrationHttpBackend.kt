...
/**
 * OkHttp implementation of [LegacyMigrationBackend] against `zode serve`.
 *
 ...
 * constraints getting in the way. Mirrors the pattern in ZodeBackendChatManager.
 *
...
class LegacyMigrationHttpBackend(
    ...
    override fun importProject(project: JsonObject): String {
        ...
        val url = "$base/zodecode/session-import/project?directory=${encode(dir)}"
    ...
    override fun importSession(session: JsonObject): LegacyImportResult {
        ...
        val url = "$base/zodecode/session-import/session?directory=${encode(dir)}"
    ...
    override fun importMessage(message: JsonObject) {
        ...
        val url = "$base/zodecode/session-import/message?sessionID=${encode(sessionId)}"
    ...
    override fun importPart(part: JsonObject) {
        ...
        val url = "$base/zodecode/session-import/part?sessionID=${encode(sessionId)}"
