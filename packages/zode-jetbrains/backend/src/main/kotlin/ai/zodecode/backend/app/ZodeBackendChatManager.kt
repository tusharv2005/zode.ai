 ...
 *
 * All JSON parsing is delegated to [ZodeCliDataParser].
 */
class ZodeBackendChatManager(
        ...
        watcher = cs.launch {
            sse.collect { event ->
                if (event.type in CHAT_EVENTS) {
                    val parsed = ZodeCliDataParser.parseChatEvent(event.type, event.data)
                    if (parsed != null) {
    ...
    fun prompt(id: String, dir: String, prompt: PromptDto) {
        ...
        val body = ZodeCliDataParser.buildPromptJson(prompt)
    ...
    fun compact(id: String, dir: String, model: ModelSelectionDto) {
        ...
        val http = requireClient()
        val url = requireBase()
        val body = ZodeCliDataParser.buildSummarizeJson(model)
        val request = Request.Builder()
    ...
    fun messages(id: String, dir: String): List<MessageWithPartsDto> {
        ...
        return http.newCall(request).execute().use { response ->
            ...
            val parsed = ZodeCliDataParser.parseMessages(raw)
    ...
    fun updateConfig(dir: String, update: ConfigUpdateDto) {
        ...
        val partial = ZodeCliDataParser.buildConfigPartial(update)
    ...
    fun replyPermission(requestId: String, dir: String, reply: PermissionReplyDto) {
        ...
        val body = ZodeCliDataParser.buildPermissionReplyJson(reply)
    ...
    fun savePermissionRules(requestId: String, dir: String, rules: PermissionAlwaysRulesDto) {
        ...
        val body = ZodeCliDataParser.buildPermissionAlwaysRulesJson(rules)
    ...
    fun replyQuestion(requestId: String, dir: String, answers: QuestionReplyDto) {
        ...
        val body = ZodeCliDataParser.buildQuestionReplyJson(answers)
    ...
    fun pendingPermissions(dir: String): List<PermissionRequestDto> {
        ...
        val parsed = ZodeCliDataParser.parsePermissionRequests(raw)
    ...
    fun pendingQuestions(dir: String): List<QuestionRequestDto> {
        ...
        val parsed = ZodeCliDataParser.parseQuestionRequests(raw)
