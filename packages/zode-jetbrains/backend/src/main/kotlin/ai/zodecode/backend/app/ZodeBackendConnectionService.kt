 ...
 *
 * Concurrency is handled by the owning [ZodeBackendAppService] — `connect`,
 * `restart`, and `reinstall` are called under its mutex. Internal reconnect
 * attempts delegate back to the owner via [onReconnect].
 *
 * Not a service — owned and instantiated by [ZodeBackendAppService].
 */
class ZodeConnectionService(
  ...
  private val appLoadTimeoutMs: Long,
    ...
    constructor(
      ...
      server: CliServer,
      onReconnect: () -> Unit,
    ) : this(cs, server, onReconnect, ZodeLog.create(ZodeConnectionService::class.java), 30_000L)
     ...
     *
     * Called under [ZodeBackendAppService]'s mutex — no internal guard needed.
     */
    suspend fun connect() {
        open()
    }
     ...
     *
     * Called under [ZodeBackendAppService]'s mutex.
     */
    suspend fun restart() {
    ...
    private suspend fun open() {
        ...
        val ac = ZodeBackendHttpClients.api(password)
        val lc = ZodeBackendHttpClients.appLoad(password, appLoadTimeoutMs)
        val hc = ZodeBackendHttpClients.health(password)
        apiClient = ac
        appLoadClient = lc
        healthClient = hc
    ...
    private val listener = object : EventSourceListener() {
        ...
        override fun onEvent(src: EventSource, id: String?, type: String?, data: String) {
            ...
            synchronized(lock) {
                ...
                lastEvent.set(System.currentTimeMillis())
                val kind = type ?: ZodeCliDataParser.extractEventType(data)
     ...
     * goes through [ZodeBackendAppService]'s mutex for a full restart.
     */
    ...
    private fun close() {
        api = null
        appLoadApi = null
        apiClient?.let { ZodeBackendHttpClients.shutdown(it) }
        apiClient = null
        appLoadClient?.let { ZodeBackendHttpClients.shutdown(it) }
        appLoadClient = null
        healthClient?.let { ZodeBackendHttpClients.shutdown(it) }
        healthClient = null
    }
    ...
    fun dispose() {
        ...
        log.info("ZodeConnectionService disposed")
    }
}
