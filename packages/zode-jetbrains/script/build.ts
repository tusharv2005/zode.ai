...
/**
 * Build the Zode JetBrains plugin.
 *
 * Usage:
 ...
 * 2. Copies them into backend/build/generated/cli/cli/{os}/zode[.exe]
 *    plus models-snapshot.json sidecars.
 *    so they end up inside the backend jar at /cli/{os}/zode.
...
const platforms = [
  { os: "darwin-arm64", exe: "zode" },
  { os: "darwin-x64", exe: "zode" },
  { os: "linux-arm64", exe: "zode" },
  { os: "linux-x64", exe: "zode" },
  { os: "windows-x64", exe: "zode.exe" },
  { os: "windows-arm64", exe: "zode.exe" },
] as const
...
function distBinPath(os: string, exe: string): string {
  return join(distDir, `@zodecode/cli-${os}`, "bin", exe)
}

function distSnapshotPath(os: string): string {
  return join(distDir, `@zodecode/cli-${os}`, "bin", "models-snapshot.json")
}
