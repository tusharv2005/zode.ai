...
rm -rf "${cli}/dist"
ZODE_VERSION="$version" ZODE_CHANNEL=rc bun "${plugin}/script/build.ts" --production --prepare-cli
./gradlew buildPlugin -Pproduction=true -Pzode.version="$version" -Pzode.channel=eap

if [[ "$sign" == "1" ]]; then
  ./gradlew signPlugin -Pproduction=true -Pzode.version="$version" -Pzode.channel=eap
fi
...
if [[ "$skip_verification" == "1" ]]; then
  ...
  if [[ "$sign" == "1" ]]; then
    ./gradlew verifyPluginSignature -Pproduction=true -Pzode.version="$version" -Pzode.channel=eap
  fi
  ./gradlew verifyPlugin -Pproduction=true -Pzode.version="$version" -Pzode.channel=eap
fi
