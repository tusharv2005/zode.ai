...
resolve() {
  case "$target" in
    client|frontend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/zode.jetbrains/zode-frontend/zode-dev.log" \
    ...
    backend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/zode.jetbrains/zode-backend/zode-dev.log" \
...

{
  sub(/\r$/, "")
  cls = logger($0)
  if ((mode == "client" || mode == "frontend") && cls !~ /ZodeSessionService$/) next
  if (mode == "backend" && cls !~ /ZodeBackendChatManager$/) next
