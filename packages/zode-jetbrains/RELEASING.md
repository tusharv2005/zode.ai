...
Maintainers can use the Zode `release-jetbrains` skill to drive this process from a version request such as `next rc` or an explicit version. The skill resolves and confirms the version, dispatches and watches the prepare workflow, helps produce a filtered human-readable JetBrains/CLI changelog draft, commits the reviewed changelog to the release PR, and watches publishing after the PR is merged.

The skill lives at `.zode/skills/release-jetbrains/SKILL.md`. It does not move or recreate release tags, and merge permission is only required if the user explicitly asks the skill to merge the release PR automatically.

## Create Release Tag And PR

1. Open the GitHub Actions workflow:

[https://github.com/Zode-Org/zodecode/actions/workflows/prepare-jetbrains-release.yml](https://github.com/Zode-Org/zodecode/actions/workflows/prepare-jetbrains-release.yml)
...
| File | Purpose |
|---|---|
| `packages/zode-jetbrains/gradle.properties` | JetBrains plugin version in `zode.jetbrains.version`. |
| `packages/zode-jetbrains/CHANGELOG.md` | Release notes packaged into the plugin. |

Review `packages/zode-jetbrains/gradle.properties` and edit `packages/zode-jetbrains/CHANGELOG.md` before merging. This changelog entry is rendered into JetBrains `<change-notes>`, so it appears on the Marketplace and inside IntelliJ plugin UI.

The PR can change release metadata such as `packages/zode-jetbrains/gradle.properties` and `packages/zode-jetbrains/CHANGELOG.md`, but it does not change the tagged source code that will be built.
...
[https://github.com/Zode-Org/zodecode/actions/workflows/publish-jetbrains.yml](https://github.com/Zode-Org/zodecode/actions/workflows/publish-jetbrains.yml)
...
4. Search for **Zode Code** in the Marketplace tab.
...
If Marketplace publishing succeeded but GitHub Release upload failed, manually create or edit the GitHub Release for the existing tag. Use the reviewed release notes from `packages/zode-jetbrains/CHANGELOG.md` in the merged release PR.
...
| Secret | Purpose |
|---|---|
| `ZODE_MAINTAINER_APP_ID` | GitHub App ID used to create/update release PRs and immediate release tags. |
| `ZODE_MAINTAINER_APP_SECRET` | GitHub App private key used to create/update release PRs and immediate release tags. |
