...
- Confirm `ai.zodecode.jetbrains` is the final permanent Marketplace plugin ID before the first upload.
...
- Confirm the plugin is owned by the correct Zode vendor or organization.
...
- Confirm `ZODE_MAINTAINER_APP_ID` and `ZODE_MAINTAINER_APP_SECRET` are available to create release PRs and immediate release tags.
...
- Review and edit `packages/zode-jetbrains/CHANGELOG.md` in the generated release PR.
