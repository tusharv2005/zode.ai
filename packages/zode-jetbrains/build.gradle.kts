...
fun port(value: String): Int {
    ...
    val n = text.toIntOrNull()
        ?: error("zode.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random high port")
    require(n in 0..65535) {
        "zode.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random high port"
    }
...
val override = providers.gradleProperty("zode.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val prop = providers.gradleProperty("zode.jetbrains.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val tag = gitTag()?.removePrefix("jetbrains/v")
val ver = override?.let(::checked) ?: prop?.let(::checked) ?: if (release) checked(
    tag ?: error("Missing JetBrains plugin version. Publish builds must set zode.jetbrains.version or run from a jetbrains/v<version> tag."),
) else checked(tag ?: "0.0.0-dev")

val channel = providers.gradleProperty("zode.channel").map { it.trim() }.orElse("default")
val splitPort = providers.gradleProperty("zode.splitModeServerPort").orNull?.let(::port) ?: fallback()
val isolated = providers.gradleProperty("zode.dev.storage.isolated").map { it.toBoolean() }.orElse(false)
val worktreeRoot = providers.gradleProperty("zode.dev.worktree.root").orElse(
...
changelog {
    ...
    repositoryUrl = "https://github.com/Zode-Org/zodecode"
    groups = listOf("Added", "Changed", "Fixed", "Removed", "Security")
    combinePreReleases = false
}

val notes = providers.gradleProperty("zode.changeNotes").orElse(
    provider {
...
tasks.withType<RunIdeTask> {
    val level = providers.gradleProperty("zode.dev.log.level").orNull ?: "DEBUG"
    val content = providers.gradleProperty("zode.dev.log.chat.content").orNull ?: "off"
    val preview = providers.gradleProperty("zode.dev.log.chat.preview.max").orNull ?: "160"
    systemProperty("zode.dev.log.level", level)
    systemProperty("zode.dev.log.chat.content", content)
    systemProperty("zode.dev.log.chat.preview.max", preview)
    systemProperty("zode.dev.storage.isolated", isolated.get().toString())
    systemProperty("zode.dev.worktree.root", worktreeRoot.get())
}
