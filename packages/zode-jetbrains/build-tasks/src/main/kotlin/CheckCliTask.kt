 ...
 *   Local:      bun run build          (from packages/zode-jetbrains/)
    ...
    fun run() {
        ...
        if (!resolved.exists() || resolved.listFiles()?.isEmpty() != false) {
            throw GradleException(
                ...
                "Run 'bun run build' from packages/zode-jetbrains/ to build CLI and plugin together."
            )
        }
        if (production.get()) {
            val missing = platforms.get().filter { platform ->
                val d = File(resolved, platform)
                val exe = if (platform.startsWith("windows")) "zode.exe" else "zode"
