...
/**
 * Verifies that every Zode-specific change in shared upstream-owned source files
 * is annotated with a zodecode_change marker.
 *
 * Usage:
 ...
 *   - contains a zodecode_change marker comment           (inline annotation)
 *   - falls inside a zodecode_change start/end block      (block annotation)
 ...
 *
 * Exempt paths (no markers needed — entirely Zode-specific):
 *   - packages/opencode/src/zodecode/**
 *   - packages/opencode/test/zodecode/**
 *   - Any path containing "zodecode" in directory or filename
 *   - Any path with a directory starting with "zode-" (e.g. zode-sessions/)
 *   - script/upstream/**
 *   - Zode-specific annotation checker support files
 */
...
function isSource(file: string) {
  ...
  if (ext) return false
  return content(file).startsWith("#!") // zodecode_change
}

// Parses the unified=0 diff for `file` against `base` and returns:
//   - added: every added line number on HEAD
//   - revert: true when the file's diff removes any zodecode_change marker.
//     In that case the changes are reverting Zode modifications back to the
...
//     Zode construct (e.g. `unixSkip(` → `unix(`) often live in different
...
// Matches the start of a zodecode_change marker in JS, JSX, YAML, TOML, and shell comments.
...
function coveredLines(text: string): { lines: string[]; covered: Set<number> } {
  ...
  // Whole-file annotation: first non-shebang non-empty line is a zodecode_change - new file marker.
...
for (const file of files) {
  ...
  if (revert) continue // zodecode_change - file is reverting Zode modifications back to upstream

  const text = content(file) // zodecode_change
...
if (violations.length === 0) {
  console.log("All shared upstream changes are annotated with zodecode_change markers.")
  process.exit(0)
}

console.error(
  [
    "Unannotated Zode changes found in shared upstream files:",
    "",
    ...violations,
    "",
    "Every Zode-specific change in shared upstream source files must be annotated.",
    "",
    "Checked paths:",
    ...SCOPES.map((scope) => `  - ${scope}/**`),
    "",
    "Inline (single line):",
    "  const url = Flag.ZODE_MODELS_URL || 'https://models.dev' // zodecode_change",
    "",
    ...
    "  - packages/opencode/src/zodecode/**",
    "  - packages/opencode/test/zodecode/**",
    "  - Any path containing 'zodecode' in the directory or filename",
    "  - Any directory starting with 'zode-' (e.g. zode-sessions/)",
    "  - script/upstream/**",
    "  - Zode-specific annotation checker support files",
    "",
