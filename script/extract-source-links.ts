...
const OUTPUT = path.join(ROOT, "packages/zode-docs/source-links.md")

const check = process.argv.includes("--check")

const DIRS = [
  path.join(ROOT, "packages/zode-vscode/src"),
  path.join(ROOT, "packages/zode-vscode/webview-ui"),
...
const EXCLUDE_PATTERNS = [
  ...
  /^https?:\/\/api\.zode\.ai\/api\//,
  /^https?:\/\/supermassive-black-hole\.zodeapps\.io\/v1\/session-export\//, // zodecode_change
  /^https?:\/\/ingest\.zodesessions\.ai/,
  /^https?:\/\/api\.openai\.com/,
  /^https?:\/\/api\.github\.com/,
  /^https?:\/\/api\.githubcopilot\.com/,
  /^https?:\/\/[^/]+\.openai\.azure\.com\/openai/, // zodecode_change
...
const SKIP_FILES = ["models-snapshot.ts", "models-snapshot.js", "check-forbidden-strings.ts"] // zodecode_change
...
function shouldSkipFile(filepath: string): boolean {
  ...
  if (parts.includes("i18n") && path.basename(filepath) !== "en.ts") return true // zodecode_change
...
if (check) {
  const committed = await Bun.file(OUTPUT)
    ...
    .catch(() => "")
  if (committed === output) {
    console.log("packages/zode-docs/source-links.md is up to date.")
    process.exit(0)
  }
  console.error(
    [
      "ERROR: packages/zode-docs/source-links.md is out of date.",
      "",
...
console.log(`Wrote ${sorted.length} unique URLs to packages/zode-docs/source-links.md`)
