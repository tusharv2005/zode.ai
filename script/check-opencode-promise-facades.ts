 ...
 * remaining Zode Promise facades are migrated away. It also prevents tests
...
const testAllow: Record<string, { count: number; reason: string }> = {
  ...
  "zodecode/config-resilience.test.ts": { count: 4, reason: "existing runtime integration test" },
  "zodecode/config-validation.test.ts": { count: 2, reason: "existing runtime integration test" },
  "zodecode/plan-followup.test.ts": { count: 4, reason: "existing runtime integration test" },
  "zodecode/server/config-overlay.test.ts": { count: 3, reason: "server config cache integration test" },
  "zodecode/session/platform-attribution.test.ts": { count: 5, reason: "existing runtime integration test" },
  "zodecode/session-prompt-queue.test.ts": { count: 5, reason: "prompt queue legacy instance bridge regression" },
  "zodecode/session/session.test.ts": { count: 4, reason: "existing runtime integration test" },
...
const owned = (file: string) => file.startsWith("zodecode/") || file.startsWith("zode-sessions/")
