...
if (prop !== ver)
  throw new Error(`packages/zode-jetbrains/gradle.properties zode.jetbrains.version is ${prop}, expected ${ver}`)

const changelog = await Bun.file("packages/zode-jetbrains/CHANGELOG.md").text()
...
async function props() {
  const text = await Bun.file("packages/zode-jetbrains/gradle.properties").text()
  const line = text.split(/\r?\n/).find((item) => item.startsWith("zode.jetbrains.version="))
  const value = line?.split("=", 2)[1]?.trim()
  if (!value) throw new Error("packages/zode-jetbrains/gradle.properties is missing zode.jetbrains.version")
  return value
}
