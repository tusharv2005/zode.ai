...
const props = new URL("../packages/zode-jetbrains/gradle.properties", import.meta.url).pathname
const log = new URL("../packages/zode-jetbrains/CHANGELOG.md", import.meta.url).pathname
const repo = process.env.GH_REPO ?? process.env.GITHUB_REPOSITORY ?? "Zode-Org/zodecode"
...
await $`git add packages/zode-jetbrains/gradle.properties packages/zode-jetbrains/CHANGELOG.md`
...
async function writeprops(ver: string) {
  ...
  const line = `zode.jetbrains.version=${ver}`
  const next = current.match(/^zode\.jetbrains\.version=/m)
    ? current.replace(/^zode\.jetbrains\.version=.*$/m, line)
    : `${current.trim()}\n${line}\n`
...
- Review \`packages/zode-jetbrains/gradle.properties\` and edit \`packages/zode-jetbrains/CHANGELOG.md\` before merging.
