...
// Sync every Zode version string across the monorepo to a single target.
//
...
// `.zode/command/upstream-manual-merge.md`). It's also handy mid-merge to
// rebase our version bumps onto any new Zode main releases.
//
// Usage:
...
//   - the five Zode-Org download URLs inside that toml
//
// Intentionally NOT touched:
//   - `packages/zode-jetbrains/**` — the JetBrains plugin has its own release
...
for await (const rel of glob.scan({ cwd: root, onlyFiles: true })) {
  ...
  if (rel.startsWith("packages/zode-jetbrains/")) continue
...
if (await Bun.file(zed).exists()) {
  ...
  const next = text
    ...
    .replace(
      /https:\/\/github\.com\/Zode-Org\/zodecode\/releases\/download\/v[^/]+\//g,
      `https://github.com/Zode-Org/zodecode/releases/download/v${target}/`,
    )
  if (next !== text) {
