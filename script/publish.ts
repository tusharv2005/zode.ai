...
// zodecode_change start - consume changesets on the publish runner so changelog
...
{
  await $`bun install`
  const paths = ["packages/zode-vscode/CHANGELOG.md", "packages/opencode/CHANGELOG.md"]
...
if (Script.release) {
  // zodecode_change start - commit, tag, and push with rebase + retry to handle
  ...
  // zodecode_change start - mark prerelease GitHub releases accordingly
  ...
  const changelogPath = fileURLToPath(new URL("../packages/zode-vscode/CHANGELOG.md", import.meta.url))
...
// zodecode_change start
console.log("\n=== vscode ===\n")
await import(`../packages/zode-vscode/script/publish.ts`)
// zodecode_change end

// zodecode_change start - Zode does not ship the opencode desktop app
// if (Script.release) {
...
// zodecode_change start - extract latest changelog section for release notes
