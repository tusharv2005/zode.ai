...
test("fixScripts preserves Zode-only root scripts from base", () => {
  const ours = {
    scripts: {
      "dev-setup": "zode dev-setup",
      postinstall: "bun run --cwd packages/opencode fix-node-pty && bun run script/setup-git.ts",
      extension: "bun --cwd packages/zode-vscode script/launch.ts",
    },
  }
...
test("fixMetadata preserves opencode publish metadata from base", () => {
  const ours = { keywords: ["cli", "zode", "opencode"], private: false }
...
test("mergeWithNewestVersions preserves ours' key order so zode-only deps don't relocate", () => {
  // Regression: when ours has a zode-only dep in the middle (e.g. rotating-file-stream
