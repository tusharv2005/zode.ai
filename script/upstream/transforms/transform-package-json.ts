#!/usr/bin/env bun
/**
 * Enhanced package.json transform with Zode dependency injection
 *
 * This script handles package.json conflicts by:
 * 1. Taking upstream's version (to get new dependencies)
 * 2. Transforming package names (opencode -> zode)
 * 3. Injecting Zode-specific dependencies
 * 4. Preserving Zode's version number
 * 5. Preserving overrides and patchedDependencies
 * 6. Preserving Zode's repository configuration
...
import { oursHasZodecodeChanges } from "../utils/git"
 ...
 *
 * Key order preserves ours' order first (so zode-only deps stay in their
  ...
  if (ours) {
    for (const [name, ourVersion] of Object.entries(ours)) {
      ...
      if (theirVersion === undefined) {
        ...
        changes.push(`${section}: preserved ${name}@${ourVersion} (zode-only)`)
        continue
      }
      ...
      if (cmp === null) {
        ...
        result[name] = ourVersion
        changes.push(`${section}: ${name} ${theirVersion} -> ${ourVersion} (zode newer)`)
      } else {
        result[name] = theirVersion
...
const PACKAGE_NAME_MAP: Record<string, string> = {
  "opencode-ai": "@zodecode/cli",
  "@opencode-ai/cli": "@zodecode/cli",
  "@opencode-ai/sdk": "@zodecode/sdk",
  "@opencode-ai/plugin": "@zodecode/plugin",
}

// Zode-specific dependencies to inject into specific packages
// NOTE: When adding new Zode-specific workspace dependencies (packages starting with @zodecode/zode-*),
// add them here to prevent them from being removed during upstream merges
const ZODE_DEPENDENCIES: Record<string, Record<string, string>> = {
  // packages/opencode/package.json needs these
  "packages/opencode/package.json": {
    "@zodecode/zode-gateway": "workspace:*",
    "@zodecode/zode-telemetry": "workspace:*",
  },
}

// Zode-specific bin entries to set on specific packages
const ZODE_BIN: Record<string, Record<string, string>> = {
  "packages/opencode/package.json": {
    zode: "./bin/zode",
    zodecode: "./bin/zode",
  },
...
const TRANSFORM_PACKAGE_NAMES: Record<string, string> = {
  "package.json": "@zodecode/zode",
  "packages/opencode/package.json": "@zodecode/cli",
  "packages/plugin/package.json": "@zodecode/plugin",
  "packages/sdk/js/package.json": "@zodecode/sdk",
}

// Zode-specific scripts to preserve from the base branch per package.json.
...
// Zode doesn't ship (desktop-electron, console/app, app) and would otherwise
...
// in by upstream features (e.g. desktop Sentry integration) that Zode doesn't
...
/**
 * Re-apply Zode-specific scripts on top of the upstream-shaped scripts block,
 * and prune upstream-only scripts that target packages Zode doesn't ship.
 */
export function fixScripts(
  ...
  for (const name of DELETE_UPSTREAM_SCRIPTS[path] || []) {
    if (theirs[name]) {
      delete theirs[name]
      changes.push(`scripts.${name}: removed (upstream-only, no Zode target)`)
    }
  }
...
/**
 * Prune upstream-only catalog entries that have no consumers in Zode.
 */
export function fixCatalog(pkg: Record<string, unknown>, path: string, changes: string[]): void {
  ...
  for (const name of DELETE_UPSTREAM_CATALOG[path] || []) {
    if (cat[name]) {
      delete cat[name]
      changes.push(`workspaces.catalog.${name}: removed (upstream-only, no Zode consumer)`)
    }
  }
}
...
export async function transformPackageJson(file: string, options: PackageJsonOptions = {}): Promise<PackageJsonResult> {
  ...
  // If our version has zodecode_change markers, flag for manual resolution
  if (await oursHasZodecodeChanges(file)) {
    warn(`${file} has zodecode_change markers — skipping auto-transform, needs manual resolution`)
    return { file, action: "flagged", changes: [], dryRun: false }
  }

  try {
    // Save Zode's version BEFORE taking theirs
    ...
    // 2. Preserve Zode version if requested
    if (options.preserveVersion !== false) {
      const zodeVersion = await getCurrentVersion()
      if (pkg.version !== zodeVersion) {
        changes.push(`version: ${pkg.version} -> ${zodeVersion}`)
        pkg.version = zodeVersion
      }
    }
    ...
    if (ourPkg) {
      ...
      // 5. Preserve patchedDependencies (Zode-specific, upstream won't have these)
      ...
      // 6. Preserve repository (Zode-specific, upstream doesn't have this)
      const ourRepo = ourPkg.repository
      if (ourRepo && JSON.stringify(pkg.repository) !== JSON.stringify(ourRepo)) {
        pkg.repository = ourRepo
        changes.push(`repository: preserved Zode's repository configuration`)
      }

      fixMetadata(pkg, relativePath, ourPkg, changes)

      // 7. Handle workspaces for root package.json
      // Zode has removed hosted platform packages (console/*, slack, etc.)
      // so we need to preserve Zode's workspace configuration instead of taking upstream's
      ...
      if (relativePath === "package.json" && ourWorkspaces?.packages) {
        ...
        changes.push(`workspaces.packages: preserved Zode's workspace configuration`)
      }
    ...
    // 7. Transform dependency names (opencode -> zode)
    if (pkg.dependencies) {
    ...
    // 8. Inject Zode-specific dependencies
    const zodeDeps = ZODE_DEPENDENCIES[relativePath]
    if (zodeDeps) {
      pkg.dependencies = pkg.dependencies || {}
      for (const [name, version] of Object.entries(zodeDeps)) {
    ...
    // 9. Set Zode-specific bin entries
    const zodeBin = ZODE_BIN[relativePath]
    if (zodeBin) {
      pkg.bin = zodeBin
      changes.push(`bin: set Zode bin entries`)
    }
...
/**
 * Get Zode's package.json from the base branch (main) for comparison
 * Used during pre-merge to compare upstream versions against Zode's versions
 */
async function getZodePackageJson(path: string, baseBranch = "main"): Promise<Record<string, unknown> | null> {
  try {
 ...
 * This function merges Zode's versions with upstream, using "newest wins" strategy
 */
export async function transformAllPackageJson(options: PackageJsonOptions = {}): Promise<PackageJsonResult[]> {
  ...
  for await (const path of glob.scan({ absolute: false })) {
    ...
    try {
      ...
      // Get Zode's version from base branch for comparison
      const zodePkg = await getZodePackageJson(path)
      ...
      // 3. Merge dependencies with "newest wins" strategy (if Zode has this file)
      if (zodePkg) {
        pkg.dependencies = mergeWithNewestVersions(
          zodePkg.dependencies as Record<string, string> | undefined,
          pkg.dependencies,
          changes,
          "dependencies",
        )
        ...
        pkg.devDependencies = mergeWithNewestVersions(
          zodePkg.devDependencies as Record<string, string> | undefined,
          pkg.devDependencies,
          changes,
          "devDependencies",
        )
        ...
        pkg.peerDependencies = mergeWithNewestVersions(
          zodePkg.peerDependencies as Record<string, string> | undefined,
          pkg.peerDependencies,
          changes,
          "peerDependencies",
        )
        ...
        // 4. Preserve/merge overrides
        const zodeOverrides = zodePkg.overrides as Record<string, string> | undefined
        if (zodeOverrides || pkg.overrides) {
          pkg.overrides = mergeWithNewestVersions(zodeOverrides, pkg.overrides, changes, "overrides")
        }

        // 5. Preserve patchedDependencies (Zode-specific, upstream won't have these)
        const zodePatchedDeps = zodePkg.patchedDependencies as Record<string, string> | undefined
        if (zodePatchedDeps) {
          pkg.patchedDependencies = pkg.patchedDependencies || {}
          for (const [name, patch] of Object.entries(zodePatchedDeps)) {
            if (!pkg.patchedDependencies[name]) {
              pkg.patchedDependencies[name] = patch
              changes.push(`patchedDependencies: preserved ${name}`)
            }
        ...
        // 6. Preserve repository (Zode-specific, upstream doesn't have this)
        const zodeRepo = zodePkg.repository
        if (zodeRepo && JSON.stringify(pkg.repository) !== JSON.stringify(zodeRepo)) {
          pkg.repository = zodeRepo
          changes.push(`repository: preserved Zode's repository configuration`)
        }

        fixMetadata(pkg, path, zodePkg, changes)

        // 7. Handle workspaces for root package.json
        // Zode has removed hosted platform packages (console/*, slack, etc.)
        // so we need to preserve Zode's workspace configuration instead of taking upstream's
        const zodeWorkspaces = zodePkg.workspaces as
        ...
        if (path === "package.json" && zodeWorkspaces?.packages) {
          pkg.workspaces = pkg.workspaces || {}
          pkg.workspaces.packages = zodeWorkspaces.packages
          changes.push(`workspaces.packages: preserved Zode's workspace configuration`)
        }

        fixScripts(pkg, path, zodePkg, changes)

        // Merge catalog with "newest wins" strategy
        if (zodeWorkspaces?.catalog || upstreamWorkspaces?.catalog) {
          pkg.workspaces = pkg.workspaces || {}
          pkg.workspaces.catalog = mergeWithNewestVersions(
            zodeWorkspaces?.catalog,
      ...
      // 8. Inject Zode-specific dependencies
      const zodeDeps = ZODE_DEPENDENCIES[path]
      if (zodeDeps) {
        pkg.dependencies = pkg.dependencies || {}
        for (const [name, version] of Object.entries(zodeDeps)) {
          if (!pkg.dependencies[name]) {
 ...
 * zodecode_change markers (manual review needed).
 */
export async function reconcilePackageJsonFromRefs(
  ...
  if (await oursHasZodecodeChanges(file)) {
    warn(`${file} has zodecode_change markers — skipping reconcile, needs manual resolution`)
...
export async function reconcileAllPackageJson(options: ReconcileOptions): Promise<PackageJsonResult[]> {
  ...
  // upstream-only and zode-only files alike.
