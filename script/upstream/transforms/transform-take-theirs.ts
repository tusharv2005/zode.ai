#!/usr/bin/env bun
/**
 * Transform files by taking upstream version and applying Zode branding
 *
 * This script handles files that have only branding differences (no logic changes).
 * It takes the upstream version and applies Zode branding transforms.
 *
 * Use this for:
 * - UI components with OpenCode -> Zode branding
 * - Config files with predictable patterns
 * - Files without zodecode_change logic blocks
 */
...
export function applyBrandingTransforms(content: string, verbose = false): { result: string; replacements: number } {
  ...
  for (const line of lines) {
    // Skip lines with zodecode_change marker (already customized)
    if (line.includes("// zodecode_change")) {
