#!/usr/bin/env bun
/**
 * Transform i18n translation files with Zode branding
 *
 * This script handles i18n files by:
 * 1. Taking upstream's version as the base (to get new translation keys)
 * 2. Applying intelligent string replacements for Zode branding
 ...
 * - anomalyco/opencode -> Zode-Org/zodecode (GitHub repo)
 *
 * Preserved (not replaced):
 * - opencode.json (actual config filename)
 * - .opencode/ (actual directory name)
 * - Lines with `// zodecode_change`
 */
...
const I18N_REPLACEMENTS: StringReplacement[] = [
  // GitHub repo references
  {
    ...
    replacement: "github.com/Zode-Org/zodecode",
    description: "GitHub URL",
  },
  {
    pattern: /anomalyco\/opencode/g,
    replacement: "Zode-Org/zodecode",
...
/**
 * Check if a line should be preserved (has zodecode_change marker)
 */
function shouldPreserveLine(line: string): boolean {
  return line.includes("// zodecode_change")
}

/**
 * Apply string replacements to content, preserving zodecode_change lines
 */
export function transformI18nContent(
  ...
  for (const line of lines) {
    // Skip lines marked with zodecode_change
 ...
 * Takes upstream version (theirs) and applies Zode branding
 */
export async function transformConflictedI18n(
  ...
  for (const file of files) {
    ...
    if (!options.dryRun && (await oursHasZodecodeChanges(file))) {
    ...
    // Then apply Zode branding transformations
    ...
    if (options.dryRun) {
      ...
      success(`Transformed ${file}: took upstream + ${result.replacements} Zode branding replacements`)
    }
  }
