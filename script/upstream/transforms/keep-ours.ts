#!/usr/bin/env bun
/**
 * Keep Zode's version of specific files during merge
 *
 * This script handles files that should always keep Zode's version
...
export function shouldKeepOurs(filePath: string, patterns: string[]): boolean {
  const dirs = defaultConfig.zodeDirectories.map((dir) => `${dir}/**`)
  return matches(filePath, [...patterns, ...dirs]) || patterns.some((pattern) => filePath.includes(pattern))
}

/**
 * Keep Zode's version of conflicted files
 */
...
/**
 * Reset specific files to Zode's version (even if not conflicted)
 */
export async function resetToOurs(files: string[], options: KeepOursOptions = {}): Promise<KeepOursResult[]> {
  ...
  for (const file of files) {
    if (options.dryRun) {
      ...
      try {
        // Get the file from HEAD (Zode's version)
...
if (import.meta.main) {
  ...
  console.log()
  success(`Kept Zode's version for ${kept.length} files`)
