...
Scripts for automating the merge of upstream opencode changes into Zode.
...
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/zode-opencode-v1.1.44
```
...
| `fix-zodecode-markers.ts` | Rebuild `zodecode_change` markers for one file against the last merged upstream |
...
| Script | Description |
|---|---|
| `transforms/package-names.ts` | Transform opencode package names to zode |
| `transforms/preserve-versions.ts` | Preserve Zode's package versions |
| `transforms/keep-ours.ts` | Keep Zode's version of specific files |
| `transforms/skip-files.ts` | Skip/remove files that shouldn't exist in Zode |
| `transforms/transform-i18n.ts` | Transform i18n files with Zode branding |
| `transforms/transform-take-theirs.ts` | Take upstream + apply Zode branding for branding-only files |
| `transforms/transform-package-json.ts` | Enhanced package.json with Zode dependency injection |
...
4. **Create branches**
   ...
   - `<author>/zode-opencode-<version>` - Merge target branch
   - `<author>/opencode-<version>` - Transformed upstream branch

5. **Apply ALL transformations to upstream branch (PRE-MERGE)**:
   - Remove files that should not exist in Zode (`skipFiles`)
   - Transform package names (opencode-ai -> @zodecode/cli)
   - Preserve Zode's versions
   - Transform i18n files with Zode branding
   - Transform branding-only files (UI components, configs)
   - Transform package.json files (names, deps, Zode injections)
   - Transform script files (GitHub API references)
   - Transform extension files (Zed, etc.)
   - Transform web/docs files
   - Reset Zode-specific files

6. **Merge** transformed upstream into Zode branch
   - Since all branding transforms are applied pre-merge, conflicts should be minimal
   - Remaining conflicts are files with actual code differences (zodecode_change markers)

7. **Auto-resolve** any remaining conflicts
   - Skip files that shouldn't exist in Zode
   - Keep Zode's version of specific files
...
```typescript
{
  ...
  packageMappings: [
    { from: "opencode-ai", to: "@zodecode/cli" },
    { from: "@opencode-ai/cli", to: "@zodecode/cli" },
    // ...
  ],

  // Files to always keep Zode's version (never take upstream)
  keepOurs: [
    "README.md",
    "CONTRIBUTING.md",
    "AGENTS.md",
  ...
  // Files to take upstream + apply Zode branding transforms
  takeTheirsAndTransform: [
    "packages/ui/src/**/*.tsx",
    // ...
  ],

  // Zode-specific directories (preserved)
  zodeDirectories: [
    "packages/opencode/src/zodecode",
    "packages/zode-gateway",
    "packages/zode-telemetry",
    // ...
  ],
}
```
...
**Key insight**: By applying all branding transforms to the upstream branch BEFORE merging, we eliminate most conflicts that would otherwise occur due to branding differences (OpenCode -> Zode).

### Transform Order (Pre-Merge)

The following transforms are applied to the opencode branch before merging:

1. **Skip files** - Remove upstream-only packages/files that should not exist in Zode
2. **Package names** - `opencode-ai` -> `@zodecode/cli`, etc.
3. **Versions** - Preserve Zode's version numbers
4. **i18n files** - OpenCode -> Zode in user-visible strings
5. **Branding files** - UI components, configs with branding only
6. **package.json** - Names, dependencies, Zode injections
...
|---|---|---|
| i18n files | `i18n-transform` | Take upstream, apply Zode branding |
| UI components | `take-theirs-transform` | Take upstream, apply branding (no logic changes) |
| package.json | `package-transform` | Take upstream, transform names, inject Zode deps |
...
| README/docs | `keep-ours` | Keep Zode's version |
| GitHub workflows | `keep-ours` | Keep Zode's version (manual review) |
| Code with markers | `manual` | Has `zodecode_change` markers, needs review |
...
- Zode had `Zode` branding
- Git saw these as conflicting changes

Now:

- We transform upstream to `Zode` branding BEFORE merge
- Both branches have the same branding
- Git sees no conflict for branding-only files

The only remaining conflicts are files with **actual code differences** - files with `zodecode_change` markers that contain Zode-specific logic.
...
| `.worktrees/opencode-merge/zode-main` | The Zode base branch snapshot used for the merge |
...
- Zode-only paths: anything under `packages/zode-*/`, any `**/zodecode/**` subdir, `script/upstream/`.
- Non-code assets: SVG, PNG, fonts, archives, lock files, etc. (see `SKIP_EXTENSIONS` / `SKIP_FILENAMES` in the script).
- Files covered by the merge config's `keepOurs` or `skipFiles` lists in `utils/config.ts` — these are intentionally preserved or removed in Zode and must not be bulk-reset.
...
| `markers-only` | Stripping `zodecode_change` markers makes local match upstream | reset |
...
| `upstream-missing` | File does not exist upstream (zode-only, intentional) | skipped |
| `local-missing` | File tracked but missing locally (deleted in Zode) | skipped |
...
# Create PR: catrielmuller/zode-opencode-v1.1.44 -> main

# Second merge: v1.1.50 based on the previous PR (without waiting for approval)
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/zode-opencode-v1.1.44

# Create PR: catrielmuller/zode-opencode-v1.1.50 -> catrielmuller/zode-opencode-v1.1.44
# OR: catrielmuller/zode-opencode-v1.1.50 -> main (once first PR is merged)
```
...
bun run analyze.ts --version v1.1.50 --base-branch catrielmuller/zode-opencode-v1.1.44

# 2. Run the merge
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/zode-opencode-v1.1.44

# 3. Create PR from catrielmuller/zode-opencode-v1.1.50
#    - Target: catrielmuller/zode-opencode-v1.1.44 (if first PR not merged yet)
...
2. Look for `zodecode_change` markers to identify Zode-specific code
3. Resolve conflicts, keeping Zode-specific changes
4. Stage and commit:
   ```bash
   git add -A
