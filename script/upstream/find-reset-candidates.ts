 ...
 *   - markers-only        : only diff is zodecode_change markers wrapping
 ...
 *   - upstream-missing    : file does not exist upstream (zode-only, skipped)
...
const ZODE_ONLY_PATHSPECS = [
  ":(exclude,glob)packages/zode-*/**",
  ":(exclude,glob)**/zodecode/**",
...
async function candidates(commit: string, scope: string | undefined, top: string): Promise<CandidateSet> {
  const pathspecs = [scope ?? ".", ...ZODE_ONLY_PATHSPECS]
...
/**
 * Files the upstream merge config marks as "keep ours" (Zode-specific preserved
 * versions) or "skip" (upstream-only, removed in Zode) should never be touched
 * by the bulk resetter. They show up in `git diff` against raw upstream but
 * resetting them would undo deliberate Zode decisions.
 */
