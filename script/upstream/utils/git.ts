...
function compatTag(message: string): string | null {
  const prefix = "refactor: zode compat for "
...
export async function findLatestCompatCommit(base: string, target: string): Promise<CompatBase | null> {
  const grep = "^refactor: zode compat for "
 ...
 * Used to clean build artifacts from Zode-specific directories after checking
...
/**
 * Check if the "ours" version of a conflicted file contains zodecode_change markers.
 ...
 */
export async function oursHasZodecodeChanges(file: string): Promise<boolean> {
  const result = await $`git show :2:${file}`.quiet().nothrow()
  if (result.exitCode !== 0) return false
  return result.stdout.toString().includes("zodecode_change")
}
...
export async function trainRerere(grep: string): Promise<number> {
  ...
  const dir = join(tmpdir(), `zode-rerere-train-${randomUUID()}`)
