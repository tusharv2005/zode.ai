...
test("recommends keep ours for Zode directories", () => {
  expect(getRecommendation("packages/zode-vscode/.prettierignore", [], []).recommendation).toBe("keep-ours")
  expect(getRecommendation("packages/zode-i18n/tsconfig.json", [], []).recommendation).toBe("keep-ours")
})
