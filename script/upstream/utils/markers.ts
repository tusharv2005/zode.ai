#!/usr/bin/env bun
/**
 * Shared zodecode_change marker helpers used by both the marker fixer and the
...
export const standalone = [
  /^\s*\/\/\s*zodecode_change\b.*$/,
  /^\s*#\s*zodecode_change\b.*$/,
  /^\s*\{?\s*\/\*\s*zodecode_change\b.*\*\/\}?\s*$/,
]
export const start = /\bzodecode_change\s+start\b/
export const end = /\bzodecode_change\s+end\b/
export const freshmark = /\bzodecode_change\s*-\s*new\s*file\b/
...
function strip(file: string, line: string): { line: string | null; mark?: string } {
  ...
  if (style(file) === "hash") return comment(line, [/^#\s*zodecode_change\b/])
  return comment(line, [/^\{\/\*\s*zodecode_change\b/, /^\/\*\s*zodecode_change\b/, /^\/\/\s*zodecode_change\b/])
}
...
function block(mode: Style, pad: string) {
  if (mode === "hash") return { start: `${pad}# zodecode_change start`, end: `${pad}# zodecode_change end` }
  if (mode === "jsx") return { start: `${pad}{/* zodecode_change start */}`, end: `${pad}{/* zodecode_change end */}` }
  if (mode === "block") return { start: `${pad}/* zodecode_change start */`, end: `${pad}/* zodecode_change end */` }
  return { start: `${pad}// zodecode_change start`, end: `${pad}// zodecode_change end` }
}

function note(mode: Style) {
  if (mode === "hash") return " # zodecode_change"
  if (mode === "jsx") return " {/* zodecode_change */}"
  if (mode === "block") return " /* zodecode_change */"
  return " // zodecode_change"
}
...
export function fresh(file: string, clean: Clean) {
  ...
  const line = clean.marks.file ?? (mode === "hash" ? "# zodecode_change - new file" : "// zodecode_change - new file")
...
export async function changed(base: Text, head: Text, opts?: { ignoreWhitespace?: boolean }): Promise<Diff> {
  const dir = await mkdtemp(path.join(tmpdir(), "zode-markers-"))
