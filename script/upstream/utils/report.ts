 ...
 * Pass currentContent (our version of the file) to detect zodecode_change markers
...
export function getRecommendation(
  ...
  // Check if file should be skipped entirely (doesn't exist in Zode, shouldn't be added)
  if (shouldSkipFile(path, skipFiles)) {
    return {
      recommendation: "skip",
      reason: "File should be skipped (does not exist in Zode fork)",
    }
  }
  ...
  if (keepOurs.some((pattern) => path.includes(pattern) || path === pattern)) {
    return {
      ...
      reason: "File is Zode-specific and should not be overwritten",
    }
  }

  // Zode directories should always keep ours
  if (
    matches(
      path,
      defaultConfig.zodeDirectories.map((dir) => `${dir}/**`),
    )
  ) {
    return {
      recommendation: "keep-ours",
      reason: "File is in a Zode-specific directory",
    }
  }
  ...
  if (shouldTakeTheirsTransform(path)) {
    // If our version has zodecode_change markers, flag for manual review
    if (currentContent?.includes("zodecode_change")) {
      return {
        recommendation: "manual",
        reason: "File has zodecode_change markers — auto-transform skipped, needs manual review",
      }
    }
    return {
      recommendation: "take-theirs-transform",
      reason: "Branding-only file: take upstream and apply Zode branding transforms",
    }
  }

  switch (type) {
    case "i18n":
      // i18n files that have zodecode_change markers need manual review
      if (currentContent?.includes("zodecode_change")) {
        return {
          recommendation: "manual",
          reason: "i18n file has zodecode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "i18n-transform",
        reason: "i18n file: take upstream translations and apply Zode branding",
      }
    case "script":
      if (currentContent?.includes("zodecode_change")) {
        return {
          recommendation: "manual",
          reason: "Script file has zodecode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
    ...
    case "extension":
      if (currentContent?.includes("zodecode_change")) {
        return {
          recommendation: "manual",
          reason: "Extension file has zodecode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "extension-transform",
        reason: "Extension file: take upstream and apply Zode branding",
      }
    case "web":
      if (currentContent?.includes("zodecode_change")) {
        return {
          recommendation: "manual",
          reason: "Web/docs file has zodecode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "web-transform",
        reason: "Web/docs file: take upstream and apply Zode branding",
      }
    case "markdown":
      return {
        recommendation: "keep-ours",
        reason: "Markdown files are typically Zode-specific documentation",
      }
    case "package":
      if (currentContent?.includes("zodecode_change")) {
        return {
          recommendation: "manual",
          reason: "package.json has zodecode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "package-transform",
        reason: "Package.json: take upstream, transform names, inject Zode deps, preserve version",
      }
    case "code":
      return {
        recommendation: "manual",
        reason: "Code files need manual review for zodecode_change markers",
      }
    case "config":
      return {
        recommendation: "manual",
        reason: "Config files may have Zode-specific settings",
      }
    default:
      return {
        recommendation: "manual",
...
export async function analyzeConflicts(
  ...
  for (const path of files) {
    const type = classifyFile(path)
    // Read current file content (our version) to detect zodecode_change markers
...
export function generateMarkdownReport(report: ConflictReport): string {
  ...
  for (const rec of order) {
    ...
    const titleMap: Record<ConflictFile["recommendation"], string> = {
      ...
      "i18n-transform": "i18n Transform (Auto-Apply Zode Branding)",
      "take-theirs-transform": "Take Upstream + Zode Branding (Auto)",
      ...
      "keep-ours": "Keep Zode Version (Ours)",
