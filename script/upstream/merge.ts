 ...
 *
 * Automates the process of merging upstream opencode changes into Zode.
 *
 * Usage:
...
function logWorktrees(refs: worktree.RefInfo, input: worktree.RefInput, baseName: string): void {
  ...
  logger.info(`  zode-main:  ${refs.main} (${baseName}, ${input.base.slice(0, 8)})`)
  ...
  logger.info(`  - Zode base main: ${refs.main}`)
...
async function main() {
  ...
  logger.header("Zode Upstream Merge Tool")
  ...
  if (keepOursCount > 0) {
    conflictReport.recommendations.push(`${keepOursCount} files will keep Zode's version`)
  }
  if (codemodCount > 0) {
  ...
  const zodeVersion = await version.getCurrentZodeVersion()
  ...
  // Create Zode merge branch
  const zodeBranch = `${author}/zode-opencode-${targetVersion.tag}`
  const zodeBackup = await git.backupAndDeleteBranch(zodeBranch)
  if (zodeBackup) {
    logger.info(`Backed up existing branch to: ${zodeBackup}`)
  }
  await git.createBranch(zodeBranch)

  if (options.push) {
    await git.push(config.originRemote, zodeBranch, true)
  }
  logger.info(`Created Zode branch: ${zodeBranch}`)
  ...
  // This reduces conflicts by transforming upstream code to Zode conventions BEFORE merging
  logger.step(6, 8, "Applying transformations to opencode branch (pre-merge)...")

  logger.info("Removing files skipped in Zode...")
  ...
  // 6a. Transform package names (opencode-ai -> @zodecode/cli)
  ...
  // 6b. Preserve Zode versions
  logger.info("Preserving Zode versions...")
  const versionResults = await preserveAllVersions({
    dryRun: false,
    verbose: options.verbose,
    targetVersion: zodeVersion,
  })
  logger.success(`Preserved versions in ${versionResults.length} files`)

  // 6c. Transform i18n files (OpenCode -> Zode branding)
  ...
  if (i18nPreCount > 0) {
    logger.success(`Transformed ${i18nPreCount} i18n files with Zode branding`)
  }
  ...
  if (brandingCount > 0) {
    logger.success(`Transformed ${brandingCount} files with Zode branding`)
  }

  // 6f. Transform package.json files (names, deps, Zode injections)
  ...
  // 6j. Reset keep-ours files to Zode's version
  logger.info("Resetting Zode-specific files...")
  const keepOursResults = await resetToOurs(config.keepOurs, { dryRun: false, verbose: options.verbose })
  logger.success(`Reset ${keepOursResults.length} files to Zode's version`)
  ...
  // Clean untracked build artifacts from Zode-specific directories.
  ...
  logger.info("Cleaning Zode-specific directory artifacts...")
  await git.cleanDirectories(config.zodeDirectories)

  // Commit all transformations
  await git.stageAll()
  const compatMessage = `refactor: zode compat for ${targetVersion.tag}`
  if (prior) {
  ...
  // Step 7: Merge into Zode branch
  logger.step(7, 8, "Merging into Zode branch...")

  await git.checkout(zodeBranch)
  if (prior) {
    const linked = await git.recordAncestor(targetVersion.commit, `merge: record upstream ${targetVersion.tag}`)
    if (linked) logger.info(`Recorded upstream ${targetVersion.tag} as Zode branch ancestry`)
  }
  ...
  if (!mergeResult.success) {
    ...
    // These are likely files with zodecode_change markers or actual logic differences.

    // Step 7a: Skip files that shouldn't exist in Zode
    logger.info("Removing files that shouldn't exist in Zode...")
    ...
    logger.info("Keeping Zode-specific files...")
    ...
    if (autoResolved.length > 0) {
      logger.success(`Auto-resolved ${autoResolved.length} conflicts (kept Zode's version)`)
    }
    ...
    // Files with zodecode_change markers are flagged for manual resolution instead.
    ...
    if (conflictedFiles.length > 0) {
      ...
      // zodecode_change markers, plus JSON/YAML/TOML key merges and other
      ...
      if (i18nFlagged.length > 0) {
        logger.warn(`${i18nFlagged.length} i18n file(s) have zodecode_change markers — flagged for manual resolution`)
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (takeFlagged.length > 0) {
          logger.warn(
            `${takeFlagged.length} branding file(s) have zodecode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (pkgFlagged.length > 0) {
          logger.warn(
            `${pkgFlagged.length} package.json file(s) have zodecode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (scriptFlagged.length > 0) {
          logger.warn(
            `${scriptFlagged.length} script file(s) have zodecode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (extFlagged.length > 0) {
          logger.warn(
            `${extFlagged.length} extension file(s) have zodecode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (webFlagged.length > 0) {
          logger.warn(
            `${webFlagged.length} web/docs file(s) have zodecode_change markers — flagged for manual resolution`,
          )
    ...
    // Combine git-reported conflicts with files flagged due to zodecode_change markers
    const allManual = [...new Set([...remaining, ...flaggedFiles])]
    if (allManual.length > 0) {
      if (flaggedFiles.length > 0) {
        logger.warn(`${flaggedFiles.length} file(s) were flagged because they contain zodecode_change markers:`)
        logger.list(flaggedFiles)
        logger.info("  These files have intentional Zode-specific changes. Keep our version or merge carefully.")
        logger.info("")
      }
      ...
      logger.info("")
      logger.info("These conflicts contain zodecode_change markers or actual code differences.")
      ...
      conflictReport.mergeBranch = zodeBranch
      const reportPath = `upstream-merge-report-${targetVersion.version}.md`
      await report.saveReport(conflictReport, reportPath)
      logger.success(`Report saved to ${reportPath}`)
      ...
      logger.info(`  3. git push ${config.originRemote} ${zodeBranch}`)
      logger.info("  4. Create PR from " + zodeBranch + " to " + config.baseBranch)
      logger.info("")
  ...
  if (options.push) {
    await git.push(config.originRemote, zodeBranch)
    logger.success(`Pushed ${zodeBranch} to ${config.originRemote}`)
  }
  ...
  logger.info(`Zode branch: ${zodeBranch}`)
  ...
  if (remainingConflicts.length > 0) {
    ...
    logger.info("  2. Create PR from " + zodeBranch + " to " + config.baseBranch)
  }
