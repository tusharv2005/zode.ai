 ...
 *
 * Enforcement scope (Zode-owned paths only, to avoid upstream-sync churn):
 *   - Any top-level markdown file (TESTING.md, AGENTS.md, README.md, …)
 *   - Any path segment containing "zodecode" or starting with "zode-"
...
function skip(file: string) {
  ...
  if (parts.some((p) => p.includes("zodecode") || p.startsWith("zode-"))) return false
