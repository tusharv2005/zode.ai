...
Guidance for the automated reviewer (zode-code-bot) on PRs in this repo.
...
- `knip` unused exports
- `zodecode_change` marker rules — both directions:
  - Missing markers on shared opencode files (`script/check-opencode-annotations.ts`)
  - Markers present in zode-only paths like `packages/zode-vscode/`, `packages/zode-ui/`, `packages/opencode/src/zodecode/` (`bun run check-zodecode-change`)
- Workflow allowlist drift (`script/check-workflows.ts`)
- Stale `packages/zode-docs/source-links.md` (`script/extract-source-links.ts`)
...
- Generated artifact freshness (`check-zode-generated-artifacts.yml`)
...
Zode CLI is a fork of opencode. Minimizing diff against upstream is a top priority.

- If a change modifies a shared opencode file (anything under `packages/opencode/` not in a path containing `zodecode`), ask whether the logic could live in a Zode-only directory instead (`packages/opencode/src/zodecode/`, `packages/zode-gateway/`, etc.) or be reduced to a smaller hook.
- Refactors or reorganizations of upstream code are a red flag — flag them unless clearly justified.
- See `.zode/skills/zodecode-merge-minimizer/SKILL.md` for the decision rules.

### 4. Cloud config schema mirror

When `Config.Info` in `packages/opencode/src/config/config.ts` gains a new `zodecode_change` field, the matching JSON Schema entry must also be added in the cloud repo (`apps/web/src/app/config.json/extras.ts`). CI does **not** check this — flag it as a reminder if you see a new config field added.
...
For changes under `packages/zode-vscode/webview-ui/`:
