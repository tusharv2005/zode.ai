...
description: Write Storybook stories and visual regression tests for the Zode VS Code extension webview UI
---

Use this skill when the user asks you to add visual regression tests, screenshot tests, or Storybook stories for components in `packages/zode-vscode/`.
...
Stories live in `packages/zode-vscode/webview-ui/src/stories/`. Existing files and their scope:
...
Snapshots live at `packages/zode-vscode/tests/visual-regression.spec.ts-snapshots/`:
...
Key settings in `packages/zode-vscode/playwright.config.ts`:
...
- `packages/util/**`
- `packages/sdk/js/**`
- `packages/zode-vscode/webview-ui/**`
- `packages/zode-vscode/.storybook/**`
- `packages/zode-vscode/tests/visual-regression*`
...
zode-ui components are imported via deep subpaths:

```tsx
import { Part } from "@zodecode/zode-ui/message-part"
import { BasicTool } from "@zodecode/zode-ui/basic-tool"
...
```tsx
import type { AssistantMessage as SDKAssistantMessage, TextPart, ToolPart } from "@zodecode/sdk/v2"
...
```
globals=colorScheme:dark;theme:zode-vscode;vscodeTheme:dark-modern
```

The `.storybook/preview.tsx` applies these via a decorator that calls `applyVscodeTheme()` / `applyZodeTheme()` from zode-ui. Stories do NOT need to handle theming — it happens automatically.
