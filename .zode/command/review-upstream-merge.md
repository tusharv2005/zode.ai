...
## ZODECODE_CHANGE_MARKERS.md

Review carefully, file by file, whether we accidentally removed any `zodecode_change` marker.

Get the full list of files changed in the reviewed PR. Check each changed file. For each file, compare both our `main` branch and the upstream-merged version in the reviewed PR. Determine whether any marker removal, marker move, or Zode-specific change makes sense, and comment on that.
...
Check whether the merge now mentions OpenCode somewhere user-facing instead of Zode, or links to OpenCode web properties.
...
Check whether any merged files now use `zodecode_change` markers without any actual difference to upstream.
...
For each `zodecode_change` in this PR, trace the full chain: where the value or behavior is introduced, where it needs to flow through, and where it is ultimately consumed. Verify every link in the chain still exists after the merge.
...
Check whether this PR removed any Zode-specific tests.

Zode-specific tests may live in paths containing `zode` or `zodecode`, or may include Zode-specific assertions, fixtures, or `zodecode_change` markers.
