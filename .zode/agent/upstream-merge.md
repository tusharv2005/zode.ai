...
**Do not load the `zodecode-merge-minimizer` skill.** That skill is for
authoring new Zode changes against shared upstream files; during an upstream
merge it gives the wrong guidance (it nudges toward extracting Zode logic out
...
- does a `zodecode_change` marker in HEAD encode a bug fix, a feature, or a
  defensive check?
...
When HEAD includes a non-obvious Zode-specific wrapper (e.g. a helper in
`packages/opencode/src/zodecode/`), find out why it exists before deciding to
keep or bypass it:

```bash
git log --all --oneline -S "<symbol>" -- packages/opencode/src/zodecode/
git log --all --oneline -- <zode-file>
```
...
- one-sentence rationale (what Zode behaviour is preserved, what upstream
...
3. State which Zode behaviour must survive and why (reference PR numbers /
   `zodecode_change` comments when possible).
...
- `.worktrees/opencode-merge/zode-main` — Zode base snapshot
- `.worktrees/opencode-merge/auto-merge` — automated merge snapshot (original
  conflict reference)

Apply in order:

- prefer upstream code and architecture whenever compatible with Zode behaviour
- preserve Zode-specific behaviour marked with `zodecode_change`
- keep `zodecode_change` markers around Zode-specific code in shared opencode
  files
- when upstream refactors a region that HEAD had annotated with
  `zodecode_change`, **check whether the marker encodes a bug fix or a feature
  ...
  `Effect.gen` block.
- when a `take-theirs` drops a line that was the target of a Zode pre-filter,
  the upstream line may be actively wrong for Zode — e.g. an inner `continue`
  filter whose condition collides with an outer filter Zode added. Re-read the
  surrounding 20 lines before committing to `take-theirs`.
- if Zode-specific code must be refactored to fit new upstream architecture,
  explain the refactor in the final summary
- if upstream moved the relevant logic to another file, port the Zode behaviour
  there and list both paths in the final summary. Verify the new file already
  carries the Zode-renamed symbols (e.g. `x-zode-directory`) by diffing against
  pristine upstream.
- if upstream extracts shared policy into a helper, move Zode-specific additions
...
- if upstream deleted a file, analyse whether the Zode behaviour should be
...
**commenting it out with `zodecode_change` markers** over deletion when the
surrounding structure (an `if`, a loop) still makes sense. That keeps the
intent visible to the next merger. Example:

```ts
} else if (input?.scope !== "project" && !Flag.ZODE_EXPERIMENTAL_WORKSPACES) {
  // zodecode_change start - directory filtering handled by ZodeSession.filters above
  // if (input?.directory) {
...
shows up in `git diff --diff-filter=U`; the new sibling may need a Zode feature
...
- `zodecode_change` comments in the *auto-merge* snapshot that didn't end up in
...
- **Orphaned imports and references.** A rename upstream may leave a Zode
...
- **Partial auto-merges.** Upstream may have refactored a region Zode
  ...
  path, the file compiles upstream but breaks on Zode.
...
- other CI guards that touched files imply (knip for `zode-vscode/`,
  `check-zodecode-change`, source-links, visual regression,
  `script/check-forbidden-strings.ts`)
- if you encounter a hardcoded upstream URL, repo path, or attribution string
  during conflict resolution that obviously shouldn't ship in Zode (e.g. another
...
shared code, but the change cascades into Zode-only files (zodecode tests,
zode-specific source, plugins) that still reference the old symbol. Those
...
### 9.6. Handle upstream-added tests that diverge from Zode

Upstream sometimes adds tests that encode design contracts Zode intentionally
breaks. These auto-merge cleanly and then fail. Three resolution patterns:

- **Rewrite the test** when the test is a contract assertion and Zode has a
  different but equally valid contract. Invert or adjust the assertion with a
  `zodecode_change` marker explaining the divergence.
- **Skip the test** when the test relies on patterns that Zode has replaced
  ...
  Zode's extensions break). Mark with `zodecode_change` and a rationale
  explaining what would need to change for the test to run.
- **Delete the test** when it covers functionality Zode deliberately removed
  (fallback paths, deprecated endpoints, products Zode doesn't ship). Note
...
`packages/extensions/zed/extension.toml` (version field + 5 Zode-Org download
...
(e.g. `1.14.30`), whose release tag does not exist on Zode's pipeline, so the
...
(excluding `node_modules`, hidden dirs, and `packages/zode-jetbrains/` which
tracks its own cadence), plus the Zed extension toml. It is idempotent — rerun
it any time to rebase the version back onto Zode main (useful during
...
- **Non-trivial merge decisions**: a short section per file (or group of
  ...
  Focus on *what Zode behaviour survived* and *what upstream features were
  adopted*. Link to Zode PRs when a `zodecode_change` encodes a specific fix.
...
- Upstream can reintroduce bugs a Zode `zodecode_change` had already fixed —
  during big refactors check every `zodecode_change` the refactor touched.
- "Take-theirs" on an inner conditional is often wrong when Zode added an outer
...
- Stricter DOM lib types (upstream TS upgrade) can surface latent casting
  issues around `WebSocket.send`, `Headers`, etc. — prefer narrowing the Zode
...
- Zode code may rely on ambient context (async-local storage, globally-set
  flags, process env) being populated at a lifecycle moment that upstream
  refactors away. If Zode behaviour reads ambient state during init, forked
...
- Tests that intercept via process-global or module-global spies can become
  ...
  the spy.
- When Zode extends a shared data shape with extra optional fields, different
  ...
  every Zode addition — audit the encoding assumption before adding fields.
...
- Upstream-added tests can encode a design contract Zode deliberately breaks.
  The test passes upstream because upstream doesn't share Zode's requirement.
  Decide between refactoring Zode to match the upstream contract or rewriting
  the test to assert Zode's divergent contract — with a `zodecode_change`
