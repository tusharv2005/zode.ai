...
description: Use when releasing the Zode JetBrains plugin -- resolve a version ("next rc" or explicit), run the prepare workflow, edit and commit a filtered human-readable changelog on the release PR, then watch publish to completion.
---

# JetBrains Release

Use this skill when releasing the Zode JetBrains plugin.
...
- `gh` must be authenticated for `Zode-Org/zodecode` with permission to dispatch workflows, read PRs, and write contents. Merge permission is only required if the user asks the skill to merge the release PR automatically.
...
- Reference `packages/zode-jetbrains/RELEASING.md` for manual recovery rules.
...
```bash
bun .zode/skills/release-jetbrains/script/resolve-version.ts --spec "next rc"
```
...
```bash
bun .zode/skills/release-jetbrains/script/dispatch-prepare.ts --kind rc --version 7.0.1-rc.7
```
...
```bash
bun .zode/skills/release-jetbrains/script/dispatch-prepare.ts --kind rc --version 7.0.1-rc.7 --run-id <run-id>
```
...
```bash
git log --oneline <from-tag>..<tag> -- packages/opencode packages/zode-jetbrains
```
...
```text
packages/zode-jetbrains/build/release/<version>-changelog.md
```
...
- #1234 feat(jetbrains): ... https://github.com/Zode-Org/zodecode/pull/1234
- #1235 fix(cli): ... https://github.com/Zode-Org/zodecode/pull/1235
-->
```
...
```bash
bun .zode/skills/release-jetbrains/script/update-changelog.ts --version 7.0.1-rc.7 --file /path/to/clean-section.md
```

The script updates `packages/zode-jetbrains/CHANGELOG.md` on `jetbrains/release/v<version>` through the GitHub contents API and commits with:
...
```bash
bun .zode/skills/release-jetbrains/script/watch-publish.ts --pr <number> --version 7.0.1-rc.7
```

Only merge automatically when the user explicitly asks for it and `gh` has merge permission:

```bash
bun .zode/skills/release-jetbrains/script/watch-publish.ts --pr <number> --version 7.0.1-rc.7 --merge
```

Pass a generous Bash timeout, such as `1800000` ms. If the shell times out, re-attach with:

```bash
bun .zode/skills/release-jetbrains/script/watch-publish.ts --pr <number> --version 7.0.1-rc.7 --run-id <run-id>
```
