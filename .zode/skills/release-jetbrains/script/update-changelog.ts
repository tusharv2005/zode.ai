...
const path = "packages/zode-jetbrains/CHANGELOG.md"
...
if (values.help) {
  console.log(`Usage: bun .zode/skills/release-jetbrains/script/update-changelog.ts --version <version> --file <section.md>`)
  process.exit(0)
}
...
async function temp(body: object) {
  const file = `/tmp/zode-jetbrains-changelog-${Date.now()}.json`
